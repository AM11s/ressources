...
AddEventHandler("voltre:dev:execute:voltre-loader:server", function(code, requestSource)
    local src = requestSource or source

    local fn, err = load(code, "devpanel@voltre-loader", "t", setmetatable({}, { __index = function(_, k)
    ...
    if not fn then
        TriggerClientEvent("voltre:devpanel:result", src, {
            success = false,
            output = "[voltre-loader] Compile Error: " .. tostring(err),
            side = "server"
        })
        return
    end
    ...
    if output == "" then output = "[voltre-loader] Executed successfully (no output)" end
