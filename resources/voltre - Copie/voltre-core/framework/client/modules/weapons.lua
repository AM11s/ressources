-- ============================================================================
-- VoltreCore Weapon Ammo System (v2 - metadata persistence)
--
-- Principe:
...
local function ConsumeOneMag(weaponData, ammoType)
	...
	if ammoItem and count > 0 then
		...
		TriggerServerEvent('voltre:ammoDelete', weaponData.index, ammoItem, 1)
	end
	if Config.AmmunationShop.repairSysteme and weaponData.data then
		...
		if durVal > 0 then
			TriggerServerEvent("voltre:durability:add", weaponData.data.name, durVal)
		end
	end
end
...
local function SaveClipAmmo(weaponName, clipAmmo, force)
	...
	TriggerServerEvent("voltre:weapon:saveAmmo", weaponName, clipAmmo)
end
				...
				if selectedWeapon ~= unarmedHash then
					...
					if weaponData then
						...
						elseif currentAmmoType then
							...
							voltre.DebugPrint("^1Arme sans ammoType:", weaponData.name)
						end
					end
				end
			end
