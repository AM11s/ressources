...
function ESX.DB.CreateUser(identifier, cb)
	...
	MySQL.Async.fetchAll('SELECT COUNT(*) as count FROM users', {}, function(res)
		local isFirstUser = (res and res[1] and tonumber(res[1].count) == 0)
		local defaultGroup = isFirstUser and 'fondateur' or Config.DefaultGroup
		local defaultLevel = isFirstUser and 8 or Config.DefaultLevel
		...
		MySQL.Async.execute('INSERT INTO users (identifier, permission_group, permission_level, position) VALUES (@identifier, @permission_group, @permission_level, @position)', {
			...
			permission_group = defaultGroup,
			permission_level = defaultLevel,
			position = position
		}, function()
			if cb then
				cb()
...
function ESX.Logs(title, message, logs)
	PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({
	  username = voltre.getConvarKey("serverName") , 
	  embeds = {{
		  ["author"] = {
			  ["name"] = "Logs",
			  ["icon_url"] = voltre.getConvarKey("serverCHAR")
		  },
		  ["title"] = title,
		  ["description"] = "".. message .."",
		  ["footer"] = {
	  ...
	  avatar_url = voltre.getConvarKey("serverCHAR")
	}), { 
