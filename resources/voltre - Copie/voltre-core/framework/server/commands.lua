	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				voltre.logs.job("setjob", xPlayer, args[2], args[3], xPlayer2)
			else
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				voltre.logs.job("setjob2", xPlayer, args[2], args[3], xPlayer2)
			else
				ESX.ChatMessage(source, 'Le job ou grade existe pas')
			end
		else
	...
	if xTarget then
		...
		if count then
			if ESX.Items[item] then
				...
				voltre.logs.item("give", xPlayer, ESX.GetItemLabel(item), count, xTarget)
			else
	...
	if xTarget then
		...
		if ESX.GetWeapon(weaponName) then
			if xTarget.hasWeapon(weaponName) then
				...
				voltre.logs.weapon("give", xPlayer, ESX.GetWeaponLabel(weaponName), xTarget)
			end
		else
			...
			if xPlayer.hasWeapon(weaponName) then
				...
				if component then
					if xPlayer.hasWeaponComponent(weaponName, args[3]) then
						...
						voltre.logs.adminCommand("giveweaponcomponent", xPlayer2, xPlayer, args[3].." sur "..weaponName)
					end
				else
...
RegisterCommand("clear", function(source)
	if (source == 0) then
		...
		if (playerSelected.getGroup() == "fondateur" or playerSelected.getGroup() == "responsable") then
			TriggerClientEvent("chat:clear", -1)
			voltre.logs.adminCommand("clear", playerSelected)
		else
...
ESX.AddGroupCommand('clearinventory', 'fondateur', function(source, args, user)
	...
	if xPlayer then
		...
		voltre.logs.adminCommand("clearinventory", xPlayer2, xPlayer)
	else
		ESX.ChatMessage(source, 'Le joueur est déconecter')
	end
...
ESX.AddGroupCommand('clearloadout', 'fondateur', function(source, args, user)
	...
	if xPlayer then
		...
		voltre.logs.adminCommand("clearloadout", xPlayer2, xPlayer)
	else
		ESX.ChatMessage(source, 'Le joueur est déconecter')
	end
			...
			for k, v in pairs(listwhitelist) do
				if args[1] == v.model then
					if args[2] then
						...
						voltre.logs.vehicle("spawn", xPlayer, args[1])
					else
...
ESX.AddGroupCommand('dv', 'helper', function(source, args, user)
	...
	voltre.logs.vehicle("delete", xPlayer, "radius: "..args[1])
...
ESX.AddGroupCommand('extras', 'admin', function(source, args, user)
	TriggerClientEvent("voltre:client:openExtrasMenu", source)
