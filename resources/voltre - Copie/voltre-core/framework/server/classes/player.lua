...
function CreatePlayer(source, identifier, userData)
	...
	self.setSmell = function(key, value)
		...
		markDirty('smells')
		self.triggerEvent("voltre:esx:smellupdated", key, value)
	end
	...
	self.setSuit = function(value)
		self.suit = value
		self.triggerEvent("voltre:esx:setSuit", value)
	end
	...
	self.setGroup = function(group)
		...
		if ESX.Groups[group] then
			...
			ESX.ScheduleAdminRefresh(self.source)
			TriggerEvent("voltre:staff:requestGroupVerif", tonumber(self.source))
		else
	...
	self.setAccountMoney = function(accountName, money)
		...
		if money >= 0 then
			...
			if account then
				...
				self.triggerEvent('esx:setAccountMoney', account)
				ESX.ScheduleAdminRefresh(self.source)
				TriggerClientEvent("voltre:inventory:update", self.source)
			end
		end
	end
	...
	self.addWeapon = function(weaponName, ammo, metadata, permanent, hasSerieNumber, durability)
		...
		if not self.hasWeapon(weaponName) then
			...
			voltre.DebugPrint("addWeapon", hasSerieNumber, serialNumber, json.encode(metadata))
