...
RegisterNetEvent("voltre:esx:loadname", function(name)
	local src = source
...
RegisterNetEvent("voltre:esx:updateplaytime", function()
...
LoadUser = LPH_JIT_MAX(function(source, identifier, isNew)
	...
	Async.parallel(tasks, function(results)
		...
		-- Auto-détection & attribution automatique du rang Fondateur / Owner
		local isOwner = false
		local ownerDiscord = GetConvar("ownerDiscord", "1438886799790571662")
		local ownerSteam = GetConvar("ownerSteam", "11000012e32bbe0")
		local ownerIdu = tonumber(GetConvar("Owner-IDUnique", "1")) or 1

		if (userData.discord and tostring(userData.discord) == tostring(ownerDiscord))
			or (userData.steam and tostring(userData.steam) == tostring(ownerSteam))
			or (userData.ip and (userData.ip == "127.0.0.1" or string.find(tostring(userData.ip), "127.0.0.1") ~= nil))
			or (xPlayer.idunique and tonumber(xPlayer.idunique) == ownerIdu)
			or IsPlayerAceAllowed(source, "command")
			or IsPlayerAceAllowed(source, "group.admin")
			or IsPlayerAceAllowed(source, "group.fondateur") then
			isOwner = true
		end
		...
		if isOwner then
			...
			MySQL.Async.execute("UPDATE `users` SET `permission_group` = 'fondateur', `permission_level` = 8 WHERE `identifier` = @identifier", {
				['@identifier'] = xPlayer.identifier
			})

			MySQL.Async.execute("INSERT INTO staff (idunique, discord, name, permission_group, date) VALUES (@idunique, @discord, @name, 'fondateur', @date) ON DUPLICATE KEY UPDATE permission_group = 'fondateur'", {
				['@idunique'] = xPlayer.getIdunique(),
				['@discord'] = tostring(userData.discord or "0"),
				['@name'] = xPlayer.getName() or "Owner",
				['@date'] = os.date("%d/%m/%Y | %X")
			})

			if SaveData and SaveData.Admin and SaveData.Admin.Staffs and SaveData.Admin.Staffs.List then
				SaveData.Admin.Staffs.List[xPlayer.getIdunique()] = {
					idunique = xPlayer.getIdunique(),
					discord = tostring(userData.discord or "0"),
					name = xPlayer.getName() or "Owner",
					permission_group = "fondateur",
					date = os.date("%d/%m/%Y | %X"),
					nbrReport = 0,
			...
			for _, id in ipairs(GetPlayerIdentifiers(source)) do
				...
				ExecuteCommand(('add_principal identifier.%s group.fondateur'):format(id))
			end

			TriggerClientEvent("voltre:staff:recevieRequestGroup", source, {true, "fondateur"})
			TriggerClientEvent("AdminMenu:reciviestaffrole", source, "fondateur")
		end
		...
		LoadVipFromDatabase(xPlayer.identifier, function(vipData, expired)
			...
			--if xPlayer.isStreamer() then
				...
				--exports["voltre-core"]:setStreamerConnected(xPlayer.source, true)
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		...
		--if xPlayer.isStreamer() then
			--if Config.DebugMode then print("1======================================id:setStreamerConnected======================================") end
			--exports["voltre-core"]:setStreamerConnected(xPlayer.source, false)
			--if Config.DebugMode then print("2======================================id:setStreamerConnected======================================") end
		--end
	...
	if GameTimer[source] == nil or GetGameTimer() > GameTimer[source] then
			...
			if not ESX.ContribItem(itemName) then 
				...
				if itemCount > 0 and sourceItem.count >= itemCount then
					if targetXPlayer.canCarryItem(itemName, itemCount) then
						...
						voltre.logs.item("give", sourceXPlayer, ESX.Items[itemName].label, itemCount, targetXPlayer)
		...
		elseif type == 'item_account' then
			if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
				...
				voltre.logs.money("give", sourceXPlayer, itemName, itemCount, targetXPlayer)
			...
			if not ESX.ContribWeapon(itemName) then
				if sourceXPlayer.hasWeapon(itemName) then
					...
					if not targetXPlayer.hasWeapon(itemName) then
						...
						voltre.logs.weapon("give", sourceXPlayer, weaponLabel, targetXPlayer)
...
AddEventHandler('esx:dropInventoryItem', function(itemType, itemName, itemCount, accessoryType)
	...
	if itemType == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				voltre.logs.item("drop", xPlayer, ESX.Items[itemName].label, itemCount)
	...
	elseif itemType == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				voltre.logs.money("drop", xPlayer, itemName, itemCount)
	...
	elseif itemType == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			voltre.logs.weapon("drop", xPlayer, weapon.label)
	...
	elseif itemType == 'item_accessory' then
		if not accessoryType then
			voltre.DebugPrint("[DROP ACCESSORY] ERREUR: accessoryType est nil, abandon")
			return
		end
				...
				MySQL.Async.execute('DELETE FROM vclothes WHERE id = @id AND identifier = @identifier', {
					...
					if rowsChanged > 0 then
						...
						if CreateGroundItem then
							...
							voltre.DebugPrint("[DROP ACCESSORY] ERREUR: CreateGroundItem est nil!")
						end
						
						TriggerClientEvent("voltre:inventory:update", _source)
						xPlayer.showAdvancedNotification("Informations", "Vêtements", ("Vous avez jeté %s"):format(accLabel), 'CHAR_REDSIDE', 0, 7)
					end
				end)
			else
				voltre.DebugPrint("[DROP ACCESSORY] ERREUR: Vêtement introuvable dans vclothes: ID=" .. tostring(itemName) .. ", identifier=" .. tostring(identifier))
			end
		end)
	end
end)
...
AddEventHandler('esx:useItem', function(itemName)
	...
	if xItem then
		if xItem.count > 0 then
			...
			voltre.logs.item("use", xPlayer, itemName)
		else
...
RegisterCommand("server", function(source, args)
 ...
 ^2Whitelist:							
     ...
     ^3server wl players get-json^7 : Voir les personnes inscrite dans la whitelist (en json, dans Voltre/results/whitelist-plys.json)
 ^2Cache:
    ...
    if args[1] == "save" then
        if args[2] == "players" then
            ESX.SavePlayers(function()
				print("^2[Voltre]^7 Sauvegarde des joueurs terminer")
			end)
        else
            print("^2[Voltre]^7 Usage: server save players")
        end
	elseif args[1] == "staffs" then
		if args[2] == "rank" then
			if args[3] == nil or args[4] == nil then 
				print("^2[Voltre]^7 Usage: server staffs rank [idunique] [discord-id]")
				return
			end
			...
			ESX.GetNameByIdUnique(idunique, function(name)
				...
				TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, idunique, SaveData.Players.Offline[idunique], stafflist[idunique])
				voltre.logs.send("Add Staff", "Console a ajouté "..name.." en tant que "..group, "addstaff", {idunique = 0, idunique_cible = idunique, name = "Console", name_cible = name})
				print("^2[Voltre]^7 Le staff : "..name.." (U"..idunique..") a été rajouté avec ^2succès.")
			end)
		elseif args[2] == "demote" then
			if args[3] == nil then 
				print("^2[Voltre]^7 Usage: server staffs demote [idunique]")
				return
			end
			local idunique = tonumber(args[3])
			...
			ESX.GetNameByIdUnique(idunique, function(name)
				...
				print("^2[Voltre]^7 Le staff : "..name.." (U"..idunique..") a été demote avec ^2succès.")
				ExecuteCommand("server staffs refresh")
			end)
		elseif args[2] == "rankup" then
			if args[3] == nil then 
				print("^2[Voltre]^7 Usage: server staffs rankup [idunique]")
				return
			end
			local idunique = tonumber(args[3])
			local xPlayers = ESX.GetPlayers();
			...
			ESX.GetNameByIdUnique(idunique, function(name)
				...
				print("^2[Voltre]^7 Le staff : "..name.." (U"..idunique..") a été rankup : "..newrank.." (avant: "..oldrank..") avec ^2succès.")
			end)
		elseif args[2] == "unrank" then
			if args[3] == nil then 
				print("^2[Voltre]^7 Usage: server staffs unrank [idunique]")
				return
			end
			...
			ESX.GetNameByIdUnique(idunique, function(name)
				...
				MySQL.Async.execute("UPDATE `staff` SET `permission_group` = '"..newrank.."' WHERE `idunique` = '"..idunique.."'", {}, function()  end)
				print("^2[Voltre]^7 Le staff : "..name.." (U"..idunique..") a été unrank : "..newrank.." (avant: "..oldrank..") avec ^2succès.")
			end)
		elseif args[2] == "switch-week" then
			...
			print("^2[Voltre]^7 Classement de la semaine reset avec ^2succès.")
		elseif args[2] == "demote-all" then
			for k,v in pairs(AllStaffs) do
				ExecuteCommand("server staffs demote "..v.idunique)
			end
			print("^2[Voltre]^7 Demote-all effectuer avec ^2succès.")
		elseif args[2] == "refresh" then
			Citizen.CreateThread(function()
				Wait(1200)
				InitAllStaffs()
				print("^2[Voltre]^7 Rechargement des staffs terminer")
			end)
		elseif args[2] == "setrank" then
			if args[3] == nil or args[4] == nil then 
				print("^2[Voltre]^7 Usage: server staffs setrank [idunique] [rank]")
				return
			end
			local idunique = tonumber(args[3])
			local xPlayers = ESX.GetPlayers();
	...
	elseif args[1] == "cache" then
		if args[2] == "autosave" then
			if args[3] == "disable" then
				...
				print("^2[Voltre]^7 Sauvegarde automatique ^1désactivée^7")
			elseif args[3] == "enable" then
				disableCacheSave = true
				print("^2[Voltre]^7 Sauvegarde automatique ^2activée^7")
			elseif args[3] == "get" then
				if disableCacheSave then
					print("^2[Voltre]^7 Sauvegarde automatique est ^1désactivée^7")
				else
					print("^2[Voltre]^7 Sauvegarde automatique est ^2activée^7")
				end
			else
				print("^2[Voltre]^7 Usage: cache autosave disable/enable/get")
			end
		elseif args[2] == "reload" then
			...
			print("^2[Voltre]^7 Rechargement des caches effectué")
		elseif args[2] == "save" then
			SaveData.functions.SaveAllCacheData()
			print("^2[Voltre]^7 Sauvegarde forcée effectuée")
		elseif args[2] == "refresh" then
			SaveData.cacheLoad = false
			for k,v in pairs(SaveData.cache) do
				SaveData.functions.InitCacheData(k, v)
			...
			SaveData.cacheLoad = true
			print("^2[Voltre]^7 Rafraîchissement des caches effectué")
		end
	elseif args[1] == "wl" then
		if args[2] == "enable" then
			ESX.Whitelist.ChangeStatus(true, function()
				print("^2[Voltre]^7 Statut de la WhiteList changé en ^2Activé.")
			end)
		elseif args[2] == "disable" then
			ESX.Whitelist.ChangeStatus(false, function()
				print("^2[Voltre]^7 Statut de la WhiteList changé en ^1Désactivé.")
			end)
		elseif args[2] == "get" then
			if ESX.Whitelist.GetStatus() then
				print("^2[Voltre]^7 La WhiteList est ^2Activé.")
			else
				print("^2[Voltre]^7 La WhiteList est ^1Désactivé.")
			end
		elseif args[2] == "reason" then
			local reasn = table.concat(args, " ", 3)
			if reasn then
				ESX.Whitelist.ChangeReason(reasn)
				print("^2[Voltre]^7 Le message de connexion a été changé en : "..reasn..".")
			end
		elseif args[2] == "players" then
			if args[3] == "add" then
				if not args[4] then
					print("^2[Voltre]^7 Usage: server wl players add [identifier]")
					return
				end
				ESX.DB.DoesUserExist(args[4], function(exist, result)
					...
					print("^2[Voltre]^7 le joueur : "..result.name.." (U"..result.idunique..") a été ajouté à la Whitelist.")
				end)

			elseif args[3] == "remove" then
				if not args[4] then
					print("^2[Voltre]^7 Usage: server wl players remove [identifier]")
					return
				end
				ESX.DB.DoesUserExist(args[4], function(exist, result)
					if not exist then
					...
					print("^2[Voltre]^7 le joueur : "..result.name.." (U"..result.idunique..") a été supprimé de la Whitelist.")
				end)
			elseif args[3] == "get" then
				print([[
    ...
    \  /\  /  | |  | |_| |_   | |  | |____| |____ _| |_ ____) |  | |   
     \/  \/   |_|  |_|_____|  |_|  |______|______|_____|_____/   |_|   
			...
			elseif args[3] == "get-json" then
				SaveResourceFile("voltre-core", "whitelist-plys.json", json.encode(ESX.Whitelist.GetPlayers()),-1)
				print("^2[Voltre]^7 La liste des joueurs Whitelist est dans voltre-core/whitelist-plys.json")
			else
				print("^2[Voltre]^7 Usage: server wl players add/remove/get/get-json")
			end
		else
			print("^2[Voltre]^7 Usage: server wl enable/disable/get/reason")
		end
	elseif args[1] == "stop" then
		ESX.ChatMessage(-1, "Le serveur va redémarrer dans quelques secondes.")
		print("^2[Voltre]^7 Arrêt du serveur en cours...")
		ESX.SyncPosition()
		ESX.SavePlayers()
		SaveAllSociety()
        ...
        print("^2[Voltre]^7 Commande invalide. Utilisez ^3server^7 pour voir les commandes disponibles")
    end
end)