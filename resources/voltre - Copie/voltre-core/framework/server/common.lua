...
function ESX.ScheduleAdminRefresh(source)
    ...
    SetTimeout(1000, function()
        ...
        TriggerEvent("voltre:admin:refresh:server", source, "forAllStaff")
    end)
end
...
MySQL.ready(function()
	...
	MySQL.Async.fetchAll('SELECT * FROM jobs', {}, function(result)
		for i = 1, #result do
			...
			TriggerEvent('Voltre:esx_phone:registerNumber', result[i].name, result[i].label, true, true)
			TriggerEvent('Voltre:esx_society:registerSociety', result[i].name, result[i].label, 'society_'..result[i].name, 'society_'..result[i].name, 'society_'..result[i].name, {type = 'private'})
		end
		...
		MySQL.Async.fetchAll('SELECT * FROM job_grades', {}, function(result2)
			for i = 1, #result2 do
				...
				if ESX.Jobs[result2[i].job_name] then
					...
					print(('[^1Voltre^7] Le job : "%s", inscrit dans : job_grades, n\'est pas inscrit dans : job.'):format(result2[i].job_name))
			...
			for k, v in pairs(ESX.Jobs) do
				if ESX.Table.SizeOf(v.grades) == 0 then
					ESX.Jobs[v.name] = nil
					print(('[^1Voltre^7] Le job : "%s", inscrit dans : job, n\'est pas inscrit dans : job_grades.'):format(v.name))
...
RegisterNetEvent('Voltre:CreateNewItem')
AddEventHandler('Voltre:CreateNewItem', function(value)
