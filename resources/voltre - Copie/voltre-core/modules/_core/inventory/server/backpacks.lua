...
MySQL.ready(function()
    MySQL.Async.execute([[
        ...
        -- Add bag_value and bag_texture columns if they don't exist (for existing tables)
        MySQL.Async.execute([[
            ALTER TABLE `vbackpacks` 
            ADD COLUMN IF NOT EXISTS `bag_value` INT NOT NULL DEFAULT 0 AFTER `clothe_id`,
            ADD COLUMN IF NOT EXISTS `bag_texture` INT NOT NULL DEFAULT 0 AFTER `bag_value`,
            ADD COLUMN IF NOT EXISTS `custom_name` VARCHAR(255) DEFAULT NULL AFTER `bag_texture`;
        ]], {}, function()
            --voltre.DebugPrint("[Backpacks] SQL migration complete")
        end)
    end)
end)
...
-- Server callbacks
ESX.RegisterServerCallback('voltre:backpack:load', function(source, cb, clotheId)
...
ESX.RegisterServerCallback('voltre:backpack:addItem', function(source, cb, clotheId, itemName, count, itemExtra)
...
ESX.RegisterServerCallback('voltre:backpack:removeItem', function(source, cb, clotheId, itemName, count, itemExtra)
    Backpacks.RemoveItem(source, clotheId, itemName, count, itemExtra, function(success, contents, weight, maxWeight)
        cb(success, weight, maxWeight)
    end)
end)

ESX.RegisterServerCallback('voltre:backpack:addWeapon', function(source, cb, clotheId, weaponName)
    Backpacks.AddWeapon(source, clotheId, weaponName, function(success, contents, weight, maxWeight)
        cb(success, weight, maxWeight)
    end)
end)

ESX.RegisterServerCallback('voltre:backpack:removeWeapon', function(source, cb, clotheId, weaponName)
    Backpacks.RemoveWeapon(source, clotheId, weaponName, function(success, contents, weight, maxWeight)
        cb(success, weight, maxWeight)
    end)
end)

ESX.RegisterServerCallback('voltre:backpack:transferMoney', function(source, cb, clotheId, moneyType, amount, toBackpack)
    Backpacks.TransferMoney(source, clotheId, moneyType, amount, toBackpack, function(success, contents)
        cb(success)
    end)
end)

ESX.RegisterServerCallback('voltre:backpack:preview', function(source, cb, clotheId)
    Backpacks.GetPreview(source, clotheId, function(success, preview)
        cb(success, preview)
    end)
end)

ESX.RegisterServerCallback('voltre:backpack:canReceive', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false) end
...
ESX.RegisterServerCallback('voltre:backpack:register', function(source, cb, clotheId, bagValue, bagTexture)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false, "Joueur introuvable") end
...
ESX.RegisterServerCallback('voltre:backpack:getAllBags', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then
        return cb({})
    end
...
ESX.RegisterServerCallback('voltre:backpack:transferOwnership', function(source, cb, targetPlayerId, clotheId)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:backpack:getBagWeight', function(source, cb, clotheId)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:backpack:belongsToPlayer', function(source, cb, clotheId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false) end
...
RegisterNetEvent('voltre:backpack:giveToPlayer')
AddEventHandler('voltre:backpack:giveToPlayer', function(targetPlayerId, clotheId, itemName)
        ...
        Backpacks.CountPlayerBackpacks(tPlayer.identifier, function(count)
            ...
            Backpacks.TransferOwnership(xPlayer.identifier, tPlayer.identifier, clotheId, function(success, message)
                if success then
                    ...
                    TriggerClientEvent("voltre:inventory:update", targetPlayerId)
                else
...
RegisterNetEvent('voltre:backpack:dropBag')
AddEventHandler('voltre:backpack:dropBag', function(clotheId)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('voltre:backpack:depositToStorage', function(source, cb, cacheKey, clotheId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false, "Joueur introuvable") end
    ...
    Backpacks.BelongsToPlayer(identifier, clotheId, function(belongs)
        ...
        MySQL.Async.fetchAll('SELECT bag_value, bag_texture, custom_name, contents FROM vbackpacks WHERE identifier = @identifier AND clothe_id = @clotheId', {
            ...
            TriggerClientEvent("voltre:inventory:update", source)
            cb(true)
        end)
    end)
end)
...
ESX.RegisterServerCallback('voltre:backpack:withdrawFromStorage', function(source, cb, cacheKey, bagItemName)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false, "Joueur introuvable") end
...
--voltre.InitPrint("Inventory Backpacks module loaded")
