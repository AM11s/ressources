...
--voltre.InitPrint("Inventory Server modules loaded")
...
ESX.RegisterServerCallback('voltre:getCoffre', function(source, cb, chestName, chestType, clientMaxWeight)
    local invType
...
ESX.RegisterServerCallback('voltre:getCoffre2', function(source, cb)
...
ESX.RegisterServerCallback("voltre:fouiller", function(source, cb, player)
...
ESX.RegisterServerCallback('voltre:getCoffreForRefresh', function(source, cb, id, invType)
...
RegisterNetEvent("voltre:closeCoffre", function(name)
...
ESX.RegisterServerCallback('voltre:inv:transfertCashToStorage', function(source, cb, id, count, moneyType, invType)
    local cacheKey = ("%s_%s"):format(invType, id)
...
ESX.RegisterServerCallback('voltre:inv:transfertStorageToCash', function(source, cb, id, count, moneyType, invType)
    local cacheKey = ("%s_%s"):format(invType, id)
...
ESX.RegisterServerCallback('voltre:inv:transfertItemToStorage', function(source, cb, id, name, count, invType, maxWeight, extra)
    local cacheKey = ("%s_%s"):format(invType, id)
...
ESX.RegisterServerCallback('voltre:inv:transfertStorageToItem', function(source, cb, id, name, count, invType, extra)
    local cacheKey = ("%s_%s"):format(invType, id)
...
ESX.RegisterServerCallback('voltre:inv:transfertWeaponToStorage', function(source, cb, id, name, invType, metadata, maxWeight)
    local cacheKey = ("%s_%s"):format(invType, id)
...
ESX.RegisterServerCallback('voltre:inv:transferStorageToWeapon', function(source, cb, id, name, invType, metadata)
    local cacheKey = ("%s_%s"):format(invType, id)
...
ESX.RegisterServerCallback('voltre:inv:getClothe', function(source, cb)
    InventoryClothes.Load(source, cb)
end)

ESX.RegisterServerCallback('voltre:inv:getClotheSkin', function(source, cb, clotheId)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('newInventory:getAllItems', function(source, cb)
    ...
    if not (Config.GroupeHighPerm[xPlayer.getGroup()] == true) then return cb(nil) end

    local items = {}

    for name, v in pairs(ESX.GetItemList()) do
        table.insert(items, {
            type = 'item',
            name = name,
    ...
    -- Money accounts
    table.insert(items, { type = 'account', name = 'money',     label = 'Argent propre',  count = 999999999 })
...
--voltre.InitPrint("Inventory Core Module loaded")
