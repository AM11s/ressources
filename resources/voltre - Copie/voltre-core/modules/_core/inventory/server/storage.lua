...
function Storage.SaveFromCache(cacheKey)
    ...
    if invType == "VEHICLE" or invType == "VEHICLE_GLOVE_BOX" then
        MySQL.Async.execute('UPDATE vtrunk SET coffre = @coffre WHERE plate = @plate', {
            ...
            --voltre.DebugPrint("[Inventory] VEHICLE save affected rows: " .. tostring(affectedRows))
        end)
    else
        MySQL.Async.execute('UPDATE vstorage SET coffre = @coffre WHERE name = @name', {
            ...
            --voltre.DebugPrint("[Inventory] STORAGE save affected rows: " .. tostring(affectedRows))
        end)
    end

    InventoryCache.ClearDirty(cacheKey)
    --voltre.DebugPrint("[Inventory] Saved cache: " .. cacheKey)
end

AddEventHandler("voltre:inventory:saveCache", function(cacheKey)
    Storage.SaveFromCache(cacheKey)
end)
...
function AddItemToStorage(storagename, name, number)
    ...
    if cached then
        ...
        --voltre.DebugPrint("[CACHE] Added Item "..name.." ("..number.."x) to cached storage: "..storagename)
        p:resolve(true)
    else
        MySQL.Async.fetchAll('SELECT coffre, id FROM vstorage WHERE name = @name', {
            ['@name'] = storagename
            ...
            if result[1] then
                ...
                MySQL.Async.execute("UPDATE `vstorage` SET `coffre` = '"..json.encode(inventory).."' WHERE `name` = '"..storagename.."'", {}, function()
                    --voltre.DebugPrint("[SQL] Added Item "..name.." ("..number.."x) to storage : "..storagename)
                    p:resolve(true)
                end)
            else
                voltre.DebugPrint("[SQL] Storage not found: "..storagename)
                p:resolve(false)
            end
        end)
    end
...
function RemoveItemToStorage(storagename, name, number)
    ...
    if cached then
        ...
        InventoryCache.MarkDirty(cacheKey)
        --voltre.DebugPrint("[CACHE] Removed Item "..name.." ("..number.."x) from cached storage: "..storagename)
    else
        MySQL.Async.fetchAll('SELECT coffre, id FROM vstorage WHERE name = @name', {
            ['@name'] = storagename
        }, function (result)
...
function AddWeaponToStorage(storagename, name, description)
    ...
    if cached then
        ...
        InventoryCache.MarkDirty(cacheKey)
        --voltre.DebugPrint("[CACHE] Added weapon "..name.." to cached storage: "..storagename)
    else
        MySQL.Async.fetchAll("SELECT coffre, id FROM vstorage WHERE name = @name", {
            ['@name'] = storagename
        }, function(result)
            if result[1] then
                ...
                MySQL.Async.execute("UPDATE `vstorage` SET `coffre` = '"..json.encode(inventory).."' WHERE `name` = '"..storagename.."'", {}, function()  end)
                --voltre.DebugPrint("[SQL] Added weapon "..name.." (Description: "..description..") to storage : "..storagename)
            end
        end)
    end
end
...
function RemoveWeaponToStorage(storagename, name)
    ...
    if cached then
        ...
        InventoryCache.MarkDirty(cacheKey)
        --voltre.DebugPrint("[CACHE] Removed weapon "..name.." from cached storage: "..storagename)
    else
        MySQL.Async.fetchAll('SELECT coffre, id FROM vstorage WHERE name = @name', {
            ['@name'] = storagename
        }, function (result)
