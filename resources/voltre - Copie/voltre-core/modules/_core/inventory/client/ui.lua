...
local InventoryPedConfig = {
    ...
    isOpenFn = function() return VoltreInventory.isOpen end,
...
function RefreshInventoryPed()
    ...
    local useClonePed = exports["voltre-core"]:getPreference("cloneped")
...
RegisterNetEvent("inventory:update", function(data, inventory)
    ...
    if inventory.id == VoltreInventory.currentChest.data.id then
        VoltreInventory.currentChest.data = data
        VoltreInventory.currentChest.weight = inventory.weight
...
UpdateInventoryStats = LPH_JIT(function()
    ...
    pcall(function()
        stats.hunger = exports["voltre-core"]:getStatus("hunger") / 10000
        stats.thirst = exports["voltre-core"]:getStatus("thirst") / 10000
        stats.alcohol = exports["voltre-core"]:getStatus("drunk") / 10000
        stats.drug = exports["voltre-core"]:getStatus("drug") / 10000
    end)
...
AddEventHandler("voltre:inventory:createClonePed", function()
    SetTimeout(120, CreateInventoryPed)
end)

AddEventHandler("voltre:inventory:destroyClonePed", function()
    DestroyInventoryPed()
end)

AddEventHandler("voltre:inventory:refreshClonePed", function()
    RefreshInventoryPed()
end)
...
_G.VoltreInventoryUILoaded = true
