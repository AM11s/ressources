        ...
        while menu do
            RageUI.IsVisible(menu, function ()
                ...
                RageUI.Button("Déposer de l'argent", nil, {}, true, {
                    onSelected = function ()
                        voltre.fct.inputCb('Montant', function(data)
                            if data == nil or data == '' then
                                return ESX.ShowNotification("~r~Le montant ne peut pas être nul", "Erreur")
                            end
                            data = tonumber(data)

                            ESX.TriggerServerCallback('voltre:inv:transfertCashToStorage', function(final)
                                if final then
                ...
                RageUI.Button("Retirer de l'argent", nil, {}, true, {
                    onSelected = function ()
                        voltre.fct.inputCb('Montant', function(data)
                            ...
                            ESX.TriggerServerCallback('voltre:inv:transfertStorageToCash', function(final)
                                if final then
                                    currentDistantInventory.cash = currentDistantInventory.cash - data
                                end
                            end, currentDistantInventory.id, data, "cash", currentDistantInventory.type)
            ...
            RageUI.IsVisible(menuDepositObject, function ()
                for i = 1, #ESX.PlayerData.inventory, 1 do
                    ...
                    RageUI.Button(("%s"):format(label), nil, { RightLabel = ("%s"):format(item.count) }, true, {
                        onSelected = function ()
                            voltre.fct.inputCb('Montant', function(result)
                                if result == nil or result == '' then
                                    return ESX.ShowNotification("~r~Le montant ne peut pas être nul", "Erreur")
                                end
                                result = tonumber(result)
                                
                                ESX.TriggerServerCallback('voltre:inv:transfertItemToStorage', function(final)
                                    if final then
                                        -- Todo Refresh l'inv
                                        local find = false
                    ...
                    RageUI.Button(("%s"):format(label), nil, { RightLabel = ("%s"):format(item.count) }, true, {
                        onSelected = function ()
                            voltre.fct.inputCb('Montant', function(result)
                                ...
                                ESX.TriggerServerCallback('voltre:inv:transfertStorageToItem', function(final)
                                    if final then
            ...
            RageUI.IsVisible(menuDepositWeapon, function ()
                for i = 1, #ESX.PlayerData.loadout, 1 do
                    ...
                    RageUI.Button(("%s"):format(label), nil, {}, true, {
                        onSelected = function ()
                            ESX.TriggerServerCallback('voltre:inv:transfertWeaponToStorage', function(final)
                                if final then
            ...
            RageUI.IsVisible(menuWithdrawWeapon, function ()
                for i = 1, #currentDistantInventory.loadout, 1 do
                    ...
                    RageUI.Button(("%s"):format(label), nil, {}, true, {
                        onSelected = function ()
                            ESX.TriggerServerCallback('voltre:inv:transferStorageToWeapon', function(final)
                                if final then
            ...
            then
                ...
                TriggerServerEvent("voltre:closeCoffre", currentDistantInventory.savename)
...
RegisterNetEvent("inventory:update", function (data, inventory)
    ...
    if not exports["voltre-core"]:getPreference("interfaces") then return end
...
RegisterNetEvent("inventory:openTarget", function (target)
    ...
    if not exports["voltre-core"]:getPreference("interfaces") then
        RageUI.CloseAll()
    ...
    exports["voltre-core"]:openInventory({
        name = target.type,
