...
-- Routes React NUI callbacks to existing voltre inventory events
-- Open/Close/Update are handled by VoltreInventory in main.lua
-- ============================================================================

while not _G.VoltreInventoryClientLoaded do
    Wait(0)
end
...
RegisterNUICallback('newInventory:shortcutSet', function(data, cb)
    exports["voltre-core"]:setWeaponKeybind(VoltreInventory._weaponBinds.numbers[data.slot-1], data.item.name, data.item.label)
    cb({})
end)

RegisterNUICallback('newInventory:shortcutRemove', function(data, cb)
    exports["voltre-core"]:setWeaponKeybind(VoltreInventory._weaponBinds.numbers[data.slot-1], "WEAPON_UNARMED", "Aucun")
    cb({})
end)
...
RegisterNUICallback('newInventory:kevlarUnequip', function(_, cb)
    TriggerEvent("voltre:kevlar:removeItem")
    cb('ok')
end)
...
RegisterNUICallback('newInventory:removeAllClothes', function(_, cb)
    cb({})
    if not VoltreInventory.isOpen then return end
    
    VoltreInventory._lastOutfit = {}
    if VoltreInventory.equippedClothes then
        for k, v in pairs(VoltreInventory.equippedClothes) do
            VoltreInventory._lastOutfit[k] = v
        end
    end
...
RegisterNUICallback('newInventory:restoreLastOutfit', function(_, cb)
    cb({})
    if not VoltreInventory.isOpen then return end
    if not VoltreInventory._lastOutfit or next(VoltreInventory._lastOutfit) == nil then 
    ...
    for clotheType, clotheData in pairs(VoltreInventory._lastOutfit) do
...
RegisterNUICallback('newInventory:settingChange', function(data, cb)
    ...
    TriggerEvent("voltre:inventory:settingChanged", data.id, data.value)
    -- Persist setting
    pcall(function()
        exports["voltre-core"]:setPreference(data.id, data.value)
    end)
end)
...
RegisterNUICallback('newInventory:togglePreference', function(data, cb)
    ...
    elseif id == "_hud" then
        ...
        voltre.DisplayHud(newEnabled, 54) 
        SendNUIMessage({ action = "newInventory:updatePreference", data = { id = id, enabled = newEnabled } })
        return
...
RegisterNetEvent("ZgegFramework:changeShortCut", function(name, weapon, isNumber)
    if not VoltreInventory.isOpen then return end
    local id = isNumber and tonumber(name) or VoltreInventory._weaponBinds.string[name]
...
RegisterNUICallback('newInventory:outfitCreate', function(data, cb)
    ...
    -- Ask for outfit name
    local outfitName = voltre.fct.input("Nom de la tenue", GetCurrentResourceName())
    ...
    ESX.TriggerServerCallback('voltre:outfits:create', function(success)
        if success then
            ...
            VoltreInventory.GetClothes(function(clothes)
...
RegisterNUICallback('newInventory:outfitDisassemble', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:outfits:disassemble', function(success, createdPieces)
        if success then
    ...
    ESX.TriggerServerCallback('voltre:outfits:equip', function(success, skinChanges, newEquipped)
        if success and skinChanges then
            ...
            if newEquipped then
                ...
                -- Save to server
                TriggerServerEvent("voltre:inventory:saveEquippedClothes", newEquipped)
            end
...
function SendBackpackDataToUI()
    TriggerEvent("Voltre:skinchanger:getSkin", function(skin)
        ...
        ESX.TriggerServerCallback('voltre:backpack:getAllBags', function(bags)
            ...
            ESX.TriggerServerCallback('voltre:backpack:load', function(success, items, weight, maxWeight)
                if success then
...
RegisterNUICallback('newInventory:backpackAddItem', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:backpack:addItem', function(success, weight, maxWeight)
        if success then
            VoltreInventory.Update()
...
RegisterNUICallback('newInventory:backpackRemoveItem', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:backpack:removeItem', function(success, weight, maxWeight)
        if success then
            VoltreInventory.Update()
            SendBackpackDataToUI()
            RefreshBagStorage()
...
RegisterNUICallback('newInventory:backpackAddWeapon', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:backpack:addWeapon', function(success)
        if success then
            VoltreInventory.Update()
            SendBackpackDataToUI()
            RefreshBagStorage()
...
RegisterNUICallback('newInventory:backpackRemoveWeapon', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:backpack:removeWeapon', function(success)
        if success then
            VoltreInventory.Update()
            SendBackpackDataToUI()
            RefreshBagStorage()
...
RegisterNUICallback('newInventory:backpackTransferMoney', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:backpack:transferMoney', function(success)
        if success then
            VoltreInventory.Update()
            SendBackpackDataToUI()
        else
...
RegisterNUICallback('newInventory:backpackPreview', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:backpack:preview', function(success, preview)
...
function SendAdvancedCraftDataToUI(tableId)
    if tableId then
        ESX.TriggerServerCallback('voltre:crafting:getRecipes', function(recipes)
            SendNUIMessage({
        ...
        ESX.TriggerServerCallback('voltre:crafting:getRecipes', function(basicRecipes)
            ...
            -- pcall(function()
            --     if not voltre or not voltre.data or not voltre.data.illegals or not voltre.data.illegals.groups
            --         or not voltre.data.illegals.groups.list then return end

            --     local job2 = ESX.PlayerData.job2
            --     if not job2 or not job2.name then return end

            --     local gangData = voltre.data.illegals.groups.list[job2.name]
            ...
            --             table.insert(reqs, { label = "Levier", itemName = "levier", icon = "nui://voltre-core/images/items/levier.webp", amount = wData.fablevier, have = leverCount })
            --         end
            --         if wData.fabplanche and wData.fabplanche > 0 then
            --             table.insert(reqs, { label = "Planche", itemName = "planche", icon = "nui://voltre-core/images/items/planche.webp", amount = wData.fabplanche, have = plancheCount })
            --         end
            --         if wData.fabacier and wData.fabacier > 0 then
            --             table.insert(reqs, { label = "Acier Pur", itemName = "acierpur", icon = "nui://voltre-core/images/items/acierpur.webp", amount = wData.fabacier, have = acierCount })
            --         end
            --         table.insert(allRecipes, {
            --             id = weaponName, item = weaponName, label = wData.label,
            --             description = "Fabriquer " .. wData.label,
            --             icon = "nui://voltre-core/images/items/" .. weaponName .. ".webp",
...
RegisterNUICallback('newInventory:advancedCraft', function(data, cb)
    ...
    Citizen.CreateThread(function()
        ...
        ESX.TriggerServerCallback('voltre:crafting:craft', function(success)
...
RegisterNUICallback('newInventory:fetchAllItems', function(_, cb)
    ESX.TriggerServerCallback('newInventory:getAllItems', function(items)
        if items then
...
RegisterNUICallback('newInventory:fetchStaffChest', function(_, cb)
    ESX.TriggerServerCallback('voltre:getCoffre', function(data, id, weight)
        if data then
...
RegisterNUICallback('newInventory:staffChestDeposit', function(data, cb)
    ...
    if data.itemType == 'weapon' then
        ESX.TriggerServerCallback('voltre:inv:transfertWeaponToStorage', function(success)
            if success then VoltreInventory.Update() end
            cb({ success = success })
        end, "staffchest", data.itemName, "SOCIETY", data.metadata)
    elseif data.itemType == 'account' then
        ESX.TriggerServerCallback('voltre:inv:transfertCashToStorage', function(success)
            if success then VoltreInventory.Update() end
            cb({ success = success })
        end, "staffchest", data.count or 1, data.itemName, "SOCIETY")
    else
        ESX.TriggerServerCallback('voltre:inv:transfertItemToStorage', function(success)
            if success then VoltreInventory.Update() end
            cb({ success = success })
...
RegisterNUICallback('newInventory:staffChestWithdraw', function(data, cb)
    ...
    if data.itemType == 'weapon' then
        ESX.TriggerServerCallback('voltre:inv:transferStorageToWeapon', function(success)
            if success then VoltreInventory.Update() end
            cb({ success = success })
        end, "staffchest", data.itemName, "SOCIETY", data.metadata)
    elseif data.itemType == 'account' then
        ESX.TriggerServerCallback('voltre:inv:transfertStorageToCash', function(success)
            if success then VoltreInventory.Update() end
            cb({ success = success })
        end, "staffchest", data.count or 1, data.itemName, "SOCIETY")
    else
        ESX.TriggerServerCallback('voltre:inv:transfertStorageToItem', function(success)
            if success then VoltreInventory.Update() end
            cb({ success = success })
...
voltre.InitPrint("New Inventory NUI Bridge loaded")
