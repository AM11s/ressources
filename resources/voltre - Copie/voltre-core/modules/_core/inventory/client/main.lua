...
VoltreInventory.Open = LPH_JIT(function(secondInventory)
    if VoltreInventory.isOpen then
        if secondInventory then
            VoltreInventory.currentChest = {
    ...
    if not VoltreInventory.canOpen then return end
    
    VoltreInventory.isOpen = true
    _G.newInventoryOpen = true
    exports["voltre-core"]:setChatCanOpen(false)
    ...
    local playerSex = VoltreInventory.IsMale() and "male" or "female"
    local bagInfo = Config.ListBags and Config.ListBags[playerSex] or {}
    local equipped = VoltreInventory.equippedClothes or {}
    ...
    local leftData = VoltreInventory.GetPlayerInventory()
    ...
    pcall(function()
        local keybinds = exports["voltre-core"]:getWeaponKeybinds()
        if keybinds then
            for i = 0, 4 do
                local key = VoltreInventory._weaponBinds.numbers[i]
    ...
    if secondInv then
        ...
        VoltreInventory.currentChest = nil
    ...
    VoltreInventory.StartControlThread()
end)

VoltreInventory.Close = LPH_JIT(function()
    if not VoltreInventory.isOpen then VoltreInventory.isOpen = false return end
    
    if VoltreInventory.currentChest then
        if VoltreInventory.currentChest.literal and VoltreInventory.currentChest.literal.savename then
            TriggerServerEvent("voltre:closeCoffre", VoltreInventory.currentChest.literal.savename)
        end
    end
    
    VoltreInventory.isOpen = false
    _G.newInventoryOpen = false
    VoltreInventory.currentChest = nil
...
VoltreInventory.Update = LPH_JIT(function()
    ...
    VoltreInventory.GetClothes(function(clothes)
        ...
        if VoltreInventory.equippedClothes then
            for clotheType, clotheData in pairs(VoltreInventory.equippedClothes) do
...
VoltreInventory.GetPlayerInventory = LPH_JIT(function()
    local data = {}
    local playerData = ESX.GetPlayerData()
...
VoltreInventory.GetClothes = LPH_JIT(function(cb)
    local now = GetGameTimer()
    
    if VoltreInventory.clothesCache and (now - VoltreInventory.clothesCacheTime) < CLOTHES_CACHE_DURATION then
        return cb(VoltreInventory.clothesCache)
    end
    
    ESX.TriggerServerCallback('voltre:inv:getClothe', function(clothes)
        VoltreInventory.clothesCache = clothes
        VoltreInventory.clothesCacheTime = now
        cb(clothes)
    end)
end)

function VoltreInventory.IsMale()
    return GetEntityModel(PlayerPedId()) == GetHashKey("mp_m_freemode_01")
end

VoltreInventory.StartControlThread = LPH_NO_VIRTUALIZE(function()
    CreateThread(function()
        while VoltreInventory.isOpen do
            Wait(0)
...
RegisterNetEvent("voltre:inventory:closeinv", function()
    VoltreInventory.Close()
end)
...
RegisterNetEvent("voltre:inventory:update", function()
    VoltreInventory.Update()
end)

RegisterNetEvent("voltre:inventory:clearClothesCache", function()
...
RegisterNetEvent("inventory:clotheRenamed", function(clotheId, newName)
    ...
    TriggerEvent("voltre:inventory:invalidateClothesCache")
    
    -- Force UI refresh
    VoltreInventory.Update()
end)

AddEventHandler("voltre:inventory:invalidateClothesCache", function()
    VoltreInventory.clothesCache = nil
    VoltreInventory.clothesCacheTime = 0
end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    if xPlayer.clothes_equiped then
        VoltreInventory.equippedClothes = xPlayer.clothes_equiped
    ...
    TriggerEvent("voltre:inventory:nui:setTitle", "Inventaire")
end)

exports('openInventory', function(secondInventory)
    VoltreInventory.Open(secondInventory)
end)
...
exports('CanOpenInventory', function(value)
    if value ~= nil then
        VoltreInventory.canOpen = value
    end
    return VoltreInventory.canOpen
end)
...
RegisterCommand("openCoffre", function()
    if VoltreInventory.isOpen then return end
...
_G.VoltreInventoryClientLoaded = true
voltre.InitPrint("Inventory Client Module loaded")