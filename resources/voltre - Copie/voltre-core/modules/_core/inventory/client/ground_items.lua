...
RegisterNetEvent('voltre:groundItems:add')
AddEventHandler('voltre:groundItems:add', function(id, data)
...
RegisterNetEvent('voltre:groundItems:remove')
AddEventHandler('voltre:groundItems:remove', function(id)
    ...
    if VoltreInventory.isOpen and VoltreInventory.currentChest and VoltreInventory.currentChest.data.type == "GROUND" then
        if VoltreInventory.currentChest.data.id == id then
            TriggerEvent("voltre:inventory:closeinv")
        end
    end
end)

RegisterNetEvent('voltre:groundItems:update')
AddEventHandler('voltre:groundItems:update', function(id, data)
    if not GroundItems.items[id] then
        return
    end
...
RegisterNetEvent('voltre:groundItems:sync')
AddEventHandler('voltre:groundItems:sync', function(items)
    for id, groundItem in pairs(GroundItems.items) do
        RemoveGroundItem3DInteraction(id)
        if DoesEntityExist(groundItem.prop) then
            DeleteEntity(groundItem.prop)
    ...
    for id, data in pairs(items) do
        TriggerEvent('voltre:groundItems:add', id, data)
    end
end)
...
function PickupSingleItem(groundItemId, itemType, itemName, count, label)
    ...
    TriggerServerEvent('voltre:groundItems:withdraw', groundItemId, itemType, itemName, count)
    ...
    Wait(500)
    TriggerServerEvent('voltre:groundItems:checkAndDelete', groundItemId)
end
...
function PickupGroundItem(groundItemId)
    ...
    ESX.TriggerServerCallback('voltre:groundItems:getData', function(data)
        if not data then
        ...
        if data.items and #data.items > 0 then
            for _, item in ipairs(data.items) do
                TriggerServerEvent('voltre:groundItems:withdraw', groundItemId, 'item', item.name, item.count)
        ...
        if data.loadout and #data.loadout > 0 then
            for _, weapon in ipairs(data.loadout) do
                TriggerServerEvent('voltre:groundItems:withdraw', groundItemId, 'weapon', weapon.name, 1)
        ...
        if data.accessories and #data.accessories > 0 then
            for _, accessory in ipairs(data.accessories) do
                TriggerServerEvent('voltre:groundItems:withdraw', groundItemId, 'accessory', accessory.name, 1)
                table.insert(itemsPickedUp, accessory.label or accessory.name)
            end
        end
        
        if data.cash and data.cash > 0 then
            TriggerServerEvent('voltre:groundItems:withdraw', groundItemId, 'cash', 'cash', data.cash)
            table.insert(itemsPickedUp, ("$%d argent propre"):format(data.cash))
        end
        
        if data.dirtycash and data.dirtycash > 0 then
            TriggerServerEvent('voltre:groundItems:withdraw', groundItemId, 'dirtycash', 'dirtycash', data.dirtycash)
