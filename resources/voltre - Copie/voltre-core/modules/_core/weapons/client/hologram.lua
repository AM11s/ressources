-- ============================================================================
-- VoltreCore — Ammo Hologram (3D DUI hologram_box_model attached to the ped)
--
...
-- Initial DUI size; updated from Voltre3DUI config on ready.
...
local function InitDui()
    ...
    local txd = CreateRuntimeTxd("VoltreAmmoHologramTxd")
    local handle = GetDuiHandle(duiObject)
    CreateRuntimeTextureFromDuiHandle(txd, "VoltreAmmoHologramTex", handle)
...
local function ApplyTextureReplacement()
    ...
    AddReplaceTexture(HOLOGRAM_TXD, HOLOGRAM_TXN, "VoltreAmmoHologramTxd", "VoltreAmmoHologramTex")
...
RegisterCommand('+voltreAmmoHologram', function()
...
RegisterKeyMapping('+voltreAmmoHologram', 'Afficher les munitions (hologramme)', 'keyboard', 'E')
...
CreateThread(function()
    ...
    if Voltre3DUI then
        Voltre3DUI.Subscribe("ammo_hologram", ApplyConfig)
    end
end)
