...
local function HasWeaponBag()
    ...
    voltre.DebugPrint("bagIndex: "..tostring(bagIndex))
    local sex = PlayerState.playerSex or "male"
    voltre.DebugPrint("sex: "..tostring(sex))
    local bagData = listBags[sex] and listBags[sex][bagIndex]
    voltre.DebugPrint(bagData)
...
CreateThread(LPH_NO_VIRTUALIZE(function()
    voltre.fct.waitPlayerLoaded(500)
    ...
    while true do
		...
		if voltreGF.InZone then goto continue end
