...
RegisterNetEvent("voltre:ammoDelete", function(index, item, count)
...
RegisterNetEvent("voltre:weapon:saveAmmo", function(weaponName, clipAmmo)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
...
RegisterNetEvent("voltre:durability:add", function(name, value)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
...
voltre.InitPrint("^2Weapons module loaded")
