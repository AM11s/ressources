...
-- Auth: déjà gérée par common.lua (Bearer + IP whitelist api.voltre.fr)
...
-- ============================================
voltre.api.registerRoute("GET", "/live/players", function()
    local players = {}
...
-- ============================================
voltre.api.registerRoute("POST", "/live/broadcast", function(body)
...
-- ============================================
voltre.api.registerRoute("POST", "/live/kick", function(body)
    ...
    if voltre and voltre.logs and voltre.logs.send then
        voltre.logs.send("Dashboard", ("Dashboard a kick %s — Raison: %s"):format(xPlayer.getName(), reason), "kick")
    end
...
-- ============================================
voltre.api.registerRoute("POST", "/live/ban", function(body)
    local playerId = tonumber(body.playerId)
    if not playerId then return err("playerId requis") end
    local xPlayer = getXPlayer(playerId)
    if not xPlayer then return err("Joueur introuvable", 404) end
    ...
    if voltre and voltre.logs and voltre.logs.send then
        voltre.logs.send("Dashboard", ("Dashboard a ban %s (%dh) — Raison: %s"):format(xPlayer.getName(), duration, reason), "ban")
    end
...
-- ============================================
voltre.api.registerRoute("POST", "/live/revive", function(body)
    local playerId = tonumber(body.playerId)
    if not playerId then return err("playerId requis") end
    local xPlayer = getXPlayer(playerId)
    if not xPlayer then return err("Joueur introuvable", 404) end
    ...
    TriggerClientEvent("voltre:revive", playerId)
...
-- ============================================
voltre.api.registerRoute("POST", "/live/giveItem", function(body)
    local playerId = tonumber(body.playerId)
    ...
    if voltre and voltre.logs and voltre.logs.send then
        voltre.logs.send("Dashboard", ("Dashboard a donné %dx %s à %s"):format(count, itemName, xPlayer.getName()), "item")
    end
...
-- ============================================
voltre.api.registerRoute("POST", "/live/giveMoney", function(body)
    local playerId = tonumber(body.playerId)
    ...
    if voltre and voltre.logs and voltre.logs.send then
        voltre.logs.send("Dashboard", ("Dashboard a donné $%d (%s) à %s"):format(amount, account, xPlayer.getName()), "money")
    end
...
-- ============================================
voltre.api.registerRoute("POST", "/live/restartResource", function(body)
    local resource = body.resource
    if not resource or resource == "" then return err("Ressource requise") end
    -- Blacklist: ne pas autoriser à redémarrer voltre-core lui-même via dashboard
    if resource == GetCurrentResourceName() then return err("Impossible de redémarrer voltre-core via le dashboard") end
...
-- ============================================
voltre.api.registerRoute("GET", "/live/economy", function()
