...
function LoadDefaultModel(malePed, cb)
	...
	Citizen.CreateThread(function()
		...
		TriggerEvent('Voltre:skinchanger:modelLoaded')
	end)
end
...
RegisterNetEvent('Voltre:skinchanger:loadDefaultModel', function(loadMale, cb)
	LoadDefaultModel(loadMale, cb)
end)

RegisterNetEvent('Voltre:skinchanger:getData', function(cb)
...
RegisterNetEvent('Voltre:skinchanger:loadDefaultSkin', function()
	ESX.TriggerServerCallback('Voltre:esx_skin:getPlayerSkin', function(skin)
        TriggerEvent('Voltre:skinchanger:loadSkin', skin) 
    end)
end)

RegisterNetEvent('Voltre:skinchanger:change', function(key, val)
	Character[key] = val

	if key == 'sex' then
		TriggerEvent('Voltre:skinchanger:loadSkin', Character)
	else
		ApplySkin(Character)
		-- voltre.DebugPrint("Skin changed: " .. key .. " = " .. val)
	end
	if key == "bags_1" then
		TriggerEvent("Voltre:changeBag", val)
	end
end)

RegisterNetEvent('Voltre:skinchanger:getSkin', function(cb)
	cb(Character)
end)

RegisterNetEvent('Voltre:skinchanger:modelLoaded', function()
...
RegisterNetEvent('Voltre:skinchanger:loadSkin', function(skin, cb)
	if skin['sex'] ~= LastSex then
		LoadSkin = skin

		if skin['sex'] == 0 then
			TriggerEvent('Voltre:skinchanger:loadDefaultModel', true, cb)
		else
			TriggerEvent('Voltre:skinchanger:loadDefaultModel', false, cb)
		end
	else
		ApplySkin(skin)
...
RegisterNetEvent('Voltre:skinchanger:loadClothes:NoSkin', function(clothesSkin)
	if Character['sex'] ~= LastSex then
		...
		if Character['sex'] == 0 then
			TriggerEvent('Voltre:skinchanger:loadDefaultModel', true)
		else
			TriggerEvent('Voltre:skinchanger:loadDefaultModel', false)
		end
	else
...
RegisterNetEvent('Voltre:skinchanger:loadClothes', function(playerSkin, clothesSkin)
...
RegisterNetEvent("voltre:skinchanger:loadall", function()
	stopLoadall = false
	...
	for k,v in pairs(data) do
		...
		if v ~= -1 and v ~= 0 and (tonumber(v) - index) ~= 0 then
			while (tonumber(v) - index) > 0 do
				if stopLoadall == true then break end
				TriggerEvent("Voltre:skinchanger:change", k, index)
				index = index + 1
				print(index, v, (tonumber(v) - index))
				Wait(100)
			end
		else
			TriggerEvent("Voltre:skinchanger:change", k, 0)
		end
		Wait(100)
		::continue::
	end
	TriggerServerEvent("voltre:streams:preload:finish")
end)

RegisterNetEvent("voltre:skinchanger:stoploadall", function()
	stopLoadall = true
end)
