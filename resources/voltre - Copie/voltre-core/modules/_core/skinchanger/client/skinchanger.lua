...
function RefreshData()
    TriggerEvent("Voltre:skinchanger:getData", function(comp_, max)
        open = true
        comp = comp_
...
RegisterNetEvent('Voltre:OpenClotheMenu')
AddEventHandler('Voltre:OpenClotheMenu', function()
    RefreshData()
    SkinChangerClothe()
end)
						...
						if v.label == vv then
							RageUI.List(v.label, v.table, indexface2[v.name], nil, {}, true, {
								onListChange = function(indexface, Items)
									...
									TriggerEvent("Voltre:skinchanger:change", v.name, indexface - 1)
								...
								onSelected = function(indexface, Items)
									ESX.TriggerServerCallback('voltre:GetMoneyOutfit', function(hasEnoughMoney)
										if hasEnoughMoney then
											TriggerEvent('Voltre:skinchanger:getSkin', function(skin)
												TriggerServerEvent('Voltre:esx_skin:save', skin)
											end)
			
											ESX.TriggerServerCallback('voltre:checkPropertyDataStore', function(foundStore)
												if foundStore then
													OpenSaveOutfit()
												end
											end)
    ...
    while menu do
        ...
        RageUI.IsVisible(menu, function()
            RageUI.Button('Sauvegarder la tenue', nil, {}, true, {
                onSelected = function()
					local LabelTenue = voltre.fct.input('Quelle nom ?')
					TriggerEvent('Voltre:skinchanger:getSkin', function(skin)
						TriggerServerEvent('voltre:SauvegardeTenue', LabelTenue, skin)
					end)
...
RegisterNetEvent('Voltre:openSkinMenu')
AddEventHandler('Voltre:openSkinMenu', function()
    RefreshData()
    SkinChanger()
end)
...
function CreateCamThread()
	...
	Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
		...
		while true do
			if not isCameraActive then break end
			controlsScaleform = voltre.fct.draw.DrawInstructionalButtons(buttonsMessage)
