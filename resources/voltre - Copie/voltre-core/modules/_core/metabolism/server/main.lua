...
CreateThread(function()
    Wait(2000)
    MySQL.query('ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `metabolism` LONGTEXT NULL DEFAULT NULL', {})
end)
...
local function sync(src)
    ...
    TriggerClientEvent('voltre:metabolism:sync', src, {
...
RegisterNetEvent('voltre:metabolism:consume', function(itemName)
...
AddEventHandler('voltre:usableitem:new', function(data)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
...
AddEventHandler('voltre:editRestaurant',    function(_, craft) processCraftTable(craft) end)
AddEventHandler('voltre:createrestaurant',  function(_, _, _, _, _, _, _, craft) processCraftTable(craft) end)
...
-- ============================================================
RegisterNetEvent('voltre:metabolism:request', function()
    local src = source
