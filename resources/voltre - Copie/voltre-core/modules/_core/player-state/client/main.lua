...
CreateThread(LPH_NO_VIRTUALIZE(function()
    while true do
        ...
        if isPaused ~= lastPauseState then
            ...
            TriggerEvent('voltre:player:pauseMenu', isPaused)
        end
...
RegisterNetEvent('esx:setAccountMoney', function(account)
    ...
    voltre.DebugPrint("PlayerState Accounts Sync : "..account.name.." - "..account.money)
...
RegisterNetEvent('Voltre:esx:changeBucket', function(value)
    ...
    TriggerEvent('voltre:player:bucketChanged', value)
end)

RegisterNetEvent('voltre:player:setWeapon', function(weaponHash)
    PlayerState.lastWeapon = PlayerState.weapon
    PlayerState.weapon = weaponHash
    
    if PlayerState.weapon ~= PlayerState.lastWeapon then
        TriggerEvent('voltre:player:weaponChanged', PlayerState.weapon, PlayerState.lastWeapon)
    end
end)

voltre.InitPrint('^2Player State module loaded^7')
