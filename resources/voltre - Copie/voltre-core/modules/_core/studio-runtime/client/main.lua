...
local function applyBlip(slug, payload, label)
    ...
    AddTextComponentString(label or payload.name or "Voltre Studio")
    EndTextCommandSetBlipName(blip)

    StudioBlips[slug] = blip
end

RegisterNetEvent("voltre:studio:applyMapPoint", function(slug, payload, label)
    applyBlip(slug, payload, label)
end)

RegisterNetEvent("voltre:studio:applyShop", function(slug, payload, label)
...
RegisterNetEvent("voltre:studio:remove", function(type, slug)
