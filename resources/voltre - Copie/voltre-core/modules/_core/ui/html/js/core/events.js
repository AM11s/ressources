/**
 * Voltre UI - Système d'événements
...
const VoltreEvents = {
    ...
    emit(action, data) {
        ...
        this._handlers[action].forEach(handler => {
            try {
                ...
                console.error(`[VoltreUI] Error in handler for "${action}":`, e);
            }
        });
    },
    ...
    async post(endpoint, data = {}) {
        try {
            const response = await fetch(`https://voltre-core/${endpoint}`, {
                method: 'POST',
            ...
            console.error(`[VoltreUI] Error posting to "${endpoint}":`, e);
            return null;
        }
    },
...
// Export global
window.VoltreEvents = VoltreEvents;
