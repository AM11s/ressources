/**
 * Voltre UI - Module Notifications
 * Système de notifications et alertes
 */

const VoltreNotifications = {
    // Container DOM
    _container: null,
    ...
    init() {
        ...
        if (!this._container) {
            console.error('[VoltreNotifications] Container #notifications not found');
        }

        // Enregistrer les handlers d'événements
        VoltreEvents.on('notify', (data) => this.show(data.text, data.type, data.duration));
        VoltreEvents.on('Notify', (data) => this.show(data.text, 'info', data.duration));
    ...
    show(text, type = 'info', duration = null) {
        ...
        const id = VoltreUtils.generateId('notif');
...
// Export global
window.VoltreNotifications = VoltreNotifications;
