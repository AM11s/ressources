...
const VoltreSound = {
     ...
     */
    init() {
        VoltreEvents.on('url', (data) => this.play(data.name, data.url, data.volume, data.loop, data.dynamic, data.x, data.y, data.z));
        VoltreEvents.on('play', (data) => this.resume(data.name));
        VoltreEvents.on('pause', (data) => this.pause(data.name));
        VoltreEvents.on('resume', (data) => this.resume(data.name));
        VoltreEvents.on('delete', (data) => this.destroy(data.name));
        VoltreEvents.on('volume', (data) => this.setVolume(data.name, data.volume));
        VoltreEvents.on('max_volume', (data) => this.setMaxVolume(data.name, data.volume));
        VoltreEvents.on('distance', (data) => this.setDistance(data.name, data.distance));
        VoltreEvents.on('soundPosition', (data) => this.setPosition(data.name, data.x, data.y, data.z));
    ...
    play(name, url, volume = 1.0, loop = false, dynamic = false, x = 0, y = 0, z = 0) {
        ...
        const sound = new VoltreAudioPlayer({
            name,
            url,
            volume,
            loop,
    ...
    _startVolumeUpdate() {
        this._updateInterval = setInterval(() => {
            const playerPos = VoltreState.getPlayerPosition();
...
class VoltreAudioPlayer {
    constructor(options) {
        ...
        this.divId = VoltreUtils.generateId('audio');
    ...
    play() {
        const youtubeId = VoltreUtils.getYoutubeId(this.url);
    ...
    _createAudio() {
        ...
        if (!this.loop) {
            this.audioElement.onended = () => {
                VoltreEvents.post('sound_finished', { name: this.name });
                this.destroy();
            };
        }
    ...
    _createYoutube(videoId) {
        ...
        if (typeof YT === 'undefined' || typeof YT.Player === 'undefined') {
            console.warn('[VoltreSound] YouTube API not loaded');
            return;
        }
    ...
    setVolume(vol) {
        this.volume = VoltreUtils.clamp(vol, 0, 1);
    ...
    setMaxVolume(vol) {
        this.maxVolume = VoltreUtils.clamp(vol, 0, 1);
    }
    ...
    updateVolume3D(playerPos) {
        ...
        const dist = VoltreUtils.distance3D(playerPos, this.position);
...
// Export global
window.VoltreSound = VoltreSound;
