-- ============================================================================
-- Voltre 3D UI — Shared Config & Event Bus
--
-- Centralizes configuration for 3D holographic UIs (ammo, speedometer, …).
-- Exposes `Voltre3DUI` global for submodules to register themselves and react
...
Voltre3DUI = Voltre3DUI or {}

local KVP_KEY = "voltre_3dui_config"
...
local function NotifyAll()
    for _, cb in pairs(subscribers) do
        ...
        if not ok then
            print("^1[Voltre3DUI] Subscriber error:", err, "^7")
        end
    end
end
...
function Voltre3DUI.GetSpeedVariant() return currentConfig.speedVariant end
function Voltre3DUI.GetAmmoVariant()  return currentConfig.ammoVariant  end

function Voltre3DUI.GetQualitySize(quality)
    return QUALITY_SIZES[quality or currentConfig.quality] or 1024
end

function Voltre3DUI.SetConfig(newConfig)
...
function Voltre3DUI.Subscribe(id, callback)
    if type(id) ~= "string" or type(callback) ~= "function" then return end
    subscribers[id] = callback
    callback(currentConfig)
end

function Voltre3DUI.Unsubscribe(id)
    subscribers[id] = nil
end
...
RegisterNUICallback("set3dUIConfig", function(data, cb)
    if data and data.config then
        Voltre3DUI.SetConfig(data.config)
    end
    cb("ok")
end)

-- Network event for server-triggered updates (optional)
RegisterNetEvent("voltre:3dui:setConfig")
AddEventHandler("voltre:3dui:setConfig", function(config)
    Voltre3DUI.SetConfig(config)
end)
