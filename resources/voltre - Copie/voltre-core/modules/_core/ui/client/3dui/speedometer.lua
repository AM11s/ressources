-- ============================================================================
-- Voltre 3D Speedometer (minimal) — hologram_box_model attached to the vehicle
-- Shows speed / gear / rpm on a holographic display above the dashboard.
--
-- Pack mode + quality come from Voltre3DUI shared config (HUD Editor).
...
local DUI_URI              = ("nui://%s/modules/_core/ui/html/3dui-speed/speed.html"):format(RESOURCE_NAME)
...
local function InitDui()
    ...
    --print(("^3[Voltre3DSpeed] InitDui — size=%d uri=%s^7"):format(DUI_WIDTH, DUI_URI))
    duiObject = CreateDui(DUI_URI, DUI_WIDTH, DUI_HEIGHT)
    local waited = 0
    while not duiReady and waited < 3000 do Wait(50); waited = waited + 50 end
    --print(("^3[Voltre3DSpeed] DUI ready after %dms (duiReady=%s)^7"):format(waited, tostring(duiReady)))
    local txd = CreateRuntimeTxd("VoltreSpeedHologramTxd")
    local handle = GetDuiHandle(duiObject)
    CreateRuntimeTextureFromDuiHandle(txd, "VoltreSpeedHologramTex", handle)
...
RegisterNUICallback("speedHologramReady", function(_, cb)
    duiReady = true
    --print("^2[Voltre3DSpeed] JS signalled ready (speedHologramReady)^7")
    cb({ ok = true })
end)
...
local function ApplyTextureReplacement()
    ...
    AddReplaceTexture(HOLOGRAM_TXD, HOLOGRAM_TXN, "VoltreSpeedHologramTxd", "VoltreSpeedHologramTex")
...
local function ApplyConfig(cfg)
    ...
    local newSize = (Voltre3DUI and Voltre3DUI.GetQualitySize(cfg.quality)) or 1024
...
CreateThread(function()
    while not (voltre and voltre.fct and voltre.fct.waitPlayerLoaded) do Wait(250) end
    voltre.fct.waitPlayerLoaded()
    Wait(2000)

    if Voltre3DUI then
        local cfg = Voltre3DUI.GetConfig()
    ...
    if Voltre3DUI then
        Voltre3DUI.Subscribe("speed_hologram", ApplyConfig)
    end
        ...
        if IsPedInAnyVehicle(ped, false) then 
            ...
            if veh and veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
                ...
                if currentVehicle ~= veh then
                    ...
                    --print(("^3[Voltre3DSpeed] Entered vehicle %d as driver — hologramEntity=%d^7"):format(veh, e))
                    AttachToVehicle(veh)
                end
                running = true
                ApplyTextureReplacement()
                ...
                local seatbelt = LocalPlayer.state.voltre_seatbelt == true
                ...
                if not _G.__voltre3dSpeedFirstSent then
                    _G.__voltre3dSpeedFirstSent = true
                    --print(("^3[Voltre3DSpeed] First DuiSend sent=%s duiObj=%s ready=%s^7"):format(tostring(sent), tostring(duiObject ~= false), tostring(duiReady)))
                end
