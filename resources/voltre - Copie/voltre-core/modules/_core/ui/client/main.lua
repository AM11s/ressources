...
RegisterNUICallback('ui_ready', function(data, cb)
    UIReady = true
    voltre.InitPrint('^2UI ready (v' .. (data.version or '?') .. ')^7')
    
    SendNUIMessage({ 
        action = 'setConfig',
        data = {
            serverName = ESX.Config and ESX.Config('serverName') or 'osnium FA', 
...
RegisterNUICallback('closeMenu', function(data, cb)
    if voltre and voltre.modules and voltre.modules.ui and voltre.modules.ui.interactions then
        local HUDEditor = voltre.modules.ui.interactions.HUDEditor
...
voltre.InitPrint('^2UI module loaded^7')
