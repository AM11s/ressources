...
function init() {
    ...
    // Start song (Fresnes de Brulux by default)
    currentSong = 0;
...
function applyConfigContent() {
    ...
    if (enterName && CFG.serverName) {
        enterName.innerText = CFG.serverName;
    }
    // Tip
    if (CFG.tip) {
...
function triggerEnter() {
    ...
    _triggerEnterFallbackTimer = setTimeout(() => {
        if (document.body.classList.contains('shutters-covering')) {
            console.warn('[voltre-loading] FADE_OUT not received after 10s, forcing fadeOut()');
            fadeOut();
        }
    }, 10000);
}
...
const TARGET_RESOURCE = 'voltre-core';
...
function sendEnterCallback() {
    ...
    console.log('[voltre-loading] sending fetch to loadingEnter');
    ...
    fetch(url, {
        ...
        .then(r => console.log('[voltre-loading] fetch status', r.status))
        .catch(err => {
            console.error('[voltre-loading] fetch error:', err);
...
function bindEnterAction() {
    ...
    const btn = document.querySelector('#enter-phase .enter-key-btn');
    if (!btn) {
        console.error('[voltre-loading] enter-key-btn NOT found in DOM');
        return;
    }
...
if (window.VoltreBlur) {
    window.VoltreBlur.start({ port: 39474 });

    var blurObserver = new MutationObserver(function (mutations) {
        mutations.forEach(function (m) {
            if (m.attributeName === 'data-voltre-blur-addon') {
                var active = document.documentElement.getAttribute('data-voltre-blur-addon') === 'active';
    ...
    blurObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['data-voltre-blur-addon'] });
}
...
document.querySelectorAll('.bp-link').forEach(function (link) {
    link.addEventListener('click', function (e) {
        ...
        fetch(`https://${TARGET_RESOURCE}/blurOpenUrl`, {
            ...
            console.error('[voltre-loading] blurOpenUrl error:', err);
        });
    });
});
...
function sendBlurCallback(action) {
    ...
    fetch(url, {
        ...
        console.error('[voltre-loading] blurProposalAction error:', err);
    });
}
