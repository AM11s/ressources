/* =============================================================================
 *  voltre-blur.js — script drop-in (mode GÉOMÉTRIE)
 * -----------------------------------------------------------------------------
 ...
 *    - la classe `.voltre-blur` (RECOMMANDÉ), OU
 *    - l'attribut `data-voltre-blur`, OU
 ...
 *
 *  API : window.VoltreBlur.start({ port })   // défaut 39474
 *        window.VoltreBlur.refresh()         // force un re-scan
 *        window.VoltreBlur.stop()
 * ========================================================================== */
(function () {
  'use strict';
  ...
  function isBlurEl(el, cs) {
    ...
    if (el.classList && (el.classList.contains('voltre-blur') ||
      el.hasAttribute('data-voltre-blur'))) return true;
  ...
  function readBlurPx(el, cs) {
    ...
    else if (el.hasAttribute('data-voltre-blur')) px = parseFloat(el.getAttribute('data-voltre-blur')) || 16;
    else px = 16;
    el.dataset.vbpx = px;
    return px;
  }
  ...
  function rebuild() {
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      if (isBlurEl(el, cs)) {
        ...
        neutralizeCEF(el, cs);
        var mm = (el.getAttribute('data-voltre-blur-mode') || '').toLowerCase();
        var mode = mm === 'radial' ? 2 : (mm === 'linear' ? 1 : 0);
        var angle = parseFloat(el.getAttribute('data-voltre-blur-angle')) || 0;
  ...
  function send(payload) {
    var url = 'http://127.0.0.1:' + opts.port + '/voltre-blur/regions?d=' + encodeURIComponent(payload);
    if (opts.debug) console.log('[voltre-blur] ->', payload === '' ? '(vide)' : payload);
    fetch(url, { method: 'GET', cache: 'no-store' })
      .then(function () { document.documentElement.setAttribute('data-voltre-blur-addon', 'active'); })
      .catch(function (e) {
        document.documentElement.setAttribute('data-voltre-blur-addon', 'inactive');
        if (opts.debug) console.warn('[voltre-blur] fetch KO', e && e.message);
      });
  }
  ...
  function startObserver() {
    ...
    observer.observe(document.body, {
      ...
      attributes: true, attributeFilter: ['class', 'data-voltre-blur']
    });
  }
