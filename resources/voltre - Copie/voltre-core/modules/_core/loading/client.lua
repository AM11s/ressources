--[[
    Voltre player lifecycle orchestrator (v2)
    -----------------------------------------
    ...
    Events émis (client) :
      voltre:player:lifecycle (state)
      voltre:player:loading
      voltre:player:noCharacter
      voltre:player:loadCharacter (skin)
      voltre:player:enterPhase
      voltre:player:spawned (isFirstSpawn)
      voltre:player:respawned
...
local function logDbg(msg) print(('^5[voltre-loading]^7 %s'):format(msg)) end
...
local function setState(s)
    ...
    currentState = s
    TriggerEvent('voltre:player:lifecycle', s)
end
...
RegisterNUICallback('blurProposalAction', function(data, cb)
    ...
    if data and data.action == 'install' then
        TriggerServerEvent('voltre:loading:blurInstallKick')
        return
    end
    if data and data.action == 'skip' then
        SetResourceKvp('voltre_blur_refused', 'true')
    end
...
RegisterNUICallback('blurOpenUrl', function(data, cb)
    ...
    if data and data.url then
        TriggerEvent('voltre:client:openUrl', data.url)
    end
end)
...
local function ShowEnterScreen(xPlayer)
    ...
    TriggerServerEvent("voltre:loading:playerInEnter", ESX.PositionBeforeEnterCam)

    local blurRefused = GetResourceKvpString('voltre_blur_refused') == 'true'
    ...
    SetupScenicCamera()
    TriggerEvent('voltre:player:enterPhase')
...
local function StartFirstSpawnSequence()
    ...
    TriggerEvent('voltre:player:loading')
    ...
    PushNui({
        type = 'INIT_CORE',
        serverName = GetConvar('serverName', 'osnium FA'),
    ...
    if not hasCharacter then
        ...
        ShutdownLoadingScreenNui()
        ShutdownLoadingScreen()
        TriggerEvent('voltre:player:noCharacter')
        TriggerEvent('voltre:newCreator:open', true)
        return
    end

    TriggerEvent('voltre:player:loadCharacter', playerSkin)
    ...
    if not LoadingConfig.enterPhase then
        ...
        DoScreenFadeIn(0)
        ClearFocus()
        TriggerServerEvent("voltre:loading:playerLoaded", ESX.PositionBeforeEnterCam)
        DisplayRadar(true)
        voltre.DisplayHud(true, 999)
        loadMapType(mapConfig and mapConfig.mapType or "square")
        setState(STATE.SPAWNED)
...
AddEventHandler('playerSpawned', function()
    ...
    if not firstSpawn then
        TriggerEvent('voltre:player:respawned')
        return
    end
    firstSpawn = false
...
RegisterCommand("debug", function()
    ...
    SetNuiFocus(false, false)
    pcall(function() exports["voltre-core"]:ActiveFrontend(false) end)
    voltre.DisplayHud(true, 999)
