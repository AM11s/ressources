for k,v in pairs(Config.CarWash.List) do
    ...
    if voltre.data.markers.list[markerKey] == nil and v.Position ~= nil then
        voltre.data.markers.list[markerKey] = {
            Position = v.Position,
            Public = true,
            Job = nil,
            Job2 = nil,
            ...
            ConditionText = function()
                if IsPedInAnyVehicle(PlayerPedId(), true) ~= 1 or IsPedInAnyVehicle(PlayerPedId(), true) == false then
                    ...
                    voltre.fct.draw.Text3DBar(v.Position.x, v.Position.y,v.Position.z, "Appuyez sur [~y~E~s~] pour intéragir~s~.")
                end
            end
        }
    else
...
for k,v in pairs(Config.Properties.apartment) do
    ...
    if voltre.data.markers.list[markerKey] == nil and v.id ~= nil and v.Position ~= nil then
        voltre.data.markers.list[markerKey] = {
            Position = v.Position,
            Public = true,
            Job = nil,
...
for k,v in pairs(Config.Barber.List) do
    ...
    if voltre.data.markers.list[markerKey] == nil and v.Position ~= nil then
        voltre.data.markers.list[markerKey] = {
            ...
            Action = function()
                exports["voltre-core"]:OpenShop("barber", v.Type)
            end
        }
    else
    ...
    if voltre.data.markers.list[markerKey] == nil and v.Position ~= nil then
        if v.Blip then
            voltre.data.markers.list[markerKey] = {
                ...
                Action = function()
                    exports["voltre-core"]:OpenShop("makeup", v.Type)
                end
            }
        else
            voltre.data.markers.list[markerKey] = {
