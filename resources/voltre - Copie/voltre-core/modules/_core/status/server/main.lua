		...
		if AlreadyDoUsableItem[k] == nil then
			...
			ESX.RegisterUsableItem(k, function(source)
				...
				elseif v.type == "heal" then
					...
					TriggerClientEvent("Voltre:UseItemsEMS", source, k, v.number, v.time*1000)
					if v.notRemove then return end
				elseif v.type == "armor" then
					if GetPedArmour(GetPlayerPed(source)) >= 100 then
						return
					end
					TriggerClientEvent("Voltre:UseItemsEMS", source, k, 0, v.time*1000)
...
RegisterServerEvent("voltre:usableitem:new", function (data)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("voltre:item:new", function (data)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() ~= "user" then
	...
	if source == 0 then
		...
		if xPlayer.getGroup() ~= 'user' then 
			...
			if tonumber(args[1]) then
				...
				if GetPlayerName(target) then
					TriggerClientEvent('esx_status:healPlayer', target)
					voltre.logs.adminCommand("heal", xPlayer, xTarget)
				else
					TriggerClientEvent('chatMessage', source, "HEAL", {255, 0, 0}, "Player not found!")
				end
			else
				voltre.logs.adminCommand("heal", xPlayer)
