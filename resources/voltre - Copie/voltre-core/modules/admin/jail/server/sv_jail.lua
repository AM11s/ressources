...
RegisterCommand("jail", function(source, args, rawCommand)
    ...
    if args[1] ~= nil and args[2] ~= nil and args[3] ~= nil then
        ...
        if xTarget == nil then
                ...
                MySQL.Async.fetchAll('SELECT * FROM account_info WHERE license = @license',{
                    ...
                    MySQL.Async.fetchAll('SELECT * FROM vjails WHERE identifier = @identifier',{
                        ...
                        if result3[1] then
                            ...
                            voltre.logs.send("Logs Staff",""..pName.." a jail "..tSource.." ("..args[1]..") pendant : "..args[2].." secondes","jail")
            ...
            TriggerClientEvent("Voltre:JailPutIn", args[1], tonumber(args[2]), tostring(reasn), xPlayer.getName())
            voltre.logs.send("Logs Staff",xPlayer.getName().." a jail "..xTarget.getName().." ("..args[1]..") pendant : "..args[2].." secondes","jail")
            MySQL.Async.execute("INSERT INTO sanction_list (target_id, staff_name, target_name, type, raison, date) VALUES (@target_id, @staff_name, @target_name, @type, @raison, @date)",{
            ...
            TriggerClientEvent("Voltre:JailPutIn", xTarget.source, tonumber(args[2]), tostring(reasn), xPlayer.getName())  
            voltre.fct.instance.Set(xTarget.source, 9201, "Jail")
        end
    else
        ...
        if xTarget == nil then
            MySQL.Async.fetchAll('SELECT * FROM users WHERE idunique = @idunique',{
                ...
                if result[1] ~= nil then
                    ...
                    voltre.logs.send("Action staff",""..xPlayer.getName().." \nA unjail : "..result[1].identifier.." ("..args[1]..")","unjail")
            ...
            voltre.logs.send("Action staff",""..xPlayer.getName().." a unjail : "..xTarget.getName().." ("..args[1]..")","unjail")
            IN_JAIL[xTarget.identifier] = nil
            TriggerClientEvent("Voltre:JailPutOut", xTarget.source)
            ...
            voltre.fct.instance.Set(xTarget.source, 0)
        end
    else
        xPlayer.chatMessage("Formulation incorrect !")
    end
...
RegisterNetEvent('Voltre:leaveJail')
AddEventHandler('Voltre:leaveJail', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:leaveJail") 
    if IN_JAIL[xPlayer.identifier] ~= nil then
        DropPlayer(source, "Désynchonisation avec le serveur ou detection de Cheat")
    end
end)

AddEventHandler("Voltre:VerifyJail", function(id)
    local xPlayer = ESX.GetPlayerFromId(id);
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:VerifyJail") 

    if (xPlayer) then
        if (IN_JAIL[xPlayer.identifier] ~= nil) then
            TriggerClientEvent("Voltre:JailPutIn", id, tonumber(IN_JAIL[xPlayer.identifier].time), tostring(IN_JAIL[xPlayer.identifier].raison),IN_JAIL[xPlayer.identifier].staffname)
            
            voltre.fct.instance.Set(id, 9201, "Jail")
        else
            MySQL.query('SELECT time, raison FROM vjails WHERE identifier = @a', {
                ["@a"] = xPlayer.identifier
            }, function(result)
                if result[1] then
                    TriggerClientEvent("Voltre:JailPutIn", id, result[1].time, result[1].raison, result[1].staffname)
                    voltre.fct.instance.Set(id, 9201, "Jail")
...
RegisterNetEvent('Voltre:JailPut')
AddEventHandler('Voltre:JailPut', function(id, seconds, desc)
    local source = source;
    local xPlayer = ESX.GetPlayerFromId(source);
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:JailPut") 
    if xPlayer.getGroup() ~= "user" then
        local xTarget = ESX.GetPlayerFromId(id)
        if IN_JAIL[xTarget.identifier] ~= nil then 
            return xPlayer.showNotification("Ce joueur est déjà en prison pour ~g~"..IN_JAIL[xTarget.identifier].raison.."~s~ pendant ~g~"..IN_JAIL[xTarget.identifier].time.."~s~ sec !") 
        end
        TriggerClientEvent("Voltre:JailPutIn", id, seconds, tostring(desc),GetPlayerName(source))
        voltre.fct.instance.Set(id, 9201, "Jail")
        MySQL.update('INSERT INTO vjails (jailname,staffname,identifier, time, raison) VALUES (@e,@d,@a, @b, @c)', {
...
RegisterNetEvent('Voltre:JailSeconds')
AddEventHandler('Voltre:JailSeconds', function()
    local source = source;
    local xPlayer = ESX.GetPlayerFromId(source);
    if not xPlayer then return end
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:JailSeconds") 
    ...
    if IN_JAIL[xPlayer.identifier] then
        SetTimeout(1000, function()
            if IN_JAIL[xPlayer.identifier] ~= nil and IN_JAIL[xPlayer.identifier].time == -1 then
                ...
                voltre.fct.instance.Set(xPlayer.source, 9201, "Jail")
            elseif IN_JAIL[xPlayer.identifier] ~= nil and IN_JAIL[xPlayer.identifier].time <= 0 then
                ...
                voltre.fct.instance.Set(xPlayer.source, 0)
                MySQL.update('DELETE FROM vjails WHERE identifier = @a', {
                    ["@a"] = xPlayer.identifier,
                })
                IN_JAIL[xPlayer.identifier] = nil;
                TriggerClientEvent("Voltre:JailPutOut", source)
...
RegisterNetEvent('Voltre:RequestJail')
AddEventHandler('Voltre:RequestJail', function()
    local source = source;
    local xPlayer = ESX.GetPlayerFromId(source);
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:RequestJail") 
    ALL_PLAYERS_IN_JAIL = {};
    ...
    if xPlayer.getGroup() ~= "user" then
        ...
        TriggerClientEvent("Voltre:StaffUpdateJail", source, ALL_PLAYERS_IN_JAIL)
    else
            ...
            if result[1] then
                MySQL.Async.execute("DELETE FROM vjails WHERE jailid = @j2",{
                    ...
                    if rowsChanged then
                        ...
                        if tPlayer then
                            TriggerClientEvent("Voltre:skinchanger:getSkin", tPlayer.source, function(skin)
                                TriggerClientEvent("Voltre:skinchanger:loadSkin", tPlayer.source, skin)
                            end)
                            IN_JAIL[tPlayer.identifier] = nil;
                            voltre.fct.instance.Set(tPlayer.source, 0)
                            TriggerClientEvent("Voltre:JailPutOut", tPlayer.source)
...
AddEventHandler('esx:playerLoaded', function(source)
	TriggerEvent("Voltre:VerifyJail", source)
end)

ESX.RegisterServerCallback("Voltre:jailList", function(source,cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    cb(IN_JAIL)
end)

ESX.RegisterServerCallback("Voltre:IsInJail", function(source,cb,id,identifier)
