...
JAIL_DURATION = 0
JAIL_STAFF = nil;
RegisterNetEvent("Voltre:JailPutIn")
AddEventHandler("Voltre:JailPutIn", function(duration, message,staffname)
	PlayerJail = true;
        ...
        while true do
            if (IS_IN_JAIL) then
                ...
                if JAIL_DURATION == -1 then
                    ...
                    time = voltre.fct.format.SecondsToClock(JAIL_DURATION)
                    pcall(function()
                        ShowInfo(
                            "Informations Jail",  
                            {
	...
	Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
		while true do
            if JAIL_DURATION ~= -1 then
                TriggerServerEvent('Voltre:JailSeconds')
            end
			Citizen.Wait(1000)
			local player = PlayerPedId();
...
RegisterNetEvent("Voltre:JailPutOut")
AddEventHandler("Voltre:JailPutOut", function()
	TriggerServerEvent('Voltre:leaveJail')
	PlayerJail = false;
	IS_UNJAIL = true
	IS_IN_JAIL = false
	JAIL_DURATION = 0
...
RegisterNetEvent("Voltre:StaffUpdateJail")
AddEventHandler("Voltre:StaffUpdateJail", function(table)
    JAIL_PLAYER = table;
