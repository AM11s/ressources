...
function ReworkLogs(data, type, cancel1, data2)
    voltre.logs.send(data[1]["title"],data[1]["description"],type)
end

RegisterNetEvent("voltre:admin:getSaveData.Players")
AddEventHandler("voltre:admin:getSaveData.Players", function()
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() ~= "user" then
        TriggerClientEvent("voltre:admin:getSaveData.Players", source, SaveData.Players.Online.List)
    end
end)
...
function WipeTable(targetId) -- Fonction qui vide la table des joueurs lors d'un wipe
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {
		...
		if result[1] ~= nil then
            ...
            voltre.DebugPrint("Final Wipe Loadout: "..json.encode(listearmeperm))
...
function InitPlayersTable(xPlayer)
    ...
    return {
        ...
        --fivem = xPlayer.fivem,
        instance = voltre.fct.instance.Get(xPlayer.source),
...
CreateThread(function()
    voltre.fct.waitPlayerLoaded(100)
    Wait(6000)
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    ...
    TriggerClientEvent("voltre:admin:addPlayer", -1, xPlayer.getIdunique(), SaveData.Players.Online[xPlayer.getIdunique()], stafflist[xPlayer.getIdunique()])
...
AddEventHandler('playerDropped', function(reason)
    ...
    TriggerClientEvent("voltre:admin:addPlayer", -1, xPlayer.getIdunique(), nil, stafflist[xPlayer.getIdunique()])
end)
...
AddEventHandler("voltre:admin:staffinit", function()
    ...
    TriggerClientEvent('voltre:admin:allrefresh', source, SaveData.Players.Online, reportsTable, stafflist, NumberPlayerTotalCached)
end)
...
AddEventHandler("voltre:admin:restart", function()
    ...
    TriggerClientEvent('voltre:admin:restart', -1, SaveData.Players.Online, reportsTable)
end)
...
function refreshPlayer(target, type, Init)
    ...
    if type == "forAll" then
        TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, targetIdunique, SaveData.Players.Online[targetIdunique])
    elseif type == "forMe" then
        TriggerClientEvent("voltre:admin:refreshOnePlayer", source, targetIdunique, SaveData.Players.Online[targetIdunique])
    elseif type == "forAllStaff" then
        for k,v in pairs(SaveData.Players.Online) do
            if v.group == "user" then goto continue end
            TriggerClientEvent("voltre:admin:refreshOnePlayer", v.source, targetIdunique, SaveData.Players.Online[targetIdunique])
            ::continue::
        end
...
AddEventHandler("voltre:admin:demanderefreshallplayerforme", function()
    ...
    TriggerClientEvent("voltre:player:refreh", source, SaveData.Players.Online)
end)
...
RegisterNetEvent("voltre:detectrppack:staff:setHave", function(pack)
    ...
    for k,v in pairs(SaveData.Players.Online) do
        ...
        TriggerClientEvent("voltre:admin:refreshOnePlayer", v.source, xPlayer.getIdunique(), SaveData.Players.Online[xPlayer.getIdunique()])
        ::continue::
    end
end)
...
AddEventHandler("vAdminMenu:addstaffglobal", function(idunique, group, discord)
    ...
    if xTarget ~= nil then
        ...
        InitAllStaffs()
        TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, idunique, SaveData.Players.Online[idunique], stafflist[idunique])
		...
		ESX.GetNameByIdUnique(idunique, function(result)
            ...
            InitAllStaffs()
            TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, idunique, SaveData.Players.Online[idunique], stafflist[idunique])
            voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a ajouter un nouveau staff : "..result.."(U"..idunique..") en tant que "..group,"addstaff", {idunique = xPlayer.getIdunique(), idunique_cible = idunique, name = xPlayer.getName(), name_cible = name})
...
AddEventHandler("AdminMenu:removeBank", function(target, amount)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer "..amount.."$ (bank) au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-account", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)
...
AddEventHandler("AdminMenu:removeBlack", function(target, amount)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer "..amount.."$ (dirtycash) au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-account", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)
