...
Citizen.CreateThread(function()
    Wait(2000)
    TriggerServerEvent("voltre:track:request")
end)

RegisterNetEvent("voltre:track:load", function(data)
    TrackList = data
    TrackLoad = true
...
RegisterNetEvent("voltre:track:lobby:setintrack", function(id, lobbyid, data)
    inTrack = true
end)

RegisterNetEvent("voltre:track:lobby:load", function(id, lobbyid, data)
    ...
    if RaceStart == true then
        ...
        voltre.fct.draw.UpdateTimerBar(PlayerBarIndex, {text = TrackList[id].lobby[lobbyid].playerInTrack.."/"..TrackList[id].lobby[lobbyid].nbrPlayers})
        voltre.fct.draw.UpdateTimerBar(CheckPointBarIndex, {text = #TrackList[id].checkpoint - #TrackList[id].lobby[lobbyid].players[ESX.PlayerData.idunique].checkpoints.."/"..#TrackList[id].checkpoint})
    end
end)

RegisterNetEvent("voltre:track:lobby:leave", function()
   -- PlaySoundFrontend(-1, "CHECKPOINT_PERFECT", "HUD_MINI_GAME_SOUNDSET", 1)
    if inTrack then
        ...
        --exports["Voltre"]:whileToDestroy("raceAmbiance", 200)
        if Zone then
            Zone:destroy()
        end
...
RegisterNetEvent("voltre:track:ply:finish", function()
    PlaySoundFrontend(-1, "CHECKPOINT_PERFECT", "HUD_MINI_GAME_SOUNDSET", 1)
    if PlayerState.isInVehicle then
        DeleteEntity(PlayerState.vehicle)
    end
...
RegisterNetEvent("voltre:track:lobby:finish", function(id, lobbyid, data)
    TrackList[id].lobby[lobbyid] = data
    inTrack = false
end)

RegisterNetEvent("voltre:track:lobby:start", function(id, lobbyid, points, canDrift)
    ...
    Citizen.CreateThread(function()
        ...
        PlayerBarIndex = voltre.fct.draw.AddTimerBar("Joueurs restant :", {text = TrackList[id].lobby[lobbyid].playerInTrack.."/"..TrackList[id].lobby[lobbyid].nbrPlayers})
        CheckPointBarIndex = voltre.fct.draw.AddTimerBar("Checkpoint restant :", {text = #TrackList[id].checkpoint - #TrackList[id].lobby[lobbyid].players[ESX.PlayerData.idunique].checkpoints.."/"..#TrackList[id].checkpoint})
        voltre.fct.draw.AddTimerBar("Temps restant :",{endTime=time+5000})
    end)
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if PlayerState.isInVehicle then
                ...
                if distanceFinish < 10.0 then
                    ...
                    if canAction then 
                        TriggerServerEvent("voltre:track:players:race:finish", id, lobbyid)
                    end
                end
                ...
                for k,v in pairs(TrackList[id].checkpoint) do
                    ...
                    if val then
                        ...
                        if distanceCheckPoint <= 10.0 then
                            ...
                            if canAction then
                                TriggerServerEvent("voltre:track:players:race:pastcheckpawn", id, lobbyid, k)
                            end
                        end
                    end
                end
        ...
        while true do
            ...
            if not Zone:isPointInside(GetEntityCoords(PlayerState.playerPed)) and RaceStart then
                ...
                if TimerToExit < 0 then
                    TriggerServerEvent("voltre:track:players:race:leave", id, lobbyid)
                    break
                end
...
RegisterNetEvent("voltre:track:solo:start", function(id, points, canDrift)
    Solo = {
        Time = nil,
        TimeonStart = nil,
        checkpoints = {}
    ...
    Citizen.CreateThread(function()
        ...
        CheckPointBarIndex = voltre.fct.draw.AddTimerBar("Checkpoint restant :", {text = #TrackList[id].checkpoint - #Solo.checkpoints.."/"..#TrackList[id].checkpoint})
        voltre.fct.draw.AddTimerBar("Temps restant :",{endTime=Solo.Time})
    end)
    Citizen.CreateThread(function()
        local FinishCoords = vector3(TrackList[id].finish.pos.x, TrackList[id].finish.pos.y, TrackList[id].finish.pos.z)
        local allCheckPoint = {}
                ...
                if distanceFinish < 10.0 then
                    ...
                    if canAction then 
                        if #TrackList[id].checkpoint - #Solo.checkpoints == 0 then 
                            TriggerServerEvent("voltre:track:solo:finish", id, GetGameTimer(), Solo.TimeonStart)
                            break
                        end
                    end
                end
                        ...
                        if distanceCheckPoint <= 10.0 then
                            ...
                            if canAction and Solo.checkpoints[k] ~= true then
                                if Solo.checkpoints[k-1] == true then 
                                    ...
                                    voltre.fct.draw.UpdateTimerBar(CheckPointBarIndex, {text = #TrackList[id].checkpoint - #Solo.checkpoints.."/"..#TrackList[id].checkpoint})
                                elseif k == 1 then
                                    Solo.checkpoints[k] = true
                                    voltre.fct.draw.UpdateTimerBar(CheckPointBarIndex, {text = #TrackList[id].checkpoint - #Solo.checkpoints.."/"..#TrackList[id].checkpoint})
                                end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if not Zone:isPointInside(GetEntityCoords(PlayerState.playerPed)) and RaceStart then
                ...
                TimerToExit = TimerToExit - 1
                if TimerToExit < 0 then
                    TriggerServerEvent("voltre:track:solo:leave", id)
                    break
                end
				DrawMissionText("~r~Vous êtes en dehors de la zone de course ! Vous avez "..TimerToExit.." secondes pour rentrer.", 2000)
            else
...
RegisterNetEvent("voltre:race:start", function(id, lobbyid, points)
    Citizen.CreateThread(function()
        ExecuteCommand("clearchat")
        --exports["Voltre"]:PlayUrl("raceAmbiance", "https://www.youtube.com/watch?v=J4t4pMZBXZg", 0.2, true)
			...
			RageUI.IsVisible(main, function()
                if TrackList[id].type == "players" then
                    ...
                    for k,v in pairs(TrackList[id].lobby) do 
                        ...
                        if v.players[ESX.PlayerData.idunique] ~= nil and v.players[ESX.PlayerData.idunique].owner == true then
                            label = "Voltre Salon #"..v.id.." ("..v.nbrPlayers.."/"..TrackList[id].maxPlayers.." Joueurs)"
                            trackOwner = true
                            inTrack = true
                        end
                            ...
                            RageUI.Button(label, nil, {RightLabel = trackOwner == false and "Rejoindre" or "Gérer"}, true, {
                                onSelected = function()
                                    ...
                                    if not trackOwner then
                                        ...
                                        TriggerServerEvent("voltre:track:players:join", id, v.id)
                                        Wait(1500)
                                        DoScreenFadeIn(500)
                                    end
                ...
                elseif TrackList[id].type == "timer" then
                    RageUI.Button("Commencer la course", nil, {}, true, {
                        onSelected = function()
                            TriggerServerEvent("voltre:track:startrace", id)
                            DoScreenFadeOut(100)
                            RageUI.CloseAll()
                            Wait(5000)
                            DoScreenFadeIn(5500)
                    ...
                    RageUI.Button("Classement", nil, {}, true, {
                        onSelected = function()
                            ESX.TriggerServerCallback("voltre:track:getClassementFromTrack", function(data) 
                                Classement = data or {}
            ...
            RageUI.IsVisible(openLobby, function()
                if trackOwner then
                    ...
                    for k,v in pairs(TrackList[id].lobby[selectedLobbyId].players) do
                        if v.idunique ~= ESX.PlayerData.idunique then
                            RageUI.Button(v.name.." (U"..v.idunique..")", "[ENTRER] Pour exclure le joueur", {}, true, {
                                onSelected = function()
                                    TriggerServerEvent("voltre:track:kick", id, TrackList[id].lobby[selectedLobbyId].id, v.idunique)
                    ...
                    RageUI.Button("Annuler la course", nil, {Color = {BackgroundColor = {255, 0, 0, 100}}}, true, {
                        onSelected = function()
                            RageUI.CloseAll()
                            TriggerServerEvent("voltre:track:players:closelobby", id, TrackList[id].lobby[selectedLobbyId].id )
                        end
                    })
                    ...
                    elseif TrackList[id].lobby[selectedLobbyId].nbrPlayers < TrackList[id].minPlayers then
                        ...
                        RageUI.Button("Lancer la course", "Vous et les autres joueurs seront débiter de "..TrackList[id].mise.."$.", {Color = {BackgroundColor = {0, 200, 0, 100}}}, true, {
                            onSelected = function()
                                RageUI.CloseAll()
                                TriggerServerEvent("voltre:track:players:start", id,TrackList[id].lobby[selectedLobbyId].id )
                            end
                        })
                    end
                else
                    ...
                    RageUI.Button("Quitter la course", nil, {}, true, {
                        onSelected = function()
                            RageUI.CloseAll()
                            TriggerServerEvent("voltre:track:players:leave", id, TrackList[id].lobby[selectedLobbyId].id)
                        end
                    })
                end
            end)
