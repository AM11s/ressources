...
local blip 
RegisterNetEvent('voltre:Eventstart', function(data)
            ...
            while event[data.position].on do
                ...
                if dist < 25 then
                    ...
                    if vehicleHealth/10 <= 0 then
                        ...
                        TriggerServerEvent('voltre:Eventbroke', data)
                    ...
                    if event[data.position].time <= 0 then
                        TriggerServerEvent('voltre:server:Eventstop', data.position, data.id)
                        entredInZone = false
                        event[data.position].on = false
                        break
                    end
...
RegisterNetEvent('voltre:Eventbroke', function(data)
    TriggerServerEvent('voltre:EventsecondBroke', data)
end)
                ...
                if dist <= 30 then
                    if dist <= 3.5 then
                        ...
                        if IsControlJustReleased(0, 38) and v.recup == nil then
                            v.recup = true
                            TriggerServerEvent('voltre:server:Eventtake', v.obj, k, data.id)
                        end
                    end
                else
                    interval = 2000
...
RegisterNetEvent('voltre:client:Eventtake', function(obj, index)
    for k,v in pairs(moneyProps) do
        if k == index then
            DeleteObject(v.obj)
            DeleteObject(obj)
...
RegisterNetEvent('voltre:client:Eventstop', function(position)
    HideInfo()
