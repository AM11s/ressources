...
RegisterNetEvent("voltre:staff:createtracks", function(data)
    local src = source
    ...
    TriggerClientEvent("voltre:track:load", -1, SaveData.json["tracks"])
end)

ESX.RegisterServerCallback("voltre:track:getClassement", function(source, cb)
    local Classements = {}
    for k,v in pairs(SaveData.json["tracks"]) do
...
ESX.RegisterServerCallback("voltre:track:getClassementFromTrack", function(source, cb, id)
    cb(SaveData.json["tracks"][id].classements)
end)

RegisterNetEvent("voltre:track:request", function()
    TriggerClientEvent("voltre:track:load", source, SaveData.json["tracks"])
end)

RegisterNetEvent("voltre:track:startrace", function(id)
    ...
    if SaveData.json["tracks"][id].type == "players" then
        ...
        TriggerClientEvent("voltre:track:lobby:load", -1, id, lobbyid, SaveData.json["tracks"][id].lobby[lobbyid])
    else
        TriggerClientEvent("voltre:track:lobby:setintrack", xPlayer.source)
        StartRace("timer", id, nil, xPlayer.source)
    end
end)

RegisterNetEvent("voltre:track:kick", function(id, idlobby, idunique)
    local xPlayer = ESX.GetPlayerFromId(source)
    local owner = false
    ...
    TriggerClientEvent("voltre:track:lobby:load", -1, id, idlobby, SaveData.json["tracks"][id].lobby[idlobby])
end)

RegisterNetEvent("voltre:track:players:leave", function(id, idlobby)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("voltre:track:players:race:leave", function(id, idlobby)
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerClientEvent("voltre:annonce", source, "COURSE", "Vous avez était exclu.", 6000)
    --ESX.ChatMessage(xPlayer.source, "Vous avez était exclu de la course. Cause: exit zone", "Race Systeme", {255, 0, 0})
    SaveData.json["tracks"][id].lobby[idlobby].players[xPlayer.getIdunique()] = nil
    SaveData.json["tracks"][id].lobby[idlobby].nbrPlayers = SaveData.json["tracks"][id].lobby[idlobby].nbrPlayers - 1
    SaveData.json["tracks"][id].lobby[idlobby].playerInTrack = SaveData.json["tracks"][id].lobby[idlobby].playerInTrack - 1
    TriggerClientEvent("voltre:track:lobby:leave", xPlayer.source)
...
RegisterNetEvent("voltre:staff:track:delete", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    if SaveData.json["tracks"][id] == nil then return end
...
RegisterNetEvent("voltre:track:solo:leave", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerClientEvent("voltre:annonce", source, "COURSE", "Vous avez était exclu.", 6000)
    TriggerClientEvent("voltre:track:lobby:leave", xPlayer.source)
    SetEntityCoords(GetPlayerPed(xPlayer.source), vector3(SaveData.json["tracks"][id].pedPosition.pos.x, SaveData.json["tracks"][id].pedPosition.pos.y, SaveData.json["tracks"][id].pedPosition.pos.z))
...
RegisterNetEvent("voltre:track:players:race:pastcheckpawn", function(id, idlobby, checkpoint)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("voltre:track:players:race:finish", function(id, idlobby)
    local xPlayer = ESX.GetPlayerFromId(source)
    local idunique = xPlayer.getIdunique()
    ...
    if nbr == SaveData.json["tracks"][id].lobby[idlobby].nbrPlayers then 
        ...
        for k,v in pairs(SaveData.json["tracks"][id].lobby[idlobby].players) do
            TriggerClientEvent("voltre:track:lobby:leave", v.source)
            SetEntityCoords(GetPlayerPed(xPlayer.source), vector3(SaveData.json["tracks"][id].pedPosition.pos.x, SaveData.json["tracks"][id].pedPosition.pos.y, SaveData.json["tracks"][id].pedPosition.pos.z))
            SetInInstance(v.source, 0, "Course")
            ESX.ChatMessage(v.source, "La course est terminer.", "Race Systeme", {255, 255, 0})
            TriggerClientEvent("voltre:annonce", v.source, "La Course est terminer", FinishLabel, 7000)
        ...
        SaveData.json["tracks"][id].lobby[idlobby].players[idunique].position = #SaveData.json["tracks"][id].lobby[idlobby].position+1
        SaveData.json["tracks"][id].lobby[idlobby].position[#SaveData.json["tracks"][id].lobby[idlobby].position+1] = idunique
        TriggerClientEvent("voltre:track:ply:finish", SaveData.json["tracks"][id].lobby[idlobby].players[idunique].source)
        SetEntityCoords(GetPlayerPed(xPlayer.source), vector3(SaveData.json["tracks"][id].pedPosition.pos.x, SaveData.json["tracks"][id].pedPosition.pos.y, SaveData.json["tracks"][id].pedPosition.pos.z))
...
RegisterNetEvent("voltre:track:solo:finish", function(id, timetostart, timetofinish)
...
RegisterNetEvent("voltre:track:players:closelobby", function(id, idlobby)
    local xPlayer = ESX.GetPlayerFromId(source)
    local owner = false
    for k,v in pairs(SaveData.json["tracks"][id].lobby[idlobby].players) do
        if v.idunique == xPlayer.getIdunique() then
...
RegisterNetEvent("voltre:staff:track:closelobby", function(id, idlobby)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    for k,v in pairs(SaveData.json["tracks"][id].lobby[idlobby].players) do
        TriggerClientEvent("voltre:track:lobby:leave", v.source)
...
RegisterNetEvent("voltre:track:players:join", function(id, idlobby)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterCommand("insertbot", function(source, args)
    ...
    TriggerClientEvent("voltre:track:lobby:load", -1, args[1], args[2], SaveData.json["tracks"][args[1]].lobby[args[2]])
end)
...
RegisterNetEvent("voltre:track:players:start", function(id, idlobby)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["tracks"][id].type == "players" then
        local owner = false
        for k,v in pairs(SaveData.json["tracks"][id].lobby[idlobby].players) do
            local xTarget = ESX.GetPlayerFromId(v.source)
            TriggerClientEvent("voltre:track:lobby:setintrack", v.source)
            if xTarget then
    ...
    Citizen.CreateThread(function()
        if type == "players" then
            ...
            for k,v in pairs(Lobby.players) do
                ...
                TriggerClientEvent("voltre:track:lobby:start", v.source, id, lobbyid, Track.zone, Track.giveDrift)
            end
            Wait(1500)
            ...
            Wait(7000)
            for k,v in pairs(Lobby.players) do
                TriggerClientEvent('voltre:race:start', v.source)
            end
            Citizen.CreateThread(function()
                while true do
                ...
                TriggerClientEvent("voltre:track:lobby:load", -1, id, lobbyid, nil)
            end)
        else
            local Track = SaveData.json["tracks"][id]
            local vehicules = nil
            ...
            TriggerClientEvent("voltre:track:solo:start", src, id, Track.zone, Track.giveDrift)
            ...
            Wait(7000)
            TriggerClientEvent('voltre:race:start', src)
