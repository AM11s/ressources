...
--             TriggerClientEvent('voltre:Eventstart', -1, data)
--         end
--     end
-- end) 

RegisterNetEvent('voltre:Eventbroke', function(data)
    ...
    TriggerClientEvent('voltre:Eventbroke', -1, data)
    SaveData.Events.CamionBlinder[data.id].broke = true
end)

RegisterNetEvent('voltre:EventsecondBroke', function(data)
    local source = source
    TriggerClientEvent('voltre:EventsecondBroke', source, data)
end)

RegisterServerEvent('voltre:server:Eventtake', function(obj, k, id)
    local source = source
    ...
    TriggerClientEvent('voltre:client:Eventtake', -1, obj, k)
end)

RegisterServerEvent('voltre:server:Eventstop', function(position, id)
    if SaveData.Events.CamionBlinder[id] == nil then return end
    if SaveData.Events.CamionBlinder[id].broke ~= true then return end
    if SaveData.Events.CamionBlinder[id].finish == true then return end
    SaveData.Events.CamionBlinder[id].finish = true
    TriggerClientEvent('voltre:client:Eventstop', -1, position)
...
RegisterNetEvent("voltre:startbrinksevent")
AddEventHandler("voltre:startbrinksevent", function(position, rewards)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("voltre:startbrinkseventauto")
AddEventHandler("voltre:startbrinkseventauto", function()
    local xPlayer = ESX.GetPlayerFromId(source)
