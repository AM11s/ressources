...
function CreateGift(coords)
    ...
    TriggerClientEvent("voltre:noel:dropgift", -1, coords, id, sleighCoordsPossible[math.random(1,#sleighCoordsPossible)])
end
...
RegisterServerEvent("voltre:noel:giftdropped")
AddEventHandler("voltre:noel:giftdropped", function(id)
...
function RewardGift(reward, xPlayer)
    ...
    elseif reward.type == "coins" then
        local identifier = voltre.fct.getIdentifiers(xPlayer.source)
...
RegisterServerEvent("voltre:noel:get")
AddEventHandler("voltre:noel:get", function(id)
    if AllCadeaux[id] == nil then return end
    if AllCadeaux[id].dropped == false then return end
    if AllCadeaux[id].get == true then return end
    local xPlayer = ESX.GetPlayerFromId(source)

    TriggerClientEvent("voltre:noel:deletegift", -1)
...
RegisterServerEvent("voltre:staff:noel:start")
AddEventHandler("voltre:staff:noel:start", function(coords)
    local xPlayer = ESX.GetPlayerFromId(source)
