...
function VoltreIncrementKillStat(killerIdu, victimIdu)
...
function VoltreIncrementReportMade(idu)
	if not idu then return end
...
ESX.RegisterServerCallback('voltre:admin:getPlayerInventory', function(source, cb, targetIdUnique)
...
RegisterNetEvent("voltre:staff:activeStreamerMode", function(bool)
    local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler("AdminMenu:updatestaffactif", function(bool)
    ...
    if xPlayer.getGroup() ~= "user" and ESX.PlayersByIdUnique[idunique] ~= nil then
        ...
        if bool then
            ...
            voltre.logs.send("Staff",xPlayer.getName().."~s~ a pris son service", "prise-service", {idunique = idunique, name = xPlayer.getName()})
        else
            finalservicemsg = "quitter"
            voltre.logs.send("Staff",xPlayer.getName().."~s~ a quitter son service", "quitte-service", {idunique = idunique, name = xPlayer.getName()})
        end
        ...
        if stafflist[idunique] then
            ...
            --voltre.DebugPrint("[admin server] [AdminMenu:updatestaffactif] ^1 aucun staff[n] pour ", idunique or "N/A", "impossible donc de l'actualiser.")
        ...
        -- TriggerClientEvent("voltre:admin:staffCountUpdate", -1, getStaffInServiceCount())
    end
end)
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    ...
    if xPlayer.getGroup() ~= "user" then
        ...
        voltre.DebugPrint("Init Staff:",xPlayer.getIdunique())
    end

    TriggerClientEvent("voltre:admin:addPlayer", -1, xPlayer.idunique, getLightPlayerData(ESX.PlayersByIdUnique[xPlayer.idunique]), stafflist[xPlayer.idunique])
    TriggerClientEvent('voltre:reports:refreh', source, reportsTable)
end)
...
AddEventHandler('playerDropped', function(reason)
    ...
    if reportsTable[source] ~= nil then
        ...
        TriggerClientEvent('voltre:reports:refreh', -1, reportsTable)
    end

    SaveData.Admin.PlayersBlips[xPlayer.idunique] = nil
    stafflist[xPlayer.idunique] = nil
    TriggerClientEvent("voltre:admin:addPlayer", -1, xPlayer.idunique, nil, stafflist[xPlayer.idunique])
end)

local NumberPlayerTotalCached = nil
RegisterNetEvent("voltre:admin:staffinit")
AddEventHandler("voltre:admin:staffinit", function()
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    if not NumberPlayerTotalCached then
        NumberPlayerTotalCached = ESX.Table.SizeOf(SaveData.Players.Offline.List)
    end
    TriggerClientEvent('voltre:admin:allrefresh', source, getLightAllPlayers(ESX.PlayersByIdUnique), reportsTable, stafflist, NumberPlayerTotalCached)
end)

RegisterNetEvent("voltre:admin:restart")
AddEventHandler("voltre:admin:restart", function()
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    if xPlayer.getGroup() == "user" then
    ...
    TriggerClientEvent('voltre:admin:restart', -1, getLightAllPlayers(ESX.PlayersByIdUnique), reportsTable)
end)


RegisterNetEvent("voltre:admin:refresh:server")
AddEventHandler("voltre:admin:refresh:server", function(target, type)
    Wait(1000)
    if source == 0 then
...
function refreshPlayer(target, type, Init)
    ...
    if type == "forAll" then
        TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, targetIdunique, lightData)
    elseif type == "forMe" then
        TriggerClientEvent("voltre:admin:refreshOnePlayer", source, targetIdunique, lightData)
    elseif type == "forAllStaff" then
        ...
        for k,v in pairs(ESX.PlayersByIdUnique) do
            ...
            TriggerClientEvent("voltre:admin:refreshOnePlayer", v.source, targetIdunique, lightData)
            TriggerClientEvent("voltre:admin:staffCountUpdate", v.source, staffCount)
            ::continue::
        end
...
RegisterNetEvent("voltre:staff:changegamertag", function(bool, oldbool)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    local idunique = xPlayer.getIdunique()
...
RegisterNetEvent("voltre:admin:demanderefreshallplayerforme")
AddEventHandler("voltre:admin:demanderefreshallplayerforme", function()
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    if xPlayer.getGroup() == "user" then
        DropPlayer(source, "Vous n'avez pas les permission de faire cela")
    ...
    TriggerClientEvent("voltre:player:refreh", source, getLightAllPlayers(ESX.PlayersByIdUnique))
end)

RegisterNetEvent("voltre:detectrppack:staff:setHave", function(pack)
    ...
    --     TriggerClientEvent("voltre:admin:refreshOnePlayer", v.source, xPlayer.getIdunique(), ESX.PlayersByIdUnique[xPlayer.getIdunique()])
    --     ::continue::
    -- end
end)


ESX.RegisterServerCallback("voltre:staff:getPlayerInstance", function(source, cb, target)
    local xPlayer = ESX.GetPlayerFromId(source)
    local xTarget = ESX.GetPlayerFromId(target)
    if xPlayer.getGroup() == "user" then cb(nil) return end
    cb(voltre.fct.instance.Get(xTarget.source))
end)


RegisterNetEvent("voltre:staff:deleteownedveh:plate")
AddEventHandler("voltre:staff:deleteownedveh:plate", function(plate)
    local xPlayer = ESX.GetPlayerFromId(source)
    if Config.GroupeHighPerm[xPlayer.getGroup()] ~= true then return end
    SaveData.json["owned_vehicles"][plate] = nil
end)

ESX.RegisterServerCallback("voltre:staff:getplayerVeh", function(source, cb, target)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
...
ESX.RegisterServerCallback("voltre:staff:getplayerVehIdUnique", function(source, cb, identifier, job, job2)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    if xPlayer == nil then cb({}) end
    if xPlayer.getGroup() == "user" then cb({}) end
...
ESX.RegisterServerCallback("voltre:staff:getsocietyVeh", function(source, cb, name)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    if xPlayer == nil then cb({}) end
    if xPlayer.getGroup() == "user" then cb({}) end
...
ESX.RegisterServerCallback("voltre:admin:gethistoconnexion", function(source, cb, idu)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    if xPlayer == nil then return end
    if xPlayer.getGroup() == "user" then return end
...
ESX.RegisterServerCallback("voltre:admin:changeweaponacc", function(source, cb, idunique,weaponname, accname, type)
    local xPlayer = ESX.GetPlayerFromId(source)
...
Citizen.CreateThread(function()
    ...
    if DayOfWeek == 1 then
        print("^7[^4Voltre^7] Nous sommes un "..os.date("%A").." il y a donc un reset des reports de la semaine")
    ...
    print("^7[^4Voltre^7] Nous sommes un "..os.date("%A").." il n'y a donc pas de reset des reports de la semaine")
end)

local WlEvalStaff = {[0] = true,[1] = true, [2] = true, [3] = true, [4] = true, [5] = true}


ESX.RegisterServerCallback("voltre:admin:getHistoEvalStaff", function(source, cb, idunique)
...
RegisterNetEvent("voltre:admin:change-evalstaff")
AddEventHandler("voltre:admin:change-evalstaff", function(idunique,new, thereason, old)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterNetEvent("voltre:staff:setInstance")
AddEventHandler("voltre:staff:setInstance", function(target,newinstace)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    voltre.fct.instance.Set(target, newinstace)
end)

RegisterNetEvent("voltre:staff:resetReportWeekly")
AddEventHandler("voltre:staff:resetReportWeekly", function()
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    if Config.GroupeHighPerm[xPlayer.getGroup()] == nil then
...
ESX.RegisterServerCallback('voltre:GetIDUnique', function(source, cb)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer ~= nil then
...
ESX.RegisterServerCallback('voltre:GetMyIDUnique', function(source, cb)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer ~= nil then
...
ESX.RegisterServerCallback('voltre:GetIDUnique2', function(source, cb, player)
    local _src = source
...
ESX.RegisterServerCallback('voltre:GetByIDUnique', function(source, cb)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    if xPlayer ~= nil then
...
RegisterNetEvent("voltre:staff:verifpackrp")
AddEventHandler("voltre:staff:verifpackrp", function(target_id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then
        DropPlayer(_src, "Vous n'avez pas à faire cela !!!")
        return
    end
    TriggerClientEvent("voltre:detectrppack", target_id)
    xPlayer.showNotification("Vérification du pack en Cours...")
end)

RegisterNetEvent("voltre:staff:addafkpoint")
AddEventHandler("voltre:staff:addafkpoint", function(idu, nbr)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:staff:getInfoForUsersTable', function(source, cb)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
...
RegisterNetEvent("voltre:staff:wipetableUsers")
AddEventHandler("voltre:staff:wipetableUsers", function(inputTable, option)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterNetEvent("voltre:staff:removeafkpoint")
AddEventHandler("voltre:staff:removeafkpoint", function(idu, nbr)
    local xPlayer = ESX.GetPlayerFromId(source)
    player = ReturnPlayerId(idu) or {id = 0}
    local xTarget = ESX.GetPlayerFromId(player.id)
    if xPlayer.getGroup() == "user" then
...
RegisterNetEvent("voltre:staff:updateplayerWeight")
AddEventHandler("voltre:staff:updateplayerWeight", function(src, newWeight)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("voltre:staff:updateinventoryIdUnique:offline")
AddEventHandler("voltre:staff:updateinventoryIdUnique:offline", function(idu, newinventory)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterNetEvent("voltre:staff:updateloadoutIdUnique:offline")
AddEventHandler("voltre:staff:updateloadoutIdUnique:offline", function(idu, newloadout)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
AddEventHandler("vAdminMenu:addstaffglobal", function(idunique, group, discord)
    ...
    if xTarget ~= nil then
        ...
        TriggerClientEvent("voltre:staff:recevieRequestGroup", xTarget.source, {true, xTarget.getGroup()})
        ...
        InitAllStaffs()
        TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, idunique, getLightPlayerData(ESX.PlayersByIdUnique[idunique]), stafflist[idunique])
        voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a ajouter un nouveau staff : "..xTarget.getName().."(U"..xTarget.getIdunique()..") en tant que "..group,"addstaff", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
		...
		ESX.GetNameByIdUnique(idunique, function(result)
            ...
            InitAllStaffs()
            TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, idunique, getLightPlayerData(ESX.PlayersByIdUnique[idunique]), stafflist[idunique])
            voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a ajouter un nouveau staff : "..staffName.."(U"..idunique..") en tant que "..group,"addstaff", {idunique = xPlayer.getIdunique(), idunique_cible = idunique, name = xPlayer.getName(), name_cible = staffName})
...
AddEventHandler("vAdminMenu:rankstaff", function(idunique, rank, name)
    ...
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a changer le rank du staff : "..SaveData.Admin.Staffs.List[idunique].name.." (U"..SaveData.Admin.Staffs.List[idunique].idunique..") en tant que "..rank,"rankstaff", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
    TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, idunique, getLightPlayerData(ESX.PlayersByIdUnique[idunique]), stafflist[idunique])
...
AddEventHandler("vAdminMenu:deletestaff", function(idunique, group, name)
    ...
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a demote le staff : "..SaveData.Admin.Staffs.List[idunique].name.." (U"..SaveData.Admin.Staffs.List[idunique].idunique..")","demotestaff", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
    TriggerClientEvent("voltre:admin:refreshOnePlayer", -1, idunique, getLightPlayerData(ESX.PlayersByIdUnique[idunique]), stafflist[idunique])
...
RegisterNetEvent("voltre:staff:teleport:id")
AddEventHandler("voltre:staff:teleport:id", function(id)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
...
RegisterNetEvent("voltre:staff:teleport:playertocoord")
AddEventHandler("voltre:staff:teleport:playertocoord", function(target, coords, label)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    if xPlayer.getGroup() == "user" then
...
RegisterNetEvent("voltre:staff:messageplayer")
AddEventHandler("voltre:staff:messageplayer", function(target, message)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
...
RegisterNetEvent("voltre:staff:removeplayeritem")
AddEventHandler("voltre:staff:removeplayeritem", function(target, amount, item, label)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer l'item "..label.." ("..item.." x"..amount..") au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-item", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)

RegisterNetEvent("voltre:staff:removeplayerweapon")
AddEventHandler("voltre:staff:removeplayerweapon", function(target, weapon, label)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer l'arme "..label.." ("..weapon..") au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-weapon", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)

RegisterNetEvent("voltre:staff:repairweapon")
AddEventHandler("voltre:staff:repairweapon", function(target, weapon, label)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a réparer l'arme "..label.." ("..weapon..") au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-weapon", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)

RegisterServerEvent("voltre:staff:setStreamer", function(idunique, bool)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if xTarget == nil then
        ...
        TriggerClientEvent("voltre:setStreamer",xTarget.source, bool)
        if bool then
...
RegisterNetEvent("voltre:staff:removeplayerweaponOffline")
AddEventHandler("voltre:staff:removeplayerweaponOffline", function(target, weapon, label)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    if xTarget ~= nil then 
        xTarget.removeWeapon(weapon, 1)
        voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer l'arme "..label.." ("..weapon..") au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-weapon", {idunique = xPlayer.getIdunique(), idunique_cible = target, name = xPlayer.getName(), name_cible = xTarget.getName()})
        return 
    end
    ...
    MySQL.Async.fetchAll('SELECT * FROM users WHERE idunique = @idunique',{
        ...
        if result[1] then
            ...
            voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer l'arme "..label.." ("..weapon..") au joueur : "..result[1].name.." (U"..target..")", "remove-weapon", {idunique = xPlayer.getIdunique(), idunique_cible = target, name = xPlayer.getName(), name_cible = result[1].name})
        else
...
RegisterNetEvent("voltre:staff:player:removemoney")
AddEventHandler("voltre:staff:player:removemoney", function(target, account, amount)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xTarget = ESX.GetPlayerFromId(target)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a retirer "..amount.."$ (cash) au joueur : "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "remove-account", {idunique = xPlayer.getIdunique(), idunique_cible = xTarget.getIdunique(), name = xPlayer.getName(), name_cible = xTarget.getName()})
end)


RegisterNetEvent("voltre:staff:player:idunique:removemoney")
AddEventHandler("voltre:staff:player:idunique:removemoney", function(idunique, account, amount)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
...
AddEventHandler("AdminMenu:giveWeapon", function(target, model, amount, option)
    ...
    voltre.logs.send("Logs Staff","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a give l'arme "..ESX.GetWeaponLabel(model).." ("..model..") a "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "give-weapon", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = xTarget.getIdunique(), name_cible = xTarget.getName()})
end)
...
AddEventHandler("AdminMenu:giveMoneyCash", function(target, amount)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a give "..amount.."$ (cash) a "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "give-account", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = xTarget.getIdunique(), name_cible = xTarget.getName()})
end)
...
AddEventHandler("AdminMenu:giveMoneyBank", function(target, amount)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a give "..amount.."$ (bank) a "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "give-account", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = xTarget.getIdunique(), name_cible = xTarget.getName()})
end)
...
AddEventHandler("AdminMenu:giveMoneyBlack", function(target, amount)
    ...
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
    voltre.logs.send("Logs Staff","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a give "..amount.."$ (dirtycash) a "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "give-account", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = xTarget.getIdunique(), name_cible = xTarget.getName()})
end)
...
AddEventHandler("AdminMenu:kickPlayer", function(target, reason)
    ...
    voltre.logs.send("Logs Staff","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a kick "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "kick", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = xTarget.getIdunique(), name_cible = xTarget.getName()})
end)
...
AddEventHandler("AdminMenu:warn", function(target, reason)
    ...
    voltre.logs.send("Logs Staff","Le joueur "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a warn le joueur "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "warn", {idunique = xPlayer.getIdunique, name = xPlayer.getName(), idunique_cible = xTarget.getIdunique(), name_cible = xTarget.getName()})
...
ESX.RegisterServerCallback("voltre:staff:getPlayerFarmList", function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("voltre:staff:boutique:addcoins", function(idboutique, nbrtoAdd, idunique)
    local xPlayer = ESX.GetPlayerFromId(source) 
    local xTarget = ESX.GetPlayerFromIdUnique(idunique)
    ...
    if xTarget then
        ...
        TriggerClientEvent("voltre:boutique:newCoinsAmount", xTarget.source, getPoints(xTarget.source))
    else
...
ESX.RegisterServerCallback("AdminMenu:getIdUniqueBoutiqueInfo", function(source, cb, idunique)
    ...
    local historique = {}
    if xTarget then
        local identifier = voltre.fct.getIdentifiers(xTarget.source)
            ...
            MySQL.Async.fetchAll("SELECT * FROM tebex_players_wallet WHERE idunique = @idunique", {['@idunique'] = idunique}, function(result)
                if result[1] ~= nil then
                    ...
                    MySQL.Async.fetchAll("SELECT SUM(points) FROM tebex_players_wallet WHERE identifiers = @identifiers", {
                        ...
                        MySQL.Async.fetchAll("SELECT * FROM tebex_voltre_fidelite WHERE license = @license", {
                            ['@license'] = idboutique
                        }, function(result2)
                            local total = 0
                            local havebuy = 0
...
RegisterNetEvent("Voltre:boutique:staff:deletepack")
AddEventHandler("Voltre:boutique:staff:deletepack", function(idunique, selectedPack)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterNetEvent("Voltre:boutique:staff:addpack")
AddEventHandler("Voltre:boutique:staff:addpack", function(idunique, selectedPack, after)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
AddEventHandler("AdminMenu:setRankbyIdUnique", function(idunique, rank)
    ...
    for i = 1, #xPlayers, 1 do
        ...
        if xTarget.getIdunique() == idunique then
            ...
            if rank == "user" then
                TriggerClientEvent("voltre:staff:recevieRequestGroup", xTarget.source, {false, xTarget.getGroup()})
            else
                TriggerClientEvent("voltre:staff:recevieRequestGroup", xTarget.source, {true, xTarget.getGroup()})
            end
        end
...
AddEventHandler("AdminMenu:giveItem", function(target, item, label, amount)
    ...
    voltre.logs.send("Logs Staff","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a give l'item "..ESX.GetItemLabel(item).." (x"..amount..") a "..xTarget.getName().." (U"..xTarget.getIdunique()..")", "give-item", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = xTarget.getIdunique(), name_cible = xTarget.getName()})
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
end)
...
AddEventHandler("AdminMenu:giveItemPerso", function(item, label, amount)
    ...
    --ReworkLogs(w, "give-item", function(err, text, headers) end, 'POST', json.encode({username = "LogsAdmin", embeds = w, avatar_url = ESX.Config("serverCHAR")}), { ['Content-Type'] = 'application/json'})
    voltre.logs.send("Logs Staff","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a give l'item "..ESX.GetItemLabel(item).." (x"..amount..") a lui même", "give-item", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = xPlayer.getIdunique(), name_cible = xPlayer.getName()})
    TriggerClientEvent("AdminMenu:serverInteractFinish", _src)
end)
...
AddEventHandler("AdminMenu:openSkinMenu", function(target)
    ...
    TriggerClientEvent('voltre:openSkinMenu', target)
...
RegisterNetEvent("voltre:admin:freezeVehicle")
AddEventHandler("voltre:admin:freezeVehicle", function(veh)
...
RegisterNetEvent("Voltre:staff2:getiduinfo")
AddEventHandler("Voltre:staff2:getiduinfo", function(idunique, type)
    ...
    if type == "idunique" then
        if ESX.PlayersByIdUnique[idunique] ~= nil then
            MySQL.Async.fetchAll('SELECT * FROM account_info WHERE license = @license',{
                ...
                TriggerClientEvent("voltre:staff:recevieSearchIdUniqueInfo", _src, idunique, toSend)
            end)
        else
            MySQL.Async.fetchAll('SELECT * FROM users WHERE idunique = @idunique',{
                ['@idunique'] = idunique,
...
RegisterNetEvent('Voltre:sendmessage')
AddEventHandler('Voltre:sendmessage', function(args)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then 
...
ESX.RegisterServerCallback('Voltre:staff2:getiduinfo', function(source, cb)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local identifier = nil
...
RegisterNetEvent("voltre:staff:newnotestaff")
AddEventHandler("voltre:staff:newnotestaff", function(Informations)
    local src = source
...
ESX.RegisterServerCallback('Voltre:staff:getStaffNotes', function(source, cb, idunique)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterCommand('report', function(source, args, rawCommand)
    ...
    elseif #table.concat(args, " ") >= 45 then
        ...
        if reportsTable[source] ~= nil then
            ...
            TriggerClientEvent("voltre:reports:refreh", -1, reportsTable)
            VoltreIncrementReportMade(xPlayer.getIdunique())
            voltre.logs.send("Logs Joueur","Le joueur "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a fais un report : "..reportsTable[source].args.."", "create-report", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
        end
    end
end, false)
...
AddEventHandler("AdminMenu:closeReport", function(reportId)
    ...
    voltre.logs.send("Logs Staff","Le Staff "..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a close le report de "..reportsTable[reportId].nom.." (RU"..reportId.." U"..reportsTable[reportId].idu..")", "close-report", {idunique = xPlayer.getIdunique(), name = xPlayer.getName(), idunique_cible = reportsTable[reportId].idu, name_cible = reportsTable[reportId].nom})
    notifStaff(GetPlayerName(source).." a cloturé le report #"..reportsTable[reportId].uniqueId)
    TriggerClientEvent("voltre:staff:openreportnote",reportsTable[reportId].id ,{
...
RegisterNetEvent("voltre:staff:requestGroupVerif", function(target)
    if target ~= nil then 
        local src = target
        local xPlayer = ESX.GetPlayerFromId(src)
        if xPlayer.getGroup() ~= "user" then
            TriggerClientEvent("voltre:staff:recevieRequestGroup", src, {true, xPlayer.getGroup()})
        else
            TriggerClientEvent("voltre:staff:recevieRequestGroup", src, {false,"user"})
        end
    else
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterNetEvent("voltre:staff:activeBlips", function(bool)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    if bool == false then bool = nil end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(SaveData.StaffWithBlips) do
            TriggerClientEvent("voltre:staff:recevieBlips", k, SaveData.Admin.PlayersBlips)
        end
    end
end)]]
