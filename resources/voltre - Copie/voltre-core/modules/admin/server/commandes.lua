...
RegisterCommand("register", function(source, args, rawCommand)
	...
	if Showcase and Showcase.IsEnabled() and source ~= 0 then
		...
		TriggerClientEvent('voltre:newCreator:open', source, true)
		return
	end
	...
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then 
        if xTarget == nil then
            ...
            TriggerClientEvent('voltre:newCreator:open', xTarget.source, true)
...
RegisterCommand("tppc", function(source, args, rawCommand)
	...
	local xTarget = nil
	if xPlayer.getGroup() == 'user' then return end
	if voltre.fct.format.isNumber(args[1]) then 
		local xTarget = ESX.GetPlayerFromId(ReturnPlayerId(args[1]).source)
		if xTarget == nil then return end 
...
RegisterCommand("goto", function(source, args, rawCommand)
	...
	if xPlayer.getGroup() ~= 'user' then 
		...
		if voltre.players.instances[xTarget.source] ~= nil and voltre.players.instances[xTarget.source].number ~= 0 and ((voltre.players.instances[xPlayer.source] ~= nil and voltre.players.instances[xPlayer.source].number ~= voltre.players.instances[xTarget.source].number) or voltre.players.instances[xPlayer.source] == nil) then
			TriggerClientEvent('chat:addMessage',source, { args = { '^3STAFF', "Ce joueur est dans une instance : "..voltre.players.instances[xTarget.source].label.. ", vous avez été déplacer dedans."} })
			voltre.fct.instance.Set(xPlayer.source, voltre.players.instances[xTarget.source].number, voltre.players.instances[xTarget.source].label.." (TPA)")
		end
...
RegisterCommand("gotomain", function(source, args, rawCommand)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() ~= 'user' then 
		if voltre.players.instances[xPlayer.source] ~= nil and voltre.players.instances[xPlayer.source].number ~= voltre.players.instances[xTarget.source].number then
			voltre.fct.instance.Set(xPlayer.source, 0, "Main")
		end
...
RegisterCommand("getid", function(source, args, rawCommand)
	...
	TriggerClientEvent("voltre:client:copy", source, xTarget.source)
...
RegisterCommand("annonce", function(source, args)
	...
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then 
		for i = 1, #xPlayers, 1 do
			...
			--TriggerClientEvent("voltre:sendAnnouce", xPlayers[i], message, 5000, "https://i.ibb.co/1YrLkQXj/Annonces.png")
			ESX.ShowAnnouncementToAll(message, 5000)
			--TriggerClientEvent("voltre:annonce", xPlayer.source, "ANNONCE", message, 5000, {"Event_Start_Text", "GTAO_FM_Events_Soundset"})
		end
