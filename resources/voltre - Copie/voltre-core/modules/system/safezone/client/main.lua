...
Citizen.CreateThread(function()
    Wait(2000)
    TriggerServerEvent('voltre:safezone:init')
end)


RegisterNetEvent('voltre:safezone:init')
AddEventHandler('voltre:safezone:init', function(Table)
	if voltre.data.system.safezone.list ~= nil then
		voltre.data.system.safezone.list = nil
	end
	if zones ~= nil then
	...
	zones = {}
	voltre.data.system.safezone.list = Table
	voltre.data.system.safezone.load = true
...
local function setSafeZoneUi(state)
	local ok, err = pcall(function()
		TriggerEvent("voltre-ui:setSafeZone", state)
	end)

	if not ok then
		voltre.DebugPrint(("Safezone UI event failed: %s"):format(tostring(err)))
	end
end
...
Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
	...
	while voltre.data.system.safezone.load == false do
		Wait(10)
	end
	...
	while true do
		...
		if PlayerState.isInSafeZone then
			...
			if voltre.data.jobs.polices.list[ESX.PlayerData.job.name] ~= nil then 
				...
				voltre.DebugPrint("Safezone, set unarmed")
			end
			if not notifIn then
				--NetworkSetFriendlyFireOption(false)
				while not DisplayHud do Wait(100) end
				setSafeZoneUi(true)
				voltre.DebugPrint("Entered safezone: " .. (PlayerState.safeZoneName or "unknown"))
			...
			if not notifOut then
				...
				while not DisplayHud do Wait(100) end
				setSafeZoneUi(false)
				voltre.DebugPrint("Exited safezone: " .. (PlayerState.safeZoneName or "unknown"))
