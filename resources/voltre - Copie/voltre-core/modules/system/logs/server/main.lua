...
local function sendToDiscord(title, message, logType, metadata)
    ...
    local serverName = ESX.Config("serverName") or "Voltre"
...
function OthersLogsDetails(title, message, logType, metadata) 
    voltre.logs.send(title, message, logType, metadata)
end
...
function voltre.logs.send(title, message, logType, metadata)
    if not title or not message or not logType then
        return voltre.DebugPrint("[Logs] Paramètres manquants pour l'envoi du log")
    end
    ...
    -- Debug
    voltre.DebugPrint(("[Logs] %s: %s"):format(logType, title))
