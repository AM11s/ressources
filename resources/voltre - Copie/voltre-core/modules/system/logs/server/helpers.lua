...
function voltre.logs.playerMeta(xPlayer)
...
function voltre.logs.transactionMeta(xPlayer, xTarget)
    local meta = voltre.logs.playerMeta(xPlayer)
    if xTarget then
...
function voltre.logs.item(action, xPlayer, itemName, count, xTarget)
    ...
    if action == "give" and xTarget then
        ...
        meta = voltre.logs.transactionMeta(xPlayer, xTarget)
    ...
    voltre.logs.send(title, message, logType, meta)
end
...
function voltre.logs.weapon(action, xPlayer, weaponName, xTarget)
...
function voltre.logs.money(action, xPlayer, accountType, amount, xTarget)
...
function voltre.logs.vehicle(action, xPlayer, vehicleName, plate)
...
function voltre.logs.adminCommand(commandName, xStaff, xTarget, details)
    ...
    local meta = voltre.logs.playerMeta(xStaff)
    
    if xTarget then
        message = message .. (" sur **%s**"):format(xTarget.getName())
        meta = voltre.logs.transactionMeta(xStaff, xTarget)
    end
    ...
    voltre.logs.send("Commande Admin", message, commandName, meta)
end
...
function voltre.logs.job(action, xPlayer, jobName, gradeName, xStaff)
    ...
    if action == "setjob" or action == "setjob2" then
        ...
        if xStaff then
            ...
            meta = voltre.logs.transactionMeta(xStaff, xPlayer)
        else
