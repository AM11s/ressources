...
local function setWeatherTime()
    exports['voltre-core']:stopTimeWhile(true)
    exports['voltre-core']:DisplayHud(false)
    SetRainLevel(0.0)
...
local function restoreWeather()
    exports['voltre-core']:stopTimeWhile(false)
    exports['voltre-core']:DisplayHud(true)
end
...
local function captureClothingItem(gender, categoryId, componentType, componentId, drawableId, cameraInfo)
    ...
    TriggerServerEvent('voltre:imagemaker:capture', 'clothing', filename, true)
...
local function captureVehicle(model)
    ...
    awaitingCapture = true
    TriggerServerEvent('voltre:imagemaker:capture', 'vehicle', model, true)
...
local function captureWeapon(weaponName)
    ...
    TriggerServerEvent('voltre:imagemaker:capture', 'weapon', weaponName, true)
...
local function capturePed(pedModel)
    ...
    awaitingCapture = true
    TriggerServerEvent('voltre:imagemaker:capture', 'ped', pedModel, true)
...
local function captureProp(propModel)
    ...
    awaitingCapture = true
    TriggerServerEvent('voltre:imagemaker:capture', 'prop', propModel, true)
...
local function captureUtilsItem(item, utilsInfo)
    ...
    awaitingCapture = true
    TriggerServerEvent('voltre:imagemaker:capture', 'utils', filenameBase, true)
...
RegisterNetEvent('voltre:imagemaker:captureResult', function(success, filename, captureType)
...
local function runBatchCapture()
    ...
    TriggerServerEvent('voltre:imagemaker:restart:cache')
...
RegisterNUICallback('imagemaker:getConfig', function(_, cb)
    if not config then
        ESX.TriggerServerCallback('voltre:imagemaker:getConfig', function(cfg)
            config = cfg
            cb(config)
        end)
    else
...
RegisterNUICallback('imagemaker:scanImages', function(_, cb)
    TriggerServerEvent('voltre:imagemaker:scanImages')
    cb('ok')
end)

RegisterNetEvent('voltre:imagemaker:scanResult', function(data)
    SendNUIMessage({
...
RegisterNUICallback('imagemaker:deleteImage', function(data, cb)
    TriggerServerEvent('voltre:imagemaker:deleteImage', data.type, data.filename)
    cb('ok')
end)

RegisterNetEvent('voltre:imagemaker:deleteResult', function(success, filename, deleteType)
    SendNUIMessage({
...
RegisterNUICallback('imagemaker:customCapture', function(data, cb)
    ...
    awaitingCapture = true
    TriggerServerEvent('voltre:imagemaker:capture', saveType, saveName, true)
