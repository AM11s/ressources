...
-- Scans game data and coordinates with voltre-image-maker resource
-- ============================================================================
...
ESX.RegisterServerCallback('voltre:imagemaker:getConfig', function(source, cb)
    cb({
...
RegisterNetEvent('voltre:imagemaker:restart:cache', function()
    local src = source
    if src ~= 0 then
        local xPlayer = ESX.GetPlayerFromId(src)
        if not xPlayer.getPermission("dev") then return end
    end
    ExecuteCommand('ensure voltre-cache')
end)

voltre.InitPrint("ImageMaker Server Module Loaded")
