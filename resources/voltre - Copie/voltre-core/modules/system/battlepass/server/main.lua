        ...
        for _, playerId in ipairs(players) do
            ...
            if xPlayer and xPlayer.identifier then
                ...
                if data then
                    ...
                    TriggerClientEvent('voltre:battlepass:xpGain', playerId, xpAmount, data.level, data.xp, BattlePass.GetRequiredXP(data.level))

                    if leveled then
                        TriggerClientEvent('voltre:battlepass:levelUp', playerId, data.level)
                    end
                end
            end
        end
...
-- SERVER CALLBACKS
-- ============================================================================
ESX.RegisterServerCallback('voltre:battlepass:getData', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:battlepass:claimReward', function(source, cb, level, track)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:battlepass:buyLevel', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then cb(false, "Joueur introuvable") return end
...
-- ============================================================================
ESX.RegisterServerCallback('voltre:vip:getFullData', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then cb(nil) return end
...
ESX.RegisterServerCallback('voltre:vip:getDeathTimer', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterCommand('addBPXP', function(source, args)
    ...
    if data then
        TriggerClientEvent('voltre:battlepass:xpGain', targetId, amount, data.level, data.xp, BattlePass.GetRequiredXP(data.level))
        if leveled then
            TriggerClientEvent('voltre:battlepass:levelUp', targetId, data.level)
        end
    end
...
voltre.InitPrint('^2Battle Pass module loaded^7')
