...
RegisterNetEvent('voltre:battlepass:xpGain', function(amount, newLevel, newXP, requiredXP)
...
RegisterNetEvent('voltre:battlepass:levelUp', function(newLevel)
    BattlePassData = BattlePassData or {}
    BattlePassData.level = newLevel
...
RegisterNUICallback('battlepass:getData', function(data, cb)
    ESX.TriggerServerCallback('voltre:battlepass:getData', function(result)
        if result then
...
RegisterNUICallback('battlepass:claimReward', function(data, cb)
    ESX.TriggerServerCallback('voltre:battlepass:claimReward', function(success, msg, updatedClaims)
        cb({
            success = success,
            message = msg,
...
RegisterNUICallback('battlepass:buyLevel', function(data, cb)
    ESX.TriggerServerCallback('voltre:battlepass:buyLevel', function(success, msg, updatedData)
        cb({
            success = success,
            message = msg,
...
RegisterNUICallback('boutique:getVipData', function(data, cb)
    ESX.TriggerServerCallback('voltre:vip:getFullData', function(result)
        cb(result or {})
    end)
end)
...
RegisterNUICallback('boutique:getVipPurchaseInfo', function(data, cb)
    ESX.TriggerServerCallback('voltre:boutique:getVipPurchaseInfo', function(result)
        cb(result or {})
    end)
end)
...
RegisterNUICallback('boutique:buyVip', function(data, cb)
    ...
    TriggerServerEvent('voltre:boutique:buyVip', data.tier)
    cb({ success = true })
end)

voltre.InitPrint('^2Battle Pass module (client) loaded^7')
