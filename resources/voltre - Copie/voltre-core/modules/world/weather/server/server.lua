...
RegisterServerEvent('voltre:weather:changeFreeze')
AddEventHandler('voltre:weather:changeFreeze', function(freezeTime2, freezeWeather2, bool)
...
RegisterServerEvent('voltre:weather:changeTime')
AddEventHandler('voltre:weather:changeTime', function(newHours, newMinutes)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterServerEvent('voltre:weather:requestWeatherTimer')
AddEventHandler('voltre:weather:requestWeatherTimer', function()
    local src = source
...
RegisterServerEvent('voltre:weather:changeBlackOutAnim')
AddEventHandler('voltre:weather:changeBlackOutAnim', function(newblackoutAnim)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterServerEvent('voltre:weather:changeBlackout')
AddEventHandler('voltre:weather:changeBlackout', function(value)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('voltre:weather:changeWeather')
AddEventHandler('voltre:weather:changeWeather', function(newWeather, bool)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    if bool then
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a /weather "..string.lower(args[1]), "weather", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
    ...
    elseif source ~= 0 then
        if isAllowedToChange(source) then
            if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
                ...
                localTime.hours, localTime.minutes = argh, argm
                voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a /time en "..newtime, "time", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
            ...
            if not freezeTime then
                ...
                if localTime.minutes >= 60 then
                    ...
                    if Config.Weather.Debug then
                        voltre.DebugPrint("Minutes Updated: 60 -> 0")
                    end
                    localTime.hours += 1
                    if Config.Weather.Debug then
                        voltre.DebugPrint("Hours Updated: "..tostring(localTime.hours - 1).." -> "..localTime.hours.."")
                    end
                end
                localTime.minutes += 1 
                if Config.Weather.Debug then
                    voltre.DebugPrint("Minutes Updated: "..tostring(localTime.minutes - 1).." -> "..localTime.minutes.."")
                end
            end
...
function NextWeatherStage()
    if freezeWeather then voltre.DebugPrint("[^5Voltre^7]  Tentative de changement de Météo alors qu'elle est Freeze.") return end
    local date = os.date("*t")
    ...
    newWeatherTimer = DefautNextWeahterTime
    if debugprint then
        voltre.DebugPrint("[^5Voltre^7]  Un nouveau type de météo aléatoire a été généré: " .. generatedWeather .. ".")
        voltre.DebugPrint("[^5Voltre^7]  Réinitialiser la minuterie à "..DefautNextWeahterTime.." minutes.")
    end
...
voltre.InitPrint("Weather Module Initialized")