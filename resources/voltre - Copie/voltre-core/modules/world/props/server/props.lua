RegisterNetEvent('Voltre:props:place', function(name, label, id, coords, heading, isForStaff)
    local source = source
        ...
        MySQL.Async.execute('INSERT INTO world_props (name, label, owner, position, heading, iid) VALUES (@name, @label, @owner, @position, @heading, @iid)', {
            ...
            MySQL.Async.fetchAll('SELECT * FROM world_props WHERE iid = @iid', {
                ...
                for k,v in pairs(result) do
                    ...
                    TriggerClientEvent('voltre:world:propsaddTable', -1, v.id,  SaveData.World.Props[v.id])
                end
            end)
        end)
    else
        local instance = voltre.fct.instance.Get(xPlayer.source)
        if instance then
...
RegisterNetEvent('voltre:world:propsserver:loadProps', function()
    local source = source
    ...
    MySQL.Async.fetchAll('SELECT * FROM world_props', {}, function(result)
        for k,v in pairs(result) do
            ...
            TriggerClientEvent('voltre:world:propsaddTable', source, v.id,  SaveData.World.Props[v.id])
        end
    end)
end)

RegisterNetEvent('voltre:world:propsserver:delete', function(data)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    MySQL.Async.execute('DELETE FROM world_props WHERE id = @id', {
        ...
        TriggerClientEvent('voltre:world:propsremoveTable', -1, data.propsId)
        SaveData.World.Props[data.propsId] = nil
    end)
end)

RegisterNetEvent('voltre:props:server:changeOwner', function(data, newidu)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    TriggerClientEvent('voltre:world:propsaddTable', source, data.propsId,  SaveData.World.Props[data.propsId])
end)
