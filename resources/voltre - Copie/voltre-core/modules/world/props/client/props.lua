    ...
    Citizen.CreateThread(function()
        ...
        while inPreviewMode do
            ...
            if IsControlJustPressed(0, 215) then -- Enter
                ...
                TriggerServerEvent('Voltre:props:place', name2, name, id, objectCoords, objectRotation, isForStaff)
...
RegisterNetEvent('Voltre:props:place', function(data)
    activePreviewMode(data.name, data.label, 'test', data.id, data.name)
end)

RegisterNetEvent('voltre:world:propsaddTable', function(iid, table)
    voltre.data.world.props.propsSpawned[iid] = table
end)

RegisterNetEvent('voltre:world:propsremoveTable', function(iid)
    local entity = voltre.data.world.props.propsSpawned[iid].entity
    voltre.data.world.props.propsSpawned[iid] = nil
...
Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
    ...
    voltre.fct.waitPlayerLoaded()
    Wait(5000)
    TriggerServerEvent('voltre:world:propsserver:loadProps')
    Wait(5000)
    ...
    while true do
        ...
        for k,v in pairs(voltre.data.world.props.propsSpawned) do
            VoltreCoord = GetEntityCoords(PlayerPedId())
            if v == nil then goto continue end
            if #(VoltreCoord - vector3(v.position.x, v.position.y, v.position.z)) > 50.0 then
            ...
            if #(VoltreCoord - vector3(v.position.x, v.position.y, v.position.z)) < 2.5 then
                loopWait = 0
                ...
                if PlayerState.WorldPropsInfo then
                    if IsControlJustPressed(0, 215) then
                        if nTable.staffMode then
                            if nTable.objets then
                                TriggerServerEvent('voltre:world:propsserver:delete', v)
                            else
...
function setobject()
    Citizen.CreateThread(function()
        while nTable.objets do
            Wait(500)
            for _,v in pairs(voltre.data.world.props.propsSpawned) do
                if not DoesBlipExist(v.blip) then
                    ...
                    voltre.data.world.props.propsSpawned[v.propsId].blip = v.blip
                end
            end
        end
	...
	while categoryMenu do
        ...
        RageUI.IsVisible(categoryMenu, function()
            if categoryResearch == nil then
                RageUI.Button(("Rechercher"):format(), nil, {}, true, {
                    ...
                    onSelected = function()
                        voltre.fct.inputCb("Nom de la catégorie",function(result) 
                            if result ~= nil then
        ...
        RageUI.IsVisible(propMenu, function()
            if research == nil then
                RageUI.Button(("Rechercher"):format(), nil, {}, true, {
                    onSelected = function()
                        voltre.fct.inputCb("Entrez le nom de l\'objet",function(result) 
                            if result ~= nil then
                                research = result
                            end
                        end);
