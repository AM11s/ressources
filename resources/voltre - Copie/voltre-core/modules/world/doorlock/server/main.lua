...
local function notify(src, msg)
    TriggerClientEvent('voltre:notification', src, msg)
end
...
local function isEditor(xPlayer)
    ...
    -- Vérification par grade (système voltre).
    local gradeTable = Config and Config.GroupeGrade or nil
...
local function loadDoors()
    local result = MySQL.query.await('SELECT * FROM voltre_doorlock') or {}
    Doors = {}
    ...
    DoorsLoaded = true
    voltre.DebugPrint(('[Doorlock] %d porte(s) chargée(s) depuis la DB'):format(#result))
    TriggerClientEvent('voltre_doorlock:syncAll', -1, buildClientPayload())
end
...
CreateThread(function()
    ...
    MySQL.ready(function()
        ...
        pcall(function()
            MySQL.query.await("ALTER TABLE `voltre_doorlock` ADD COLUMN IF NOT EXISTS `code` VARCHAR(32) DEFAULT NULL")
            MySQL.query.await("ALTER TABLE `voltre_doorlock` ADD COLUMN IF NOT EXISTS `admin_rank` VARCHAR(32) DEFAULT NULL")
        end)
        loadDoors()
    end)
end)
...
RegisterNetEvent('voltre_doorlock:requestSync', function()
    local src = source
    if not DoorsLoaded then return end
    TriggerClientEvent('voltre_doorlock:syncAll', src, buildClientPayload())
end)

-- Le menu éditeur demande la liste détaillée (admins only).
ESX.RegisterServerCallback('voltre_doorlock:getEditorList', function(source, cb)
...
local function scheduleAutolock(door)
    ...
    SetTimeout(door.autolock * 1000, function()
        ...
        if d and not d.locked then
            d.locked = true
            MySQL.update('UPDATE voltre_doorlock SET locked = 1 WHERE id = @id', { ['@id'] = d.id })
            TriggerClientEvent('voltre_doorlock:setState', -1, d.id, true, true)
        end
    end)
end
...
local function applyToggle(door)
    ...
    MySQL.update('UPDATE voltre_doorlock SET locked = @locked WHERE id = @id', {
        ['@locked'] = door.locked and 1 or 0,
        ['@id']     = door.id,
    })
    TriggerClientEvent('voltre_doorlock:setState', -1, door.id, door.locked, false)
    if not door.locked then scheduleAutolock(door) end
end

RegisterNetEvent('voltre_doorlock:toggle', function(doorId, viaLockpick)
    local src = source
...
RegisterNetEvent('voltre_doorlock:tryCode', function(doorId, code)
    local src = source
    local door = Doors[doorId]
...
ESX.RegisterServerCallback('voltre_doorlock:canAccess', function(source, cb, doorId)
    local door = Doors[doorId]
...
RegisterNetEvent('voltre_doorlock:lockpickResult', function(doorId, success)
    local src = source
    local door = Doors[doorId]
...
ESX.RegisterServerCallback('voltre_doorlock:create', function(source, cb, data)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    local insertId = MySQL.insert.await(
        'INSERT INTO voltre_doorlock (label, locked, doors, groups, items, code, admin_rank, lockpick, autolock) VALUES (@label, @locked, @doors, @groups, @items, @code, @admin_rank, @lockpick, @autolock)',
        {
    ...
    TriggerClientEvent('voltre_doorlock:addOne', -1, buildClientDoor(Doors[insertId]))
    cb(true, insertId)
end)

ESX.RegisterServerCallback('voltre_doorlock:update', function(source, cb, doorId, data)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not isEditor(xPlayer) then return cb(false, 'Accès refusé') end
    local door = Doors[doorId]
    ...
    MySQL.update('UPDATE voltre_doorlock SET label=@label, groups=@groups, items=@items, code=@code, admin_rank=@admin_rank, lockpick=@lockpick, autolock=@autolock WHERE id=@id', {
        ['@label']      = door.label,
    ...
    TriggerClientEvent('voltre_doorlock:addOne', -1, buildClientDoor(door))
    cb(true)
end)

ESX.RegisterServerCallback('voltre_doorlock:delete', function(source, cb, doorId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not isEditor(xPlayer) then return cb(false, 'Accès refusé') end
    if not Doors[doorId] then return cb(false, 'Porte introuvable') end
    MySQL.update('DELETE FROM voltre_doorlock WHERE id = @id', { ['@id'] = doorId })
    Doors[doorId] = nil
    TriggerClientEvent('voltre_doorlock:removeOne', -1, doorId)
    cb(true)
end)
