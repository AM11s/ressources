...
local function notify(msg)
    exports['voltre-core']:SendNotification(msg)
end
...
local function playDoorSound(coords, locked)
    ...
    pcall(function()
        exports['voltre-deps']:PlayUrlPos(soundId, url, DoorlockConfig.Sounds.volume, coords, false)
        exports['voltre-deps']:Distance(soundId, DoorlockConfig.Sounds.maxDistance)
        exports['voltre-deps']:setVolumeMax(soundId, DoorlockConfig.Sounds.volume)
    end)
end
...
RegisterNetEvent('voltre_doorlock:syncAll', function(payload)
...
RegisterNetEvent('voltre_doorlock:addOne', function(door)
    if Doors[door.id] then unregisterDoor(door.id) end
    registerDoor(door)
end)

RegisterNetEvent('voltre_doorlock:removeOne', function(id)
    unregisterDoor(id)
end)

RegisterNetEvent('voltre_doorlock:setState', function(id, locked, silent)
    local door = Doors[id]
    if not door then return end
    door.locked = locked
...
AddEventHandler('voltre:player:spawned', function()
    TriggerServerEvent('voltre_doorlock:requestSync')
end)
CreateThread(function()
    Wait(2000)
    TriggerServerEvent('voltre_doorlock:requestSync')
        ...
        if id and center then
            ...
            if dist <= DoorlockConfig.InteractDistance and not codePadOpen then
                ...
                if IsControlJustReleased(0, ConfigAllKeys[DoorlockConfig.InteractKey] or 51) then
                    TriggerServerEvent('voltre_doorlock:toggle', id, false)
                end
...
RegisterNUICallback('doorlock:submitCode', function(data, cb)
    if data and data.id and data.code then
        TriggerServerEvent('voltre_doorlock:tryCode', data.id, tostring(data.code))
    end
    closeCodePad()
    cb({ ok = true })
end)
