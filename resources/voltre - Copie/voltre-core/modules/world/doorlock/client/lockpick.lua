-- ============================================================================
-- Voltre Doorlock — Crochetage (lockpick)
...
local function attemptLockpick()
    ...
    local id = exports['voltre-core']:GetNearestDoorId()
    if not id then return end

    local doors = exports['voltre-core']:GetClientDoors()
    ...
    -- Mini-jeu ox_lib (lib est dispo via @voltre-deps/modules/ox_lib/init.lua).
    ...
    TriggerServerEvent('voltre_doorlock:lockpickResult', id, success)

    if success then
        notify("Serrure crochetée.")
        -- Déverrouille via le serveur (flag viaLockpick = bypass permission).
        TriggerServerEvent('voltre_doorlock:toggle', id, true)
    else
...
RegisterCommand('+voltre_lockpick', attemptLockpick, false)
RegisterCommand('-voltre_lockpick', function() end, false)
RegisterKeyMapping('+voltre_lockpick', 'Crocheter la porte ciblée', 'keyboard', '')
