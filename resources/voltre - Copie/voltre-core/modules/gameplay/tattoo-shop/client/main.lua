...
RegisterNetEvent("Voltre1buyTattoo")
AddEventHandler("Voltre1buyTattoo", function(ok)
    if ok then
...
RegisterNetEvent("voltre:tattoo:player:callback")
AddEventHandler("voltre:tattoo:player:callback", function(ok)
    if ok ~= nil then
...
AddEventHandler("Voltre:skinchanger:loadSkin", function(skin)
...
RegisterNetEvent("voltre:tattoo:clean")
AddEventHandler("voltre:tattoo:clean", function(ok)
    if ok then
        playerTatoos = {}
...
--   `voltre:tattoo:player:callback`, `voltre:tattoo:clean`) reste active et
--   nécessaire au chargement des tattoos du joueur.
-- ============================================================================

function OpenTattooRageUIMenu()
    if exports["voltre-core"] and exports["voltre-core"].OpenShop then
        exports["voltre-core"]:OpenShop("tattoo", nil)
    end
end
			...
			while Tatouage.Menu do
                ...
                RageUI.IsVisible(RMenu:Get('tattoo', 'main'), function()
                    RageUI.Button("Supprimer vos Tatouages actuelles", nil, { RightLabel = "~g~50000 ~s~$" }, not cleaning, {
                        onSelected = function()
                            cleaning = true
                            TriggerServerEvent("voltre:tattoo:pay:clean")
                        end
                    })
                    for k,v in pairs(Config.Tattoo.TattooCategories) do
                            ...
                            RageUI.Button("Tatouage #"..index, nil, { RightLabel = "~g~"..tatoo.price.."~s~ $ →" }, true, {
                                onSelected = function()
                                    ...
                                    if needModif then
                                        ...
                                        TriggerServerEvent("voltre:tattoo:pay", tatoo.price, playerTatoos)
                                    else
