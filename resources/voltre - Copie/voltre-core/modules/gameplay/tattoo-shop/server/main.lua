RegisterNetEvent("voltre:tattoo:pay")
AddEventHandler("voltre:tattoo:pay", function(price,playerTatoos)
    local _src = source
    ...
    if xMoney >= price then
        ...
        TriggerClientEvent("voltre:tattoo:callback:pay", _src, true)
        performDbUpdate(playerTatoos,_src)
    else
        TriggerClientEvent("voltre:tattoo:callback:pay", _src, false)
    end
end)

-- ============================================================================
-- Voltre:tattoo:purchase
--   Variante de `voltre:tattoo:pay` qui MERGE les nouveaux tattoos avec les
...
-- ============================================================================
RegisterNetEvent("Voltre:tattoo:purchase")
AddEventHandler("Voltre:tattoo:purchase", function(price, newTattoos)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `playerstattoos` WHERE identifier = @identifier",
        ...
        function(rslt)
            ...
            TriggerClientEvent("voltre:tattoo:player:callback", _src, json.encode(existing))
        end)
end)
...
RegisterNetEvent("voltre:tattoo:pay:clean")
AddEventHandler("voltre:tattoo:pay:clean", function()
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
    local xMoney = xPlayer.getAccount('cash').money
    if xMoney >= 50000 then
        ...
        TriggerClientEvent("voltre:tattoo:clean", _src, 1)

    else
        TriggerClientEvent("voltre:tattoo:clean", _src, 0)
    end
end)

RegisterNetEvent("voltre:tattoo:player:request:tattoo")
AddEventHandler("voltre:tattoo:player:request:tattoo", function()
    local _src = source
    local license = getPlayerLicense(_src)
    local result = nil
    ...
    Citizen.Wait(150)
    TriggerClientEvent("voltre:tattoo:player:callback", _src, result)
end)
