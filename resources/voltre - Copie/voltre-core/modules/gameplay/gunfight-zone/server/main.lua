...
RegisterServerEvent('voltre:zonegf:getallzone')
AddEventHandler('voltre:zonegf:getallzone', function()
    while not GF.isLoad do Wait(100) end
    TriggerClientEvent("voltre:zonegf:initzonegf", source, SaveData.json["zonegf"])
end)


RegisterServerEvent('voltre:zonegf:create')
AddEventHandler('voltre:zonegf:create', function(data)
    ...
    TriggerClientEvent("voltre:zonegf:initzonegf", -1, SaveData.json["zonegf"])
end)

RegisterServerEvent('voltre:zonegf:delete')
AddEventHandler('voltre:zonegf:delete', function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
...
RegisterServerEvent('voltre:zonegf:close')
AddEventHandler('voltre:zonegf:close', function(id, bool)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    if SaveData.json["zonegf"][id] ~= nil then SaveData.json["zonegf"][id].isOpen = bool end
    TriggerClientEvent("voltre:zonegf:editOpen", -1, id, bool)
end)

ESX.RegisterServerCallback('voltre:zonegf:join', function(source, cb, zoneid, type)
    if SaveData.json["zonegf"][zoneid] ~= nil then
        if type == "Join" then
            ...
            voltre.fct.instance.Set(source, 75576, "Zone GunFight")
            ...
            TriggerClientEvent('voltre:gunfight:newStat', -1, 'p', SaveData.json["zonegf"][zoneid].nbrPlayers, zoneid)
...
local Anti2FoisVoltreGF = {}
RegisterServerEvent('voltre:gunfight:newStat')
AddEventHandler('voltre:gunfight:newStat', function(...)
    local Args = {...}
    local CoolDown = {}
    if not Args then return end

    if Anti2FoisVoltreGF[source] ~= nil then
        if os.time() - Anti2FoisVoltreGF[source] < 3 then 
            return 
        else
            Anti2FoisVoltreGF[source] = nil
        end
    end

    Anti2FoisVoltreGF[source] = os.time()

    if Args[1] and Args[2] then
        local xDeath = ESX.GetPlayerFromId(Args[2])
        local xKiller = ESX.GetPlayerFromId(Args[1])
        TriggerClientEvent("voltre:zonegf:refreshStats", xDeath.source, "death")
        TriggerClientEvent("voltre:zonegf:refreshStats", xKiller.source, "kill")
...
RegisterServerEvent('voltre:gunfight:admin:resetleader')
AddEventHandler('voltre:gunfight:admin:resetleader', function()
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('voltre:gunfight:admin:kickzonegf')
AddEventHandler('voltre:gunfight:admin:kickzonegf', function(type, zoneid)
    local xPlayer = ESX.GetPlayerFromId(source)
    if Config.GroupeHighPerm[xPlayer.getGroup()] == nil then return end
    if type == "all" then
        ...
        for k,v in pairs(SaveData.json["zonegf"]) do
            for i,x in pairs(v.Players) do
                TriggerClientEvent("voltre:zonegf:leave", i)
            end
        end
...
ESX.RegisterServerCallback('voltre:gunfight:getAll', function(source, cb, ...)
    local Args = {...}
    if not Args then return end
