        ...
        for k,v in pairs(cfg_garage_entreprise) do
            --if ESX.PlayerData.job.name == v.job then
                ...
                if distance < 2.0 then
                    sleep = 0
                    voltre.fct.draw.Text3DBar(v.pos.x, v.pos.y, v.pos.z, "Appuyez sur [~y~E~s~] pour ouvrir le garage~s~.")
                    ...
                    onSelected = function()
                        ...
                        if found then 
                            ESX.Game.SpawnVehicle(k, coords, heading, function(vehicle)
                                ...
                                TriggerServerEvent('Voltre:garage:addTempKey', GetVehicleNumberPlateText(vehicle))
                            end)
                        end
                        RageUI.CloseAll()
                    end
...
Citizen.CreateThread(function()
    while not voltre.data.markers.loaded do
        Wait(100)
    end
    for k,v in pairs(Config.Garage.JobsGarages) do 
        voltre.data.markers.register("jobgarage_"..k.."_spawn", {
            Position = v.coords,
            Public = false,
            Job = k,
            Blip = {
        ...
        voltre.data.markers.register("jobgarage_"..k.."_delete", {
            Position = v.delete, 
            Public = false,
            Job = k,
            Action = function()
