RegisterNetEvent('voltre:opencreategarage', function(Table)
    openCreateGarage()
end)
VehicleSort = {}
...
function openMenuGarage(SpawnPoint, TypeGarage, garageid)
    ...
    ESX.TriggerServerCallback('voltre:getOwnedCars', function(ownedCars, ownedCarsJobs, OwnedCarsOrg)
    ...
    while menu do
        ...
        RageUI.IsVisible(menu, function()
            ...
            RageUI.Button('Renommer le véhicule', nil, {}, true, {
                onSelected = function()
                    voltre.fct.inputCb("Quelle nom voulez vous mettre?",function(label) 
                        TriggerServerEvent('voltre:renameVehicle', vehicle.owner, vehicle.vehicle, label)
                        RageUI.CloseAll()
                        Wait(150)
                ...
                RageUI.Button('Donner le véhicule', 'Permet de donner le véhicule au joueurs le plus proche de vous', {}, true, {
                    ...
                    onSelected = function()
                        ...
                        if player == -1 or closestplayer > 3.0 then
                            ...
                            TriggerServerEvent('Voltre:changevehicleowner', GetPlayerServerId(player), vehicle)
                            RageUI.CloseAll()
                        end
                    end
                })
                ...
                RageUI.Button('Attribuer le véhicule à son entreprise', 'Permet d\'attribuer le véhicule à votre entreprise\n~r~IMPOSSIBLE DE LE RECUPERER PAR LA SUITE', {}, true, {
                    onSelected = function()
                        ...
                        if not vehicle.boutique then
                            TriggerServerEvent('Voltre:AttribuerVehicule', 'job', vehicle)
                        else
            ...
            if not gang and not isBoutique then
                RageUI.Button('Attribuer le véhicule à son Gang/Organisation', 'Permet d\'attribuer le véhicule à votre Gang/Organisation\n~r~IMPOSSIBLE DE LE RECUPERER PAR LA SUITE', {}, true, {
                    onSelected = function()
                        RageUI.CloseAll()
                        if not vehicle.boutique then
                            TriggerServerEvent('Voltre:AttribuerVehicule', 'job2', vehicle)
                        else
...
function SpawnVehicle(vehicle, plate, SpawnPoint)
	...
	ESX.Game.SpawnVehicle(vehicle.model, coords, heading, function(vehicleSpawn)
        ...
        TriggerServerEvent('voltre:setstatevehicle', plate, false, vehicleSpawn)
	end)
end
...
function PutVehicleInGarage(vehicle, vehicleProps)
	...
	TriggerServerEvent('voltre:setstatevehicle', vehicleProps.plate, true)
    ...
    while menu do
        ...
        RageUI.IsVisible(menu, function()
            ...
            RageUI.Button('Type de garage', TypeGarage, {RightLabel='car, aircraft ou boat'}, true, {
                onSelected = function()
                    voltre.fct.inputCb("Type de garage (car/aircraft/boat)",function(label) 
                        if label ~= nil then 
                            TypeGarage = label
                        end
                    end)
            ...
            RageUI.Button('~g~Confirmer', nil, {}, true, {
                onSelected = function()
                    RageUI.CloseAll()
                    TriggerServerEvent('voltre:createGarage', NameGarage, PositionGarage, PositionPointSpawn, PositionPointDelete, TypeGarage, BlipActive)
                end
            })
        end, function()
        end)
...
RegisterNetEvent('voltre:refreshGarage', function(Table)
    GarageList = Table
    GarageLoaded = true
...
Citizen.CreateThread(function()
    Wait(1000)
    TriggerServerEvent('voltre:InitGarage')
end)
    ...
    while true do 
        ...
        for k,v in pairs(GarageList) do
            ...
            if DistanceGarage < 3 then
                voltre.fct.draw.Text3DBar(PointGarage.x, PointGarage.y, PointGarage.z, "Appuyez sur ["..ESX.Config("serverColor").."E~s~] pour "..ESX.Config("serverColor").."intéragir~s~")
                    ...
                    if IsControlJustPressed(1,51) then
                        ...
                        if vehicleProps then
                            ...
                            if NewPosition < 20 then
                                ESX.TriggerServerCallback('voltre:storevehicle', function(valid)
                                    if valid then
        ...
        for k,v in pairs(Config.Garage.Impound) do 
            ...
            if FourriereDistance < 3 then
                voltre.fct.draw.Text3DBar(v.PointFourriere.x, v.PointFourriere.y, v.PointFourriere.z, "Appuyez sur ["..ESX.Config("serverColor").."E~s~] pour "..ESX.Config("serverColor").."intéragir~s~")
                 if IsControlJustPressed(1,51) then
                    ...
                    if v.type == 'car' and not v.state then
                        if v.label ~= nil then 
                            RageUI.Button('['..GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))..'] - '..v.label, v.plate, starsBoutique(v.boutique), true, {
                                onSelected = function()
                                    RageUI.CloseAll()
                                    ESX.TriggerServerCallback('voltre:storeVehicleFourriere', function(valid)
                                        if valid then
        ...
        RageUI.IsVisible(menu, function() 
            ...
            if GetEntityHealth(vehicle) >= GetEntityMaxHealth(vehicle)-50 then
                ...
                RageUI.Button('Payer '..priceRepair..'$ et ranger le véhicule', nil, {}, true, {
                    onSelected = function()
                        ESX.TriggerServerCallback('voltre:storevehiclewithmoney', function(valid)
                            if valid then
                                RageUI.CloseAll()
                                PutVehicleInGarage(vehicle, vehicleProps)
                            else
...
RegisterNetEvent('voltre:DeleteEntity',function(Entity)
...
RegisterNetEvent('voltre:UpdateTableVehicleSort', function(table)
    VehicleSort = table
end)
