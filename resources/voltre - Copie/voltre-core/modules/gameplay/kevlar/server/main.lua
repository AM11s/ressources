...
RegisterNetEvent("voltre:kevlar:equip", function(itemName, itemExtra)
    local src = source
    ...
    TriggerClientEvent("voltre:kevlar:equipped", src, {
        itemName = itemName,
        extraIdentifier = itemExtraId,
        durability = durability,
        maxDurability = kevlarConfig.maxDurability,
    ...
    TriggerClientEvent("voltre:inventory:update", src)
end)
...
RegisterNetEvent("voltre:kevlar:unequip", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    TriggerClientEvent("voltre:kevlar:unequipped", src)
    TriggerClientEvent("voltre:inventory:update", src)
end)
...
RegisterNetEvent("voltre:kevlar:damageTick", function(newArmor)
    local src = source
    local equipped = equippedKevlar[src]
    ...
    if newDurability < equipped.durability then
        ...
        if newDurability <= 0 then
            ...
            equippedKevlar[src] = nil
            TriggerClientEvent("voltre:kevlar:destroyed", src)
...
ESX.RegisterServerCallback("voltre:kevlar:getEquipped", function(source, cb)
                ...
                if kevlarConfig and row.durability > 0 then
                    ...
                    SetTimeout(5000, function()
                        if equippedKevlar[playerId] then
                            TriggerClientEvent("voltre:kevlar:equipped", playerId, {
                                itemName = row.item_name,
                                extraIdentifier = extraId,
                                durability = row.durability,
                                maxDurability = kevlarConfig.maxDurability,
