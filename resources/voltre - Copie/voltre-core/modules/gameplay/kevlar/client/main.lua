...
RegisterNetEvent("voltre:kevlar:equipped", function(data)
    ...
    if data.bproof then
        ...
        if bproofData then
            ...
            VoltreInventory.equippedClothes["bproof_1"] = {
                id = data.itemName,
                name = data.itemName,
                data = bproofData,
                label = data.itemName,
...
RegisterNetEvent("voltre:kevlar:unequipped", function()
...
RegisterNetEvent("voltre:kevlar:destroyed", function()
    isKevlarEquipped = false
    kevlarData = nil
    damageCheckActive = false
    lastArmor = 0
    ...
    CreateThread(function()
        while damageCheckActive and isKevlarEquipped do
            ...
            if currentArmor < lastArmor then
                ...
                TriggerServerEvent("voltre:kevlar:damageTick", currentArmor)
...
AddEventHandler("voltre:kevlar:useItem", function(itemName, itemExtra)
    if isKevlarEquipped then
        ...
        if kevlarData and itemExtra and kevlarData.extraIdentifier
            ...
            TriggerServerEvent("voltre:kevlar:unequip")
        else
            TriggerEvent("inventory:sendMessage", "~r~Vous portez déjà un gilet")
        end
        return
    end
    TriggerServerEvent("voltre:kevlar:equip", itemName, itemExtra)
end)

AddEventHandler("voltre:kevlar:removeItem", function()
