...
AddEventHandler('voltre:fuel:startTick', function(pumpObject, ped, vehicle)
    ...
    while isFueling do
        ...
        if currentCash >= currentCost then
            ...
            TriggerEvent('voltre:fuel:update3DStation', currentCost, currentFuel)
        else
            isFueling = false
        end
    end
    ...
    if pumpObject then
        TriggerServerEvent('voltre:fuel:pay', currentCost)
    end
    currentCost = 0.0
end)

AddEventHandler('voltre:fuel:refuelFromPump', function(pumpObject, ped, vehicle)
    ...
    TriggerEvent('voltre:fuel:startTick', pumpObject, ped, vehicle)
    Wait(200)
    TriggerEvent('voltre:fuel:start3DStation', pumpObject, currentCost or 0, currentFuel or 0)

    CreateThread(function()
        while isFueling do
            TriggerEvent('voltre:fuel:update3DStation', currentCost or 0, currentFuel or 0, pumpObject)
            Wait(350)
        end
    end)
    ...
    TriggerEvent('voltre:fuel:stop3DStation')
    isFueling = false
end)
                        ...
                        if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
                            ...
                            if currentCash > 0 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    ...
                                    isFueling = true
                                    TriggerEvent('voltre:fuel:refuelFromPump', isNearPump, ped, vehicle)
                                    FuelLoadAnimDict("timetable@gardener@filling_can")
                                end
                            else
