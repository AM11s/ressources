local url = "nui://voltre-core/modules/_core/ui/html/3dui-fuel/index.html"
    ...
    while true do
        if isDisplayed and sfHandle and drawPos then
            if not txdHasBeenSet then
                ...
                PushScaleformMovieMethodParameterString('voltrefuelTxd')
                PushScaleformMovieMethodParameterString('voltrefuelTex')
...
RegisterNetEvent('voltre:fuel:start3DStation', function(entity, newPrice, newFuel)
    ...
    local txd = CreateRuntimeTxd("voltrefuelTxd")
    ...
    CreateRuntimeTextureFromDuiHandle(txd, "voltrefuelTex", dui)
...
RegisterNetEvent('voltre:fuel:stop3DStation', function()
    isFueling = false
    if duiObj then
...
RegisterNetEvent('voltre:fuel:update3DStation', function(newPrice, newFuel, obj)
