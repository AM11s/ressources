    ...
    MySQL.Async.fetchAll('SELECT * FROM properties_list', {}, function(data)
        ...
        Citizen.CreateThread(function()
            ...
            for _, playerId in ipairs(players) do
                TriggerClientEvent("voltre:properties:UpdatePropertiesList", playerId, SaveData.PropertiesList)
            end
        end)
    end)
end
...
RegisterServerEvent("voltre:properties:CreatedProperties", function(data) 
    ...
    if (xPlayer.getPermission("create-properties")) or (xPlayer.getJob() == "realestateagent" and #(GetEntityCoords(GetPlayerPed(_source)) - data.POSITION.EXIT) < 50) then 
        ...
        if not SaveData.PropertiesList[info.name] then 
            ...
            TriggerClientEvent("voltre:properties:UpdatePropertiesList", -1, SaveData.PropertiesList)
        else 
            TriggerClientEvent('esx:showNotification', _source, "~p~Création de propriété~s~\nUne propriété comporte déjà ce nom.") 
        end
    end
end)
RegisterServerEvent("voltre:properties:deleteProperties", function(data) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent("voltre:properties:virerOwnerProperties", function(data) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    if xPlayer.getJob().name ~= "realestateagent" then
        return
...
RegisterServerEvent("voltre:properties:PlayerBuyPropeties", function(data, player) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    --if #(GetEntityCoords(GetPlayerPed(_source)) - vector3(data.positions.EXIT.x, data.positions.EXIT.y, data.positions.EXIT.z)) < 50 then 
        ...
        if xMoney >= data.price then 
            if SaveData.PropertiesList[data.name] then
                ...
                TriggerClientEvent("voltre:properties:UpdatePropertiesBuyed",-1, data.name, true, xTarget.identifier)
...
RegisterServerEvent("voltre:properties:PlayerHasRenderProperties", function(data, priceRender) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    if itsMyProperties then
        if #(GetEntityCoords(GetPlayerPed(_source)) - vector3(data.positions.COFFRE.x, data.positions.COFFRE.y, data.positions.COFFRE.z)) < 50 then 
            if SaveData.PropertiesList[data.name] then
                ...
                TriggerClientEvent("voltre:properties:UpdatePropertiesBuyed",-1, data.name, false, nil)
                TriggerClientEvent("h:propertySyncProperties", -1)
                TriggerClientEvent("voltre:properties:ExitProperties", _source, data.name)
                TriggerClientEvent("h:propertySyncProperties", -1)
            else 
...
RegisterServerEvent("voltre:properties:PlayerAttribuetPropreties", function(data, value) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local itsMyProperties = SaveData.PropertiesList[data.name] and SaveData.PropertiesList[data.name].owner == xPlayer.identifier or SaveData.PropertiesList[data.name].owner == xPlayer.job.name or SaveData.PropertiesList[data.name].owner == xPlayer.job2.name
        ...
        if SaveData.PropertiesList[data.name] then
            if xPlayer.job.name == "unemployed" or xPlayer.job.name == "unemployed2" then 
                ...
                if value == 1 then
                    ...
                    TriggerClientEvent("voltre:properties:UpdatePropertiesJob",-1, data.name, xPlayer.job.name)
                else
                    ...
                    TriggerClientEvent("voltre:properties:UpdatePropertiesJob",-1, data.name, xPlayer.job2.name)
                end
                TriggerClientEvent("voltre:properties:ExitProperties", _source, data.name)
            end
        else 
...
RegisterServerEvent("voltre:properties:SetBucket", function(interact, number)
    local _source = source
    
    if interact == "solo" then
        voltre.fct.instance.Set(_source, number, "Propriété")
        PlayersInPropertiesForExtended[_source] = true
    elseif interact == "group" then 
        voltre.fct.instance.Set(source, number, "Propriété")
        PlayersInPropertiesForExtended[_source] = true
    elseif interact == "remettre" then 
        voltre.fct.instance.Set(source, 0)
...
RegisterServerEvent("voltre:properties:PlayerSonnedToPropreties", function(data)
    local _source = source
        ...
        if PlayerInProperties[data.name] then 
            if SizeOfTable(PlayerInProperties[data.name]) > 0 then 
                for k,v in pairs(PlayerInProperties[data.name]) do 
                    TriggerClientEvent("voltre:properties:NotificationSonnedProperties", v, data.name, _source)
                end
            else 
                Wait(waitTime)
...
RegisterServerEvent("voltre:properties:ReturnPlayerSonnedProperties", function(interact, id, data, isLonguer)
    local _source = source
    ...
    if interact == "accept" then
        ...
        TriggerClientEvent("voltre:properties:EnterPropertiesBySonnet", id, data, data.bucketID)
...
RegisterServerEvent("voltre:properties:PlayerEntrerOrExitThisProperties", function(data, exitOrEnter) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterNetEvent("voltre:properties:forceExitProperty", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getPermission("FORCEEXITPROPERTY") then
        TriggerClientEvent("voltre:properties:forceExitProperty", id)
    end
end)
ESX.RegisterServerCallback("voltre:properties:getAllInProperty", function(source, cb)
    cb(PlayerInProperties)
end)

ESX.RegisterServerCallback("voltre:properties:inProperty", function(source, cb, id)
    local areIN = false
...
ESX.RegisterServerCallback("voltre:properties:GetProperties", function(source, cb)
    local attempts = 0
...
ESX.RegisterServerCallback("voltre:properties:GetPlayerProperties", function(source, cb)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent("voltre:properties:GetCoffreProperties", function(propertiesName)
    if SaveData.PropertiesList[propertiesName] then
        local _source = source
        TriggerClientEvent("voltre:properties:GetCoffreProperties", _source, SaveData.PropertiesList[propertiesName].data)
    end
end)

RegisterServerEvent("voltre:properties:AddItemToPropertiesCoffre", function(data, item, count) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
                ...
                if infoItem then
                    if CheckQuantityIsValid(count) then 
                        if (weightProperties-weightRestant) >= (infoItem.weight*count) then
                            ...
                            TriggerClientEvent("voltre:properties:UpdateCoffreProperties", _source, PropertiesData.data)
                        else 
...
RegisterServerEvent("voltre:properties:SuppItemToPropertiesCoffre", function(data, item, count) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    local perm = PropertiesExist and GetPlayerPermForProperty(xPlayer, PropertiesData)
...
RegisterServerEvent("voltre:properties:AddWeaponsToPropertiesCoffre", function(data, weaponInfo, ammo)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    local perm = PropertiesExist and GetPlayerPermForProperty(xPlayer, PropertiesData)
...
RegisterServerEvent("voltre:properties:SuppWeaponsToPropertiesCoffre", function(data, weaponInfo, random, ammo)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    local perm = PropertiesExist and GetPlayerPermForProperty(xPlayer, PropertiesData)
...
RegisterServerEvent("voltre:properties:AddMoneyToPropertiesCoffre", function(data, moneyType, amount) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    local perm = PropertiesExist and GetPlayerPermForProperty(xPlayer, PropertiesData)
...
RegisterServerEvent("voltre:properties:SuppMoneyToPropertiesCoffre", function(data, moneyType, amount) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    local perm = PropertiesExist and GetPlayerPermForProperty(xPlayer, PropertiesData)
...
RegisterServerEvent('voltre:properties:playerOpenedCoffre', function(propertiesName)
    local _source = source 
...
RegisterServerEvent('voltre:properties:playerClosedCoffre', function(propertiesName)
    local _source = source
...
ESX.RegisterServerCallback('voltre:properties:CheckIfPlayerInCoffre', function(source, cb, propertiesName)
	local _source = source
...
RegisterServerEvent('voltre:properties:GetGradeListProperties', function(data)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local itsMyProperties = SaveData.PropertiesList[data.name] and SaveData.PropertiesList[data.name].owner == xPlayer.identifier or SaveData.PropertiesList[data.name].owner == xPlayer.job.name or SaveData.PropertiesList[data.name].owner == xPlayer.job2.name
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    ...
    if #(GetEntityCoords(GetPlayerPed(_source)) - vector3(data.positions.COFFRE.x, data.positions.COFFRE.y, data.positions.COFFRE.z)) < 5 then 
        if itsMyProperties then 
            ...
            TriggerClientEvent("voltre:properties:ResultOfGradeListProperties", _source, GradeList, PropertiesData.parms.GradesAlloweds)
        end
    end 
end)

RegisterServerEvent('voltre:properties:UpdateGradeListProperties', function(data, newGradesAlloweds)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local itsMyProperties = SaveData.PropertiesList[data.name] and SaveData.PropertiesList[data.name].owner == xPlayer.identifier or SaveData.PropertiesList[data.name].owner == xPlayer.job.name or SaveData.PropertiesList[data.name].owner == xPlayer.job2.name
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    ...
    if #(GetEntityCoords(GetPlayerPed(_source)) - vector3(data.positions.COFFRE.x, data.positions.COFFRE.y, data.positions.COFFRE.z)) < 5 then 
        if itsMyProperties and PropertiesExist then 
            ...
            PropertiesSaved[data.name] = SaveData.PropertiesList[data.name]
            -- TriggerClientEventAll("voltre:properties:UpdatePropertiesList", SaveData.PropertiesList)
        end
    end 
end)

RegisterServerEvent('voltre:properties:CheckIfGradeNotExistProperties', function(data, jobType)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local itsMyProperties = SaveData.PropertiesList[data.name] and SaveData.PropertiesList[data.name].owner == xPlayer.identifier or SaveData.PropertiesList[data.name].owner == xPlayer.job.name or SaveData.PropertiesList[data.name].owner == xPlayer.job2.name
    local PropertiesExist, PropertiesData = GetProperties(data.name)
...
RegisterServerEvent('voltre:properties:PlayerDonnedKeysToPlayerProperties', function(data, targetId, perms)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local itsMyProperties = SaveData.PropertiesList[data.name] and SaveData.PropertiesList[data.name].owner == xPlayer.identifier or SaveData.PropertiesList[data.name].owner == xPlayer.job.name or SaveData.PropertiesList[data.name].owner == xPlayer.job2.name
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    ...
    if #(GetEntityCoords(GetPlayerPed(_source)) - vector3(data.positions.COFFRE.x, data.positions.COFFRE.y, data.positions.COFFRE.z)) < 5 then 
        if itsMyProperties and PropertiesExist then 
            ...
            if xTarget then 
                ...
                TriggerClientEvent("voltre:properties:UpdatePropertiesList", targetId, SaveData.PropertiesList)
                TriggerClientEvent("voltre:properties:UpdatePropertiesList", _source, SaveData.PropertiesList)
                -- Notify recipient to refresh permissions if already inside the property
                TriggerClientEvent("voltre:properties:KeysUpdated", targetId, data.name, safePerms)
            else 
...
RegisterServerEvent('voltre:properties:RevokeKeysProperties', function(data, idx)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local itsMyProperties = SaveData.PropertiesList[data.name] and SaveData.PropertiesList[data.name].owner == xPlayer.identifier or SaveData.PropertiesList[data.name].owner == xPlayer.job.name or SaveData.PropertiesList[data.name].owner == xPlayer.job2.name
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    ...
    if itsMyProperties and PropertiesExist then
        ...
        if person then
            ...
            if xRevoked then
                ...
                TriggerClientEvent("voltre:properties:UpdatePropertiesList", xRevoked.source, SaveData.PropertiesList)
                -- Notify to refresh access (will remove coffre marker if inside)
                TriggerClientEvent("voltre:properties:KeysRevoked", xRevoked.source, data.name)
            end
        end
    end
end)

RegisterServerEvent('voltre:properties:UpdateAccessPermsProperties', function(data, idx, newPerms)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local itsMyProperties = SaveData.PropertiesList[data.name] and SaveData.PropertiesList[data.name].owner == xPlayer.identifier or SaveData.PropertiesList[data.name].owner == xPlayer.job.name or SaveData.PropertiesList[data.name].owner == xPlayer.job2.name
    local PropertiesExist, PropertiesData = GetProperties(data.name)
    ...
    if itsMyProperties and PropertiesExist then
        ...
        if person then
            ...
            if xTarget then
                TriggerClientEvent("voltre:properties:UpdatePropertiesList", xTarget.source, SaveData.PropertiesList)
                -- Notify about permission changes
                TriggerClientEvent("voltre:properties:KeysUpdated", xTarget.source, data.name, person.perms)
            end
...
RegisterNetEvent('voltre:properties:buy')
AddEventHandler('voltre:properties:buy', function(properties, type, immeuble, typebought)
    ...
    --if #(GetEntityCoords(GetPlayerPed(_source)) - vector3(data.positions.EXIT.x, data.positions.EXIT.y, data.positions.EXIT.z)) < 50 then 
        ...
        if xMoney >= properties.prices[typebought] then 
            if SaveData.PropertiesList[data.name] == nil then
                ...
                TriggerClientEvent("voltre:properties:UpdatePropertiesBuyed",-1, data.name, true, xPlayer.identifier)
                TriggerClientEvent("h:propertySyncProperties", -1)
            else
...
RegisterServerEvent('voltre:properties:RequestData')
AddEventHandler('voltre:properties:RequestData', function()
    local _source = source
    local attempts = 0
    while not PropertiesLoaded and attempts < 100 do
        Wait(100)
