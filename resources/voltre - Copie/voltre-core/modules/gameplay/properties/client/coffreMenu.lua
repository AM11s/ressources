...
function OpenMenuCoffreProperties(info, notPersonnal, typeJob, sharedPerms)
    ...
    if true then
        ...
        TriggerServerEvent("voltre:properties:GetCoffreProperties", info.name)
        ...
        while menu do
            ...
            RageUI.IsVisible(menu, function()
                if canDeposit or canWithdraw then
                    RageUI.Button("Coffre", nil, {}, true, {
                        onSelected = function()
                            RageUI.CloseAll()
                            voltre.DebugPrint("proprieties_"..info.name)
            ...
            RageUI.List("Attribuer la propriété", {
                    ...
                    onSelected = function()
                        if not notPersonnal then
                            if idxList == 1 then 
                                TriggerServerEvent("voltre:properties:PlayerAttribuetPropreties", info, 1)
                            elseif idxList == 2 then 
                                TriggerServerEvent("voltre:properties:PlayerAttribuetPropreties", info, 2)
                            end
                        else
                ...
                --[[RageUI.Button("Rendre la propriété", "~r~Vous renderez cette propriété pour "..math.floor((price*75)/100).."$", {}, not notPersonnal, {
                    onSelected = function()
                        TriggerServerEvent("voltre:properties:PlayerHasRenderProperties", info, math.floor((price*75)/100))
                    end
                })]]
                ...
                RageUI.Button("Confirmer et donner les clés", nil, { RightBadge = RageUI.BadgeStyle.Tick }, true, {
                    onSelected = function()
                        ...
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            TriggerServerEvent("voltre:properties:PlayerDonnedKeysToPlayerProperties", info, GetPlayerServerId(closestPlayer), KeysPerms)
                            RageUI.GoBack()
                        else
                ...
                if parms and parms.PeopleAlloweds and #parms.PeopleAlloweds > 0 then
                    for idx, person in ipairs(parms.PeopleAlloweds) do
                        ...
                        RageUI.Button("Retirer les clés de "..person.name, nil, { RightBadge = RageUI.BadgeStyle.Lock }, true, {
                            onSelected = function()
                                TriggerServerEvent("voltre:properties:RevokeKeysProperties", info, idx)
                                RageUI.GoBack()
                            end
                        })
                        RageUI.Separator("")
            ...
            RageUI.IsVisible(editAccessSubMenu, function()
                ...
                RageUI.Button("Confirmer les modifications", nil, { RightBadge = RageUI.BadgeStyle.Tick }, true, {
                    onSelected = function()
                        TriggerServerEvent("voltre:properties:UpdateAccessPermsProperties", info, editPersonIdx, editPerms)
                        RageUI.GoBack()
                    end
                })
            ...
            RageUI.IsVisible(gerePropertiesMenu, function()
                ...
                RageUI.Button("Grade ayant accès a la propriété", nil, {}, true, {
                    onSelected = function()
                        TriggerServerEvent("voltre:properties:GetGradeListProperties", info)
                    end
                }, gradeListMenu)
            ...
            RageUI.IsVisible(gradeListMenu, function()
                ...
                RageUI.Button("Confirmer les modifications", nil, {RightBadge = RageUI.BadgeStyle.Tick}, true, {
                    onSelected = function()
                        TriggerServerEvent('voltre:properties:UpdateGradeListProperties', info, GradesAllowed)
                        RageUI.GoBack()
                    end
                })
...
RegisterNetEvent("voltre:properties:GetCoffreProperties")
AddEventHandler("voltre:properties:GetCoffreProperties", function(data)
    Coffre = data
end)

RegisterNetEvent("voltre:properties:UpdateCoffreProperties")
AddEventHandler("voltre:properties:UpdateCoffreProperties", function(data)
    Coffre = data
end)

RegisterNetEvent("voltre:properties:ResultOfGradeListProperties")
AddEventHandler("voltre:properties:ResultOfGradeListProperties", function(GradeList, GradesAlloweds)
    Grades = GradeList
