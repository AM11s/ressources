    ...
    while menu do
        ...
        RageUI.IsVisible(menu, function()
            RageUI.Button('Nom de la propriété', nil, {RightLabel = DATA.LABEL or "~r~Indéfini~s~"}, true, {
                onSelected = function() 
                    local input = voltre.fct.input("Nom de la propriété")
            ...
            RageUI.Button('Valider', nil, {Color = { BackgroundColor = {50, 230, 50, 150} }}, true, {
                onSelected = function() 
                    if DATA.NAME ~= "" and DATA.MaxWeight ~= nil and DATA.POSITION.EXIT ~= nil and DATA.POSITION.ENTER ~= nil then
                        TriggerServerEvent("voltre:properties:CreatedProperties", DATA)
                        RageUI.CloseAll()
                        DestroyCam()
                    else
...
RegisterNetEvent("voltre:properties:createmenu")
AddEventHandler("voltre:properties:createmenu", function(staff)
    OpenMenu(staff)
end)