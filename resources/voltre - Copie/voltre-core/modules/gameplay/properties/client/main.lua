...
MyLicense = nil
local function Init()
    while voltre.data == nil or voltre.data.markers == nil or voltre.data.markers.list == nil do
        Wait(1)
    end
    for k,v in pairs(Properties) do 
        if not voltre.data.markers.list[k] then 
            if v.positions and v.immeuble == '0' then 
                voltre.data.markers.list[k] = {}
                voltre.data.markers.list[k].Position = vector3(v.positions.EXIT.x, v.positions.EXIT.y, v.positions.EXIT.z)
                voltre.data.markers.list[k].Public = true 
                voltre.data.markers.list[k].Job = nil
                voltre.data.markers.list[k].Job2 = nil
                voltre.data.markers.list[k].Blip = false
                voltre.data.markers.list[k].Action = function()
                    ActionProperties(k)
                end
            end
        end
    ...
    if buynotcreatedproperties then
        ...
        if Properties[propertiesId] then 
            if not Properties[propertiesId].isBuy or Properties[propertiesId].isBuy == 0 then 
                ...
                ESX.TriggerServerCallback("voltre:properties:GetPlayerProperties", function(personalPropertiesList, jobProperties, job2Properties) 
                            ...
                            if Properties[propertiesId].name == v.name then 
                                if Properties[propertiesId].parms.GradesAlloweds[ESX.PlayerData.job.grade_label] or SizeOf(Properties[propertiesId].parms.GradesAlloweds) == 0 then 
                                    if SizeOf(Properties[propertiesId].parms.GradesAlloweds) == 0 then 
                                        TriggerServerEvent("voltre:properties:CheckIfGradeNotExistProperties", Properties[propertiesId], typeJob)
                                    end
...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback("voltre:properties:GetProperties", function(propertiesList)
...
Citizen.CreateThread(function()
    while true do
        ...
        if Properties == nil then
            TriggerServerEvent("voltre:properties:RequestData")
        end
    end
end)
...
RegisterNetEvent("voltre:properties:UpdatePropertiesList")
AddEventHandler("voltre:properties:UpdatePropertiesList", function(propertiesList)
    Properties = propertiesList
    Init()
end)

RegisterNetEvent("voltre:properties:UpdatePropertiesBuyed")
AddEventHandler("voltre:properties:UpdatePropertiesBuyed", function(propertiesId, isBuy, newOwner)
    if Properties[propertiesId] and voltre.data.markers.list[propertiesId] then 
        Properties[propertiesId].isBuy = isBuy
        voltre.data.markers.list[propertiesId].isBuy = isBuy
        Properties[propertiesId].owner = newOwner
        UpdateBlips(Properties[propertiesId])
    end
end)

RegisterNetEvent("voltre:properties:UpdatePropertiesJob")
AddEventHandler("voltre:properties:UpdatePropertiesJob", function(propertiesId, jobName)
    if Properties[propertiesId] then 
...
RegisterNetEvent("voltre:properties:KeysUpdated")
AddEventHandler("voltre:properties:KeysUpdated", function(propName, perms)
    ...
    if InProperties and CurrentProperties and CurrentProperties.name == propName then
        voltre.data.markers.register("propertiesCoffre", {
...
RegisterNetEvent("voltre:properties:KeysRevoked")
AddEventHandler("voltre:properties:KeysRevoked", function(propName)
    ...
    if InProperties and CurrentProperties and CurrentProperties.name == propName then
        ...
        voltre.data.markers.unregister("propertiesCoffre")
    end
end)

RegisterNetEvent("voltre:properties:EnterPropertiesBySonnet")
AddEventHandler("voltre:properties:EnterPropertiesBySonnet", function(propreties, bucketID)
    if Properties[propreties.name] then 
        EnterProperties("bysonnet", propreties, bucketID)
    end
end)

RegisterNetEvent("voltre:properties:ExitProperties")
AddEventHandler("voltre:properties:ExitProperties", function(propertiesId)
    if Properties[propertiesId] then 
...
RegisterNetEvent("voltre:properties:forceExitProperty")
AddEventHandler("voltre:properties:forceExitProperty", function()
...
function EnterProperties(enterType, info, bucketID, sharedPerms)
    ...
    elseif enterType == "myproperties" then 
        ...
        Citizen.CreateThread(function()
            ...
            TriggerServerEvent("voltre:properties:SetBucket", "solo", info.bucketID)
            TriggerServerEvent("voltre:properties:PlayerEntrerOrExitThisProperties", info, "enter")
            SetEntityCoords(pPed, vector3(info.positions.ENTER.x, info.positions.ENTER.y, info.positions.ENTER.z))
            DoScreenFadeIn(800)
            Wait(1000)
        end)
        voltre.data.markers.register("propertiesExit", {
    ...
    elseif enterType == "jobproperties" then 
        ...
        Citizen.CreateThread(function()
            ...
            TriggerServerEvent("voltre:properties:SetBucket", "group", info.bucketID)
            TriggerServerEvent("voltre:properties:PlayerEntrerOrExitThisProperties", info, "enter")
            SetEntityCoords(pPed, vector3(info.positions.ENTER.x, info.positions.ENTER.y, info.positions.ENTER.z))
            DoScreenFadeIn(800)
            Wait(1000)
    ...
    elseif enterType == "bysonnet" then 
        ...
        Citizen.CreateThread(function()
            ...
            TriggerServerEvent("voltre:properties:SetBucket", "group", bucketID)
            TriggerServerEvent("voltre:properties:PlayerEntrerOrExitThisProperties", info, "enter")
            SetEntityCoords(pPed, vector3(info.positions.ENTER.x, info.positions.ENTER.y, info.positions.ENTER.z))
            DoScreenFadeIn(800)
            Wait(1000)
...
function ExitProperties(exitType)
    ...
    elseif exitType == "myproperties" then 
        ...
        Citizen.CreateThread(function()
            ...
            TriggerServerEvent("voltre:properties:SetBucket", "remettre")
            TriggerServerEvent("voltre:properties:PlayerEntrerOrExitThisProperties", CurrentProperties, "exit")
            SetEntityCoords(pPed, vector3(CurrentProperties.positions.EXIT.x, CurrentProperties.positions.EXIT.y, CurrentProperties.positions.EXIT.z))
            DoScreenFadeIn(800)
            Wait(1000)
            CurrentProperties = nil
        end)
        voltre.data.markers.unregister("propertiesExit")
        voltre.data.markers.unregister("propertiesCoffre")
...
RegisterNetEvent("voltre:properties:NotificationSonnedProperties")
AddEventHandler("voltre:properties:NotificationSonnedProperties", function(propertiesId, id)
    local timed = 0
    if Properties[propertiesId] then 
        ...
        ESX.ShowAccept("Une personne viens de sonner à votre propriété.", function(result)
            if result then
                TriggerServerEvent("voltre:properties:ReturnPlayerSonnedProperties", "accept", id, Properties[propertiesId])
                PlaySoundFrontend(-1, "WOODEN_DOOR_OPEN_HANDLE_AT", 0, 1)
            else
                TriggerServerEvent("voltre:properties:ReturnPlayerSonnedProperties", "decline", id, Properties[propertiesId])
            end
        end)
    end
end)
