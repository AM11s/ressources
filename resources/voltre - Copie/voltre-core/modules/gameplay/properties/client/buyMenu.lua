        ...
        RageUI.IsVisible(menu, function()
            ...
            if ESX.PlayerData.job.name == 'realestateagent' then
                ...
                RageUI.Button("Attribuer la maison ( Vente )", nil, {}, closestDistance ~= -1 and true or false, {
                    ...
                    onSelected = function()
                        TriggerServerEvent("voltre:properties:PlayerBuyPropeties", info, GetPlayerServerId(closestPlayer))
                        RageUI.CloseAll()
                    end
                })
                RageUI.Button("Attribuer la maison a vous ( Vente )", nil, {}, true, {
                    onSelected = function()
                        TriggerServerEvent("voltre:properties:PlayerBuyPropeties", info)
                        RageUI.CloseAll()
                    end
                })
            else
