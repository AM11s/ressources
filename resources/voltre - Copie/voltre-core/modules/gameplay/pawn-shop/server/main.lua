...
function LoadListingsFromDB()
    MySQL.Async.fetchAll("SELECT * FROM pawnshop_listings", {}, function(result)
        ...
        voltre.DebugPrint("[PawnShop] Loaded " .. #result .. " listings from DB")
    end)
end
...
local function RefreshClient(playerId)
    BuildPawnShopData(playerId, function(data)
        if data then TriggerClientEvent("voltre:pawnshop:update", playerId, data) end
    end)
end
...
-- ============================================================================
ESX.RegisterServerCallback('voltre:pawnshop:getData', function(source, cb)
...
RegisterNetEvent("voltre:pawnshop:buy")
AddEventHandler("voltre:pawnshop:buy", function(listingId, quantity, paymentType, isNpc)
    local src = source
    ...
    if not listing then
        TriggerClientEvent("voltre:pawnshop:buyResult", src, { success = false, message = "Annonce introuvable" })
        return
    end

    -- Check stock
    if not isNpc and listing.count < quantity then
        TriggerClientEvent("voltre:pawnshop:buyResult", src, { success = false, message = "Stock insuffisant" })
        return
    end

    -- Can't buy own listing
    if not isNpc and listing.identifier == xPlayer.identifier then
        TriggerClientEvent("voltre:pawnshop:buyResult", src, { success = false, message = "Vous ne pouvez pas acheter votre propre annonce" })
        return
    end
    ...
    if xPlayer.getAccount(account).money < totalCost then
        TriggerClientEvent("voltre:pawnshop:buyResult", src, { success = false, message = "Fonds insuffisants" })
        return
    end

    -- Check carry
    if not xPlayer.canCarryItem(listing.itemName, quantity) then
        TriggerClientEvent("voltre:pawnshop:buyResult", src, { success = false, message = "Inventaire plein" })
        return
    end
    ...
    TriggerClientEvent("voltre:pawnshop:buyResult", src, { success = true, message = "Achat de x" .. quantity .. " " .. listing.itemLabel .. " effectue !" })
    RefreshClient(src)
end)

-- Sell item (create listing)
RegisterNetEvent("voltre:pawnshop:sell")
AddEventHandler("voltre:pawnshop:sell", function(itemName, itemLabel, count, pricePerUnit)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    if count < 1 or pricePerUnit < 1 then
        TriggerClientEvent("voltre:pawnshop:sellResult", src, { success = false, message = "Valeurs invalides" })
        return
    end

    -- Check blacklist
    if Config.PawnShop.BlackListItem[itemName] or ESX.ContribItem(itemName) then
        TriggerClientEvent("voltre:pawnshop:sellResult", src, { success = false, message = "Cet item ne peut pas etre vendu" })
        return
    end
    ...
    if myCount >= (Config.PawnShop.MaxListingsPerPlayer or 8) then
        TriggerClientEvent("voltre:pawnshop:sellResult", src, { success = false, message = "Nombre max d'annonces atteint" })
        return
    end
    ...
    if not invItem or invItem.count < count then
        TriggerClientEvent("voltre:pawnshop:sellResult", src, { success = false, message = "Vous n'avez pas assez de cet item" })
        return
    end
    ...
    MySQL.Async.execute("INSERT INTO pawnshop_listings (identifier, seller_name, item_name, item_label, count, price_per_unit) VALUES (@id, @name, @itemName, @itemLabel, @count, @price)", {
        ...
        LoadListingsFromDB()
        Wait(200)
        TriggerClientEvent("voltre:pawnshop:sellResult", src, { success = true, message = "Annonce creee: x" .. count .. " " .. itemLabel })
        RefreshClient(src)
    end)
end)

-- Cancel listing
RegisterNetEvent("voltre:pawnshop:cancel")
AddEventHandler("voltre:pawnshop:cancel", function(listingId)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    if not listing or listing.identifier ~= xPlayer.identifier then
        TriggerClientEvent("voltre:pawnshop:cancelResult", src, { success = false, message = "Annonce introuvable" })
        return
    end
    ...
    TriggerClientEvent("voltre:pawnshop:cancelResult", src, { success = true, message = "Annonce annulee, items rendus" })
    RefreshClient(src)
end)

-- Collect all pending sales
RegisterNetEvent("voltre:pawnshop:collectAll")
AddEventHandler("voltre:pawnshop:collectAll", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    MySQL.Async.fetchAll("SELECT * FROM pawnshop_sales WHERE identifier = @id", {
        ...
        if not result or #result == 0 then
            TriggerClientEvent("voltre:pawnshop:collectResult", src, { success = false, message = "Aucun revenu en attente" })
            return
        end
        local total = 0
        ...
        TriggerClientEvent("voltre:pawnshop:collectResult", src, { success = true, message = "Encaisse: $" .. total })
        RefreshClient(src)
    end)
end)
