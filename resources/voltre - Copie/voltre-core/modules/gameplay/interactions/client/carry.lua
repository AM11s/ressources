	...
	if not PlayerIsDead then
		if not piggyBackInProgress then
			...
			if closestDistance ~= -1 and closestDistance <= 3 then
				...
				TriggerServerEvent('Voltre:cmg2_animations:sync', lib, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget)
			else
				ESX.TriggerServerCallback("voltre:piggyback:checkOfflineIsNear", function(result) 
					if not result then
			...
			if closestDistance ~= -1 and closestDistance <= 3 then
				local target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent('Voltre:cmg2_animations:stop', target)
			end
		end
	end
end, false)
...
RegisterNetEvent('Voltre:cmg2_animations:syncTarget', function(targetId, animationLib, animation2, distans, distans2, height, length, spin, controlFlag)
...
RegisterNetEvent('Voltre:cmg2_animations:syncMe', function(animationLib, animation, length, controlFlag, animFlag)
	local plyPed = PlayerPedId()
	RequestAnimDict(animationLib)
...
RegisterNetEvent('Voltre:cmg2_animations:cl_stop', function()
	local plyPed = PlayerPedId()
	piggyBackInProgress = false
	ClearPedSecondaryTask(plyPed)
	DetachEntity(plyPed, true, false)
