...
function releaseHostage()
	...
	if closestDistance ~= -1 and closestDistance <= 3 then
		...
		TriggerServerEvent('Voltre:cmg3_animations:sync', lib, lib2, anim1, anim2, distans, distans2, height, target, length, spin, controlFlagMe, controlFlagTarget, animFlagTarget, attachFlag)
	end
end
				...
				if canTakeHostage then
					if not holdingHostageInProgress then
						...
						if closestDistance ~= -1 and closestDistance <= 3 then
							...
							ESX.TriggerServerCallback("voltre:dead:IsPlayerDead", function(result)
								if (result) then
...
RegisterNetEvent('Voltre:cmg3_animations:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length, spin, controlFlag, animFlagTarget, attach)
	local plyPed = PlayerPedId()
...
RegisterNetEvent('Voltre:cmg3_animations:syncMe', function(animationLib, animation, length, controlFlag, animFlag)
	local plyPed = PlayerPedId()
...
RegisterNetEvent('Voltre:cmg3_animations:cl_stop', function()
	local plyPed = PlayerPedId()
	...
	while true do
		...
		if holdingHostage then
			...
			if IsEntityDead(plyPed) then
				...
				if closestDistance ~= -1 and closestDistance <= 3 then
					local target = GetPlayerServerId(closestPlayer)
					TriggerServerEvent('Voltre:cmg3_animations:stop', target)
				end
