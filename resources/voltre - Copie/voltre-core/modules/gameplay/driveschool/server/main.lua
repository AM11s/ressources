...
ESX.RegisterServerCallback('voltre:driveschool:getData', function(source, cb)
...
RegisterNetEvent('voltre:driveschool:giveLicense', function(license)
    local src = source
    ...
    AddLicense(src, license, function()
        ...
        MySQL.Async.fetchAll("SELECT * FROM user_licenses WHERE owner = @identifier", {
            ...
            TriggerClientEvent('voltre:driveschool:updateData', src, licenses)
        end)
    end)
end)

RegisterNetEvent('voltre:driveschool:removeMoney', function(account, amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
