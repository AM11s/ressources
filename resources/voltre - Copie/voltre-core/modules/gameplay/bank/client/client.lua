...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('Voltre:GetInformationsBank')
end)

RegisterNetEvent('Voltre:ReceiveInformaionsBank', function(table)
    Bank = table
end)
    ...
    while menu do
        ...
        RageUI.IsVisible(OpenVirementSend, function()
            RageUI.Button('IBAN', nil, {RightLabel = Virement.IBAN}, true, {
                onSelected = function()
                    Virement.IBAN = voltre.fct.input('IBAN')
            ...
            RageUI.Button('Raison', nil, {RightLabel = Virement.RAISON}, true, {
                onSelected = function()
                    Virement.RAISON = voltre.fct.input('RAISON')
            ...
            RageUI.Button('Montant', nil, {RightLabel = Virement.MONTANT}, true, {
                onSelected = function()
                    Virement.MONTANT = voltre.fct.input('MONTANT')
            ...
            RageUI.Button('~g~Confirmer', nil, {}, Virement.Validate, {
                onSelected = function()
                    TriggerServerEvent('Voltre:banque:sendVirement', Virement)
                ...
                for k,v in pairs(Bank.Virement) do 
                    RageUI.Button(v.Description, nil, {}, BanqueTimeoutButton, {
                        ...
                        onSelected = function()
                            ...
                            TriggerServerEvent('Voltre:RVirement', k)
...
RegisterNUICallback('atm:getData', function(data, cb)
    ESX.TriggerServerCallback('voltre:atm:getData', function(result)
        cb(result or {})
    end)
end)

RegisterNUICallback('atm:verifyPin', function(data, cb)
    ESX.TriggerServerCallback('voltre:atm:verifyPin', function(success)
        cb({ success = success })
    end, data)
end)

RegisterNUICallback('atm:withdraw', function(data, cb)
    TriggerServerEvent('Voltre:atm:withdraw', data)
    cb({})
end)

RegisterNUICallback('atm:deposit', function(data, cb)
    TriggerServerEvent('Voltre:atm:deposit', data)
    cb({})
end)

RegisterNetEvent('voltre:atm:opResult', function(data)
    SendNUIMessage({ action = 'atm:opResult', data = data })
end)

RegisterNetEvent('voltre:atm:update', function(data)
    SendNUIMessage({ action = 'atm:update', data = data })
end)

function OpenAtmUI()
    ESX.TriggerServerCallback('voltre:atm:getData', function(atmData)
        if not atmData then
