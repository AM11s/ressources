...
Citizen.CreateThread(function()
    MySQL.Async.execute([[
        ALTER TABLE vbank ADD COLUMN IF NOT EXISTS card_number VARCHAR(25) DEFAULT NULL
    ]], {})
    MySQL.Async.execute([[
        ALTER TABLE vbank ADD COLUMN IF NOT EXISTS credits TEXT DEFAULT '[]'
    ]], {})
    MySQL.Async.execute([[
        ALTER TABLE vbank ADD COLUMN IF NOT EXISTS pending_transfers TEXT DEFAULT '[]'
    ]], {})
    MySQL.Async.execute([[
        ALTER TABLE vbank ADD COLUMN IF NOT EXISTS created_at BIGINT DEFAULT 0
    ]], {})
    MySQL.Async.execute([[
        ALTER TABLE vbank ADD COLUMN IF NOT EXISTS pin VARCHAR(10) DEFAULT NULL
    ]], {})
    MySQL.Async.execute([[
        ALTER TABLE vbank ADD COLUMN IF NOT EXISTS has_card TINYINT(1) DEFAULT 0
    ]], {})
    Wait(500)
    ...
    MySQL.Async.fetchAll("SELECT * FROM vbank", {}, function(result)
        for _, row in pairs(result) do
            ...
            local histRaw = row.history and json.decode(row.history) or {}
            local creditsRaw = row.credits and json.decode(row.credits) or {}
            local pendingRaw = row.pending_transfers and json.decode(row.pending_transfers) or {}
            -- Migrate old Virement column to pending_transfers
            if (not pendingRaw or #pendingRaw == 0) and row.Virement and row.Virement ~= '[]' and row.Virement ~= '' then
            ...
            -- Convert old history format (keyed table) to array
            local histArray = {}
            if histRaw then
            ...
            Bank.Accounts[ident] = {
                bankid = row.bankid,
                cardNumber = row.card_number or GenerateCardNumber(),
                identifier = ident,
                history = histArray,
                credits = creditsRaw,
                ...
                pin = row.pin,
                hasCard = row.has_card == 1,
            }
            Bank.IBANMap[row.bankid] = ident

            -- Save card number if just generated
            if not row.card_number then
                MySQL.Async.execute('UPDATE vbank SET card_number = @card WHERE identifier = @id', {
                    ['@card'] = Bank.Accounts[ident].cardNumber,
                    ['@id'] = ident,
                })
            end
        end
...
function Bank.GetAccount(identifier)
    return Bank.Accounts[identifier]
end
...
function Bank.CreateAccount(identifier)
    ...
    Bank.Accounts[identifier] = {
        ...
        history = {{
            ...
            description = 'Bienvenue ! Votre compte bancaire a été créé.',
            category = 'other',
            date = GetDate(),
            timestamp = now,
        }},
    ...
    MySQL.Async.execute("INSERT INTO vbank (identifier, history, bankid, card_number, credits, pending_transfers, created_at, has_card) VALUES (@id, @history, @bankid, @card, '[]', '[]', @created, 0)", {
        ["@id"] = identifier,
        ["@history"] = json.encode(Bank.Accounts[identifier].history),
        ['@bankid'] = bankid,
        ['@card'] = cardNum,
        ['@created'] = now,
    })
...
ESX.RegisterServerCallback('voltre:bank:getData', function(source, cb)
    local bankData = Bank.BuildUIData(source)
    cb(bankData)
end)

RegisterNetEvent('Voltre:GetInformationsBank', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    TriggerClientEvent('Voltre:ReceiveInformaionsBank', src, Bank.GetAccount(xPlayer.identifier))
end)
...
RegisterNetEvent('Voltre:ActionBank', function(actionType, amount)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
...
RegisterNetEvent('Voltre:banque:sendVirement', function(data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterNetEvent('Voltre:RVirement', function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
    local account = Bank.GetAccount(xPlayer.identifier)
    if not account then return end
...
RegisterNetEvent('Voltre:bank:buyCard', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    local account = Bank.GetAccount(xPlayer.identifier)
    ...
    if account.hasCard then
        TriggerClientEvent('voltre:bank:cardResult', src, { success = false, message = 'Vous avez déjà une carte' })
        return
    end

    if xPlayer.getAccount(eBank.Bank.AccountName).money < price then
        TriggerClientEvent('voltre:bank:cardResult', src, { success = false, message = 'Solde insuffisant' })
        return
    end
    ...
    TriggerClientEvent('voltre:bank:cardResult', src, { success = true })
end)

RegisterNetEvent('Voltre:bank:setPin', function(data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not data then return end
    local account = Bank.GetAccount(xPlayer.identifier)
    ...
    if not pin:match('^%d%d%d%d$') then
        TriggerClientEvent('voltre:bank:pinResult', src, { success = false, message = 'Code invalide' })
        return
    end

    account.pin = pin
    Bank.SaveAccount(xPlayer.identifier)
    TriggerClientEvent('voltre:bank:pinResult', src, { success = true })
end)
...
ESX.RegisterServerCallback('voltre:atm:getData', function(source, cb)
    cb(Bank.BuildATMData(source))
end)

ESX.RegisterServerCallback('voltre:atm:verifyPin', function(source, cb, data)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('Voltre:atm:withdraw', function(data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not data then return end
    local isOk, amount = QuantityOk(data.amount)
    if not isOk then
        TriggerClientEvent('voltre:atm:opResult', src, { success = false, message = 'Montant invalide' })
        return
    end
    if xPlayer.getAccount(eBank.Bank.AccountName).money < amount then
        TriggerClientEvent('voltre:atm:opResult', src, { success = false, message = 'Solde insuffisant' })
        return
    end
    xPlayer.removeAccountMoney(eBank.Bank.AccountName, amount)
    xPlayer.addAccountMoney(eBank.Money.AccountName, amount)
    ...
    TriggerClientEvent('voltre:atm:opResult', src, { success = true, message = 'Retrait effectué', type = 'withdraw', amount = amount })
    TriggerClientEvent('voltre:atm:update', src, Bank.BuildATMData(src))
end)

RegisterNetEvent('Voltre:atm:deposit', function(data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not data then return end
    local isOk, amount = QuantityOk(data.amount)
    ...
    if xPlayer.getAccount(eBank.Money.AccountName).money < amount then
        TriggerClientEvent('voltre:atm:opResult', src, { success = false, message = 'Pas assez d\'espèces' })
        return
    end
    xPlayer.removeAccountMoney(eBank.Money.AccountName, amount)
    xPlayer.addAccountMoney(eBank.Bank.AccountName, amount)
    ...
    TriggerClientEvent('voltre:atm:opResult', src, { success = true, message = 'Dépôt effectué', type = 'deposit', amount = amount })
    TriggerClientEvent('voltre:atm:update', src, Bank.BuildATMData(src))
end)
...
RegisterNetEvent('Voltre:bank:requestLoan', function(data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not data then return end
...
RegisterNetEvent('Voltre:bank:repayLoan', function(data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not data then return end
