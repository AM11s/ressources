...
function SyncVipToClient(source, identifier)
    ...
    TriggerClientEvent('voltre:vip:sync', source, data)
    
    if data then
        TriggerClientEvent('voltre:updateVIP', source, data.isVip, data.type, {
            d = data.time.days,
            h = data.time.hours,
            m = data.time.minutes,
        ...
        pcall(function()
            ...
            if xPlayer then
                local vipWeight = exports["voltre-core"]:GetVIPMaxWeight(identifier)
        ...
        TriggerClientEvent('voltre:updateVIP', source, false)
...
RegisterNetEvent('voltre:vip:request', function()
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('osnium:GetVIP', function()
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
...
RegisterCommand('addVIP', function(source, args)
    if source ~= 0 then
        local xPlayer = ESX.GetPlayerFromId(source)
        if not xPlayer or (xPlayer.getGroup() ~= 'admin' and xPlayer.getGroup() ~= 'fondateur') then
            return
        end
    end
...
ESX.RegisterServerCallback('voltre:vip:check', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
voltre.InitPrint('^2VIP module loaded^7')
