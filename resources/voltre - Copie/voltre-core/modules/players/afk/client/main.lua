function MainAFKMenu()
	TriggerServerEvent("voltre:afk:gettimer")
			...
			RageUI.IsVisible(main, function()
				...
				for k,v in pairs(Config.Afk.Invest.Lot) do
					RageUI.Button(v.label, nil, {RightLabel = v.price, RightBadge = RageUI.BadgeStyle.Coins}, true, {
						...
						onSelected = function()
							TriggerServerEvent('voltre:afk:shopbuy', k)
						end
					})
				end
			end)
...
Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
    ...
    while true do
        loopFrames = 2500
        if #voltre.data.afk >= 1 then
            for k,v in pairs(voltre.data.afk) do 
...
function IAmAfk()
	...
	time = 1000
	timepassed = 15*60
	TriggerServerEvent("voltre:afk:iamafk:sleep", PlayerState.coords)
	Citizen.CreateThread(function()
		while PlayerState.AfkSleep do
			...
			TriggerServerEvent("voltre:afk:iamafk:sleep:udapte", timepassed)
			Wait(1000)
		end
		time = 60*1000
		timepassed = 0
		TriggerServerEvent("voltre:afk:iamnotafk:sleep")
...
RegisterNetEvent('voltre:afk:sleep:recevieplayers', function(data)
	voltre.data.afk = data
end)

RegisterCommand('afk', function(source,args)
    if GetSafeZone() then
        TriggerServerEvent('voltre:afk:join')
    else
        ESX.ShowNotification('Vous n\'êtes pas en Zone Safe')
    end
end)

RegisterNetEvent('voltre:afk:refresh', function(newtimer, point)
	if not PlayerState.inAfk then return end
	PlayerState.afkTimer = newtimer
	PlayerState.afkPoint = point
end)

RegisterNetEvent('voltre:afk:init', function()
	PlayerState.inAfk = true
	if voltre.data.markers.isRegister("afk_exit") then return end
	if voltre.data.markers.isRegister("afk_shop") then return end
	voltre.data.markers.register("afk_exit", {
		Position = vec3(490.118896, 4808.295898, -58.382812),
		Public = true,
		Job = nil,
		Job2 = nil,
		Action = function()
			TriggerServerEvent("voltre:afk:exit")
		end
	})
	voltre.data.markers.register("afk_shop", {
