                ...
                for _, v in ipairs(catData.vehicles) do
                    RageUI.Button(v.label, nil, {RightLabel = "~g~" .. v.price .. "$"}, true, {
                        ...
                        onSelected = function()
                            ...
                            -- Dialog de paiement
                            local paymentResult = VoltreInput({
                            ...
                            ESX.Game.SpawnVehicle(v.model, spawnPos, spawnCoords.w, function(vehicle)
                                ...
                                TriggerServerEvent('voltre:carrental:rent', v.model, v.price, plate, VehToNet(vehicle), paymentType)
                                TriggerServerEvent('Voltre:garage:addTempKey', plate)
                            end)

                            TriggerEvent('voltre:carrental:cooldown')
                            RageUI.CloseAll()
                        end
                    })
                end
                ...
                if Config.StarterPack and Config.StarterPack.Pack then
                    for k, v in pairs(Config.StarterPack.Pack) do
                        RageUI.Button("" .. (v.Type or k), v.Description, {}, true, {
                            onSelected = function()
                                TriggerServerEvent("Voltre:starterpack", k)
                            end
                        })
                    end
                else
...
RegisterNetEvent('voltre:carrental:cooldown')
AddEventHandler('voltre:carrental:cooldown', function()
    rentalCooldown = true
...
RegisterNetEvent('voltre:carrental:startTimer')
AddEventHandler('voltre:carrental:startTimer', function(duration)
    rentalActive = true
    rentalTimer = duration
end)

RegisterNetEvent('voltre:carrental:stopTimer')
AddEventHandler('voltre:carrental:stopTimer', function()
    rentalActive = false
    rentalTimer = 0
    rentalVehicle = nil
    pcall(function()
        ...
        if rentalActive and rentalTimer > 0 then
            ...
            if veh and veh ~= 0 then
                ...
                if plate and string.find(plate, "RENT") then
                    local time = voltre.fct.format.SecondsToClock(rentalTimer)
                    pcall(function()
                        ShowInfo(
    ...
    while true do
        ...
        if rentalActive and rentalTimer > 0 then
            ...
            if rentalTimer <= 0 then
                ...
                TriggerServerEvent('voltre:carrental:cleanup')
            end
        end
    end
end)
