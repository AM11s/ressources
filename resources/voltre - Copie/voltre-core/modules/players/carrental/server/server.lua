...
RegisterNetEvent('voltre:carrental:rent')
AddEventHandler('voltre:carrental:rent', function(model, price, plate, netId, paymentType)
    local source = source
    ...
    TriggerClientEvent('voltre:carrental:startTimer', source, Config.CarRental.duration)
    ...
    SetTimeout(Config.CarRental.duration * 1000, function()
        ...
        if GetPlayerName(source) then
            TriggerClientEvent('voltre:carrental:stopTimer', source)
...
RegisterNetEvent('voltre:carrental:cleanup')
AddEventHandler('voltre:carrental:cleanup', function()
    local source = source
    for rentalId, rental in pairs(ActiveRentals) do
        if rental.source == source then
...
RegisterCommand('endrental', function(source, args)
    ...
    for rentalId, rental in pairs(ActiveRentals) do
        if rental.source == targetId then
            ...
            TriggerClientEvent('voltre:carrental:stopTimer', rental.source)
            ActiveRentals[rentalId] = nil
            if source == 0 then
                print("^2[Voltre]^7 Location terminée pour le joueur " .. targetId)
            end
        end
    end
end, false)
...
RegisterNetEvent("Voltre:starterpack")
AddEventHandler("Voltre:starterpack", function(pack)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
