function AddLicense(target, type, cb)
	...
	MySQL.Async.execute('INSERT INTO user_licenses (type, owner) VALUES (@type, @owner)', {
		...
		GetLicenses(target, function(result)
			TriggerClientEvent("Voltre:licenses:recevie", target, result)
		end)
...
RegisterNetEvent("voltre:buy:newlicense", function(type)
	local src = source
...
RegisterNetEvent("Voltre:licenses:open", function(licenseType, targetId, metadata)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	...
	TriggerClientEvent("voltre:idcard:show", targetId, cardData)
end)

RegisterNetEvent("Voltre:licenses:askmylicenses", function()
	local src = source
	GetLicenses(src, function(result)
		TriggerClientEvent("Voltre:licenses:recevie", src, result)
	end)
end)

AddEventHandler('Voltre:esx_license:getLicenses', function(target, cb)
	GetLicenses(target, cb)
end)

AddEventHandler('Voltre:esx_license:checkLicense', function(target, type, cb)
	CheckLicense(target, type, cb)
end)

ESX.RegisterServerCallback('Voltre:esx_license:getLicenses', function(source, cb, target)
	GetLicenses(target, cb)
end)

ESX.RegisterServerCallback('Voltre:esx_license:checkLicense', function(source, cb, target, type)
	CheckLicense(target, type, cb)
end)