RegisterNetEvent('voltre:use-scissors', function()
    if not GetSafeZone() then
        ...
        if closestPlayer ~= -1 and closestDistance <= 3.0 then
            ...
            Wait(10000)
            DeleteObject(ciseau)
            TriggerServerEvent('voltre:scissors:cut', GetPlayerServerId(closestPlayer))
        else
...
RegisterNetEvent('voltre:scissors:cut', function()
    TriggerEvent('Voltre:skinchanger:change', 'hair_1', 0)
