...
RegisterNetEvent("voltre:territories:npcSellResult", function(pedNetId, result)
...
function _Peds()
    ...
    if PlayerState and PlayerState.Illegals and PlayerState.Illegals.laboratories and PlayerState.Illegals.laboratories.inLabo then
        ...
        ESX.TriggerServerCallback("voltre:labo:getEmployeeInfoFromPed", function(info)
            employeeInfo = info
        end, pedNetId)
        ...
        if employeeInfo.inventory and #employeeInfo.inventory > 0 then
            ...
            for _, item in ipairs(employeeInfo.inventory) do
                table.insert(inventoryActions, {
                    ...
                    function()
                        local count = voltre.fct.input("Quantité à retirer (max: " .. item.count .. ")")
                        if count == nil then return end
                        count = tonumber(count)
                        if count and count > 0 and count <= item.count then
                            TriggerServerEvent("voltre:labo:removeEmployeeItem", employeeInfo.labId, employeeInfo.employeeId, item.name, count)
