...
function EnableTutorialEventHooks()
    ...
    if OpenDriveSchool and not originalFunctions.OpenDriveSchool then
        ...
        OpenDriveSchool = function()
            ...
            if tutorialActive then
                TriggerEvent('voltre:tutorial:eventDetected', 'driveschool_opened')
            end
        end
    end
    ...
    if CloseDriveSchool and not originalFunctions.CloseDriveSchool then
        ...
        CloseDriveSchool = function()
            ...
            if tutorialActive then
                TriggerEvent('voltre:tutorial:eventDetected', 'driveschool_closed')
            end
        end
    end
end
...
RegisterNetEvent('voltre:driveschool:updateData', function(licenses)
    ...
    for _, lic in ipairs(licenses) do
        if lic.id == 'drive' and lic.theory and lic.practice then
            TriggerEvent('voltre:tutorial:eventDetected', 'license_completed')
            break
        end
    end
end)

voltre.InitPrint('Tutorial event detection system loaded')
