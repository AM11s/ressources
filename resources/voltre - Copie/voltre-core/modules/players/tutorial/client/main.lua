...
local function spawnVehiclePickupPed()
    ...
    Add3DInteraction({
        ...
        Action = function()
            TriggerEvent('voltre:tutorial:pickupVehicle')
        end,
    })
end
...
goToStep = function(step, extraData)
    tutorialStep = step
    TriggerServerEvent('voltre:tutorial:updateStep', step, tutorialPath)
...
RegisterNUICallback('tutorial:skip', function(data, cb)
    cleanupAll()
    TriggerServerEvent('voltre:tutorial:skip')
    cb('ok')
end)

-- Player chose to skip via objectives skipCommand
RegisterNetEvent('voltre:tutorial:skipFromObjectives', function()
...
RegisterNUICallback('tutorial:choosePath', function(data, cb)
    ...
    TriggerServerEvent('voltre:tutorial:updateStep', tutorialStep, tutorialPath)
...
RegisterNUICallback('tutorial:continueToDriveschool', function(data, cb)
    ...
    TriggerServerEvent('voltre:tutorial:giveBmx')
...
RegisterNUICallback('tutorial:complete', function(data, cb)
    cleanupAll()
    TriggerServerEvent('voltre:tutorial:complete')
    cb('ok')
end)
...
RegisterNetEvent('voltre:tutorial:eventDetected', function(eventType)
...
RegisterNetEvent('voltre:tutorial:pickupVehicle', function()
    ...
    ESX.Game.SpawnVehicle(cfg.vehicleModel, vector3(spawnPos.x, spawnPos.y, spawnPos.z), spawnPos.w, function(vehicle)
        ...
        TriggerServerEvent('voltre:tutorial:giveGarageVehicle', vehicleProps, cfg.vehicleModel)
        
        -- Donner les clés au joueur
        TriggerEvent('voltre:vehicle:addKeys', customPlate)
...
RegisterNetEvent('voltre:tutorial:started', function(data)
    tutorialActive = true
...
RegisterNetEvent('voltre:tutorial:ended', function()
    cleanupAll()
end)
...
RegisterNetEvent('voltre:tutorial:spawnBmx', function()
    deleteBmx()
...
RegisterNetEvent('esx:charCreator:finish', function()
    ...
    Citizen.SetTimeout(2000, function()
        ESX.TriggerServerCallback('voltre:tutorial:needsTutorial', function(needsTutorial)
            if needsTutorial then
                TriggerServerEvent('voltre:tutorial:start')
            end
        end)
    end)
end)
...
RegisterNetEvent('voltre:tutorial:check', function()
    if not Config.Tutorial or not Config.Tutorial.enabled then return end
...
voltre.InitPrint('Tutorial system (client) loaded')
