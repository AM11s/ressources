...
ESX.RegisterServerCallback('voltre:tutorial:needsTutorial', function(source, cb)
...
RegisterNetEvent('voltre:tutorial:start', function()
    local src = source
...
RegisterNetEvent('voltre:tutorial:updateStep', function(step, path)
    local src = source
...
RegisterNetEvent('voltre:tutorial:skip', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    TriggerClientEvent('voltre:tutorial:ended', src)
end)
...
RegisterNetEvent('voltre:tutorial:complete', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
RegisterNetEvent('voltre:tutorial:giveBmx', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
Citizen.CreateThread(function()
    ...
    ESX.RegisterUsableItem(bmx, function(source)
        ...
        TriggerClientEvent('voltre:tutorial:spawnBmx', src)
    end)
end)
...
RegisterNetEvent('voltre:tutorial:giveGarageVehicle', function(vehicleProps, vehicleModel)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
RegisterNetEvent('voltre:tutorial:pickupBmx', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
Citizen.CreateThread(function()
    Wait(5000)
    MySQL.Async.execute([[
        ALTER TABLE users ADD COLUMN IF NOT EXISTS tutorial_completed TINYINT(1) NOT NULL DEFAULT 0
    ]], {})
...
voltre.InitPrint('Tutorial system (server) loaded')
