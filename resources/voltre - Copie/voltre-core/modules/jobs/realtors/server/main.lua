-- ================================================================
-- VOLTRE REALTORS — Agence Immobilière (Boss + Employés)
-- ================================================================
...
local function regenerateListings(force)
    ...
    LAST_REFRESH = now()
    -- voltre.InitPrint(("^2[REALTOR] %d listings générés (total = %d)^0"):format(target, count + target))
end
...
CreateThread(function()
    ...
    -- voltre.InitPrint("^2[REALTOR] Module agence chargé.^0")
...
ESX.RegisterServerCallback("realtor:sellToPlayer", function(source, cb, propName, targetId)
    ...
    TriggerClientEvent("voltre:properties:UpdatePropertiesList", -1, SaveData.PropertiesList)
    TriggerClientEvent("h:propertySyncProperties", -1)
    TriggerClientEvent("voltre:properties:UpdatePropertiesBuyed", -1, p.name, true, xTarget.identifier)
...
local function evictTenant(propName)
    ...
    TriggerClientEvent("voltre:properties:UpdatePropertiesList", -1, SaveData.PropertiesList)
    TriggerClientEvent("h:propertySyncProperties", -1)
    TriggerClientEvent("voltre:properties:UpdatePropertiesBuyed", -1, p.name, true, CFG.SocietyName)

    voltre.InitPrint(("^3[REALTOR] Bail expiré : %s repris par l'agence^0"):format(p.name))
end
        ...
        for _, r in ipairs(rows) do
            ...
            for _, pid in ipairs(ESX.GetPlayers()) do
                ...
                if px and px.identifier == r.tenant_identifier then
                    ...
                    TriggerClientEvent("voltre:properties:forceExitProperty", pid)
                    break
                end
            end
    ...
    while true do
        ...
        for _, r in ipairs(done) do
            ...
            if p then
                ...
                TriggerClientEvent("voltre:properties:UpdatePropertiesList", -1, SaveData.PropertiesList)
                TriggerClientEvent("h:propertySyncProperties", -1)
                TriggerClientEvent("voltre:properties:UpdatePropertiesBuyed", -1, r.property_name, true, r.owner_identifier)
            end
