Citizen.CreateThread(function()
    Wait(2000)
    TriggerServerEvent('Voltre:initFarmSociety')
end)

local FarmBlips = {}
local currentFarm = nil  

RegisterNetEvent('Voltre:SendEntrepriseFarmList', function(Table)
    voltre.data.jobs.farms.list = Table
    voltre.data.jobs.farms.loaded = true
end)
...
AddEventHandler('esx:setJob', function(job)
    ...
    FarmBlips = {}
    Wait(100)
    for k,v in pairs(voltre.data.jobs.farms.list) do
        -- Cache la farm du joueur
        if job.name == v.name then
            currentFarm = v
        end
        voltre.DebugPrint(job, v)
        voltre.DebugPrint(job.name, v.name)
        voltre.DebugPrint(currentFarm)
...
Citizen.CreateThread(function()
    while not voltre.data.jobs.farms.loaded do 
        Wait(1)
    end
    ...
    for k,v in pairs(voltre.data.jobs.farms.list) do
        if v.type == 'Farm' then
            ...
            voltre.data.markers.register("job_farm_vestiaire_"..v.name, {
            ...
            voltre.data.markers.register("job_farm_boss_"..v.name, {
                Position = vector3(v.PosBoss.x, v.PosBoss.y, v.PosBoss.z),
                Public = false,
                Job = v.name,
                Job2 = nil,
...
CreateThread(function()
    voltre.fct.waitPlayerLoaded()
    while not voltre.data.jobs.farms.loaded do Wait(100) end
    for _, v in pairs(voltre.data.jobs.farms.list) do
        if ESX.PlayerData.job.name == v.name then
            currentFarm = v
            voltre.DebugPrint("Set currentFarm", v.name, v.type)
            break
        end
    end
    if not currentFarm then
        voltre.DebugPrint("No currentFarm found")
    end
end)
...
CreateThread(LPH_NO_VIRTUALIZE(function()
    voltre.fct.waitPlayerLoaded()
    while not voltre.data.jobs.farms.loaded do Wait(500) end
    while not voltre.data.jobs.restaurants.loaded do Wait(500) end
