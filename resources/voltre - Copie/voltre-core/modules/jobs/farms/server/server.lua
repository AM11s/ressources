...
RegisterNetEvent('Voltre:CreateFarmEntreprise', function(namejob, labeljob, namerecolteitem, labelrecolteitem, PositionRecolte, nametraitementitem, labeltraitementitem, PositionTraitement, PositionVente, PositionCoffreEntreprise, PositionVestiaire)
	...
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
        ...
        TriggerEvent('Voltre:CreateNewItem', {src = source, name = namerecolteitem, label = labelrecolteitem})
        TriggerEvent('Voltre:CreateNewItem', {src = source, name = nametraitementitem, label = labeltraitementitem})
        ...
        TriggerClientEvent('Voltre:SendEntrepriseFarmList', -1, SaveData.json["entreprises"]["Farm"])
    end
end)
...
RegisterNetEvent('Voltre:initFarmSociety', function()
    local src = source
    TriggerClientEvent('Voltre:SendEntrepriseFarmList', src, SaveData.json["entreprises"]["Farm"])
end)
	...
	if SaveData.FarmPlayers[source] then
        ...
        if type2 == "Restaurant" then
            ...
            if not xPlayer.canCarryItem(itemRecolte, 1) then
                ...
                local harvestQty = 1
                pcall(function()
                    local boostPct = exports["voltre-core"]:GetVIPFarmHarvestBoost(xPlayer.identifier)
            ...
            elseif type == 3 then -- Vente
                voltre.DebugPrint(ListeItemTraitement[ItemTraitement], ListeItemTraitement, ItemTraitement)
                if ListeItemTraitement[ItemTraitement] == nil then
                    ...
                    if ListeItemTraitement[ItemTraitement].name == ItemTraitement then
                        ...
                        if Quantity <= 0 then
                            ...
                            pcall(function()
                                farmBoostPct = exports["voltre-core"]:GetVIPFarmSellBoost(xPlayer.identifier) or 0
                            end)
...
RegisterNetEvent('Voltre:DeleteFarms', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
