...
RegisterNUICallback("societyTablet:rename", function(data, cb)
    local newname = voltre.fct.input("Nouveau nom pour votre entreprise :", "voltre-core")
...
RegisterNUICallback("societyTablet:togglePolitique", function(data, cb)
    ...
    TriggerServerEvent("Voltre:EditPolitique", data.key, data.value, currentSociety.name)
    cb({})
end)

RegisterNUICallback("societyTablet:govRenameSociety", function(data, cb)
    local newname = voltre.fct.input("Nouveau nom de l'entreprise ?")
    if newname and newname ~= "" then
...
RegisterNUICallback("societyTablet:govChangeSalary", function(data, cb)
    local newValue = voltre.fct.input("Nouvelle valeur :", false, 9000000, "number")
    if newValue then
        TriggerServerEvent("Voltre:gouv:updateSalary", data.salaryType, newValue, data.subType)
        cb({ success = true })
    else
        cb({ success = false })
    end
...
RegisterNUICallback("societyTablet:govChangeTax", function(data, cb)
    local newValue = voltre.fct.input("Nouveau montant :", false, 100, "number")
    if newValue then
        TriggerServerEvent("Voltre:gouv:updateTaxes", data.taxType, newValue)
        cb({ success = true })
    else
        cb({ success = false })
    end
...
RegisterNUICallback("societyTablet:govToggleElectricity", function(data, cb)
    TriggerServerEvent("voltre:weather:changeBlackout", not inBlackout)
    cb({ success = true })
end)

RegisterNUICallback("societyTablet:govTogglePolitique", function(data, cb)
    TriggerServerEvent("Voltre:EditPolitique", data.key, data.value, data.societyName)
    cb({})
end)
