...
AddEventHandler("Core:InitAllJob", function(ESXJOB)
    ...
    MySQL.Async.fetchAll('SELECT name FROM jobs', {}, function(result)
        if result then
            for _, v in pairs(result) do TriggerEvent('Voltre:esx_phone:registerNumber', v.name, v.name, true, true) end
        end
    end)
end)

RegisterNetEvent("Voltre:gouvernement")
AddEventHandler("Voltre:gouvernement", function(ESXJOB)
    TriggerEvent("Core:InitAllJob", ESXJOB)
end)

RegisterNetEvent("Voltre:gouv:updateSalary")
AddEventHandler("Voltre:gouv:updateSalary", function(salarytype, value, type2)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer or xPlayer.getJob().name ~= "gouvernement" then return end
    SaveData.json["salary"][salarytype][type2] = value
    TriggerClientEvent("Voltre:updateSalary", -1, GetSalary())
end)

RegisterNetEvent("Voltre:gouv:updateTaxes")
AddEventHandler("Voltre:gouv:updateTaxes", function(taxes, value)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer or xPlayer.getJob().name ~= "gouvernement" then return end
    SaveData.json["taxes"][taxes] = value
    TriggerClientEvent("Voltre:updateTaxes", -1, SaveData.json["taxes"])
end)
...
ESX.RegisterServerCallback("Voltre:GetMemberOfSociety", function(source, cb, type, name)
    local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('vsociety:updateSocietyStatus', function(societyname, state)
    ...
    TriggerClientEvent('voltre:notificationAdvanced', -1, msg, displayName, "Annonce", SocietyList[societyname].brandColor, logo)
end)

RegisterNetEvent("voltre:sendSocietyNotifToSource")
AddEventHandler("voltre:sendSocietyNotifToSource", function(jobName, msg, title, subject)
    ...
    TriggerClientEvent("voltre:notificationAdvanced", _source, msg, title, subject, color, logo)
end)
...
AddEventHandler('vsociety:changelabelsociety', function(societyname, newname)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. _source .. " U" .. xPlayer.getIdunique() .. ") a changé le nom de son entreprise en " .. newname, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
end)
...
AddEventHandler("Core:AddMoneyBusiness", function(position, ActionType, societyInfo, moneyType, amount)
    ...
    voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (T" .. _source .. " U" .. xPlayer.getIdunique() .. ") a déposé " .. amount .. "$ (" .. moneyType .. ")", "society", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
...
AddEventHandler("Core:AddMoneyToSocietyCache", function(position, ActionType, societyInfo, moneyType, amount, job2)
    ...
    elseif ActionType == "remove" then
        ...
        voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (T" .. _source .. " U" .. xPlayer.getIdunique() .. ") a retiré " .. amount .. "$ (" .. moneyType .. ")", "society", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    else
...
ESX.RegisterServerCallback("Core:GetSocietyInfo", function(source, cb, society)
    ...
    LoadSociety(society, function()
        if SocietyCache[society] then
            ...
            print("[^1Voltre^7] Société introuvable en DB: " .. tostring(society))
            cb(nil, 0)
        end
    end)
end)

RegisterNetEvent('Voltre:EditPolitique')
AddEventHandler('Voltre:EditPolitique', function(index, value, job)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
    ...
    SaveSociety(job)
    TriggerClientEvent("voltre:reciveNewPolitique", -1, job, value)
end)
...
RegisterNetEvent("voltre:entreprise:updateBrand")
AddEventHandler("voltre:entreprise:updateBrand", function(entrepriseType, jobname, brand)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    pcall(function() exports["voltre-core"]:SaveCacheData("entreprises") end)
...
ESX.RegisterServerCallback('Voltre:bossmenu:getMoneyFromEntreprise', function(source, cb, jobname)
    local xPlayer = ESX.GetPlayerFromId(source)
