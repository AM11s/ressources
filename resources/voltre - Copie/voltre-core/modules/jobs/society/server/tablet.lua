...
local function isBarJob(jobName)
    if voltre and voltre.data and voltre.data.jobs and voltre.data.jobs.bars then
        for _, v in pairs(voltre.data.jobs.bars.list) do
...
local function isMecanoJob(jobName)
    if voltre and voltre.data and voltre.data.jobs and voltre.data.jobs.mecanos then
        for _, v in pairs(voltre.data.jobs.mecanos.list) do
            if v.name == jobName then return true end
        end
    end
    return false
...
local function isAmbulanceJob(jobName)
    if voltre and voltre.data and voltre.data.jobs and voltre.data.jobs.ambulances then
        for _, v in pairs(voltre.data.jobs.ambulances.list) do
            if v.name == jobName then return true end
        end
    end
    return false
...
ESX.RegisterServerCallback("societyTablet:fireEmployee", function(source, cb, societyName, identifier, idunique, job2)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a viré " .. identifier, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:promoteEmployee", function(source, cb, societyName, identifier, idunique, currentGrade, job2)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a promu " .. identifier, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:demoteEmployee", function(source, cb, societyName, identifier, idunique, currentGrade, job2)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a rétrogradé " .. identifier, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:deposit", function(source, cb, societyName, moneyType, amount, position, job2)
    ...
    voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a déposé " .. amount .. "$ (" .. moneyType .. ")", "society", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:withdraw", function(source, cb, societyName, moneyType, amount, position, job2)
    ...
    SaveSociety(societyName)
    voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a retiré " .. amount .. "$ (" .. moneyType .. ")", "society", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:buyDrink", function(source, cb, itemValue, itemName, itemPrice, count, societyName)
    ...
    TriggerEvent('Voltre:esx_addoninventory:getSharedInventory', society, function(inventory)
        inventory.addItem(itemValue, count)
        xPlayer.removeAccountMoney("cash", itemPrice)
        voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a acheté " .. itemName .. " " .. count .. "X", "society", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    end)
...
ESX.RegisterServerCallback("societyTablet:buyHeal", function(source, cb, itemValue, itemName, itemPrice, count, societyName)
    ...
    TriggerEvent('Voltre:esx_addoninventory:getSharedInventory', 'society_ambulance', function(inventory)
        inventory.addItem(itemValue, count)
        xPlayer.removeAccountMoney("cash", itemPrice)
        voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a acheté " .. itemName .. " " .. count .. "X", "society", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    end)
...
ESX.RegisterServerCallback("societyTablet:createGrade", function(source, cb, societyName, gradeData, job2)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a créé le grade '" .. gradeData.label .. "' pour " .. societyName, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
...
ESX.RegisterServerCallback("societyTablet:deleteGrade", function(source, cb, societyName, grade, job2)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a supprimé le grade " .. grade .. " de " .. societyName, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    TriggerEvent("Core:InitAllJob", ESX.Jobs)
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:editGradeMeta", function(source, cb, societyName, grade, newName, newLabel, job2)
    ...
    TriggerEvent("Core:InitAllJob", ESX.Jobs)
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a modifié le grade " .. grade .. " de " .. societyName .. " (" .. oldName .. " → " .. newName .. " / " .. newLabel .. ")", "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:recruitEmployee", function(source, cb, societyName, targetId, job2)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. " U" .. xPlayer.getIdunique() .. ") a recruté " .. xTarget.getName() .. " (T" .. targetId .. ")", "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:evictProperty", function(source, cb, propertyId, propertyName)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. ") a viré le propriétaire de " .. propertyName, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:assignProperty", function(source, cb, propertyId, propertyName, targetId, toSelf)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. ") a attribué la propriété " .. propertyName .. " à " .. targetIdentifier, "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
...
ESX.RegisterServerCallback("societyTablet:deleteProperty", function(source, cb, propertyId, propertyName)
    ...
    voltre.logs.send("Logs Society", "Le joueur : " .. xPlayer.getName() .. " (T" .. source .. ") a supprimé la propriété " .. propertyName .. " (ID: " .. propertyId .. ")", "societyadmin", { idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName() })
    cb({ success = true })
end)
