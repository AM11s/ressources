...
RegisterNetEvent('Voltre:createbarbuilder', function(namejob, labeljob, PosVestiaire, PosBar, PosBoss)
    ...
    if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
        ...
        InitSociety2()
        TriggerClientEvent('Voltre:receiveBarBuilder', -1, SaveData.json["entreprises"]["Bar"])
    else
        ExecuteCommand("ban " .. source .. " Tentative de triche creation barbuilder (0)")
    end
end)

RegisterNetEvent('Voltre:bar:refreshSetJob', function(namejob)
    local xPlayer = ESX.GetPlayerFromId(source)
    if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
        if SaveData.json["entreprises"]["Bar"][namejob] == nil then 
            return 
...
RegisterNetEvent('Voltre:DeleteBars', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
...
RegisterNetEvent('Voltre:initBarBuilder', function()
	TriggerClientEvent('Voltre:receiveBarBuilder', source, SaveData.json["entreprises"]["Bar"], BarBuilderCounter)
end)

RegisterCommand('createbar', function(source,args)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
		TriggerClientEvent('Voltre:createbarbuildermenu', source)
	end
end)

RegisterNetEvent("Voltre:bar:take", function(societyName, name, quantity)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == societyName then
        TriggerEvent('Voltre:esx_addoninventory:getSharedInventory', 'society_'..societyName, function(inventory)
            ...
            exports["voltre-core"]:SendLogs("Logs","Le joueur : "..xPlayer.getName().." (T"..source.." U"..xPlayer.getIdunique()..") a récuperer "..name.." "..quantity.."X dans le Bar ("..societyName..")","society", {idunique_auteur = xPlayer.getIdunique(), name = xPlayer.getName()})
        end)
    end
end)

ESX.RegisterServerCallback('Voltre:getStockItemsBar', function(source, cb, societyName)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name == societyName then
        TriggerEvent('Voltre:esx_addoninventory:getSharedInventory', 'society_'..societyName, function(inventory)
            local finalinv = {}
