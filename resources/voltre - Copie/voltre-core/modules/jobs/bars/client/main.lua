...
OpenFridge = function()
    ESX.TriggerServerCallback('Voltre:getStockItemsBar', function(items)
        elementsObjectSaisi = {}
        for i=1, #items, 1 do
            ...
            for e, f in pairs(elementsObjectSaisi) do
                if f.count > 0 then
                    RageUI.Button(f.name, false, { RightLabel = "~g~x"..f.count }, true , {
                        onSelected = function()
                            local input = voltre.fct.input('Choisir une quantité ?', false, 10., "number")
                            if tonumber(input) <= f.count then
                                TriggerServerEvent("Voltre:bar:take", ESX.PlayerData.job.name, f.value, tonumber(input))
                                Wait(500)
                                ESX.TriggerServerCallback('Voltre:getStockItemsBar', function(items)
                                    elementsObjectSaisi = {}
                                    for i=1, #items, 1 do
...
RegisterNetEvent('Voltre:receiveBarBuilder', function(Table)
    voltre.data.jobs.bars.list = Table
    voltre.data.jobs.bars.loaded = true
    for k,v in pairs(BarBlips) do
        RemoveBlip(v)
    end

    for k,v in pairs(voltre.data.jobs.bars.list) do
        if voltre.data.jobs.bars.list.type == 'Bar' then
            BarBlips[v.name] = AddBlipForCoord(voltre.data.jobs.bars.list.PosBoss.x, voltre.data.jobs.bars.list.PosBoss.y, voltre.data.jobs.bars.list.PosBoss.z)
            ...
            AddTextComponentString(voltre.data.jobs.bars.list.label)
            ...
            if not ZoneExist("job_bar_vestiaire_"..v.name) then
                voltre.data.markers.register("job_bar_vestiaire_"..v.name, {
                ...
                voltre.data.markers.register("job_bar_fridge_"..v.name, {
                ...
                voltre.data.markers.register("job_bar_boss_"..v.name, {
...
Citizen.CreateThread(function()
    Wait(2000)
    TriggerServerEvent('Voltre:initBarBuilder')
end)