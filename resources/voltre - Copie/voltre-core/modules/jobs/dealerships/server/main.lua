...
CreateThread(function()
    ...
    for shopName in pairs(Config.Dealerships.Shops) do
        if not SaveData.json["dealerships"][shopName] then
            ...
            --voltre.InitPrint("^3[DEALERSHIP] " .. shopName .. " initialisé (aucun stock existant)^0")
        end
        local stockCount = #SaveData.json["dealerships"][shopName].stock
        --voltre.InitPrint(string.format("^2[DEALERSHIP] %s chargé - %d véhicules en stock^0", shopName, stockCount))
    end
    -- Log tax rates (SaveData is loaded at this point)
    local initTaxes = GetTaxRates()
    --voltre.InitPrint(string.format("^2[DEALERSHIP] Taxes actives — Salaire: %d%% | Retrait (TVA): %d%%^0", initTaxes.salaire, initTaxes.retrait))
...
ESX.RegisterServerCallback('dealership:getTabletData', function(source, cb, shopName, mode)
    ...
    if mode == 'employee' or mode == 'boss' then
        if xPlayer.job.name == shopName then
            ...
            --voltre.InitPrint(string.format("^3[DEALERSHIP] getTabletData: %s mode=%s stock=%d history=%d^0",
    ...
    if mode == 'boss' and xPlayer.job.name == shopName then
       ...
       local salesByName     = {}
        ...
        --voltre.InitPrint(string.format("^3[DEALERSHIP DEBUG] Sellers in DB: [%s]^0", table.concat(sellerNames, ", ")))

        for _, u in ipairs(users) do
            local empName = string.lower((u.firstname or '') .. ' ' .. (u.lastname or ''))
            --voltre.InitPrint(string.format("^3[DEALERSHIP DEBUG] Matching emp='%s' | salesFound=%d^0", empName, salesByName[empName] and #salesByName[empName] or 0))
            local totalCA  = 0
            local activity = {}
...
RegisterNetEvent('dealership:purchaseForStock', function(shopName, model, colorId, catalogPrice)
    ...
    --voltre.InitPrint(string.format("^3[DEALERSHIP] AchatStock: %s | Modèle: %s | Plaque: %s | Coût: %d$^0",
...
ESX.RegisterServerCallback('dealership:sellToPlayer', function(source, cb, shopName, stockIndex, targetId)
    ...
    --voltre.InitPrint(string.format("^3[DEALERSHIP] Demande vente: %s → %s | %s | %d$^0",
    ...
    while SaleBillingWaiting[billId] == 'pending' and waited < maxWait do
        ...
        if not ESX.GetPlayerFromId(targetId) then
            ...
            --voltre.InitPrint("^1[DEALERSHIP] Acheteur déconnecté pendant la demande^0")
    ...
    --voltre.InitPrint(string.format("^2[DEALERSHIP] Vente confirmée: %s | Prix: %d$ | TVA(%d%%): %d$ | Net: %d$^0",
...
RegisterNetEvent('dealership:payEmployee', function(shopName, identifier)
    ...
    --voltre.InitPrint(string.format("^3[DEALERSHIP] PayEmployee: %s | CA=%d$ | Brut=%d$ | Impôts=%d$ | Net=%d$ | TaxeCA=%d$^0",
...
RegisterNetEvent('dealership:enterPreviewInstance', function()
    ...
    voltre.fct.instance.Set(_source, instanceId, "Aperçu véhicule")
end)
...
RegisterNetEvent('dealership:exitPreviewInstance', function()
    ...
    voltre.fct.instance.Set(_source, 0, "Monde RolePlay")
end)
...
local function SpawnShowroomEntityServer(shopName, slotKey, vehicleData)
    ...
    -- --voltre.InitPrint(string.format("^2[DEALERSHIP] Showroom server entity spawned: %s/%s → %s^0",
...
AddEventHandler('dealership:setShowroomVehicle', function(shopName, slotKey, stockIndex)
    ...
    if not stockIndex then
        ...
        --voltre.InitPrint(string.format("^3[DEALERSHIP] Showroom %s/%s vidé par %s^0", shopName, slotKey, xPlayer.getName()))
    else
        local stock = SaveData.json["dealerships"][shopName].stock
        ...
        --voltre.InitPrint(string.format("^3[DEALERSHIP] Showroom %s/%s → %s (%s) par %s^0",
