               ...
               if IsPedShooting(PlayerState.ped) and PlayerState.weapon ~= GetHashKey("WEAPON_PETROLCAN") and PlayerState.weapon ~= GetHashKey("WEAPON_PLASMAP") and PlayerState.weapon ~= GetHashKey("WEAPON_FIREEXTINGUISHER") and PlayerState.weapon ~= GetHashKey("WEAPON_APPISTOL") then
                    if hassuppressor then
                         ...
                         if shoot == Config.Police.Pourcent.withsupp.thenbr then
                              ...
                              TriggerServerEvent('voltre:police:alert:firelisten', plyPos.x, plyPos.y, plyPos.z)
                              alerteEnCours = true
                         end
                    else
...
RegisterNetEvent('voltre:police:listen:fire:blips')
AddEventHandler('voltre:police:listen:fire:blips', function(gx, gy, gz)
...
AddEventHandler('vPriseAppel', function(name)
     if ESX.PlayerData.job ~= nil and (voltre.data.jobs.polices.list[ESX.PlayerData.job.name] == nil) then
...
Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
    while true do
        interval = 750
        if ESX.PlayerData.job ~= nil and voltre.data.jobs.polices.list[ESX.PlayerData.job.name] == nil and alerteEnCours then
            interval = 1
            if IsControlJustPressed(1, 246) and alerteEnCours then --Y
                TriggerServerEvent('voltre:police:alert:take')
                AlertePrise = true
                TriggerEvent('voltre:police:listen:fire:blips', coordsX, coordsY, coordsZ)
                alerteEnCours = false
            elseif IsControlJustPressed(1, 73) and alerteEnCours and (voltre.data.jobs.polices.list[ESX.PlayerData.job.name] == nil) then
                AlertePrise = false
                alerteEnCours = false
            end
         end
