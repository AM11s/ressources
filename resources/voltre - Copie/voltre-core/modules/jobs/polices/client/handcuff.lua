...
RegisterNetEvent('voltre:handcuff:cbClosestPlayerID')
AddEventHandler('voltre:handcuff:cbClosestPlayerID', function(wannacuff, method)
	...
	if distance ~= -1 and distance <= 3 and not IsPedInAnyVehicle(PlayerPedId()) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) and not isArresting then
		...
		if method ~= 'policecuff' and wannacuff then 
			if ESX.isHandsUp(targetPed) then  
				TriggerServerEvent('voltre:handcuff:handcuff', GetPlayerServerId(closestPlayer), wannacuff, method)
			else
...
AddEventHandler("voltre:handcuff:startThread", function()
	cuffedN = true
	while cuffedN do
		Citizen.Wait(0)

		SetCurrentPedWeapon(PlayerState.ped, "WEAPON_UNARMED", true)
        voltre.DebugPrint("handcuff, set unarmed")
...
RegisterNetEvent("voltre:handcuff:forceCuff", function()
	RageUI.setKeyState(21, true)
	RageUI.setKeyState(22, true)
	TriggerEvent("voltre:handcuff:startThread")
	SetEnableHandcuffs(PlayerState.ped, true)
end)

RegisterNetEvent('voltre:handcuff:thecuff')
AddEventHandler('voltre:handcuff:thecuff', function(wannamove, cufferSource)
	...
	if wannamove then
		...
		TriggerServerEvent('voltre:InteractSound_SV:PlayWithinDistance', 3.0, 'cuff', 0.7)
		Citizen.Wait(2300)
	...
	elseif not wannamove then
		TriggerServerEvent('voltre:InteractSound_SV:PlayWithinDistance', 3.0, 'uncuff', 0.7)
...
RegisterNetEvent('voltre:handcuff:arresting')
AddEventHandler('voltre:handcuff:arresting', function()
	ESX.Streaming.RequestAnimDict(cuffdict)
...
RegisterNetEvent('voltre:handcuff:unarresting')
AddEventHandler('voltre:handcuff:unarresting', function()
