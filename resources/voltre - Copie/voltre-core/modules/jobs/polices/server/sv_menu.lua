...
RegisterNetEvent("Voltre:servicepolice")
AddEventHandler("Voltre:servicepolice", function(value, policename)
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:servicepolice") 
    ...
    if SaveData.json["entreprises"]["Police"][xPlayer.job.name] ~= nil then 
        ...
        if value then 
            ...
            for k, v in pairs(PlayerPolice) do 
                TriggerClientEvent("voltre:notificationAdvanced", k, "L'agent "..ESX.Config("serverColor")..xPlayer.getName().." ~s~viens de prendre son service", label, "Prise de Service", color, logo)
                TriggerClientEvent("Voltre:recieveagentpolice", k, InService)
            end
        else
            PlayerPolice[source] = nil
            InService = InService-1
            for k, v in pairs(PlayerPolice) do 
                TriggerClientEvent("voltre:notificationAdvanced", k, "L'agent "..ESX.Config("serverColor")..xPlayer.getName().." ~s~viens de finir son service", label, "Fin de Service", color, logo)
                TriggerClientEvent("Voltre:recieveagentpolice", k, InService)
            end
        end
    else
...
RegisterNetEvent("Voltre:confiscateitem")
AddEventHandler("Voltre:confiscateitem", function(count, item, player, action, label)
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:confiscateitem") 
...
RegisterNetEvent("Voltre:demandederenfort")
AddEventHandler("Voltre:demandederenfort", function(type)
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:demandederenfort") 
    if SaveData.json["entreprises"]["Police"][xPlayer.job.name] ~= nil then 
        if PlayerPolice[source] then 
            local jobName = xPlayer.job.name
            local logo, color, label
            ...
            for k, v in pairs(PlayerPolice) do 
                if type == "pause" then 
                    TriggerClientEvent("voltre:notificationAdvanced", k, "L'agent "..ESX.Config("serverColor")..xPlayer.getName().."~s~ viens de se mettre en pause !", label, "Radio", color, logo)
                elseif type == 'control' then
                    TriggerClientEvent("voltre:notificationAdvanced", k, "L'agent "..ESX.Config("serverColor")..xPlayer.getName().."~s~ est actuellement en control !", label, "Radio", color, logo)
                elseif type == "retrourpdp" then 
                    TriggerClientEvent("voltre:notificationAdvanced", k, "L'agent "..ESX.Config("serverColor")..xPlayer.getName().."~s~ est en route vers le commisariat", label, "Radio", color, logo)
                else
                    TriggerClientEvent("voltre:notificationAdvanced", k, "L'agent "..ESX.Config("serverColor")..xPlayer.getName().."~s~ à besoin de renfort, je t'ai mis les coordonnées sur ton GPS !", label, "Renfort", color, logo)
                    TriggerClientEvent("Voltre:demandederenfort", k, type, GetEntityCoords(GetPlayerPed(source)))
                end
            end
        else
            DropPlayer(source, 'Désynchonisation avec le serveur ou detection de Cheat')
...
RegisterNetEvent("Voltre:newcasierpolice")
AddEventHandler("Voltre:newcasierpolice", function(casier)
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerEvent("ratelimit", xPlayer.source, "Voltre:newcasierpolice") 
    if SaveData.json["entreprises"]["Police"][xPlayer.job.name] ~= nil then 
        if PlayerPolice[source] then 
...
RegisterNetEvent("Voltre:miseencelule")
AddEventHandler("Voltre:miseencelule", function(player, timer, cellule)
    ...
    if SaveData.json["entreprises"]["Police"][xPlayer.job.name] ~= nil then 
        ...
        if distance < 50.00 then 
            ...
            if not Cellule[tPlayer.identifier] then 
                ...
                TriggerClientEvent("Voltre:setTimerPrison", player, timer, Cellule[tPlayer.identifier].code, PosCommico)
            end
        else
...
RegisterNetEvent("Voltre:updatetimerprison")
AddEventHandler("Voltre:updatetimerprison", function(timer, code)
    local xPlayer = ESX.GetPlayerFromId(source)
    if Cellule[xPlayer.identifier] == nil then
        ...
        if tonumber(Cellule[xPlayer.identifier].code) == tonumber(code) then 
            if tonumber(Cellule[xPlayer.identifier].timecellule) == 0 or tonumber(Cellule[xPlayer.identifier].timecellule) == 1 then 
                ...
                TriggerClientEvent("voltre:police:notif", -1, "✅ Le temps de la cellule "..Cellule[xPlayer.identifier].cellulenbr.." est terminer.\nVeuillez venir le liberer le plus vite possible.\nId Unique : "..xPlayer.getIdunique(), true)
...
AddEventHandler('esx:playerLoaded', function(source)
    ...
    if Cellule[xPlayer.identifier] ~= nil then
        TriggerClientEvent("Voltre:setTimerPrison", src, Cellule[xPlayer.identifier].timecellule, Cellule[xPlayer.identifier].code, Cellule[xPlayer.identifier].policepos)
    end
end)
