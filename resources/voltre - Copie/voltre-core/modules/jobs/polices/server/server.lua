...
RegisterNetEvent('Voltre:createpolice', function(namejob, labeljob, PosVestiaire, PosArmory, PosBoss, color, Cellule)
    ...
    if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
        ...
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Chef", "boss", 6)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Capitaine", "intendent", 5)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Lieutenant", "lieutenant", 4)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Sergent", "chef", 3)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Caporal", "sergeant", 2)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Officier", "officer", 1)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Recrue", "recruit", 0)
        ...
        InitSociety2()
        TriggerClientEvent('Voltre:receivePolice', -1, SaveData.json["entreprises"]["Police"])
        TriggerClientEvent('Voltre:receivePolice2', -1, SaveData.json["entreprises"]["Police"])
    else
        ExecuteCommand("ban " .. source .. " Tentative de triche creation mécano (0)")
    end
end)

RegisterNetEvent('Voltre:initPolice', function()
	TriggerClientEvent('Voltre:receivePolice', source, SaveData.json["entreprises"]["Police"])
    TriggerClientEvent('Voltre:receivePolice2', -1, SaveData.json["entreprises"]["Police"])
end)

RegisterNetEvent('Voltre:DeletePolice', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
		if SaveData.json["entreprises"]["Police"][value] then
            ...
            TriggerClientEvent('Voltre:receivePolice', -1, SaveData.json["entreprises"]["Police"])
            TriggerClientEvent('Voltre:receivePolice2', -1, SaveData.json["entreprises"]["Police"])
            TriggerClientEvent('Voltre:deletepoliceblips', -1, value)
        else
...
RegisterNetEvent("voltre:policebuilder:takearmory")
AddEventHandler("voltre:policebuilder:takearmory", function()
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerEvent("ratelimit", xPlayer.source, "voltre:policebuilder:takearmory") 
...
RegisterNetEvent("voltre:police:removeservice", function()
    local src = source;
...
AddEventHandler('esx:playerLoaded', function(source)
    ...
    if (SaveData.json["entreprises"]["Police"][xPlayer.job.name] ~= nil) then
        if ArmedeServiceList[xPlayer.getIdunique()] ~= nil then
            Wait(5000)
            TriggerClientEvent("voltre:job:police:armeservice",source,true)
