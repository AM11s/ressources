...
RegisterServerEvent('voltre:handcuff:handcuff')
AddEventHandler('voltre:handcuff:handcuff', function(target, wannacuff, method)
			...
			if wannacuff then
				if not targetCuff.isCuffed then
					if method == 'policecuff' then
						TriggerClientEvent('voltre:handcuff:arresting', xPlayer.source)
						TriggerClientEvent('voltre:handcuff:thecuff', target, true, xPlayer.source)
			...
			elseif not wannacuff then
				if targetCuff.isCuffed then
					if (method == targetCuff.cuffMethod) or (method == 'all') then
						TriggerClientEvent('voltre:handcuff:unarresting', xPlayer.source)
						TriggerClientEvent('voltre:handcuff:thecuff', target, false)
...
ESX.RegisterUsableItem('basic_cuff', function(source)
	TriggerClientEvent('voltre:handcuff:cbClosestPlayerID', source, true, 'basiccuff')
end)

ESX.RegisterUsableItem('basic_key', function(source)
	TriggerClientEvent('voltre:handcuff:cbClosestPlayerID', source, false, 'basiccuff')
end)

ESX.RegisterUsableItem('police_cuff', function(source)
	TriggerClientEvent('voltre:handcuff:cbClosestPlayerID', source, true, 'policecuff')
end)

ESX.RegisterUsableItem('police_key', function(source)
	TriggerClientEvent('voltre:handcuff:cbClosestPlayerID', source, false, 'policecuff')
end)

ESX.RegisterUsableItem('lockpick', function(source)
	TriggerClientEvent('voltre:handcuff:cbClosestPlayerID', source, false, 'all')
end)
...
ESX.AddGroupCommand('demenotter', "admin", function(source, args, user)
	...
	if xPlayer then
		xPlayer.triggerEvent('voltre:handcuff:thecuff', false)
		xPlayer.set('cuffState', {isCuffed = false, cuffMethod = nil})
	else
