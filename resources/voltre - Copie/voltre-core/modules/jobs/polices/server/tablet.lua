...
local function broadcastRadio(jobName)
    ...
    for _, playerId in ipairs(GetPlayers()) do
        ...
        if xp and xp.getJob().name == jobName then
            TriggerClientEvent("voltre:policeTablet:radioUpdate", pid, list)
        end
    end
end
...
local function broadcastDispatch(jobName)
    ...
    for _, playerId in ipairs(GetPlayers()) do
        ...
        local xp = ESX.GetPlayerFromId(pid)
        if xp and xp.getJob().name == jobName then
            TriggerClientEvent("voltre:policeTablet:dispatchUpdate", pid, state)
        end
    end
end
...
-- ============================================================================
ESX.RegisterServerCallback("voltre:policeTablet:getData", function(source, cb)
    local src = source
...
-- ============================================================================
RegisterNetEvent("voltre:policeTablet:createCard", function(payload)
    local src = source
...
RegisterNetEvent("voltre:policeTablet:moveCard", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp then return end
    local jobName = xp.getJob().name
...
RegisterNetEvent("voltre:policeTablet:deleteCard", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp then return end
    local jobName = xp.getJob().name
...
RegisterNetEvent("voltre:policeTablet:createGroup", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp then return end
    local jobName = xp.getJob().name
...
RegisterNetEvent("voltre:policeTablet:moveGroup", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp then return end
    local jobName = xp.getJob().name
...
RegisterNetEvent("voltre:policeTablet:deleteGroup", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp then return end
    local jobName = xp.getJob().name
...
ESX.RegisterServerCallback("voltre:policeTablet:searchPlayers", function(source, cb, query)
    local src = source
...
ESX.RegisterServerCallback("voltre:policeTablet:recentCasier", function(source, cb)
    local src = source
    if not isPoliceJob(getJobName(src)) then return cb({}) end
    MySQL.Async.fetchAll([[
...
ESX.RegisterServerCallback("voltre:policeTablet:getPlayerDetails", function(source, cb, identifier)
    local src = source
...
ESX.RegisterServerCallback("voltre:policeTablet:getCasier", function(source, cb, identifier)
    local src = source
    if not isPoliceJob(getJobName(src)) then return cb({}) end
...
ESX.RegisterServerCallback("voltre:policeTablet:searchVehicles", function(source, cb, query)
    local src = source
    if not isPoliceJob(getJobName(src)) then return cb({}) end
...
ESX.RegisterServerCallback("voltre:policeTablet:searchProperties", function(source, cb, query)
    local src = source
    if not isPoliceJob(getJobName(src)) then return cb({}) end
...
-- CASIER : add entry
-- ============================================================================
RegisterNetEvent("voltre:policeTablet:addCasier", function(payload)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    MySQL.Async.execute([[
        ...
        TriggerClientEvent("voltre:policeTablet:casierAdded", src, payload.identifier)
    end)
end)

RegisterNetEvent("voltre:policeTablet:deleteCasier", function(casierId)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp then return end
...
-- ============================================================================
RegisterNetEvent("voltre:policeTablet:addPenal", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
        ...
        loadPenalCode(jobName, function(penal)
            for _, pid in ipairs(GetPlayers()) do
                ...
                if p and p.getJob().name == jobName then
                    TriggerClientEvent("voltre:policeTablet:penalUpdate", tonumber(pid), penal)
                end
            end
        end)
    end)
...
RegisterNetEvent("voltre:policeTablet:updatePenal", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp or not isBoss(src) then return end
    local jobName = xp.getJob().name
...
RegisterNetEvent("voltre:policeTablet:deletePenal", function(penalId)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp or not isBoss(src) then return end
    local jobName = xp.getJob().name
...
-- ============================================================================
RegisterNetEvent("voltre:policeTablet:addRadio", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp or not isBoss(src) then return end
    local jobName = xp.getJob().name
...
RegisterNetEvent("voltre:policeTablet:updateRadio", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp or not isBoss(src) then return end
    local jobName = xp.getJob().name
...
RegisterNetEvent("voltre:policeTablet:deleteRadio", function(payload)
    local src = source
    local xp = ESX.GetPlayerFromId(src)
    if not xp or not isBoss(src) then return end
    local jobName = xp.getJob().name
...
voltre.InitPrint("Police Tablet Server loaded")
