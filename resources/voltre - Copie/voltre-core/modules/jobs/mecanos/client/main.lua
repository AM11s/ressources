                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        if DoesEntityExist(vehicle) and dist4 < 4 then
                            ...
                            Citizen.CreateThread(function()
                                ...
                                TriggerServerEvent("voltre:mecano:verifOwner", GetPlayerServerId(closestPlayer), GetVehicleNumberPlateText(vehicle))
                            end)
                        else
...
RegisterNetEvent('voltre:VerifOwnerReceive', function(plate)
...
RegisterNetEvent('Voltre:receiveMecano', function(Table)
    voltre.data.jobs.mecanos.list = Table
    voltre.data.jobs.mecanos.loaded = true
    Wait(1000)
    for k,v in pairs(BlipsMecano) do
        RemoveBlip(v)
    end

    for k,v in pairs(voltre.data.jobs.mecanos.list) do
        ...
        if v.type == 'Mécano' then
            ...
            if not voltre.data.markers.isRegister("job_mecano_vestiaire_"..v.name) then
                voltre.data.markers.register("job_mecano_vestiaire_"..v.name, {
                ...
                voltre.data.markers.register("job_mecano_custom1_"..v.name, {
                    ...
                    Action = function()
                        if PlayerState.isInVehicle then
                            ...
                            TriggerEvent('voltre:openMenuCustom', v.name, v.label, vector3(v.PosCustom.x, v.PosCustom.y, v.PosCustom.z))
                        else
                            ESX.ShowNotification('Vous devez être dans un véhicule')
                        end
                    end
                })
                voltre.data.markers.register("job_mecano_custom2_"..v.name, {
                    ...
                    Action = function()
                        if PlayerState.isInVehicle then
                            ...
                            TriggerEvent('voltre:openMenuCustom', v.name, v.label, vector3(v.PosCustom2.x, v.PosCustom2.y, v.PosCustom2.z))
                        else
                            ESX.ShowNotification('Vous devez être dans un véhicule')
                        end
                    end
                })
                voltre.data.markers.register("job_mecano_custom3_"..v.name, {
                    ...
                    Action = function()
                        if PlayerState.isInVehicle then
                            ...
                            TriggerEvent('voltre:openMenuCustom', v.name, v.label, vector3(v.PosCustom3.x, v.PosCustom3.y, v.PosCustom3.z))
                        else
                            ESX.ShowNotification('Vous devez être dans un véhicule')
                        end
                    end
                })
                voltre.data.markers.register("job_mecano_boss_"..v.name, {
...
RegisterNetEvent('Voltre:deletemecanoblips', function(value)
    for k,v in pairs(BlipsMecano) do
        if k == value then
            RemoveBlip(v)
            break
...
Citizen.CreateThread(function()
    Wait(2000)
    TriggerServerEvent('Voltre:initMecano')
end)

RegisterNetEvent('voltre:mecano:addchoicenotif')
AddEventHandler('voltre:mecano:addchoicenotif', function(id, name, pos,sprite, color, tag,msg)
    if voltre.data.jobs.mecanos.list[ESX.PlayerData.job.name] then
