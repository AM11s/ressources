...
RegisterNetEvent('Voltre:createmecano', function(namejob, labeljob, PosVestiaire, Custom1, Custom2, Custom3, PosBoss)
    ...
    if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
        ...
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "PDG", "boss", 2)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Responsable", "responsable", 1)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Employer", "employer", 0)
        ...
        InitSociety2()
        TriggerClientEvent('Voltre:receiveMecano', -1, SaveData.json["entreprises"]["Mécano"])
    else
        ExecuteCommand("ban " .. source .. " Tentative de triche creation mécano (0)")
    end
end)

RegisterNetEvent('Voltre:mecano:refreshSetJob', function(namejob)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('Voltre:initMecano', function()
	TriggerClientEvent('Voltre:receiveMecano', source, SaveData.json["entreprises"]["Mécano"])
end)

RegisterCommand('createmecano', function(source,args)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
		TriggerClientEvent('Voltre:createmecanomenu', source)
	end
end)


ESX.RegisterServerCallback('Voltre:Mechanic:getVehicleInfos', function(source, cb, plate, name)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('Voltre:esx_lscustom:buyMod')
AddEventHandler('Voltre:esx_lscustom:buyMod', function(price, token, vehname, owner, plaque)
    VerifyToken(source, token, 'Voltre:esx_lscustom:buyMod', function()
        local xPlayer = ESX.GetPlayerFromId(source)
        ...
        if tonumber(SocietyCache[xPlayer.job.name].data["accounts"].cash) >= price then
            ...
            TriggerClientEvent('Voltre:esx_lscustom:installMod', xPlayer.source)
            ...
            exports["voltre-core"]:SendLogs("Logs","Le joueur : "..xPlayer.getName().." (T"..source.." U"..xPlayer.getIdunique()..") a custom le véhicule : "..vehname.." (Plaque : "..plaque..", Owner : "..owner..") pour un total de : "..price.."$","society")
        else
            TriggerClientEvent('Voltre:esx_lscustom:cancelInstallMod', xPlayer.source)
...
RegisterNetEvent('Voltre:DeleteMecano', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
		if SaveData.json["entreprises"]["Mécano"][value] then
            ...
            Cache.SaveOne('entreprises')
            TriggerClientEvent('Voltre:receiveMecano', -1, SaveData.json["entreprises"]["Mécano"])
            TriggerClientEvent('Voltre:deletemecanoblips', -1, value)
        else
...
AddEventHandler('mecano:callMechanic', function(vehicle)
    ...
    TriggerClientEvent("voltre:mecano:addchoicenotif",-1, math.random(1,9999),"callMechanic",pos,317,59,"towrequest","⚠️  Un citoyen a besoin d'un mécano !")
end)

RegisterServerEvent("voltre:mecano:verifOwner", function(target, plate)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    local xTarget = ESX.GetPlayerFromId(target)
        ...
        if SaveData.json["owned_vehicles"][string.upper(plate)] ~= nil then
            if SaveData.json["owned_vehicles"][string.upper(plate)].owner == xTarget.identifier then
                ...
                TriggerClientEvent("voltre:VerifOwnerReceive", xPlayer.source, plate)
            else
                TriggerClientEvent("inventory:sendMessage", xPlayer.source, "Résultat :\nPropriétaire du véhicule = ❌\n\n(Un avertissement a étais envoyer a la police la plus proche)", 6000)
                TriggerClientEvent('voltre:police:addchoicenotif', -1, math.random(0,9999), "Appel vol véhicule", xPlayer.getCoords(), 225, 1,"volvehi-mecano", "Un mécano vous a signalé un véhicule volé.")
            end
        else
            TriggerClientEvent("inventory:sendMessage", xPlayer.source, "Résultat :\nPropriétaire du véhicule = ❌\n\n(Un avertissement a étais envoyer a la police la plus proche)", 6000)
            TriggerClientEvent('voltre:police:addchoicenotif', -1, math.random(0,9999), "Appel vol véhicule", xPlayer.getCoords(), 225, 1,"volvehi-mecano", "Un mécano vous a signalé un véhicule volé.")
