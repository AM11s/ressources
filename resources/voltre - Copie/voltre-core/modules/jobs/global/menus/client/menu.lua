...
--[[function ShowJobBadge(job)
    TriggerServerEvent("Voltre:jsfour-idcard:open", GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), "job")
end]]
...
function ShowJobBadge(jobName)
    ...
    Citizen.CreateThread(function()
        ...
        if closestDistance ~= -1 and closestDistance <= 3.0 then
            TriggerServerEvent('Voltre:jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer), 'job')
        end
...
RegisterCommand("f6", function()
    if not PlayerIsDead and ESX.PlayerData.job ~= nil then 
        ...
        for k,v in pairs(voltre.data.jobs.polices.list) do
            if ESX.PlayerData.job.name == v.name then
        ...
        for k,v in pairs(voltre.data.jobs.ambulances.list) do
            if ESX.PlayerData.job.name == v.name then
