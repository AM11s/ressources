RegisterServerEvent('Voltre:esx_billing:sendBill')
AddEventHandler('Voltre:esx_billing:sendBill', function(target, sharedAccountName, label, amount)
		...
		if xTarget ~= nil then
			if target == source or xPlayer.job.name ~= 'unemployed' and #(GetEntityCoords(GetPlayerPed(source))-GetEntityCoords(GetPlayerPed(target))) < 15 then 
				MySQL.Async.execute('INSERT INTO billing (identifier,date, sender, target_type, target, label, amount) VALUES (@identifier,@date, @sender, @target_type, @target, @label, @amount)', {
					...
					TriggerClientEvent('Voltre:esx_billing:newBill', target)
...
ESX.RegisterServerCallback('Voltre:esx_billing:getBills', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('Voltre:esx_billing:getTargetBills', function(source, cb, target)
...
ESX.RegisterServerCallback('Voltre:esx_billing:payBill', function(source, cb, id)
	local xPlayer = ESX.GetPlayerFromId(source)
