...
local DemandeList = {}
local ReponseList = {}
RegisterNetEvent('voltre:billing:receviereponse')
AddEventHandler('voltre:billing:receviereponse', function(id, bool, passed)
...
ESX.RegisterServerCallback('Core:AddChoiceBilling', function(source, cb, target, amount, society)
    ...
    if xTarget == nil then 
        print("Probleme Facture (045) Merci de contacter Voltre")
        return 
    end
    if xPlayer.job.name ~= "unemployed" then
        if target and amount then 
            if amount <= 0 then 
                ...
                if xTarget.getAccount("cash").money >= amount then
                    ...
                    TriggerClientEvent("voltre:billing:receviedemande", target,id,amount,society)
