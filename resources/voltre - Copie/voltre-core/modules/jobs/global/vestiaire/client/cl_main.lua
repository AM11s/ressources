...
function GetAllClothJob()
    VestiaireJobMenu = {}
    ESX.TriggerServerCallback('voltre:jobclothes:getVetements', function(VestiaireJob)
            ...
            for k,v in pairs(VestiaireJobMenu) do 
                RageUI.List(('Tenue %s'):format(v.label), {"Modifier le nom","~r~Supprimer~s~"}, index.vestiaire, nil, {}, true, {
                    ...
                    onSelected = function(Index)
                        if Index == 1 then
                            local newLabel = voltre.fct.input(('Entrez un nouveau nom  (Nom actuel : %s)'):format(v.label))
                            if newLabel then 
                                TriggerServerEvent('voltre:jobclothes:editname', v.id, newLabel)
                                Wait(150)
                                GetAllClothJob()
                        ...
                        elseif Index == 2 then
                            local result = voltre.fct.confirm(('Êtes-vous sûr de vouloir supprimer : %s ?'):format(v.label))
                            if result then
                                TriggerServerEvent('voltre:jobclothes:deleteJobTenue', v.id)
                                Wait(150)
                                GetAllClothJob()
                            else
                ...
                onSelected = function()
                    ...
                    if name then 
                        ...
                        TriggerEvent("Voltre:skinchanger:getSkin", function(skin)
                            ...
                            TriggerServerEvent("voltre:jobclothes:addtenuvestiaire", name, TempoSkin)
                        end)
...
function apllyTenueVestiaire(tenue)
    ...
    TriggerEvent('Voltre:skinchanger:getSkin', function(skin)
        TriggerEvent('Voltre:skinchanger:loadClothes', skin, json.decode(clothes))
    end)
    -- -- save la tenue 
