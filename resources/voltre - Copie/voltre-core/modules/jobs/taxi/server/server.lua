...
ESX.RegisterServerCallback('voltre:taxi:getProfile', function(source, cb)
...
ESX.RegisterServerCallback('voltre:taxi:startMission', function(source, cb, missionKey)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    local ok, pending = pcall(function() return exports["voltre-core"]:HasPendingTaxiRequest() end)
...
-- ---- Cancel mission --------------------------------------------
RegisterNetEvent('voltre:taxi:cancelMission', function()
    local src = source
    Active[src] = nil
end)

-- ---- Finalisation : paiement + XP ------------------------------
ESX.RegisterServerCallback('voltre:taxi:finishMission', function(source, cb, payload)
    local xPlayer = ESX.GetPlayerFromId(source)
...
--AddEventHandler('Ouvre:taxi', function()
  ...
  --  TriggerClientEvent('voltre:notificationAdvanced', -1,
