...
local function broadcastQueueToTaxis()
    ...
    for _, src in ipairs(listOnServiceTaxis()) do
        TriggerClientEvent('voltre:taxi:queueUpdated', src, snap)
    end
end
...
ESX.RegisterServerCallback('voltre:taxi:requestRide', function(source, cb, payload)
    ...
    for _, driverSrc in ipairs(listOnServiceTaxis()) do
        TriggerClientEvent('voltre:taxi:newRequest', driverSrc, requestSnapshot(req))
    end
...
ESX.RegisterServerCallback('voltre:taxi:cancelMyRequest', function(source, cb)
    ...
    if req.takenBy then
        ...
        TriggerClientEvent('voltre:taxi:requestCanceled', req.takenBy, id)
    end
...
ESX.RegisterServerCallback('voltre:taxi:getMyRequest', function(source, cb)
    local id = AssignedByCitizen[source]
...
ESX.RegisterServerCallback('voltre:taxi:listDrivers', function(source, cb)
...
ESX.RegisterServerCallback('voltre:taxi:listRequests', function(source, cb)
    if not isOnServiceTaxi(source) then return cb({}) end
    cb(listPendingSnapshots())
end)

ESX.RegisterServerCallback('voltre:taxi:acceptRequest', function(source, cb, reqId)
    ...
    TriggerClientEvent('voltre:taxi:requestAccepted', req.citizenSrc, {
        id          = req.id,
        driverSrc   = source,
        driverName  = driverName,
    })
...
ESX.RegisterServerCallback('voltre:taxi:releaseRequest', function(source, cb)
    local id = AssignedByDriver[source]
    if not id or not Requests[id] then return cb(false) end
    local req = Requests[id]

    -- Notifier le citoyen
    TriggerClientEvent('voltre:taxi:requestEnded', req.citizenSrc, id)
...
RegisterNetEvent('voltre:taxi:driverPing', function(coords)
    local src = source
    local id  = AssignedByDriver[src]
    if not id or not Requests[id] then return end
    local req = Requests[id]
    if type(coords) ~= "table" then return end
    TriggerClientEvent('voltre:taxi:driverPos', req.citizenSrc, {
...
-- ============================================================
RegisterNetEvent('voltre:taxi:setService', function(onOff)
    local src = source
    ...
    if not onOff then
        ...
        if dReq and Requests[dReq] then
            ...
            TriggerClientEvent('voltre:taxi:requestEnded', req.citizenSrc, dReq)
            Requests[dReq] = nil
            AssignedByCitizen[req.citizenSrc] = nil
...
RegisterNetEvent('voltre:taxi:beginRide', function()
    local src = source
    local id  = AssignedByDriver[src]
    if not id or not Requests[id] then return end
    local req = Requests[id]
    ...
    TriggerClientEvent('voltre:taxi:rideStarted', src,           { id = id, citizen = req.citizenName })
    TriggerClientEvent('voltre:taxi:rideStarted', req.citizenSrc, { id = id, driverName = req.driverNameCached })
    -- Premier tick pour poser le HUD
    TriggerClientEvent('voltre:taxi:rideUpdate', src,             { meters = 0, price = req.ride.price })
    TriggerClientEvent('voltre:taxi:rideUpdate', req.citizenSrc,  { meters = 0, price = req.ride.price })
end)

RegisterNetEvent('voltre:taxi:rideTick', function(meters)
    local src = source
    local id  = AssignedByDriver[src]
    if not id or not Requests[id] then return end
    local req = Requests[id]
    ...
    TriggerClientEvent('voltre:taxi:rideUpdate', src,             { meters = meters, price = req.ride.price })
    TriggerClientEvent('voltre:taxi:rideUpdate', req.citizenSrc,  { meters = meters, price = req.ride.price })
end)

ESX.RegisterServerCallback('voltre:taxi:endRide', function(source, cb)
    local src = source
    local id  = AssignedByDriver[src]
    if not id or not Requests[id] then return cb(false) end
    local req = Requests[id]
    ...
    -- Notifs HUD "end"
    TriggerClientEvent('voltre:taxi:rideEnded', src,             { price = price, paid = paid })
    TriggerClientEvent('voltre:taxi:rideEnded', req.citizenSrc,  { price = price, paid = paid })
...
AddEventHandler('playerDropped', function()
    ...
    if cReq and Requests[cReq] then
        ...
        if req.takenBy then
            AssignedByDriver[req.takenBy] = nil
            TriggerClientEvent('voltre:taxi:requestCanceled', req.takenBy, cReq)
        end
        Requests[cReq] = nil
