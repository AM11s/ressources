...
function TaxiClient.RefreshProfile(cb)
    ESX.TriggerServerCallback('voltre:taxi:getProfile', function(profile)
        ...
        while serviceMenuOpen do
            RageUI.IsVisible(serviceMenu, function()
                RageUI.Checkbox("Prendre mon service", "Vous rend disponible pour les appels taxi des joueurs.", serviceStatus, {}, {
                    onChecked = function()
                        serviceStatus = true
                        TriggerServerEvent("voltre:taxi:setService", true)
                    ...
                    onUnChecked = function()
                        serviceStatus = false
                        TriggerServerEvent("voltre:taxi:setService", false)
                        if ESX and ESX.ShowNotification then
                ...
                RageUI.Button("Facturer un client proche", "Envoie une facture au joueur le plus proche.", {}, true, {
                    onSelected = function()
                        ...
                        if closest ~= -1 and dist <= 3.0 then
                            local amount = voltre.fct.input("Montant de la facture")
...
function TaxiClient.CancelMission()
    ...
    TriggerServerEvent('voltre:taxi:cancelMission')
...
function TaxiClient.StartMission(missionKey)
    ...
    ESX.TriggerServerCallback('voltre:taxi:startMission', function(ok, missionData)
        ...
        Citizen.CreateThread(function()
            ...
            ESX.TriggerServerCallback('voltre:taxi:finishMission', function(ok, result)
    ...
    CreateThread(function()
        while pingThread do
            ...
            if TaxiClient.accepted then
                ...
                TriggerServerEvent('voltre:taxi:driverPing', { x = c.x, y = c.y, z = c.z })
            else
                pingThread = false
                return
            end
...
RegisterNUICallback('taxi:listRequests', function(_, cb)
    ESX.TriggerServerCallback('voltre:taxi:listRequests', function(list)
        cb({ requests = list or {} })
    end)
end)
...
RegisterNUICallback('taxi:acceptRequest', function(data, cb)
    ...
    ESX.TriggerServerCallback('voltre:taxi:acceptRequest', function(ok, payload)
...
RegisterNetEvent('voltre:taxi:requestCanceled', function(reqId)
...
RegisterNetEvent('voltre:taxi:requestEnded', function(reqId)
    if TaxiClient.accepted and TaxiClient.accepted.id == reqId then
        TaxiClient.accepted = nil
    end
end)
...
-- ------------------------------------------------------------
-- HUD bridge — passe par voltre-taxi-app (phone iframe)
-- Les messages sont routés via TriggerEvent local pour que
-- voltre-taxi-app/client.lua les intercepte et les envoie
-- dans son propre NUI (l'iframe lb-phone), jamais en fullscreen.
-- ------------------------------------------------------------
local function openHud(role, payload)
    TriggerEvent('voltre:taxi:rideHUDMsg', 'taxiRide:start', {
        role       = role,
...
local function updateHud(meters, price)
    TriggerEvent('voltre:taxi:rideHUDMsg', 'taxiRide:update', {
...
local function closeHud(summary)
    TriggerEvent('voltre:taxi:rideHUDMsg', 'taxiRide:end', summary or {})
end
        ...
        if acc and not ride.active then
            ...
            if veh and veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
                ...
                if passengerSeat then
                    ...
                    TriggerServerEvent('voltre:taxi:beginRide')
                end
            end
        end
    end
    ...
    while true do
        ...
        if ride.active and ride.asDriver then
            ...
            if not anyPassenger then
                ...
                ESX.TriggerServerCallback('voltre:taxi:endRide', function(ok, result)
                    if ok and result then
                ...
                TriggerServerEvent('voltre:taxi:rideTick', math.floor(ride.meters))
            end
        end
...
RegisterNetEvent('voltre:taxi:rideStarted', function(payload)
    payload = payload or {}
    local ride = TaxiClient.ride
...
RegisterNetEvent('voltre:taxi:rideUpdate', function(payload)
    payload = payload or {}
    updateHud(payload.meters or 0, payload.price or 0)
end)

RegisterNetEvent('voltre:taxi:rideEnded', function(payload)
    payload = payload or {}
...
Citizen.CreateThread(function()
    ...
    voltre.data.markers.register("taxi_garage", {
        Position = CFG.Positions.Garage,
        Public   = false,
        Job      = "taxi",
        Action   = function() OpenGarageTaxi() end,
    })
    voltre.data.markers.register("taxi_ranger", {
