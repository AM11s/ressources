if Config.Gouvernement == nil then Config.Gouvernement = {} print("[^1Voltre^7] Vous n'avez pas mit a jour le gouv shared") end


CreateThread(function()
    while not voltre.data.markers.loaded do Wait(1000) end
    while Config.Gouvernement == nil do Wait(1000) end
    voltre.data.markers.register("job_gouv_boss", {
    ...
    voltre.data.markers.register("job_gouv_vestiaire", {
    ...
    voltre.data.markers.register("job_gouv_garage", {
    ...
    voltre.data.markers.register("job_gouv_garage_ranger", {
                ...
                if servicegouv then
                        ...
                        if pedArmour > 0 then
                            ...
                            RageUI.Button("Mettre un kevlar", nil,{RightLabel = ""}, true,{
                                onSelected = function()
                                    TriggerEvent('Voltre:skinchanger:getSkin', function(skin)
                                        if skin.sex == 0 then
                                            TriggerEvent('Voltre:skinchanger:loadClothes', skin, Config.Gouvernement.Kevlar.clothes["male"])
                                            SetPedArmour(PlayerPedId(), 100)
                                            pedArmour = 100
                                        else
                                            TriggerEvent('Voltre:skinchanger:loadClothes', skin, Config.Gouvernement.Kevlar.clothes["female"])
                                            SetPedArmour(PlayerPedId(), 100)
                                            pedArmour = 100
                                        end
                                    end)
                    ...
                    if hasEquipement then
                        RageUI.Button("Déposer son équipement", nil,{}, true,{
                            onSelected = function()
                                TriggerServerEvent("Voltre:gouv:vestaiaire:déposer")
                                hasEquipement = false
                            end
                        })
                    else
                        RageUI.Button("Equiper votre équipement", nil,{}, true,{
                            onSelected = function()
                                TriggerServerEvent("Voltre:gouv:vestaiaire:equipement")
                                hasEquipement = true
                            end
                        })
                    end
                    ...
                    RageUI.Button(v.label, nil, {}, true, {
                        onSelected = function()
                            if not ESX.Game.IsSpawnPointClear(vector3(v.spawnPoint.x, v.spawnPoint.y, v.spawnPoint.z), 10.0) then
                                ...
                                TriggerServerEvent('Voltre:garage:addTempKey', newPlate)
