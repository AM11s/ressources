	...
	MySQL.Async.fetchAll('SELECT * FROM activity', {}, function(Activity)
        for i=1, #Activity, 1 do
			...
			if Activity[i].illegal == 0 then 
				--TriggerClientEvent("Voltre:AddClientBlipFarm", -1, vector3(json.decode(Activity[i].recolte).x, json.decode(Activity[i].recolte).y, json.decode(Activity[i].recolte).z), spritblip, ESX.Config("serverColor").."Activité Farming ~s~ | Récolte "..Activity[i].name)
				--TriggerClientEvent("Voltre:AddClientBlipFarm", -1, vector3(json.decode(Activity[i].traitement).x, json.decode(Activity[i].traitement).y, json.decode(Activity[i].traitement).z), spritblip, ESX.Config("serverColor").."Activité Farming ~s~ | Traitement "..Activity[i].name)
				--TriggerClientEvent("Voltre:AddClientBlipFarm", -1, vector3(json.decode(Activity[i].vente).x, json.decode(Activity[i].vente).y, json.decode(Activity[i].vente).z), spritblip, ESX.Config("serverColor").."Activité Farming ~s~ | Vente "..Activity[i].name)
			end
		end

		TriggerEvent("voltre:core:recevieload:ActivityCount", #Activity)
		--Wait(10000)
		--voltre.DebugPrint('[^4LOAD^0] [^4'..#Activity..'^0] Activités ont été load avec succès')
    end)
end
				...
				if ListeItemTraitement[itemTraitement] == itemTraitement then
					if xPlayer.getInventoryItem(ItemTraitement).count <= 0 then
						...
						if BoostOrVip == 2 then
							...
							voltre.DebugPrint("==============VENTE FARMING (Avec Boosts et Vip)================")
							voltre.DebugPrint("Prix initial : "..ListeItemTraitement[ItemTraitement].price)
							voltre.DebugPrint("Prix avec Boost : "..(ListeItemTraitement[ItemTraitement].price*BoostBonus.sell))
							voltre.DebugPrint("Prix avec Boost + VIP : "..(ListeItemTraitement[ItemTraitement].price*BoostBonus.sell) + ListeItemTraitement[ItemTraitement].price)
							voltre.DebugPrint("=========================================================")
							if illegal ~= 1 then
						...
						elseif BoostOrVip == 1 then
							voltre.DebugPrint("==============VENTE FARMING (Avec Boosts/Vip)================")
							voltre.DebugPrint("Prix initial : "..ListeItemTraitement[ItemTraitement].price)
							voltre.DebugPrint("Prix avec Boost/Vip : "..(ListeItemTraitement[ItemTraitement].price*BoostBonus.sell))
							voltre.DebugPrint("=========================================================")
							xPlayer.removeInventoryItem(ItemTraitement, 1)
							if illegal ~= 1 then
...
RegisterCommand('createfarming', function(source,args)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
		TriggerClientEvent('Voltre:openFarmBuilder', source)
	end
end)
