...
RegisterNetEvent("voltre:restaurant:recevieData", function(result)
    voltre.fct.waitPlayerLoaded()
    voltre.data.jobs.restaurants.list = result
    voltre.data.jobs.restaurants.loaded = true
    Wait(1000)
    ...
    for k,v in pairs(voltre.data.jobs.restaurants.list) do
        ...
        if not voltre.data.markers.isRegister("job_restaurant_vestiaire_"..v.name) then
            voltre.data.markers.register("job_restaurant_vestiaire_"..v.name, {
        ...
        if not voltre.data.markers.isRegister("job_restaurant_boss_"..v.name) then
            voltre.data.markers.register("job_restaurant_boss_"..v.name, {
                Position = vector3(v.PosBoss.x, v.PosBoss.y, v.PosBoss.z),
                Public = false,
                Job = v.name,
                Job2 = nil,
        ...
        if not voltre.data.markers.isRegister("job_restaurant_craft_"..v.name) then
            local craftTableId = "restaurant_" .. v.name
            voltre.data.markers.register("job_restaurant_craft_"..v.name, {
...
function OpenRestaurantOrderShop(jobname)
    ESX.TriggerServerCallback('voltre:restaurant:getShopItems', function(data)
        if not data then
        ...
        if exports['voltre-core'] and exports['voltre-core'].OpenShopUI then
            exports['voltre-core']:OpenShopUI(storeConfig, { orderMode = true, orderContext = { restaurantName = jobname } })
        else
...
RegisterNetEvent('voltre:restaurant:orderReceived', function(info)
    local msg = ("🛎 Nouvelle commande (%s) — $%s"):format(info.customerName or "Client", info.total or "?")
    if info.logo or info.color then
        TriggerEvent("voltre:notificationAdvanced", msg, info.restaurantLabel or info.restaurantName, "Commande", info.color, info.logo)
    else
...
RegisterNetEvent('voltre:restaurant:orderReady', function(info)
    ...
    if info.logo or info.color then
        TriggerEvent("voltre:notificationAdvanced", msg, info.restaurantLabel or info.restaurantName, "Commande Prête", info.color, info.logo)
    else
        ESX.ShowNotification(msg)
    end
end)
...
Citizen.CreateThread(function()
    Wait(1000)
    TriggerServerEvent("voltre:restaurant:getData")
end)
            ...
            RageUI.Button('Emettre une facture', nil, {}, true, {
                onSelected = function() 
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        local string = voltre.fct.input('Montant de la facture')
                        if string ~= "" then
