...
local function IsRestaurantWorker()
    ...
    if voltre.data.jobs then
        if voltre.data.jobs.restaurants then
            if voltre.data.jobs.restaurants.list then
                for _, v in pairs(voltre.data.jobs.restaurants.list) do
...
local function OpenSupplierTablet()
    ...
    TriggerServerEvent('voltre:restaurant:requestSupplierData')
end

RegisterNetEvent('voltre:restaurant:receiveSupplierData')
AddEventHandler('voltre:restaurant:receiveSupplierData', function(data)
    if not data then
...
Citizen.CreateThread(function()
    ...
    local attempts = 0
    while (not voltre.data.jobs.restaurants.loaded or #voltre.data.jobs.restaurants.list == 0) and attempts < 50 do
        Wait(100)
        attempts = attempts + 1
    end
...
RegisterNetEvent('voltre:restaurant:recevieData')
AddEventHandler('voltre:restaurant:recevieData', function(result)
    if IsRestaurantWorker() and not Exist3DInteraction('restaurant_supplier') then
        RegisterSupplierInteraction()
    end
end)

RegisterNetEvent('voltre:restaurant:ordersUpdate')
AddEventHandler('voltre:restaurant:ordersUpdate', function(orders)
    if isSupplierTabletOpen then
        SendNUIMessage({ action = 'supplierTablet:updateOrders', data = orders })
    end
end)

RegisterNetEvent('voltre:restaurant:orderResult')
AddEventHandler('voltre:restaurant:orderResult', function(result)
...
RegisterNUICallback('supplierTablet:order', function(data, cb)
    TriggerServerEvent('voltre:restaurant:placeOrder', data.items or {})
    cb('ok')
end)

RegisterNetEvent('voltre:restaurant:spawnPalette', function(orderId, jobName, items)
...
RegisterNetEvent('voltre:restaurant:despawnPalette', function(orderId)
...
function OpenPaletteStash(orderId)
    ESX.TriggerServerCallback('voltre:restaurant:getPaletteInventory', function(inventory, coffreid)
...
AddEventHandler("esx:inventory:withdraw", function(data)
    ...
    ESX.TriggerServerCallback('voltre:restaurant:withdrawPaletteItem', function(success, message)
        if success then
            ESX.ShowNotification("~g~ Article récupéré")
            ESX.TriggerServerCallback('voltre:restaurant:getPaletteInventory', function(inventory)
                if inventory and inventory.items and #inventory.items > 0 then
                    local rightInventory = exports["voltre-core"]:FormatInventoryForUI(inventory)
