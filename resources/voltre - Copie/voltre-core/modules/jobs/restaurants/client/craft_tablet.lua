...
function OpenCraftTabletRestaurant()
    ...
    ESX.TriggerServerCallback('voltre:restaurant:getCraftData', function(data)
        if not data then
...
RegisterNUICallback('craftTablet:refreshData', function(data, cb)
    ESX.TriggerServerCallback('voltre:restaurant:getCraftData', function(craftData)
        if craftData then
            SendNUIMessage({
...
RegisterNUICallback('craftTablet:craft', function(data, cb)
    ESX.TriggerServerCallback('voltre:restaurant:startCraft', function(result)
        if result and result.started then
            ...
            -- Show the voltre-core progressbar
...
RegisterNetEvent('voltre:restaurant:craftDone', function(success, message)
    StopCraftAnimation()
    HideProgressBar()
...
RegisterNUICallback('craftTablet:getOrders', function(_, cb)
    ESX.TriggerServerCallback('voltre:restaurant:getOrders', function(orders)
...
RegisterNUICallback('craftTablet:claimOrder', function(data, cb)
    ESX.TriggerServerCallback('voltre:restaurant:claimOrder', function(ok, msg)
        if not ok then
...
RegisterNUICallback('craftTablet:markReady', function(data, cb)
    ESX.TriggerServerCallback('voltre:restaurant:markReady', function(ok, msg)
        if ok then
...
RegisterNUICallback('craftTablet:handOverOrder', function(data, cb)
    ESX.TriggerServerCallback('voltre:restaurant:handOverOrder', function(ok, msg)
        if ok then
...
RegisterNUICallback('craftTablet:cancelOrder', function(data, cb)
    ESX.TriggerServerCallback('voltre:restaurant:cancelOrder', function(ok)
        refreshCraftTabletData()
        cb({ success = ok })
    end, data.orderId)
end)
...
RegisterNetEvent('voltre:restaurant:orderReceived', function()
    if isCraftTabletOpen then refreshCraftTabletData() end
end)

-- Listen for server NUI messages
RegisterNetEvent('voltre:nui:send', function(msg)
    SendNUIMessage(msg)
end)
