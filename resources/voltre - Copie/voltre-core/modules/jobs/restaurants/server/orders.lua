...
ESX.RegisterServerCallback('voltre:restaurant:getShopItems', function(source, cb, jobname)
...
ESX.RegisterServerCallback('voltre:restaurant:placeOrder', function(source, cb, paymentType, cartArray, jobname)
    ...
    for _, pid in ipairs(players) do
        ...
        if p and p.job and p.job.name == jobname then
            TriggerClientEvent('voltre:restaurant:orderReceived', pid, {
...
ESX.RegisterServerCallback('voltre:restaurant:getOrders', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:restaurant:claimOrder', function(source, cb, orderId)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:restaurant:markReady', function(source, cb, orderId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false, "Joueur introuvable") end
    local jobname = xPlayer.job.name
    local order = findOrder(jobname, orderId)
    ...
    if customer then
        ...
        TriggerClientEvent('voltre:restaurant:orderReady', customer.source, {
            restaurantName = jobname,
            restaurantLabel = (GetRestaurantBrand(jobname) and GetRestaurantBrand(jobname).label) or jobname,
            orderId = orderId,
            logo = GetRestaurantBrand(jobname) and GetRestaurantBrand(jobname).logo,
...
ESX.RegisterServerCallback('voltre:restaurant:handOverOrder', function(source, cb, orderId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false, "Joueur introuvable") end
    local jobname = xPlayer.job.name
...
ESX.RegisterServerCallback('voltre:restaurant:cancelOrder', function(source, cb, orderId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false) end
    local jobname = xPlayer.job.name
    local order, idx = findOrder(jobname, orderId)
