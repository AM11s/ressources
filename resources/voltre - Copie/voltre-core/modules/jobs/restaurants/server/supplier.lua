...
function ProcessOrder(src, xPlayer, jobName, orderItems)
    if not orderItems or #orderItems == 0 then
        TriggerClientEvent('voltre:restaurant:orderResult', src, { success = false, message = "Commande vide" })
        return
    end
    
    local rawMaterials = _G.GetRawMaterialsForJob(jobName)
    if not rawMaterials then
        TriggerClientEvent('voltre:restaurant:orderResult', src, { success = false, message = "Aucun matériau disponible" })
        return
    end
    ...
    if #validItems == 0 then
        TriggerClientEvent('voltre:restaurant:orderResult', src, { success = false, message = "Aucun article valide" })
        return
    end
    ...
    if societyMoney < totalPrice then
        TriggerClientEvent('voltre:restaurant:orderResult', src, { success = false, message = "Fonds insuffisants dans la caisse" })
        return
    end
    ...
    TriggerClientEvent('voltre:restaurant:orderResult', src, { success = true, message = "Commande #" .. orderId .. " passée ! Livraison dans " .. math.floor(deliveryTime / 60) .. " min" })
...
function SendOrdersUpdate(src, jobName)
    ...
    TriggerClientEvent('voltre:restaurant:ordersUpdate', src, orders)
end
...
function StartDeliveryTimer(orderId)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerClientEvent('voltre:restaurant:spawnPalette', -1, orderId, order.jobName, order.items)
...
ESX.RegisterServerCallback('voltre:restaurant:getPaletteInventory', function(source, cb, orderId)
...
ESX.RegisterServerCallback('voltre:restaurant:getPaletteItems', function(source, cb, orderId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(nil) end
...
ESX.RegisterServerCallback('voltre:restaurant:withdrawPaletteItem', function(source, cb, orderId, itemName, count)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if #palette.items == 0 then
        ...
        TriggerClientEvent('voltre:restaurant:despawnPalette', -1, orderId)
...
RegisterNetEvent('voltre:restaurant:requestSupplierData')
AddEventHandler('voltre:restaurant:requestSupplierData', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then
        TriggerClientEvent('voltre:restaurant:receiveSupplierData', src, nil)
        return
    end
    ...
    TriggerClientEvent('voltre:restaurant:receiveSupplierData', src, {
...
ESX.RegisterServerCallback('voltre:restaurant:getSupplierData', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then
        return cb(nil)
    end
...
RegisterNetEvent('voltre:restaurant:placeOrder')
AddEventHandler('voltre:restaurant:placeOrder', function(orderItems)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    if not restKey then
        TriggerClientEvent('voltre:restaurant:orderResult', src, { success = false, message = "Vous ne travaillez pas dans un restaurant" })
        return
    end
    ...
    if Restaurant.Supplier.BossOnly then
        MySQL.Async.fetchAll('SELECT MAX(grade) as maxGrade FROM job_grades WHERE job_name = @job', {
            ...
            if not isBoss then
                TriggerClientEvent('voltre:restaurant:orderResult', src, { success = false, message = "Seul le patron peut passer des commandes" })
                return
            end
