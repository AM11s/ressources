...
ESX.RegisterServerCallback('voltre:restaurant:getCraftData', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:restaurant:startCraft', function(source, cb, recipeId)
    local xPlayer = ESX.GetPlayerFromId(source)
        ...
        while CraftingSessions[src] do
            ...
            if elapsed >= totalMs then
                ...
                if canCraft and xPlayer.canCarryItem(recipe.item, recipe.count) then
                    ...
                    TriggerClientEvent('voltre:restaurant:craftDone', src, true, recipe.count .. "x " .. recipe.label .. " crafté !")
                else
                    TriggerClientEvent('voltre:restaurant:craftDone', src, false, "Craft échoué - vérifiez votre inventaire")
                end
                break
            end
        end
