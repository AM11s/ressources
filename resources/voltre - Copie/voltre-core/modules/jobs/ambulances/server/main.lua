...
RegisterNetEvent('voltre:createambulancesociety', function(namejob, labeljob, PosVestiaire, PosBoss, color)
    ...
    if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
        ...
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Directeur", "boss", 3)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Médecin-Chef", "chief", 2)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Médecin", "doctor", 1)
        voltre.fct.sql.CheckJobGradeAndCreate(namejob, "Ambulancier", "ambulance", 0)
        ...
        InitSociety2()
        TriggerClientEvent('voltre:ambulance:recevie', -1, SaveData.json["entreprises"]["Ambulance"])
    else
        ExecuteCommand("ban " .. src .. " Tentative de triche creation mécano (0)")
    end
end)

RegisterNetEvent('Voltre:DeleteAmbulance', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
...
RegisterNetEvent('voltre:initAmbulance', function()
    local src = source
    Wait(2000)
	TriggerClientEvent('voltre:ambulance:recevie', src, SaveData.json["entreprises"]["Ambulance"])
end)
        ...
        if id ~= nil and id ~= "Inconnu" and tonumber(id) and tonumber(id) ~= 0 then
            ...
            if xTueur ~= nil then
                ...
                if VoltreIncrementKillStat then
                    VoltreIncrementKillStat(xTueur.idunique, xPlayer.idunique)
                end
            end
        end

        TriggerClientEvent("voltre:playerDied", -1, {
...
RegisterNetEvent('Voltre:RetreiveIsDead', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    MySQL.Async.fetchAll('SELECT * FROM `visdead` WHERE `license` = @license', {
        ...
        if result[1] then
            ...
            TriggerClientEvent('Voltre:PlayerIsDead', xPlayer.source)
...
local AppelsEMSList = {}
local CountAppel = 0
RegisterNetEvent('Voltre:CreateEmsSignal', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if AppelsEMSList[xPlayer.source] then
        ...
        voltre.DebugPrint("Create EMS Signal : "..xPlayer.source)
        ...
        for k,v in pairs(PlayersInJobs) do 
            ...
            if (xPlayers) then
                ...
                voltre.DebugPrint("Notif EMS for signal ("..k..")")
                TriggerClientEvent('Voltre:UpdateTableSignalEms', k, AppelsEMSList)
            end
        end
    end
end)
...
RegisterNetEvent('Voltre:RespawnHopital', function(Freekill, SID, UID)
    local src = source
        ...
        if args[1] and tonumber(args[1]) ~= nil then
            ...
            for k,v in pairs(GetPlayers()) do
                ...
                if coordsSpecial <= tonumber(args[1]) then
					...
					voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a /revive (/reviver "..args[1].."m) le joueur "..xPlayers.getName().." (U"..xPlayers.getIdunique()..")", "revive", {idunique = xPlayer.getIdunique(), idunique_cible = xPlayers.getIdunique(), name = xPlayer.getName(), name_cible = xPlayers.getName()})
                end
            end
        end
    end
        ...
        if xPlayer.getGroup() ~= 'user' then
            ...
            if args[1] ~= nil and args[1] ~= 0 then
                ...
                if PlayerRevive then 
                    ...
                    voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a /revive le joueur "..PlayerRevive.getName().." (U"..PlayerRevive.getIdunique()..")", "revive", {idunique = xPlayer.getIdunique(), idunique_cible = PlayerRevive.getIdunique(), name = xPlayer.getName(), name_cible = PlayerRevive.getName()})
                ...
                voltre.logs.send("Logs Staff",""..xPlayer.getName().." (U"..xPlayer.getIdunique()..") a /revive lui meme", "revive", {idunique = xPlayer.getIdunique(), idunique_cible = xPlayer.getIdunique(), name = xPlayer.getName(), name_cible = xPlayer.getName()})
...
RegisterNetEvent("Voltre:UseItemsEMS", function(name)
    local xPlayer = ESX.GetPlayerFromId(source)
...
--[[ESX.RegisterUsableItem('medikit', function(source)
    ...
    TriggerClientEvent("Voltre:UseItemsEMS", source, "medikit")
...
ESX.RegisterUsableItem('bandage', function(source)
    ...
    TriggerClientEvent("Voltre:UseItemsEMS", source, "bandage")
