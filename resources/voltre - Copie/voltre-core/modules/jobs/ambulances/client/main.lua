...
Citizen.CreateThread(function()
    Wait(3000)
    TriggerServerEvent("voltre:initAmbulance")
end)

RegisterNetEvent("voltre:ambulance:recevie", function(table)
    while voltre.data.markers.loaded ~= true do Wait(1000) end
    voltre.data.jobs.ambulances.list = table
    voltre.data.jobs.ambulances.loaded = true
    Wait(1000)
    ...
    for k,v in pairs(EntrepriseAmbulanceZone) do
        voltre.data.markers.unregister(k)
    end
    EntrepriseAmbulanceZone = {}
    ...
    for k,v in pairs(table) do
        if v.type == 'Ambulance' then 
            ...
            if not voltre.data.markers.isRegister("job_ambulance_vestiaire_"..v.name) then
                EntrepriseAmbulanceZone["job_ambulance_vestiaire_"..v.name] = true
                voltre.data.markers.register("job_ambulance_vestiaire_"..v.name, {
            ...
            --[[if not voltre.data.markers.isRegister("job_ambulance_helico_"..v.name) then
                EntrepriseAmbulanceZone["job_ambulance_helico_"..v.name] = true
                voltre.data.markers.register("job_ambulance_helico_"..v.name, {
            ...
            if not voltre.data.markers.isRegister("job_ambulance_boss_"..v.name) then
                EntrepriseAmbulanceZone["job_ambulance_boss_"..v.name] = true
                voltre.data.markers.register("job_ambulance_boss_"..v.name, {
                    Position = vector3(v.PosBoss.x, v.PosBoss.y, v.PosBoss.z),
                    Public = false,
                    Job = v.name,
                    Job2 = nil,
            ...
            RageUI.Button('Mettre une Cane', nil, { RightBadge = nil }, true, {
                onSelected = function()
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        TriggerServerEvent('EMS:RevivePlayer', GetPlayerServerId(closestPlayer))
                        Time = tonumber(voltre.fct.input("Temps a mettre en Cane (5-30 Minutes) ?"))
