...
Citizen.CreateThread(function()
    ...
    MySQL.Async.execute([[
        ...
        ADD COLUMN IF NOT EXISTS `missions` LONGTEXT DEFAULT NULL,
        ADD COLUMN IF NOT EXISTS `missions_reset_daily` VARCHAR(20) DEFAULT NULL,
        ADD COLUMN IF NOT EXISTS `missions_reset_weekly` VARCHAR(20) DEFAULT NULL,
        ADD COLUMN IF NOT EXISTS `total_territories_won` INT NOT NULL DEFAULT 0,
        ADD COLUMN IF NOT EXISTS `gangcolor` VARCHAR(10) DEFAULT '#e74c3c',
        ADD COLUMN IF NOT EXISTS `tier` INT NOT NULL DEFAULT 1,
        ADD COLUMN IF NOT EXISTS `posFabrication` LONGTEXT DEFAULT NULL
    ]], {}, function()
...
function AddGangXP(gangname, amount)
    ...
    if leveledUp then
        for _, v in pairs(SaveData.gangs[gangname].StatePly) do
            if v.id then
                TriggerClientEvent('voltre:tablet:levelUp', v.id, currentLevel)
            end
        end
    end

    TriggerClientEvent('voltre:SendGangListToClient', -1, SaveData.gangs)
end
...
ESX.RegisterServerCallback('voltre:tablet:getData', function(source, cb)
...
-- Change gang color
RegisterNetEvent('voltre:tablet:changeColor')
AddEventHandler('voltre:tablet:changeColor', function(color)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
...
RegisterNetEvent('voltre:tablet:buyWeapon')
AddEventHandler('voltre:tablet:buyWeapon', function(weaponId, quantity)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if SaveData.gangs[gangname] == nil then return end
    local gang = SaveData.gangs[gangname]
...
RegisterNetEvent('voltre:tablet:buyItem')
AddEventHandler('voltre:tablet:buyItem', function(itemId, quantity)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if SaveData.gangs[gangname] == nil then return end
    local gang = SaveData.gangs[gangname]
...
ESX.RegisterServerCallback('voltre:tablet:validateGroup', function(source, cb, groupName, groupLabel)
...
RegisterNetEvent('voltre:tablet:createGroup')
AddEventHandler('voltre:tablet:createGroup', function(groupName, groupLabel)
    local src = source
    ...
    TriggerClientEvent('voltre:tablet:groupCreated', src)
end)

-- Accept a mission
RegisterNetEvent('voltre:tablet:acceptMission')
AddEventHandler('voltre:tablet:acceptMission', function(missionId, missionType)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if SaveData.gangs[gangname] == nil then return end
...
RegisterNetEvent('voltre:tablet:claimMission')
AddEventHandler('voltre:tablet:claimMission', function(missionId, missionType)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if SaveData.gangs[gangname] == nil then return end
...
RegisterNetEvent('voltre:tablet:addGrade')
AddEventHandler('voltre:tablet:addGrade', function(label)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if xPlayer.getJob2().grade_name ~= "boss" then return end
    if SaveData.gangs[gangname] == nil then return end
...
RegisterNetEvent('voltre:tablet:removeGrade')
AddEventHandler('voltre:tablet:removeGrade', function(gradeName, gradePos)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if xPlayer.getJob2().grade_name ~= "boss" then return end
...
RegisterNetEvent('voltre:tablet:changePerms')
AddEventHandler('voltre:tablet:changePerms', function(permType, gradeKey, value)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if xPlayer.getJob2().grade_name ~= "boss" then return end
    if SaveData.gangs[gangname] == nil then return end
...
RegisterNetEvent('voltre:tablet:changeMemberGrade')
AddEventHandler('voltre:tablet:changeMemberGrade', function(targetIdunique, newGrade)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if SaveData.gangs[gangname] == nil then return end
    ...
    local player = exports["voltre-core"]:getPlayerWithUniqueID(targetIdunique) or {id = 0}
...
RegisterNetEvent('voltre:tablet:kickMember')
AddEventHandler('voltre:tablet:kickMember', function(targetIdunique)
    local xPlayer = ESX.GetPlayerFromId(source)
    local gangname = xPlayer.getJob2().name
    if SaveData.gangs[gangname] == nil then return end
...
ESX.RegisterServerCallback('voltre:staff:getGangTabletData', function(source, cb, gangname)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('voltre:staff:setGangLevel')
AddEventHandler('voltre:staff:setGangLevel', function(gangname, newLevel)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('voltre:staff:addGangXP')
AddEventHandler('voltre:staff:addGangXP', function(gangname, amount)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer.getPermission("GESTION_BUILDER") then return end
    if SaveData.gangs[gangname] == nil then return end
...
RegisterServerEvent('voltre:staff:resetGangMissions')
AddEventHandler('voltre:staff:resetGangMissions', function(gangname)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer.getPermission("GESTION_BUILDER") then return end
    if SaveData.gangs[gangname] == nil then return end
...
RegisterServerEvent('voltre:staff:setGangStat')
AddEventHandler('voltre:staff:setGangStat', function(gangname, statKey, value)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer.getPermission("GESTION_BUILDER") then return end
...
RegisterServerEvent('voltre:staff:resetGangXP')
AddEventHandler('voltre:staff:resetGangXP', function(gangname)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer.getPermission("GESTION_BUILDER") then return end
    if SaveData.gangs[gangname] == nil then return end
