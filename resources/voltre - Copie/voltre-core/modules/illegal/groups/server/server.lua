...
ESX.RegisterServerCallback("voltre:groupe:getStats", function(source, cb, name)
...
RegisterNetEvent('voltre:createGang', function(namegang, labelgang, positonCoffre, KitArme, FabArme, zone, haveStarterpack, type, gangTier, posFabrication)
...
Citizen.CreateThread(function()
    ...
    MySQL.Async.fetchAll('SELECT * FROM vgangs', {}, function(Gangs)
		...
		syncGangCraftTables()
		TriggerEvent("voltre:core:recevieload:GangCount", gangbuildercounter)
		--Wait(10000)
		--print('[^4LOAD^0] [^4'..gangbuildercounter..'^0] Gang ont été load avec succès')
    end)
end)

RegisterNetEvent("voltre:illegal:gangsbuilder:addgrade")
AddEventHandler("voltre:illegal:gangsbuilder:addgrade", function(job, label)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("voltre:illegal:gangsbuilder:removegrade")
AddEventHandler("voltre:illegal:gangsbuilder:removegrade", function(job, name, pos)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getJob2().name ~= job then return end
	if xPlayer.getJob2().grade_name ~= "boss" then return end
...
RegisterNetEvent("voltre:changeperms")
AddEventHandler("voltre:changeperms", function(type, new, job)
    local _src = source
...
RegisterNetEvent('voltre:initGangs', function()
    TriggerClientEvent('voltre:SendGangListToClient', source, SaveData.gangs)
end)
...
RegisterNetEvent("voltre:changerank")
AddEventHandler("voltre:changerank", function(gangs,idunique, job, grade, bool)
    local _src = source
    local xPlayer = ESX.GetPlayerFromId(_src)
	local player = exports["voltre-core"]:getPlayerWithUniqueID(idunique) or {id = 0}
...
ESX.RegisterServerCallback('voltre:GetPlayerData', function(source, cb, target)
    local xPlayer = ESX.GetPlayerFromId(target)
    if xPlayer then
        local data = {
...
ESX.RegisterServerCallback("Voltre:GetMemberOfGangs", function(source, cb, type, gangname)
	local xPlayer = ESX.GetPlayerFromId(source)
	if type == "staff" then
...
RegisterNetEvent('Voltre:wipeallGangMember', function(gangname)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:UpdateGangs', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:UpdateGangsPoint', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
		if SaveData.gangs[value.name] then
			SaveData.gangs[value.name].nbrpoint = value.NBRPoint
...
RegisterNetEvent('Voltre:GivePointGangs', function(Gang, PointNbr)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:DeleteGangs', function(value)
	local xPlayer = ESX.GetPlayerFromId(source)
	if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil then
...
AddEventHandler('playerDropped', function(reason)
	...
	if SaveData.gangs[gangname] ~= nil then
		...
		voltreopti = voltreopti + 1
	end
end)

RegisterNetEvent('Voltre:GangInfoSetJob2')
AddEventHandler('Voltre:GangInfoSetJob2', function(source)
    local xPlayer = ESX.GetPlayerFromId(source)
	local gangname = xPlayer.getJob2().name
	if SaveData.gangs[gangname] ~= nil then
...
Citizen.CreateThread(function()
    while true do 
		...
		if voltreopti >= Config.OptimisationSys then
			TriggerClientEvent("voltre:SendGangListToClient", -1, SaveData.gangs)
