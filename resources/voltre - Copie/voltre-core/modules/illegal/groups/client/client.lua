...
RegisterNetEvent('voltre:SendGangListToClient', function(table)
    voltre.data.illegals.groups.list = table
    voltre.data.illegals.groups.loaded = true
end) 

exports("getGroupIllegalInfo", function()
    return voltre.data.illegals.groups.list, voltre.data.illegals.groups.loaded
end)

Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
    voltre.fct.waitPlayerLoaded()
    while not voltre.data.illegals.groups.loaded do 
        Wait(1)
    end

    for k,v in pairs(voltre.data.illegals.groups.list) do
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(voltre.data.illegals.groups.list) do
        ...
        if v.posCoffre and v.posCoffre.x and v.posCoffre.y and v.posCoffre.z
            and not voltre.data.markers.isRegister("gang_coffre_"..v.name) then
            voltre.data.markers.register("gang_coffre_"..v.name, {
                Position = vector3(v.posCoffre.x, v.posCoffre.y, v.posCoffre.z),
                Public = false,
                Job = nil,
                Job2 = v.name,
                Action = function()
                    local gangData = voltre.data.illegals.groups.list[ESX.PlayerData.job2.name]
        ...
        if (v.FabArme == 1 or v.FabArme == true) and v.posFabrication then
            if not voltre.data.markers.isRegister("gang_fabrication_"..v.name) then
                local craftTableId = "gang_craft_" .. v.name
                voltre.data.markers.register("gang_fabrication_"..v.name, {
...
function OpenCoffreGang(pos)
	...
	while menu do
        if VoltreInventory.isOpen then  
            return
        end
		Citizen.Wait(0)
        RageUI.IsVisible(menu, function()
            ...
            --     }, GestionGrade)
            -- end
            -- if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_promouvoir[tostring(ESX.PlayerData.job2.grade)] or voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_recruter[tostring(ESX.PlayerData.job2.grade)] then
            ...
            --[[RageUI.Button('Facture', nil, {}, true, {
                onSelected = function() 
                    ...
                    if closestPlayer == -1 or closestDistance > 3.0 then
                        ...
                        local string = voltre.fct.input('Montant', false, 9000000, "number")
                        if string ~= "" then
            ...
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_vente[tostring(ESX.PlayerData.job2.grade)] then
                if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].KitArme == 1 or voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].KitArme == true then 
            ...
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_coffre[tostring(ESX.PlayerData.job2.grade)] then
            ...
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_fabrication[tostring(ESX.PlayerData.job2.grade)] then
                if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].FabArme == 1 or voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].FabArme == true then 
            ...
            -- if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_gestionmembre[tostring(ESX.PlayerData.job2.grade)] then
            --     RageUI.Button("Liste des employés", nil, {Voltre = {id = "boss-plylistgang", LoadingOnSelect = 400, description = "Chargement de la liste des membres."}}, true, {
            --         onSelected = function()
            --             ESX.TriggerServerCallback("Voltre:GetMemberOfGangs", function(result) 
            --                 voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].PlyList = result
        ...
        RageUI.IsVisible(GestionGrade1, function()
            RageUI.Button("Ajouter un grade", nil, {Color = { BackgroundColor = {255,244,79, 150} }}, true, {
                onSelected = function()
                    LabelGrade = voltre.fct.input("Nom du Grade")
                    if LabelGrade == nil then return end
                    TriggerServerEvent("voltre:illegal:gangsbuilder:addgrade", ESX.PlayerData.job2.name, LabelGrade)
                end
            })
            RageUI.Line()
            for k,v in pairs(voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].GradeList) do
                if v.name == "boss" then
                    ...
                    RageUI.Button(v.label.."", "Appuyez sur [ENTRER] pour supprimer le grade", {}, true, {
                        onSelected = function() 
                            TriggerServerEvent("voltre:illegal:gangsbuilder:removegrade", ESX.PlayerData.job2.name, v.name, k)
                        end
                    })
                end
            end
        ...
        RageUI.IsVisible(OpenBuyWeaponMenu, function()
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].KitArme then
                for k,v in pairs(Config.IllegalGroups.Sell.Weapons) do 
                    RageUI.Button("Arme: "..v.label.." (x999)", "L'arme sera distribuer dans votre coffre", {RightLabel = v.price..'$'}, true, {
                        onSelected = function() 
                            local quantity = tonumber(voltre.fct.input("Combien en voulez-vous ? (stock: 999)"))
        ...
        RageUI.IsVisible(OpenFabWeaponMenu, function()
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].FabArme then
        ...
        RageUI.IsVisible(GestionGradePerm, function()
            for k,v in pairs(voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].GradeList) do
                if v.name == "boss" then
                    ...
                    if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name][PermSelected][k] ~= nil then
                        RageUI.Checkbox(v.label, nil, voltre.data.illegals.groups.list[ESX.PlayerData.job2.name][PermSelected][k], {}, {
                            onSelected = function(Index)
                                voltre.data.illegals.groups.list[ESX.PlayerData.job2.name][PermSelected][k] = Index
                            end
                        })
                    else
                        voltre.data.illegals.groups.list[ESX.PlayerData.job2.name][PermSelected][k] = false
                    end
                end
            end
            RageUI.Button("Confirmer les modifications", nil, { Color = { BackgroundColor = {255,244,79, 200} } }, true, {
                onSelected = function() 
                    TriggerServerEvent("voltre:changeperms", PermSelected, voltre.data.illegals.groups.list[ESX.PlayerData.job2.name][PermSelected], ESX.PlayerData.job2.name)
                    RageUI.GoBack()
                end
            })
        end)
        ...
        RageUI.IsVisible(GestionRecrutement, function()
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_recruter[tostring(ESX.PlayerData.job2.grade)] then
                RageUI.Button("Recruté le plus proche", nil, {RightLabel = ""}, true, {
                    ...
                    onSelected = function()
                        local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            TriggerServerEvent("Voltre:personalmenu:Boss_recruterplayer2", GetPlayerServerId(closestPlayer), ESX.PlayerData.job2.name)
                        else
                ...
                RageUI.Button("Virer le plus proche", nil, {RightLabel = ""}, true, {
                    ...
                    onSelected = function()
                        local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            TriggerServerEvent("Voltre:personalmenu:Boss_virerplayer2", GetPlayerServerId(closestPlayer))
                        else
                            ESX.ShowNotification('Aucun joueur à proximité',"error")
                        end
                    end
            ...
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].perms_promouvoir[tostring(ESX.PlayerData.job2.grade)] then
                RageUI.Button("Promouvoir le plus proche", nil, {RightLabel = ""}, true, {
                    ...
                    onSelected = function()
                        local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            TriggerServerEvent("Voltre:personalmenu:Boss_promouvoirplayer2", GetPlayerServerId(closestPlayer))
                        else
                            ESX.ShowNotification('Aucun joueur à proximité',"error")
                        end
                    end
                ...
                RageUI.Button("Rétrograder le plus proche", nil, {RightLabel = ""}, true, {
                    ...
                    onSelected = function()
                        local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                        if closestDistance ~= -1 and closestDistance <= 3 then
                            TriggerServerEvent("Voltre:personalmenu:Boss_destituerplayer2", GetPlayerServerId(closestPlayer))
                        else
                            ESX.ShowNotification('Aucun joueur à proximité',"error")
                        end
                    end
        ...
        RageUI.IsVisible(MembreList, function()
            for k,v in pairs(voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].PlyList) do 
                if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].GradeList[tostring(v.job2_grade)] ~= nil then
                    RageUI.Button(v.firstname..' '..v.lastname.." ("..v.idunique..")", nil, {RightLabel = voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].GradeList[tostring(v.job2_grade)].label}, ESX.PlayerData.idunique ~= v.idunique and true or false, {
                        onSelected = function() 
                            SelectIdUnique = v
                        end
                    }, GestionMembre)
        ...
        RageUI.IsVisible(GestionMembre, function()
            RageUI.Button("Exclure", nil, {}, true, {
                onSelected = function()
                    TriggerServerEvent("voltre:changerank", voltre.data.illegals.groups.list[ESX.PlayerData.job2.name], SelectIdUnique.idunique, "unemployed", 0, true)
                    voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].PlyList[SelectIdUnique.idunique] = nil
                ...
                RageUI.Button(v.label, nil, {RightLabel = v.grade}, v.name ~= "boss" and true or false, {
                    onSelected = function() 
                        if v.name == "boss" then
                            ...
                            voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].PlyList[SelectIdUnique.idunique].job2_grade = v.grade
                            TriggerServerEvent("voltre:changerank", voltre.data.illegals.groups.list[ESX.PlayerData.job2.name],  SelectIdUnique.idunique ,ESX.PlayerData.job2.name, v.grade, false)
                ...
                for k,v in pairs(data) do
                    if v.count ~= 0 then
                        RageUI.Button(v.label, nil, {RightLabel = ESX.Config("serverColor")..'Quantités : '..v.count}, true, {
                            onSelected = function()
                                quantity = voltre.fct.input('Combien voulez vous déposer ?')
...
function OpenPutItemsGang()
    ...
    ESX.TriggerServerCallback('voltre:GetPlayerData', function(result)
        data = result
        name = result.name
        money = result.money
	...
	while inventaireplayer do
        ...
        RageUI.IsVisible(inventaireplayer, function()
                ...
                onSelected = function()
                    ...
                    if distance ~= -1 and distance <= 3.0 then
                        ...
                        if ESX.isHandsUp(targetPed) then  
                            RageUI.CloseAll()
                            ESX.TriggerServerCallback('voltre:fouiller', function(data, id)
                                if data then
                                    local inventory = data
                                    inventory.weight = 0
            ...
            --[[RageUI.Button('Classement Point', nil, {}, true, {
                onSelected = function() 
                    TriggerServerEvent("voltre:initGangs")
                    print(#voltre.data.illegals.groups.list)
                    for i = 1, #voltre.data.illegals.groups.list do
                        table.sort(voltre.data.illegals.groups.list[i])
                    end
                end
            }, pointlist)]]
        end)
        RageUI.IsVisible(pointlist, function()
            if voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].Point ~= nil then
                RageUI.Separator("Votre nombre de point : "..ESX.Config("serverColor")..voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].Point)
            else
        ...
        while GangMenu do
            ...
            RageUI.IsVisible(GangsBuilderMenu, function()
                RageUI.Button('Nom du Gang (setjob2)', "Aucune majuscule ni espace !", {RightLabel = namegang}, true, {
                    onSelected = function()
                        namegang = voltre.fct.input('Quelle nom veux tu mettre ?')
                end})
                RageUI.Button('Label du Gang', nil, {RightLabel = labelgang}, true, {
                    onSelected = function()
                        labelgang = voltre.fct.input('Quelle nom veux tu mettre ?')
                end})
                ...
                RageUI.Button('Confirmer', nil, { Color = { BackgroundColor = {255,244,79, 120} } }, true, {
                    onSelected = function()
                        TriggerServerEvent('voltre:createGang', namegang, labelgang, posCoffre, KitArme, KitArmeFab, nil, nil, nil, nil, posFabCreation)
                        RageUI.GoBack()
                        KitArme = false
                        KitArmeFab = false
                        Point = 0
                ...
                if SelectedGang.name ~= nil then
                    ...
                    RageUI.Button('Confirmer', nil, { Color = { BackgroundColor = {255,244,79, 120} } }, true, {
                        onSelected = function()
                            ...
                            TriggerServerEvent('voltre:UpdateGangs', updateData)
                            ...
                            if NbrPointGangSave == NbrPointGang then
                                ...
                                voltre.data.illegals.groups.list[ESX.PlayerData.job2.name].Point = NbrPointGang
                ...
                for k,v in pairs(voltre.data.illegals.groups.list) do
                    RageUI.Button(v.name, nil, {}, true, {
                        onSelected = function()
                            local result = voltre.fct.confirm("Êtes-vous sur ?")
                            if result ~= true then return end
                            TriggerServerEvent('voltre:DeleteGangs', v.name)
                    end})
                end
            end)
            ...
            RageUI.IsVisible(OpenInfoGang, function()
                for k,v in pairs(voltre.data.illegals.groups.list) do
                    if v.State then
                        RageUI.Button(v.name, nil, {RightLabel = '~g~En-Ligne~s~ ('..#voltre.data.illegals.groups.list[v.name].StatePly..')'}, true, {
                            onSelected = function()
                                SelectedGang = v
                            end
                        }, OpenInfoGang2)
            ...
            RageUI.IsVisible(OpenInfoGang2, function()
                if SelectedGang.name ~= nil then
                    ...
                    for k,v in pairs(voltre.data.illegals.groups.list[SelectedGang.name].StatePly) do
