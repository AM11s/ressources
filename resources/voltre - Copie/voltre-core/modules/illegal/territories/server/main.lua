...
function getTerritoryLevelBonus(gangname)
	...
	local level = 1
	local ok, result = pcall(function() return exports["voltre-core"]:GetGangLevel(gangname) end)
...
Citizen.CreateThread(function()
	Wait(1000)
	drugsss = exports["voltre-core"]:getDrugs()
	while drugsss == nil do 
		Wait(10)
	end

	for k,v in pairs(exports["voltre-core"]:getDrugs()) do
...
function updateTerritoiresOwner(id, ownerLabel, ownerCount, owner)
    ...
    TriggerClientEvent('Voltre:RecevieTerritories', -1, SaveData.json["territories"])

	exports["voltre-core"]:ProgressGangMission(owner, "daily_territory_capture", 1)
	exports["voltre-core"]:ProgressGangMission(owner, "weekly_territory_captures", 1)
	exports["voltre-core"]:IncrementGangTerritoriesWon(owner)
	exports["voltre-core"]:AddGangXP(owner, 50)
end
...
RegisterNetEvent('Voltre:initTerritories', function()
    TriggerClientEvent('Voltre:RecevieTerritories', source, SaveData.json["territories"])
end)
RegisterNetEvent('Voltre:resetTerritoires', function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('Voltre:resetAllTerritoires', function()
    local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() ~= "user" then
		if Config.GroupeHighPerm[xPlayer.getGroup()] ~= nil and Config.GroupeHighPerm[xPlayer.getGroup()] == true then
...
RegisterNetEvent('voltre:territories:addShop', function(data)
    if data ~= nil then
...
RegisterNetEvent('voltre:territories:create', function(data)
    if data ~= nil then
		if data.name == nil then return end
...
RegisterNetEvent('voltre:territories:delete', function(id)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:territories:buyShop', function(territoriesid, itemid, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:territories:deleteShopItem', function(id)
...
local function alertLSPDDrugSale(src, coords)
	...
	for _, playerId in ipairs(ESX.GetPlayers()) do
		...
		if xTarget and xTarget.job and string.lower(xTarget.job.name or "") == "lspd" then
			...
			TriggerClientEvent('voltre:police:blips', playerId, id, "Vente de drogue", vector3(coords.x, coords.y, coords.z), 51, 1, "drugsale")
		end
	end
	...
	SetTimeout(120000, function()
		for _, playerId in ipairs(ESX.GetPlayers()) do
			local xTarget = ESX.GetPlayerFromId(playerId)
			if xTarget and xTarget.job and string.lower(xTarget.job.name or "") == "lspd" then
				TriggerClientEvent('voltre:police:removeallblipswithtag', playerId, "drugsale")
			end
		end
	end)
end
...
local function getSellableDrug(xPlayer)
	...
	for _, drugData in pairs(exports["voltre-core"]:getDrugs()) do
...
RegisterNetEvent('Voltre:territories:enterZone', function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not find then 
		voltre.DebugPrint("not drugs find")
	...
	if vv == nil then
		voltre.DebugPrint("territories not found")
		playerProcessing[source] = nil
		return 
	end
	if vv.minCops > getCops() then
		voltre.DebugPrint("not enough cops")
		playerProcessing[source] = nil
		return
	end
	if vv.maxSell >= Config.Drugs.maxSells then
		voltre.DebugPrint("max sell")
		playerProcessing[source] = nil
		return
	end
	...
	TriggerClientEvent("voltre:territories:addPoint", source,generateRandomPositionInZone(vv.points, source),vv.PNJ.pos.z-1,vv.id, confianceMult)
end)

RegisterNetEvent('Voltre:territories:retryPoint', function(id)
	local src = source
	if not playerProcessing[src] then
		voltre.DebugPrint("retryPoint for player "..src.." but not processing, ignoring")
		return
	end
	...
	TriggerClientEvent("voltre:territories:addPoint", src, generateRandomPositionInZone(vv.points, src), vv.PNJ.pos.z-1, vv.id, confianceMultRetry)
end)

RegisterNetEvent('Voltre:territories:exitZone', function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
	local vv = {}
	for k,v in pairs(SaveData.json["territories"]) do
		if v.id == id then
	...
	playerProcessing[source] = nil
	TriggerClientEvent("voltre:territories:clearPoint", source)
end)
...
local function sellDrugsToNpc(src, territoryId, pedNetId, coords, shouldContinueTerritoryLoop)
	...
	if not selectDrugs then
		...
		TriggerClientEvent("voltre:territories:clearPoint", src)
		if pedNetId then
			TriggerClientEvent("voltre:territories:npcSellResult", src, pedNetId, "nodrugs")
		end
		return
	end
	...
	if pedNetId and npcSellCooldown[src] and npcSellCooldown[src] > os.time() then
		...
		TriggerClientEvent("voltre:territories:npcSellResult", src, pedNetId, "cooldown")
		return
	end
	...
	if pedNetId and math.random(1, 3) == 1 then
		...
		TriggerClientEvent("voltre:territories:npcSellResult", src, pedNetId, "refused")
		return
	end
	...
	if inTerritory then
		...
		if vv.minCops > nbrCops then
			...
			if pedNetId then
				TriggerClientEvent("voltre:territories:npcSellResult", src, pedNetId, "blocked")
			end
			return
		end
	...
	if inTerritory and gangname ~= "unemployed" and SaveData.gangs[gangname] ~= nil then
		...
		if isDefending then
			exports["voltre-core"]:ProgressGangMission(gangname, "daily_territory_defend", 1)
		end
	end
	...
	local BoostOrVip, BoostId, BoostTime, BoostBonus = voltre.fct.BoostAndVip(xPlayer.identifier, xPlayer.source)
	...
	if pedNetId then
		TriggerClientEvent("voltre:territories:npcSellResult", src, pedNetId, "success")
	end
	...
	if gangname ~= "unemployed" then
		...
		if inTerritory then
			exports["voltre-core"]:ProgressGangMission(gangname, "daily_territory_points", soldCount)
		end
		exports["voltre-core"]:ProgressGangMission(gangname, "daily_sell_drugs", soldCount)
		exports["voltre-core"]:ProgressGangMission(gangname, "weekly_sell_drugs_mass", soldCount)
		exports["voltre-core"]:ProgressGangMission(gangname, "daily_dirty_money", payment)
		exports["voltre-core"]:ProgressGangMission(gangname, "weekly_dirty_money_mass", payment)
		exports["voltre-core"]:AddGangXP(gangname, 10 * soldCount)
	end
	...
	if shouldContinueTerritoryLoop and inTerritory then
		...
		TriggerClientEvent("voltre:territories:addPoint", src, generateRandomPositionInZone(vv.points, src), vv.PNJ.pos.z - 1, vv.id, confianceMultSell)
	else
		playerProcessing[src] = nil
	end
end
...
local RobbedPed = {}
RegisterNetEvent("voltre:territories:startSelledPedReply", function(ped)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent("voltre:territories:takeMoney", function(ped, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.getJob2().name ~= "unemployed" then
		...
		exports["voltre-core"]:ProgressGangMission(gangname, "daily_dirty_money", random)
		exports["voltre-core"]:ProgressGangMission(gangname, "weekly_dirty_money_mass", random)
	end
end)
