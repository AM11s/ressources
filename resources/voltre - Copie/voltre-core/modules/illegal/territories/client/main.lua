...
Citizen.CreateThread(function()
    TriggerServerEvent('Voltre:initTerritories')
end)
local createdPeds = {}

RegisterNetEvent('Voltre:RecevieTerritories', function(data)
    territories = data
		...
		polyzone:onPlayerInOut(function(isPointInside, point)
            if not insideZone then
                if isPointInside then
                    ...
                    currentZone = v
                    currentZoneSave = v
                    TriggerServerEvent("Voltre:territories:enterZone", v.id)
                end
            else
                if currentZone ~= nil and not isPointInside then
                    insideZone = false
                    timetocheck = 2500
                    TriggerServerEvent("Voltre:territories:exitZone", currentZone.id)
                    currentZone = nil
                end
            end
		end)
			...
			RageUI.IsVisible(main, function()
                for _, v in pairs(shop) do
                    ...
                    RageUI.Button(("%s"):format(v.label), nil, { RightLabel = ("~r~%s~s~ $"):format(ESX.Math.Round(v.price)) }, true, {
                        onSelected = function ()
                            -- local type = voltre.fct.input("Avec quoi voulez-vous payer", true, {
                            ...
                            TriggerServerEvent("voltre:territories:buyShop", id, v.id, type)
                        end
                    })
                end
			end)
...
function StartSelledPedReply(ped)
    TriggerServerEvent("voltre:territories:startSelledPedReply", ped)
        ...
        while true do
            ...
            if pedDropSac then
                ...
                if distance < 2.0 then
                    voltre.fct.draw.Text3DBar(bagCoords.x, bagCoords.y, bagCoords.z, "[~y~E~s~] Récupérer le sac.")
                    if IsControlJustReleased(0, 51) then
                        local dict, anim = "random@domestic", "pickup_low"
                        ESX.Streaming.RequestAnimDict(dict)
                        TaskPlayAnim(PlayerPedId(), dict, anim, -1.0, -1.0, 3000, 0, 0, true, true, true)
                        Wait(1200)
                        TriggerServerEvent("voltre:territories:takeMoney", ped, "kill")
                        isSelling = false
                        ...
                        Citizen.CreateThread(function()
                            Wait(4000)
                            TriggerServerEvent("Voltre:territories:enterZone", currentZone.id)
                        end)
                        break
                    end
                end
...
RegisterNetEvent('voltre:territories:addPoint')
AddEventHandler('voltre:territories:addPoint', function(point,z, PointID, confianceMult)
    if currentZone == nil then 
        voltre.DebugPrint("currentZone == nil")
        return 
    end
    if isSelling or isProcessingPoint then
        voltre.DebugPrint("already selling or processing, ignoring addPoint")
        return
    end
    ...
    if (z - GetEntityCoords(PlayerState.ped).z) > 4.0 then
        TriggerServerEvent("Voltre:territories:retryPoint", currentZone.id)
        voltre.DebugPrint("point to high, respawn")
        isProcessingPoint = false
        return
    end
    ...
    local spawnCoords = voltre.fct.GenerateRandomCoordAroundPoint(coords, spawnRadius)
        ...
        while isSelling do
            ...
            if (IsPlayerFreeAiming(PlayerId()) or IsPedInMeleeCombat(PlayerPedId())) and RobSelledPed == nil then
                ...
                if isPedWantReply <= Config.Territories.ChanceToPedReply or IsPedInMeleeCombat(PlayerPedId()) or GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE') then
                    ...
                    TriggerServerEvent("voltre:territories:startSelledPedReply", CreatedSellerPed[id].ped)
                end
            end
            if RobSelledPed == "rob" then
                local distance = #(pedCoords - playerCoords)
                if distance < 1.0 then
                    voltre.fct.draw.Text3DBar(pedCoords.x, pedCoords.y, pedCoords.z, "[~y~E~s~] Récupérer son argent.")
                    if IsControlJustReleased(0, 51) then
                        RobSelledPed = nil
                        ...
                        TriggerServerEvent("voltre:territories:takeMoney", CreatedSellerPed[id].ped, "rob")
                        isSelling = false
                        isProcessingPoint = false
                        Citizen.CreateThread(function()
                            Wait(2000)
            ...
            if #(pedCoords - playerCoords) < 1.5 and RobSelledPed == nil then
                voltre.fct.draw.Text3DBar(pedCoords.x, pedCoords.y, pedCoords.z, "[~y~E~s~] Lui vendre de la drogue.")
...
RegisterNetEvent("voltre:territories:clearPoint", function ()
    isSelling = false
    isProcessingPoint = false
    DeleteSellerPed()
    FreezeEntityPosition(PlayerPedId(), false)
