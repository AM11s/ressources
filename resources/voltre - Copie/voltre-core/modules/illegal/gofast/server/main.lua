...
MySQL.Async.execute([[
    ALTER TABLE `gofast_reputation`
        ADD COLUMN IF NOT EXISTS `stolen_cargo_value` INT NOT NULL DEFAULT 0,
        ADD COLUMN IF NOT EXISTS `was_crew_betrayal` TINYINT(1) NOT NULL DEFAULT 0;
]], {})
...
function GoFast.ProgressGang(xPlayer, payment)
    ...
    for _, missionId in ipairs(cfg.gangMissions) do
        pcall(function() exports["voltre-core"]:ProgressGangMission(gangname, missionId, 1) end)
    end
    pcall(function() exports["voltre-core"]:ProgressGangMission(gangname, "daily_dirty_money", payment) end)
    pcall(function() exports["voltre-core"]:ProgressGangMission(gangname, "weekly_dirty_money_mass", payment) end)
    pcall(function() exports["voltre-core"]:AddGangXP(gangname, cfg.gangXP) end)
end
...
RegisterServerEvent("Voltre:gofast:requestMission")
AddEventHandler("Voltre:gofast:requestMission", function(missionType)
    local src = source
    ...
    GoFast.GetReputation(identifier, function(rep)
        ...
        if rep.blacklisted == 1 then
            TriggerClientEvent("Voltre:gofast:missionResponse", src, { error = "BLACKLISTED" })
            return
        end
        ...
        if GoFast.cooldowns[identifier] and now - GoFast.cooldowns[identifier] < Config.GoFast.Reputation.soloCooldown then
            ...
            TriggerClientEvent("Voltre:gofast:missionResponse", src, { error = "COOLDOWN", remaining = remaining })
            return
        end

        -- Check if already in a mission
        if GoFast.activeMissions[identifier] then
            TriggerClientEvent("Voltre:gofast:missionResponse", src, { error = "ALREADY_ACTIVE" })
            return
        end
        ...
        if isCrew then
            ...
            if gangname == "unemployed" or gangname == "unemployed2" then
                TriggerClientEvent("Voltre:gofast:missionResponse", src, { error = "NO_GANG" })
                return
            end
            ...
            if onlineCount < Config.GoFast.Reputation.crewMinPlayers then
                TriggerClientEvent("Voltre:gofast:missionResponse", src, {
...
RegisterServerEvent("Voltre:gofast:start")
AddEventHandler("Voltre:gofast:start", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
RegisterServerEvent("Voltre:gofast:complete")
AddEventHandler("Voltre:gofast:complete", function(engineHealth, plate)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    GoFast.VerifyCargo(missionPlate, mission.cargo, function(cargoComplete, ratio)
        ...
        if not cargoComplete then
            ...
            TriggerClientEvent("Voltre:gofast:result", src, {
                success = false,
                cargoMissing = true,
                cargoRatio = ratio,
...
RegisterServerEvent("Voltre:gofast:fail")
AddEventHandler("Voltre:gofast:fail", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
-- Player failed / abandoned the mission
RegisterServerEvent("voltre:gofast:pickupkey")
AddEventHandler("voltre:gofast:pickupkey", function(plate)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
RegisterServerEvent("voltre:gofast:deliveryped:takecargo")
AddEventHandler("voltre:gofast:deliveryped:takecargo", function(plate)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    if hasStolenCargo then
        ...
        for _, item in ipairs(stolenItems) do
            ...
            voltre.DebugPrint("[GoFast] Removed stolen cargo from player: " .. item.name .. " x" .. item.count)
        end
        ...
        voltre.DebugPrint("[GoFast] Player " .. identifier .. " stashed cargo - triggering wipe")
...
-- ============================================================================
ESX.RegisterServerCallback("Voltre:gofast:getCrewMembers", function(src, cb)
    local xPlayer = ESX.GetPlayerFromId(src)
...
-- ============================================================================
RegisterServerEvent("Voltre:gofast:requestCrewMission")
AddEventHandler("Voltre:gofast:requestCrewMission", function(selectedMemberIds)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    GoFast.GetReputation(identifier, function(rep)
        ...
        if not selectedMemberIds or type(selectedMemberIds) ~= "table" then
            TriggerClientEvent("Voltre:gofast:missionResponse", src, { error = "NOT_ENOUGH_CREW", required = Config.GoFast.Reputation.crewMinPlayers, online = 1 })
            return
        end
        ...
        for _, member in ipairs(validMembers) do
            TriggerClientEvent("Voltre:gofast:crewMissionNotify", member.serverId, {
...
function GoFast.HandleBetrayal(identifier, src)
    ...
    if src and GetPlayerName(src) then
        TriggerClientEvent("Voltre:gofast:betrayed", src)
    end
    ...
    if mission and mission.crewMembers then
        for _, member in ipairs(mission.crewMembers) do
            ...
            if member.serverId and GetPlayerName(member.serverId) then
                TriggerClientEvent("Voltre:gofast:betrayed", member.serverId)
            end
            voltre.DebugPrint("[GoFast] Crew member " .. member.identifier .. " blacklisté (trahison crew)")
        end
    end
    ...
    voltre.DebugPrint("[GoFast] Joueur " .. identifier .. " blacklisté pour trahison")
end

-- Ambush trigger (called from CrimeNet app when blacklisted player tries to start a mission)
RegisterServerEvent("Voltre:gofast:triggerAmbush")
AddEventHandler("Voltre:gofast:triggerAmbush", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    GoFast.GetReputation(xPlayer.identifier, function(rep)
        if rep.blacklisted == 1 then
            ...
            TriggerClientEvent("Voltre:gofast:ambush", src, {
...
RegisterServerEvent("Voltre:gofast:dangerZoneCheck")
AddEventHandler("Voltre:gofast:dangerZoneCheck", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    GoFast.GetReputation(xPlayer.identifier, function(rep)
        if rep.blacklisted == 1 or rep.blacklisted == true then
            ...
            voltre.DebugPrint("[GoFast] Drive-by check — cargo: $" .. stolenValue .. " — chance: " .. chance .. " — roll: " .. nbrRandom)
            ...
            if nbrRandom < chance then
                if tier then
                    ...
                    TriggerClientEvent("Voltre:gofast:driveBy", src, {
...
-- ============================================================================
ESX.RegisterServerCallback("Voltre:gofast:getProfileData", function(src, cb)
    local xPlayer = ESX.GetPlayerFromId(src)
...
function GoFast.SendProfileRefresh(src)
    ...
    GoFast.GetReputation(identifier, function(rep)
        ...
        TriggerClientEvent("Voltre:gofast:profileRefresh", src, {
            isIllegal = isIllegal,
            gangname = gangname,
            xp = rep.xp,
            tier = { name = tier.name, label = tier.label, index = tierIndex, description = tier.description },
...
voltre.DebugPrint("[GoFast V2] Module serveur chargé")