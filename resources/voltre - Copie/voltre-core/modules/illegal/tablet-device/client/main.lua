...
function IDevice.Open()
    ...
    sendCfg()
    ESX.TriggerServerCallback('voltre:illegalDevice:gofast:status', function(status)
...
RegisterNetEvent('voltre:illegalDevice:open')
AddEventHandler('voltre:illegalDevice:open', IDevice.Open)
...
RegisterNUICallback('illegalDevice:gofast:request', function(data, cb)
    IDevice.Close()
    TriggerServerEvent('voltre:illegalDevice:gofast:request', data.difficulty)
    cb({ ok = true })
end)
