        ...
        while M do
            ...
            if M.taken then
                if #(GetEntityCoords(veh) - M.route.delivery) < 12.0 then
                    TriggerServerEvent('voltre:missions:fourgon:complete', M.gang)
...
local function SpawnMission(data)
    ...
    CreateThread(function()
        ...
        AddRelationshipGroup("VOLTRE_FOURGON_GUARDS")
        local guardGroup  = GetHashKey("VOLTRE_FOURGON_GUARDS")
...
RegisterNetEvent('voltre:missions:fourgon:init')
AddEventHandler('voltre:missions:fourgon:init', function(data)
    SpawnMission(data)
end)
