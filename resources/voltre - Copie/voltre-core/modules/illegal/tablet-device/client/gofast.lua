    ...
    CreateThread(function()
        while SGF.active and SGF.phase == 'delivery' do
            ...
            if SGF.vehicle and DoesEntityExist(SGF.vehicle) and IsEntityDead(SGF.vehicle) then
                TriggerServerEvent('voltre:illegalDevice:gofast:fail', 'VEHICLE_DESTROYED')
                cleanup()
                return
            end
            if d < 25.0 then
                ...
                if d < 6.0 and not inVeh then
                    ...
                    if IsControlJustPressed(0, 51) then
                        TriggerServerEvent('voltre:illegalDevice:gofast:complete')
...
RegisterNetEvent('voltre:illegalDevice:gofast:response')
AddEventHandler('voltre:illegalDevice:gofast:response', function(data)
    if data.error then
...
RegisterNetEvent('voltre:illegalDevice:gofast:result')
AddEventHandler('voltre:illegalDevice:gofast:result', function(data)
