-- ============================================================================
-- ILLEGAL TABLET DEVICE - Crew bridge (reuses existing voltre:tablet:* backend)
-- ============================================================================
...
RegisterNUICallback('illegalDevice:crew:recruitNearest', function(_, cb)
    nearestActionAndClose('Voltre:personalmenu:Boss_recruterplayer2', true)
    cb({ ok = true })
end)

RegisterNUICallback('illegalDevice:crew:fireNearest', function(_, cb)
    nearestActionAndClose('Voltre:personalmenu:Boss_virerplayer2', false)
    cb({ ok = true })
end)

RegisterNUICallback('illegalDevice:crew:promoteNearest', function(_, cb)
    nearestActionAndClose('Voltre:personalmenu:Boss_promouvoirplayer2', false)
    cb({ ok = true })
end)

RegisterNUICallback('illegalDevice:crew:demoteNearest', function(_, cb)
    nearestActionAndClose('Voltre:personalmenu:Boss_destituerplayer2', false)
    cb({ ok = true })
end)

RegisterNUICallback('illegalDevice:crew:startMission', function(_, cb)
    TriggerServerEvent('voltre:missions:fourgon:start')
    cb({ ok = true })
end)
