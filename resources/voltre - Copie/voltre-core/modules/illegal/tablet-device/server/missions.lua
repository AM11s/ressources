...
RegisterNetEvent('voltre:missions:fourgon:start')
AddEventHandler('voltre:missions:fourgon:start', function()
    local src     = source
    ...
    TriggerClientEvent('voltre:missions:fourgon:init', src, {
        gangName   = gang,
        routeIndex = math.random(1, FOURGON_ROUTES),
    })
end)

RegisterNetEvent('voltre:missions:fourgon:complete')
AddEventHandler('voltre:missions:fourgon:complete', function(gangName)
    local src     = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
