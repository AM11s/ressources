...
RegisterNetEvent('voltre:illegalDevice:gofast:request')
AddEventHandler('voltre:illegalDevice:gofast:request', function(difficultyId)
    local src = source
    ...
    if not diff then
        TriggerClientEvent('voltre:illegalDevice:gofast:response', src, { error = 'INVALID' })
        return
    end

    if activeMissions[src] then
        TriggerClientEvent('voltre:illegalDevice:gofast:response', src, { error = 'ALREADY_ACTIVE' })
        return
    end
    ...
    if cooldowns[ident] and cooldowns[ident] > now then
        TriggerClientEvent('voltre:illegalDevice:gofast:response', src, {
            error = 'COOLDOWN',
    ...
    if not delivery then
        TriggerClientEvent('voltre:illegalDevice:gofast:response', src, { error = 'NO_ROUTE' })
        return
    end
    ...
    TriggerClientEvent('voltre:illegalDevice:gofast:response', src, { success = true, mission = mission })
end)

RegisterNetEvent('voltre:illegalDevice:gofast:complete')
AddEventHandler('voltre:illegalDevice:gofast:complete', function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    TriggerClientEvent('voltre:illegalDevice:gofast:result', src, {
        success = true,
        payment = mission.payment,
    })
end)

RegisterNetEvent('voltre:illegalDevice:gofast:fail')
AddEventHandler('voltre:illegalDevice:gofast:fail', function(reason)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
    ...
    activeMissions[src] = nil
    TriggerClientEvent('voltre:illegalDevice:gofast:result', src, { success = false, reason = reason })
end)

ESX.RegisterServerCallback('voltre:illegalDevice:gofast:status', function(src, cb)
    local xPlayer = ESX.GetPlayerFromId(src)
