			...
			RageUI.IsVisible(vente, function()
				if isSwat then
					...
					RageUI.Button('Vendre Bijoux', nil, {}, true, {
						onSelected = function() 
							local nbr = voltre.fct.input('Combien souhaitez vous vendre ? ', false, 9000000, "text")
							TriggerServerEvent('lester:vendeur', tonumber(nbr)) 
						end
					})
					RageUI.Button('Contacter un Hacker', nil, {}, true, {
						onSelected = function() 
							ESX.TriggerServerCallback('voltre:lester:phone', function(good)
...
function InitLesterSwat()
	...
	TriggerServerEvent("voltre:lester:swat")
	HaveReport = true
end


RegisterNetEvent('voltre:isswat:client')
AddEventHandler('voltre:isswat:client', function(bool)
	isSwat = bool
	if isSwat then
...
RegisterNetEvent('voltre:swat:client')
AddEventHandler('voltre:swat:client', function()
