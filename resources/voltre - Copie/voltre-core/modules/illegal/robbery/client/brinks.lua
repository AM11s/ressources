...
RegisterNetEvent('voltre:braquage:brinks:PoliceNotify')
AddEventHandler('voltre:braquage:brinks:PoliceNotify', function()
...
RegisterNetEvent('voltre:braquage:brinks:removeblips')
AddEventHandler('voltre:braquage:brinks:removeblips', function()
	local player = ESX.GetPlayerData()
    if voltre.data.jobs.polices.list[ESX.PlayerData.job.name] ~= nil then
...
RegisterNetEvent('voltre:braquage:brinks:openvehicle')
AddEventHandler('voltre:braquage:brinks:openvehicle', function()
...
RegisterNetEvent('voltre:braquage:brinks:initFin')
AddEventHandler('voltre:braquage:brinks:initFin', function()
	voltre.data.markers.register("waitingbrinks", {
		...
		Action = function()
			if PlayerStateisInVehicle then
				if IsVehicleModel(PlayerStatevehicle, "stockade") then
					...
					TriggerServerEvent("voltre:brinks:finish")
					voltre.data.markers.unregister("waitingbrinks")
				else
...
RegisterNetEvent('voltre:braquage:brinks:PoliceBlip')
AddEventHandler('voltre:braquage:brinks:PoliceBlip', function(targetCoords)
	local player = ESX.GetPlayerData()
	if voltre.data.jobs.polices.list[player.job.name] ~= nil then
        ESX.removeBlip("brinks_location")
		ESX.addBlips({
...
RegisterNetEvent('voltre:braquage:brinks:spawn', function()
	...
	ESX.Game.SpawnVehicle("stockade", vec3(934.05261230469, -3.533684015274, 78.763984680176), 144.2540435791, function(vehicle)
		...
		Citizen.CreateThread(function ()
			...
			--TriggerServerEvent("voltre:brinks:everypedisdead")
		end)
		Citizen.CreateThread(function ()
			while true do
				...
				if total <= 200 then
					...
					TriggerServerEvent("voltre:brinks:updatepedalive", nbrAlive)
				end
				Wait(10000)
			end
		end)
		Citizen.CreateThread(function ()
			while BrinksDrive do
				TriggerServerEvent("voltre:brinks:receviecoords", GetEntityCoords(vehicle))
				Wait(5000)
			end
		end)
	end)
