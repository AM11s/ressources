...
RegisterNetEvent('voltre:hacker:recevietalk', function(bool)
    istalk = bool
end)
...
function MenuHacker()
	...
	local close = false
	TriggerServerEvent("voltre:lester:isswat")
	ESX.TriggerServerCallback('voltre:hacker:GetDemande', function(hasDemande)
		hasBraquageDemande = hasDemande
	end)
	ESX.TriggerServerCallback('voltre:lester:getnbrpolice', function(nbr)
		if nbr < Config.Robbery.RequiredPolice then
			ESX.ShowNotification("Je suis pas disponible.")
			RageUI.CloseAll()
		end
	end)
	TriggerServerEvent("voltre:hacker:istalk", true)
	if istalk then 
		ESX.ShowNotification("Quelqu'un parle déjà avec le hacker.")
		return
	 end
	IsTalking = true
	TriggerServerEvent("voltre:hacker:istalk", false, true)
	...
	while vente do
						...
						if reponse[1] == 1 then
							...
							RageUI.List("Votre réponse : ", {"Oui", "Non"}, SaveIndex, "Entrée pour confirmer votre réponse", {}, true, {
								...
								onSelected = function(Index)
									...
									haveReponse[3] = true
									SaveIndex = 1
									ESX.TriggerServerCallback('voltre:hacker:payinfo', function(good)
										if good then
		...
		if not RageUI.Visible(vente) and not RageUI.Visible(braquagelist) then
			...
			if IsTalking then
				TriggerServerEvent("voltre:hacker:istalk", false, false)
			end
...
Citizen.CreateThread(function ()
    while voltre.data.markers.loaded ~= true do Wait(100) end
    voltre.data.markers.register("hackerzone", {
