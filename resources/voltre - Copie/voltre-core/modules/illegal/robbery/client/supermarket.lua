...
RegisterNetEvent('voltre:supermarket-robbery:OnPedDeathRobbery')
AddEventHandler('voltre:supermarket-robbery:OnPedDeathRobbery', function(store)
    SetEntityHealth(Config.SupermarketRobbery.Peds[store], 0)
end)

RegisterNetEvent('voltre:supermarket-robbery:RemovePickupRobbery')
AddEventHandler('voltre:supermarket-robbery:RemovePickupRobbery', function(bank)
...
RegisterNetEvent('voltre:supermarket-robbery:RobberyStartOver')
AddEventHandler('voltre:supermarket-robbery:RobberyStartOver', function()
    Config.SupermarketRobbery.IsRobbing = false
end)

RegisterNetEvent('voltre:supermarket-robbery:RobberyStart')
AddEventHandler('voltre:supermarket-robbery:RobberyStart', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                voltre.fct.draw.Text3DBar(GetEntityCoords(bag).x, GetEntityCoords(bag).y, GetEntityCoords(bag).z, "Appuyez sur ~g~E~s~ pour ~g~ramasser le sac~s~.")
                                if IsControlJustPressed(0, 38) then 
                                    ...
                                    voltre.fct.draw.UpdateTimerBar(Config.SupermarketRobbery.BarUpdate, {percentage = 0})
                                    voltre.fct.draw.RemoveTimerBar()
                                    StopSound(Config.SupermarketRobbery.IsRobbing)
                                    ReleaseScriptAudioBank("Alarms")
                                    TriggerServerEvent('voltre:supermarket-robbery:PickupRobbery', Config.SupermarketRobbery.List[i], i)
                                    SetFakeWantedLevel(0)
                                    break
                                end
                            end
...
RegisterNetEvent('voltre:supermarket-robbery:ResetPedDeath')
AddEventHandler('voltre:supermarket-robbery:ResetPedDeath', function(i)
...
Citizen.CreateThread(function()
  ...
  --            if IsPedDeadOrDying(Config.SupermarketRobbery.Peds[i]) then
  --                TriggerServerEvent('voltre:supermarket-robbery:PedDeadRobbery', i)
  --            end
  --        end
  --        Wait(30000)
  --    end
                    ...
                    if HasEntityClearLosToEntityInFront(player, Config.SupermarketRobbery.Peds[i], 19) and not IsPedDeadOrDying(Config.SupermarketRobbery.Peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(Config.SupermarketRobbery.Peds[i]), true) <= 5.0 then
                        ...
                        if not Config.SupermarketRobbery.IsRobbing then
							...
							if not timerBraquage then
								timerBraquage = true
								ESX.TriggerServerCallback('voltre:supermarket-robbery:CanRobbery', function(cb)
									canRob = cb
                                ...
                                if canRob == true then
                                    ...
                                    if chance >= 0 and chance <= 25 then
                                        ...
                                        Config.SupermarketRobbery.BarUpdate = voltre.fct.draw.AddTimerBar("Menace :",{percentage = 0.0, bg = {100,0,0,255}, fg = {200,0,0,255}})
                                        PlayAmbientSpeechWithVoice(Config.SupermarketRobbery.Peds[i], "SHOP_HURRYING", "MP_M_SHOPKEEP_01_PAKISTANI_MINI_01", "SPEECH_PARAMS_FORCE", 1)
                                        ...
                                        TriggerServerEvent('voltre:supermarket-robbery::CallPolice', Config.SupermarketRobbery.List[i], i)
                                        ...
                                        while scared < 50 and not IsPedDeadOrDying(Config.SupermarketRobbery.Peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(Config.SupermarketRobbery.Peds[i]), true) <= 7.5 do
                                            ...
                                            voltre.fct.draw.UpdateTimerBar(Config.SupermarketRobbery.BarUpdate, {percentage = scared / 50})
                                            scared = scared + 1
                                        end
                                        if GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(Config.SupermarketRobbery.Peds[i]), true) <= 7.5 then
                                            if not IsPedDeadOrDying(Config.SupermarketRobbery.Peds[i]) then
                                                TriggerServerEvent('voltre:supermarket-robbery:RobberyStart', i)
                                                while Config.SupermarketRobbery.IsRobbing do
                                                    Wait(0) 
                                                    if IsPedDeadOrDying(Config.SupermarketRobbery.Peds[i]) then 
                                                        Config.SupermarketRobbery.IsRobbing = false 
                                ...
                                elseif canRob == 'no_cops' then
                                    ...
                                    while wait >= GetGameTimer() do
                                        ...
                                        voltre.fct.draw.Text3DBar(pedcoord.x, pedcoord.y, pedcoord.z, '~r~Pas assez de Policier en ville.')
                                        StopSound(Config.SupermarketRobbery.IsRobbing)
                                        SetFakeWantedLevel(0)
                                    end
                                else
                                    ...
                                    while wait >= GetGameTimer() do
                                        Wait(0)
                                        local pedcoord = GetEntityCoords(Config.SupermarketRobbery.Peds[i], true)
                                        voltre.fct.draw.Text3DBar(pedcoord.x, pedcoord.y, pedcoord.z, '~r~Cette superette a déjà été braqué, le vendeur n\'a plus rien.')
                                        StopSound(Config.SupermarketRobbery.IsRobbing)
                                        SetFakeWantedLevel(0)
                                    end
                                end
...
local BlipSuperette = {}
RegisterNetEvent('voltre:supermarket-robbery::MsgPolice')
AddEventHandler('voltre:supermarket-robbery::MsgPolice', function(store, i)
