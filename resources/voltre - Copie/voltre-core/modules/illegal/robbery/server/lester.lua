...
ESX.RegisterServerCallback('voltre:lester:phone', function(source, cb)
...
ESX.RegisterServerCallback('voltre:lester:getnbrpolice', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("voltre:lester:swat", function ()
	local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["entreprises"]["Police"][xPlayer.getJob().name] == nil then return end
    if LesterisSwat then return end

    print("[^5Voltre^7] Police Swat Lester")
    LesterisSwat = true
    TriggerClientEvent("voltre:swat:client", -1)
end)


RegisterServerEvent("voltre:lester:LesterisSwat", function ()
    TriggerClientEvent("voltre:LesterisSwat:client", source,LesterisSwat)
end)
...
RegisterNetEvent('lester:vendeur', function(nbr)
	...
	elseif bijoux >= nbr then  
		...
		if gangname ~= "unemployed" then
			exports["voltre-core"]:ProgressGangMission(gangname, "daily_dirty_money", price)
			exports["voltre-core"]:ProgressGangMission(gangname, "weekly_dirty_money_mass", price)
			exports["voltre-core"]:AddGangXP(gangname, 20)
		end
	end
end)
