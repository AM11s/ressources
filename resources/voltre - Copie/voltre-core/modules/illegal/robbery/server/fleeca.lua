...
RegisterNetEvent("voltre:fleeca:heistDrill", function(id, drillid)
    ...
    if gangname ~= "unemployed" then
        --exports["voltre-core"]:ProgressGangMission(gangname, "daily_robbery_atm", 1)
        ...
        exports["voltre-core"]:ProgressGangMission(gangname, "daily_dirty_money", money)
        exports["voltre-core"]:ProgressGangMission(gangname, "weekly_dirty_money_mass", money)
        exports["voltre-core"]:AddGangXP(gangname, 100)
    end
end)
...
RegisterServerEvent("voltre:fleeca:braquage:supp")
AddEventHandler("voltre:fleeca:braquage:supp", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer == nil then return end
    if not xPlayer.getPermission("gestion_builder") then return end
