...
RegisterServerEvent("voltre:hacker:istalk", function (get, bool)
	if get then
        TriggerClientEvent("voltre:hacker:recevietalk", source, talk)
    else
        talk = bool
    end
end)

RegisterServerEvent("voltre:brinks:receviecoords", function(coords)
...
RegisterServerEvent("voltre:brinks:everypedisdead", function()
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
	if BrinksSpawn[xPlayer.getIdunique()] == nil then return end

    TriggerClientEvent("voltre:braquage:brinks:openvehicle", -1)
    BrinksSpawn[xPlayer.getIdunique()].nbrPedAlive = 0
    Wait(5000)
    BrinksSpawn[xPlayer.getIdunique()].finish = true
    TriggerClientEvent("voltre:braquage:brinks:removeblips", -1)
    ...
    TriggerClientEvent("voltre:braquage:brinks:initFin", source)
end)

RegisterServerEvent("voltre:brinks:finish", function()
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if gangname ~= "unemployed" then
        exports["voltre-core"]:ProgressGangMission(gangname, "weekly_robbery_bank", 1)
        exports["voltre-core"]:ProgressGangMission(gangname, "weekly_robbery_spree", 1)
        exports["voltre-core"]:ProgressGangMission(gangname, "daily_dirty_money", total)
        exports["voltre-core"]:ProgressGangMission(gangname, "weekly_dirty_money_mass", total)
        exports["voltre-core"]:AddGangXP(gangname, 150)
    end
end)

RegisterServerEvent("voltre:brinks:updatepedalive", function(nbr)
    local xPlayer = ESX.GetPlayerFromId(source)
	if BrinksSpawn[xPlayer.getIdunique()] == nil then return end
...
function FinishBraquageDemande(idu, source)
    ...
    if Config.Phone.votre_telephone["lbphone"] then
        ...
        if savedemande.type == "Brinks" then
            ...
            TriggerClientEvent("voltre:braquage:brinks:spawn", -1)
            Wait(2*60*1000)
            ...
            Wait(5000)
            TriggerClientEvent("voltre:braquage:brinks:PoliceNotify", -1) 
            Citizen.CreateThread(function ()
                while BrinksSpawn[idu] ~= nil and not BrinksSpawn[idu].finish do
                    if BrinksSpawn[idu] == nil then break end
                    TriggerClientEvent("voltre:braquage:brinks:PoliceBlip", -1, BrinksSpawn[idu].position)
                    Wait(30000)
                end
                print("[^5Voltre^7] Braquage Brinks Fini")
            end)
...
function initBraquageWaiting(source)
    Citizen.CreateThread(function ()
        ...
        print("[^5Voltre^7] Demande de braquage effectuer (^5"..tostring(ESX.Math.Round(secondetonotif/60)).."^7 Minutes d'attente)")
        ...
        local xPlayer = ESX.GetPlayerFromId(source)
        if xPlayer == nil then
            print("[^5Voltre^7] Demande de braquage mise en attente (Joueur deconnecter)")
            WaitingPlayerForFinishDemande[xPlayerIdU] = savedemande
        else
            print("[^5Voltre^7] Demande de braquage fini (Temps fini)")
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    if WaitingPlayerForFinishDemande[xPlayer.getIdunique()] ~= nil then
        ...
        Wait(3000)
        print("[^5Voltre^7] Demande de braquage fini (Joueur reconnnecter)")
...
ESX.RegisterServerCallback('voltre:hacker:payinfo', function(source, cb, infotype)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:hacker:GetDemande', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:staff:hacker', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
