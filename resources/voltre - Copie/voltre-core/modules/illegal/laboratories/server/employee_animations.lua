...
local function WalkToCoords(ped, targetCoords, timeout)
    ...
    if not success then
        voltre.DebugPrint("[EMPLOYEE] Failed to reach destination")
    end
...
local function PlayAnimation(ped, animType, duration, extraParam)
    ...
    if animType == "water" then
        TriggerClientEvent("voltre:labo:playEmployeeWaterAnim", -1, pedNetId, duration)
    elseif animType == "fertilizer" then
        TriggerClientEvent("voltre:labo:playEmployeeFertilizerAnim", -1, pedNetId, duration)
    elseif animType == "seed" then
        TriggerClientEvent("voltre:labo:playEmployeeSeedAnim", -1, pedNetId, duration)
    elseif animType == "harvest" then
        TriggerClientEvent("voltre:labo:playEmployeeHarvestAnim", -1, pedNetId, duration)
    elseif animType == "dry_area" then
        TriggerClientEvent("voltre:labo:playEmployeeDryAreaAnim", -1, pedNetId, duration)
    elseif animType == "treat" then
        TriggerClientEvent("voltre:labo:playEmployeeTreatAnim", -1, pedNetId, extraParam or 5)
    elseif animType == "storage" then
        TriggerClientEvent("voltre:labo:playEmployeeStorageAnim", -1, pedNetId, duration)
    elseif animType == "meth_cuve" then
        TriggerClientEvent("voltre:labo:playEmployeeMethCuveAnim", -1, pedNetId, duration)
    elseif animType == "meth_four" then
        TriggerClientEvent("voltre:labo:playEmployeeMethFourAnim", -1, pedNetId, duration)
    elseif animType == "meth_break" then
        TriggerClientEvent("voltre:labo:playEmployeeMethBreakAnim", -1, pedNetId, duration)
    end
    ...
    if success and DoesEntityExist(ped) then
        ...
        if spot.type == "couch" then
            ...
            if spots and spots.couchAnims and spots.couchAnims[spot.spotIndex] then
                ...
                TriggerClientEvent("voltre:labo:playEmployeeIdleAnim", -1, pedNetId, couchAnim.dict, couchAnim.anim, -1)
            else
                -- Fallback sur l'ancien système
                local employeeConfig = Config.laboratoire.employees[employeeId]
                if employeeConfig and employeeConfig.anim then
                    TriggerClientEvent("voltre:labo:playEmployeeIdleAnim", -1, pedNetId, employeeConfig.anim[1], employeeConfig.anim[2], -1)
                end
            end
        else
...
function ServerEmployeeAnimWaterFertilize(labId, employeeId, plantId, animType)
    if not createdEmployees[labId] or not createdEmployees[labId][employeeId] then
        voltre.DebugPrint("[EMPLOYEE] Employee not found: "..employeeId)
        return
    end
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[EMPLOYEE] Ped does not exist")
        return
    end
    ...
    Citizen.CreateThread(function()
        ...
        if not plant then
            voltre.DebugPrint("[EMPLOYEE] Plant not found")
        ...
        local pedNetId = NetworkGetNetworkIdFromEntity(ped)
        TriggerClientEvent("voltre:labo:client:unfreezeEmployee", -1, pedNetId)
...
function ServerEmployeeAnimSeed(labId, employeeId, plantId)
    ...
    Citizen.CreateThread(function()
        ...
        MySQL.update('UPDATE weedplants SET hasBase = @hasBase, time = @time WHERE id = @id', {
            ...
            --voltre.DebugPrint("[SEED] MySQL UPDATE affected rows:", affectedRows, "for plant", plantId)
        end)
        
        TriggerClientEvent("voltre:weed:client:recevieDataPlant", -1, plantId, SaveData.Illegal.WeedPlants[plantId])
...
ServerEmployeeAnimHarvest = function(labId, employeeId, plantId)
    ...
    if not ped or not DoesEntityExist(ped) then
        voltre.DebugPrint("[EMPLOYEE] Ped not found for "..employeeId)
        return false
    end
    ...
    Citizen.CreateThread(function()
        ...
        local plant = SaveData.Illegal.WeedPlants[plantId]
        if not plant then
            voltre.DebugPrint("[EMPLOYEE] Plant not found: "..plantId)
