...
NavigatePedToPosition = LPH_NO_VIRTUALIZE(function(ped, targetPos, timeout)
    ...
    if not startWaypoint then
        --voltre.DebugPrint("[PATHFINDING] Could not find start waypoint")
        return false
    end
    ...
    if not path or #path == 0 then
        --voltre.DebugPrint("[PATHFINDING] No path found")
        return false
    end
    ...
    for i, waypointPos in ipairs(path) do
        ...
        if not waypointReached then
            --voltre.DebugPrint("[PATHFINDING] Failed to reach waypoint, teleporting")
