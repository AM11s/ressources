...
Citizen.CreateThread(function()
    ...
    Wait(13000)
    voltre.DebugPrint("[^1Drugs^7] Génération du marché effecuter :")
    for k,v in pairs(Config.DrugsMarket.market) do 
        if LastMarket[k] and not v.hide then
            voltre.DebugPrint("[^1Drugs^7:^2"..k.."^7] Prix: ^3"..LastMarket[k].DrugsRequestPrice.."$^7")
            voltre.DebugPrint("[^1Drugs^7:^2"..k.."^7] Nombre de Demande: "..LastMarket[k].DrugsRequestCount.."")
        end
    end
end)
...
PeopleInLabo = {}
RegisterNetEvent("voltre:labo:get", function()
    local src = source
    TriggerClientEvent("voltre:labo:recevie", src, SaveData.json["illegals"]["laboratorys"])
end)
                    ...
                    if v2 == v.id then
                        ...
                        if xPlayer then
                            if changedEmployees then
                                TriggerClientEvent("voltre:labo:updateEmployeesData", xPlayer.source, v.id, v.employees)
                            end
                            if changed then
                                TriggerClientEvent("voltre:labo:updateindrying", xPlayer.source, v.id, v.treatmentequipment.indrying)
                            end
                            if changedMethFours then
                                TriggerClientEvent("voltre:labo:updatefour", xPlayer.source, v.id, v.treatementmeth.fours)
                            end
...
function RewardDealer(id)
    ...
    if (os.time() - data.dealer.lastRequest.time) >= (Config.laboratoire.timeDealer*60) and not data.dealer.lastRequest.finish and data.dealer.lastRequest.item ~= nil then
        ...
        voltre.logs.send("Logs", "Le labo: "..id.." a reçu les résultat vente dealer : Total gagné: "..selled*ProposedPrice.."$, Max Vente: "..max.."/"..data.dealer.lastRequest.count.." ("..maxServ..")", "laboratoires", {})

        TriggerClientEvent("voltre:labo:edit", -1, id, SaveData.json["illegals"]["laboratorys"][id])
    else
        return
    end
end
 
RegisterNetEvent("voltre:labo:setincamera", function(bool, id)
    local src = source
    ...
    if data.camera == nil then return end
    if bool then
        voltre.fct.instance.Set(src, 0)
    else
        voltre.fct.instance.Set(src, SaveData.json["illegals"]["laboratorys"][id].instance, "In Camera")
    end
end)

RegisterNetEvent("voltre:labo:enter", function(id)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    ...
    if SaveData.json["illegals"]["laboratorys"][id].upgrades["security"] then
        ...
        TriggerClientEvent("voltre:labo:history", -1, id, SaveData.json["illegals"]["laboratorys"][id].History)
    end

    voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") est entrer dans le labo : "..id, "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})

    voltre.fct.instance.Set(src, SaveData.json["illegals"]["laboratorys"][id].instance, "Laboratoire")
    TriggerClientEvent("voltre:labo:enter", src, id, true)
end)

RegisterNetEvent("voltre:labo:forceenter", function(id, time)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    local coords1 = SaveData.json["illegals"]["laboratorys"][id].coords
    ...
    if SaveData.json["illegals"]["laboratorys"][id].upgrades["security"] then
        TriggerClientEvent("voltre:labo:someoneenter", -1, id)
    end

    voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") crocheté la porte du labo : "..id, "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
    ...
    TriggerClientEvent("voltre:labo:forcedoor", -1, id, true)
    Citizen.CreateThread(function()
        Wait(60000)
        if SaveData.json["illegals"]["laboratorys"][id].lastdooropened == forcetime then
            ...
            TriggerClientEvent("voltre:labo:forcedoor", -1, id, false)
        end
    end)
...
AddEventHandler('esx:playerLoaded', function(source)
    ...
    voltre.fct.instance.Set(src, SaveData.json["illegals"]["laboratorys"][PeopleInLabo[xPlayer.getIdunique()]].instance, "Laboratoire")
    TriggerClientEvent("voltre:labo:enter", src, id, false)
end)


RegisterNetEvent("voltre:labo:leave", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    if PeopleInLabo[xPlayer.getIdunique()] == nil then
        ...
        voltre.fct.instance.Set(src, 0)
        TriggerClientEvent("voltre:labo:exit", src, PeopleInLabo[xPlayer.getIdunique()])
        ...
        voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") sort du labo : "..id, "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})

        PeopleInLabo[xPlayer.getIdunique()] = nil
    end
end)

RegisterNetEvent("voltre:labo:forceLeaveByTp", function(id)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    TriggerClientEvent("voltre:labo:exitTp", src, PeopleInLabo[xPlayer.getIdunique()])
    voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") sort du labo : "..id, "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
...
RegisterCommand("restartlabotutorial", function(source, args)
    ...
    TriggerClientEvent("voltre:labo:start-tutorial", source, tonumber(args[1]), "player", SaveData.json["illegals"]["laboratorys"][tonumber(args[1])].coords)
end)
...
RegisterCommand("restartlaboweedtutorial", function(source, args)
    ...
    TriggerClientEvent("voltre:labo:start-weed-tutorial", source, tonumber(args[1]))
end)

ESX.RegisterServerCallback("voltre:labo:buy", function(source, cb, itemId, option, metadata)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if option == "ind" then
        ...
        voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (U" .. xPlayer.getIdunique() .. " T" .. xPlayer.source .. ") a acheter le labo : " .. itemId, "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
        
        TriggerClientEvent("voltre:labo:start-tutorial", xPlayer.source, itemId, "player", SaveData.json["illegals"]["laboratorys"][itemId].coords)
        TriggerClientEvent("voltre:labo:edit", -1, itemId, SaveData.json["illegals"]["laboratorys"][itemId], true)
    ...
    elseif option == "group" then
        if xPlayer.getJob2() ~= nil and xPlayer.getJob2().name ~= nil and xPlayer.getJob2().name ~= "unemployed2" then
            ...
            voltre.logs.send("Logs", "Le joueur : " .. xPlayer.getName() .. " (U" .. xPlayer.getIdunique() .. " T" .. xPlayer.source .. ") a acheter le labo : " .. itemId .. " (pour le groupe : " .. xPlayer.getJob2().name .. ")", "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})

            TriggerClientEvent("voltre:labo:start-tutorial", xPlayer.source, itemId, "group", SaveData.json["illegals"]["laboratorys"][itemId].coords)
            TriggerClientEvent("voltre:labo:edit", -1, itemId, SaveData.json["illegals"]["laboratorys"][itemId], true)
...
RegisterNetEvent("voltre:labo:staff:create", function(id, data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    ...
    voltre.logs.send("Logs","Le staff : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a créer le labo : "..id, "staffs", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})


    TriggerClientEvent("voltre:labo:add", -1, id, SaveData.json["illegals"]["laboratorys"][id])
end) 

RegisterNetEvent("voltre:labo:staff:editPos", function(id, new)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer.getGroup() == "user" then return end
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    ...
    voltre.logs.send("Logs","Le staff : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a modifier la position du labo : "..id, "staffs", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})


    TriggerClientEvent("voltre:labo:edit", -1, id, SaveData.json["illegals"]["laboratorys"][id], false, true)
end)

RegisterNetEvent("voltre:labo:staff:delete", function(id)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer.getGroup() == "user" then return end
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    ...
    voltre.logs.send("Logs","Le staff : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a supprimer le labo : "..id, "staffs", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})


    TriggerClientEvent("voltre:labo:edit", -1, id, nil, false, true)
end)

RegisterNetEvent("voltre:labo:staff:updateUpgrade", function(id, data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer.getGroup() == "user" then return end
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    ...
    voltre.logs.send("Logs","Le staff : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a modifier les upgrades du labo : "..id, "staffs", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})


    TriggerClientEvent("voltre:labo:edit", -1, id, SaveData.json["illegals"]["laboratorys"][id])
end)  

RegisterNetEvent("voltre:labo:staff:updateType", function(id, new)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer.getGroup() == "user" then return end
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
...
RegisterNetEvent("voltre:labo:staff:edit", function(id, data)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if xPlayer.getGroup() == "user" then return end
    ...
    voltre.logs.send("Logs","Le staff : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a modifier le type du labo : "..id, "staffs", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})


    TriggerClientEvent("voltre:labo:edit", -1, id, data)
end)


local lastPlyRequest = {}
RegisterNetEvent("voltre:labo:cutDriedHead", function(id, isFirst)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    ...
    if gangname ~= "unemployed" then
        exports["voltre-core"]:AddGangXP(gangname, 2)
    end
end)

RegisterNetEvent("voltre:labo:cutWeed", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    local gangname = xPlayer.getJob2() and xPlayer.getJob2().name or "unemployed"
    if gangname ~= "unemployed" then
        exports["voltre-core"]:AddGangXP(gangname, 2 * number)
    end
end)

RegisterNetEvent("voltre:labo:packageWeedBud", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].upgrades["treatment-equipment"] == nil then return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then return end
    ...
    if budItem == nil or budItem.count <= 0 then
        TriggerClientEvent("voltre:labo:packageResult", xPlayer.source, false, "Vous n'avez plus de buds de weed.")
        return
    end
    local poochItem = xPlayer.getInventoryItem(Config.laboratoire.items["pooch"])
    if poochItem == nil or poochItem.count <= 0 then
        TriggerClientEvent("voltre:labo:packageResult", xPlayer.source, false, "Vous n'avez plus de pochon vide.")
        return
    end
    ...
    local gangname = xPlayer.getJob2() and xPlayer.getJob2().name or "unemployed"
    if gangname ~= "unemployed" then
        exports["voltre-core"]:ProgressGangMission(gangname, "daily_lab_produce", 1)
        exports["voltre-core"]:ProgressGangMission(gangname, "daily_lab_produce_large", 1)
        exports["voltre-core"]:ProgressGangMission(gangname, "weekly_lab_mass", 1)
        exports["voltre-core"]:AddGangXP(gangname, 5)
    end

    TriggerClientEvent("voltre:labo:packageResult", xPlayer.source, true)
end)
...
function SyncCuveToClients(id)
    ...
    for k, v in pairs(PeopleInLabo) do
        if v == id then
            ...
            if xP then
                TriggerClientEvent("voltre:labo:updatecuve", xP.source, id, clientData)
            end
        end
    end
end

RegisterNetEvent("voltre:labo:methCuveAddIngredient", function(id, cuveId, ingredientKey)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
...
RegisterNetEvent("voltre:labo:methCuveAddSolvant", function(id, cuveId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "meth" then return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then return end
...
RegisterNetEvent("voltre:labo:methCuveRetrieveTray", function(id, cuveId)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "meth" then return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then return end
    ...
    local gangname = xPlayer.getJob2() and xPlayer.getJob2().name or "unemployed"
    if gangname ~= "unemployed" then
        exports["voltre-core"]:AddGangXP(gangname, 3)
    end
...
RegisterNetEvent("voltre:labo:methSmashTray", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "meth" then return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then return end
...
RegisterNetEvent("voltre:labo:methPackCrystal", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "meth" then return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then return end
...
RegisterNetEvent("voltre:labo:setmethinfour", function(id, fourid)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "meth" then return end
    ...
    for k,v in pairs(PeopleInLabo) do
        if v == id then
            TriggerClientEvent("voltre:labo:updatefour", -1, id, SaveData.json["illegals"]["laboratorys"][id].treatementmeth.fours)
        end
    end
end)

RegisterNetEvent("voltre:labo:recolteMethFour", function(id, fourid)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "meth" then return end
    if SaveData.json["illegals"]["laboratorys"][id].treatementmeth.fours[fourid] == nil then return end
...
RegisterNetEvent("voltre:labo:dryweed", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].upgrades["treatment-equipment"] == nil then return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then return end
    ...
    for k,v in pairs(PeopleInLabo) do
        if v == id then
            TriggerClientEvent("voltre:labo:updateindrying", -1, id, SaveData.json["illegals"]["laboratorys"][id].treatmentequipment.indrying)
        end
    end
end)

RegisterNetEvent("voltre:labo:recoltedryweed", function(id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].upgrades["treatment-equipment"] == nil then return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then return end
...
RegisterNetEvent("voltre:labo:selltoDealer", function(id, count, price, type)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    ...
    voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a vendu au dealer (N:"..type..", Q:"..count.."X, Price: "..price.."$) (labo : "..id..")", "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})


    TriggerClientEvent("voltre:labo:edit", -1, id, SaveData.json["illegals"]["laboratorys"][id])
end)

RegisterNetEvent("voltre:labo:takeReward", function(id)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then return end
    if SaveData.json["illegals"]["laboratorys"][id].upgrades["dealer"] == nil then return end
    ...
    local gangname = xPlayer.getJob2() and xPlayer.getJob2().name or "unemployed"
    if gangname ~= "unemployed" then
        exports["voltre-core"]:ProgressGangMission(gangname, "daily_dirty_money", finalDirty)
        exports["voltre-core"]:ProgressGangMission(gangname, "weekly_dirty_money_mass", finalDirty)
        exports["voltre-core"]:AddGangXP(gangname, 25)
    end
...
ESX.RegisterServerCallback("voltre:labo:purchaseUpgrade", function(source, cb, upgrade, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if xPlayer.getAccount("bank").money < Config.laboratoire.upgrades[upgrade].prices.oneTime then
        ...
        if SaveData.json["illegals"]["laboratorys"][id].upgrades[upgrade] == true then 
            ...
            if upgrade == "treatment-equipment" then
                TriggerClientEvent("voltre:labo:start-weed-traitement-tutorial", source, id)
            end
            return
        end
    end
...
ESX.RegisterServerCallback("voltre:labo:purchaseLabType", function(source, cb, newType, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then 
        cb(false, "Ce laboratoire n'existe pas/plus.") 
        return
    ...
    voltre.DebugPrint("Change Lab Type : "..id.." - "..newType.." ("..oldType..")")
    if xPlayer.getAccount("bank").money < Config.laboratoire.type[newType].price then
        ...
        if oldType == newType then 
            ...
            if oldType == "weed" then
                voltre.DebugPrint("Delete All Weedplants from labo : "..id)
                ...
                for k,v in pairs(SaveData.Illegal.WeedPlants) do 
                    if tostring(v.inLabo) == tostring(id) then
                        ...
                        voltre.DebugPrint("Delete Weed Plant From Labo : "..id.." ("..k..") (Cause: changeType)")
                    end
                end
                TriggerClientEvent("voltre:weed:client:recevie", -1, SaveData.Illegal.WeedPlants)
            end

            voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a acheter le type "..newType.." (labo : "..id..")", "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})

            SaveData.json["illegals"]["laboratorys"][id].type = newType
            cb(true, "Paiement reçu.")
            TriggerClientEvent("voltre:labo:edit", -1, id, SaveData.json["illegals"]["laboratorys"][id])
            if newType == "weed" then
                TriggerClientEvent("voltre:labo:start-weed-tutorial", xPlayer.source, id)
            end
            return
        end
    end
...
ESX.RegisterServerCallback("voltre:labo:hireEmployee", function(source, cb, employee, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then 
        cb(false, "Ce laboratoire n'existe pas/plus.") 
        return
...
ESX.RegisterServerCallback("voltre:labo:fireEmployee", function(source, cb, employee, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then 
        cb(false, "Ce laboratoire n'existe pas/plus.") 
        return
...
ESX.RegisterServerCallback("voltre:labo:payEmployee", function(source, cb, employee, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then 
        cb(false, "Ce laboratoire n'existe pas/plus.") 
        return
...
RegisterNetEvent("voltre:labo:removeEmployeeItem", function(labId, employeeId, itemName, count)
    ...
    for k, v in pairs(employee.inventory) do
        if v.name == itemName then
            ...
            TriggerClientEvent("voltre:labo:edit", -1, labId, lab)
            break
        end
    end
end)

ESX.RegisterServerCallback("voltre:labo:inviteMember", function(source, cb, members, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    TriggerClientEvent("voltre:setNewWayPoint", xMember.source, SaveData.json["illegals"]["laboratorys"][id].coords)
    ...
    TriggerClientEvent("voltre:labo:edit", -1, id, SaveData.json["illegals"]["laboratorys"][id], true)
end)

ESX.RegisterServerCallback("voltre:labo:removeMember", function(source, cb, members, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if members == nil then
        cb(false, "...") 
        return
...
ESX.RegisterServerCallback("voltre:labo:transferOwnership", function(source, cb, newOwnerId, labId)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    TriggerClientEvent("voltre:setNewWayPoint", xNewOwner.source, SaveData.json["illegals"]["laboratorys"][labId].coords)
    
    -- Logger l'action
    voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a transféré la propriété du labo "..labId.." à "..xNewOwner.getName().." (U"..newOwnerId..")", "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
    
    TriggerClientEvent("voltre:labo:edit", -1, labId, SaveData.json["illegals"]["laboratorys"][labId], true)
    cb(true, "Transfert effectué avec succès.", xNewOwner.getName())
end)

ESX.RegisterServerCallback("voltre:labo:updateMemberPermissions", function(source, cb, members, id, newperms)
    local xPlayer = ESX.GetPlayerFromId(source)
    if members == nil then
        cb(false, "...") 
        return
...
ESX.RegisterServerCallback("voltre:labo:canCutHeads", function(source, cb, id)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("voltre:labo:canCut", function(source, cb, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then cb(false) return end
    if SaveData.json["illegals"]["laboratorys"][id].upgrades["treatment-equipment"] == nil then cb(false) return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "weed" then cb(false) return end
...
ESX.RegisterServerCallback("voltre:labo:canMixMeth", function(source, cb, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then cb(false) return end
...
ESX.RegisterServerCallback("voltre:labo:canBreakMeth", function(source, cb, id)
    local xPlayer = ESX.GetPlayerFromId(source)
    if SaveData.json["illegals"]["laboratorys"][id] == nil then cb(false) return end
    if SaveData.json["illegals"]["laboratorys"][id].type ~= "meth" then cb(false) return end
    if PeopleInLabo[xPlayer.getIdunique()] == nil then cb(false) return end
...
ESX.RegisterServerCallback("voltre:labo:getMarket", function(source, cb, id)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('voltre:labo:placeDeliveryOrder', function(source, cb, items, address, note, total, laboid)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    TriggerClientEvent("voltre:objectives:setStep", source, "labo_weed_production", "wait_delivery")
    
    voltre.DebugPrint(string.format("[Delivery Service] Nouvelle commande #%s de %s pour un total de $%s. Prévue pour dans "..math.ceil(deliveryTime/60) .. " minutes.", 
...
function DeliverOrder(orderId, source)
    ...
    voltre.DebugPrint(string.format("[Delivery Service] Commande #%s livrée au labo %s", 
    orderId, labo.id))
    ...
    TriggerClientEvent("voltre:objectives:setStep", source, "labo_weed_production", "collect_dropbox")
    ...
    if xPlayer then
        ...
        voltre.DebugPrint(string.format("[Delivery Service] Commande #%s livrée à %s", 
            orderId, GetPlayerName(source)))
    else
        order.status = "pending"
        voltre.DebugPrint(string.format("[Delivery Service] Impossible de livrer la commande #%s, joueur déconnecté", orderId))
    end]]
end
...
local function GenerateLabBill(labId)
    ...
    if billAmount > 0 then
        ...
        voltre.logs.send("Logs","Facture générée pour le labo "..labId.." : $"..billAmount.." (Échéance: "..os.date('%Y-%m-%d %H:%M:%S', deadline)..")", "laboratoires", {})
    end
end

ESX.RegisterServerCallback("voltre:labo:payBill", function(source, cb, labId)
    local xPlayer = ESX.GetPlayerFromId(source)
    local lab = SaveData.json["illegals"]["laboratorys"][labId]
    ...
    voltre.logs.send("Logs","Le joueur : "..xPlayer.getName().." (U"..xPlayer.getIdunique().." T"..xPlayer.source..") a payé la facture du labo "..labId.." : $"..lab.billing.currentBill, "laboratoires", {idunique = xPlayer.getIdunique(), name = xPlayer.getName()})
...
ESX.RegisterServerCallback("voltre:labo:getBillingInfo", function(source, cb, labId)
    local lab = SaveData.json["illegals"]["laboratorys"][labId]
    if not lab then 
        cb(nil)
        return
            ...
            if lab.billing then
                ...
                if lab.billing.currentBill and lab.billing.billDeadline then
                    if currentTime >= lab.billing.billDeadline then
                        ...
                        voltre.logs.send("Logs","Facture impayée pour le labo "..labId.." (Total impayées: "..lab.billing.unpaidBills..")", "laboratoires", {})
