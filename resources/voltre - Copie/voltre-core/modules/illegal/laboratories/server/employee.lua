...
function PlayEmployeeIdleAnim(labId, employeeId, duration)
    ...
    TriggerClientEvent("voltre:labo:playEmployeeIdleAnim", -1, pedNetId, dict, anim, duration)
...
function EmployeeWaterPlant(labId, employeeId, plantId)
    if not SaveData.Illegal.WeedPlants[plantId] then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[WATER FAIL] Plant", plantId, "does not exist")
        end
        return false
    end
    if SaveData.Illegal.WeedPlants[plantId].health == 0 then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[WATER FAIL] Plant", plantId, "health = 0")
        end
        return false
    end
    ...
    GetItemCountFromStorage(storageId, "water-canister", function(waterCount)
        if waterCount > 0 then
            ...
            if Config.laboratoire.debug then
                voltre.DebugPrint("[WATER SUCCESS] Employee", employeeId, "watered plant", plantId)
            end
            ...
            if Config.laboratoire.debug then
                voltre.DebugPrint("[WATER FAIL] No water-canister in storage (labo_coffre_big_"..labId..")")
            end
        end
    end)
...
function EmployeeSeedPlant(labId, employeeId, plantId)
    ...
    if not lab or not lab.employees[employeeId] then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[SEED FAIL] Laboratory or employee does not exist")
        end
        return false
    end
    ...
    if not plant then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[SEED FAIL] Plant", plantId, "does not exist")
        end
        return false
    end
    ...
    if plant.hasBase ~= 2 then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[SEED FAIL] Plant", plantId, "hasBase =", plant.hasBase, "(expected 2 for seeding)")
        end
        return false
    end
    ...
    GetItemCountFromStorage(storageId, "weed-seed-female", function(seedCount)
        if seedCount > 0 then
            ...
            if Config.laboratoire.debug then
                voltre.DebugPrint("[SEED FAIL] No weed-seed-female in storage (labo_coffre_big_"..labId..")")
            end
        end
    end)
...
function EmployeeFertilizePlant(labId, employeeId, plantId)
    if not SaveData.Illegal.WeedPlants[plantId] then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[FERTILIZE FAIL] Plant", plantId, "does not exist")
        end
        return false
    end
    if SaveData.Illegal.WeedPlants[plantId].health == 0 then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[FERTILIZE FAIL] Plant", plantId, "health = 0")
        end
        return false
    end
    ...
    GetItemCountFromStorage(storageId, "fertilizer", function(fertilizerCount)
        if fertilizerCount > 0 then
            ...
            if Config.laboratoire.debug then
                voltre.DebugPrint("[FERTILIZE FAIL] No fertilizer in storage (labo_coffre_big_"..labId..")")
            end
        end
    end)
...
function EmployeeHarvestPlant(labId, employeeId, plantId)
    if not SaveData.Illegal.WeedPlants[plantId] then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[HARVEST FAIL] Plant", plantId, "does not exist")
        end
        return false
    end
    if SaveData.Illegal.WeedPlants[plantId].growth ~= 100 then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[HARVEST FAIL] Plant", plantId, "growth =", SaveData.Illegal.WeedPlants[plantId].growth, "(expected 100)")
        end
        return false
    end
    if SaveData.Illegal.WeedPlants[plantId].health == 0 then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[HARVEST FAIL] Plant", plantId, "health = 0")
        end
        return false
    end
    ...
    if not employeeHasdrugs_scissors then
        GetItemCountFromStorage(storageId, "drugs_scissors", function(drugs_scissorsCount)
            if drugs_scissorsCount > 0 then
                ...
                if Config.laboratoire.debug then
                    voltre.DebugPrint("[HARVEST FAIL] No drugs_scissors in storage (labo_coffre_big_"..labId..")")
                end
                return
            end
        end)
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[HARVEST] Plant", plantId, "health:", SaveData.Illegal.WeedPlants[plantId].health, "-> weed count:", nbrweed)
    end
...
function EmployeeDryWeed(labId, employeeId)
    ...
    if weedCount == 0 then 
        if Config.laboratoire.debug then
            voltre.DebugPrint("[DRY_WEED] No weed_plant in inventory")
        end
        return false 
    end
    ...
    if nbr >= nbrMax then 
        if Config.laboratoire.debug then
            voltre.DebugPrint("[DRY_WEED] Drying area full")
        end
        return false 
    end
    ...
    for k,v in pairs(PeopleInLabo) do
        if v == labId then
            TriggerClientEvent("voltre:labo:updateindrying", -1, labId, lab.treatmentequipment.indrying)
        end
    end
...
function EmployeeGetFreshWeedForDrying(labId, employeeId)
    ...
    if not hasFreshWeed then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[GET_FRESH_FOR_DRY] No fresh weed in storage")
        end
        return false
    end
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[GET_FRESH_FOR_DRY] Employee "..employeeId.." got 1 fresh weed from storage")
    end
    ...
    GetItemCountFromStorage(storageId, "weed_plant_dry", function(dryWeedCount)
        ...
        GetItemCountFromStorage(storageId, Config.laboratoire.items["pooch"], function(emptyPouchCount)
            ...
            if lab.upgrades["security"] then
                ...
                TriggerClientEvent("voltre:labo:history", -1, labId, lab.History)
            end
...
function EmployeeMethFillCuve(labId, employeeId, cuveId)
    ...
    for _, ingredientKey in ipairs(cuveConf.ingredients) do
        ...
        if not alreadyAdded then
            ...
            if not hasItem then
                if Config.laboratoire.debug then
                    voltre.DebugPrint("[METH TASK] No "..itemName.." in storage for cuve "..cuveId)
                end
                return false
            end
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee "..employeeId.." filled cuve "..cuveId.." (state: "..cuve.state..")")
    end
    return true
end
...
function EmployeeMethAddSolvant(labId, employeeId, cuveId)
    ...
    if not hasItem then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[METH TASK] No solvant in storage for cuve "..cuveId)
        end
        return false
    end
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee "..employeeId.." added solvant to cuve "..cuveId)
    end
    return true
end
...
function EmployeeMethCollectTray(labId, employeeId, cuveId)
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee "..employeeId.." collected tray from cuve "..cuveId.." (remaining: "..(cuve.traysRemaining or 0)..")")
    end
    return true
end
...
function EmployeeMethLoadFour(labId, employeeId, fourId)
    ...
    for k,v in pairs(PeopleInLabo) do
        if v == labId then
            TriggerClientEvent("voltre:labo:updatefour", -1, labId, lab.treatementmeth.fours)
        end
    end
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee "..employeeId.." loaded four "..fourId)
    end
    return true
end
...
function EmployeeMethUnloadFour(labId, employeeId, fourId)
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee "..employeeId.." unloaded four "..fourId)
    end
    return true
end
...
function EmployeeMethDepositStorage(labId, employeeId)
    ...
    if deposited then
        Cache.SaveOne("illegals")
        if Config.laboratoire.debug then
            voltre.DebugPrint("[METH TASK] Employee "..employeeId.." deposited meth brute to storage")
        end
    end
    return deposited
end
...
function EmployeeMethBreakTray(labId, employeeId, breakId)
    ...
    if not hasTray then
        if Config.laboratoire.debug then
            voltre.DebugPrint("[METH TASK] Employee "..employeeId.." has no meth tray to break")
        end
        return false
    end
    ...
    if not hasPooch then
        ...
        if Config.laboratoire.debug then
            voltre.DebugPrint("[METH TASK] Employee "..employeeId.." no pooch available for packing")
        end
        return false
    end
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee "..employeeId.." broke tray and packed meth into pooch at breakpoint "..breakId)
    end
...
function EmployeeMethPackPooch(labId, employeeId)
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee "..employeeId.." packed meth into pooch")
    end
...
local function GetNextTaskMeth(labId, employeeId)
    ...
    if isTaskAllowed("meth_break_tray") then
        ...
        if hasTrays and hasPooch then
            ...
            for breakId, breakConf in pairs(Config.laboratoire.type["meth"].breakpoints) do
                ...
                if not EmployeeTaskLocks[taskKey] then
                    ...
                    if Config.laboratoire.debug then
                        voltre.DebugPrint("[METH TASK] Employee", employeeId, "assigned BREAK_TRAY (tray from "..(traySource or "unknown")..") at breakpoint", breakId)
                    end
        ...
        elseif hasTrays and not hasPooch then
            if Config.laboratoire.debug then
                voltre.DebugPrint("[METH TASK] Employee", employeeId, "has trays but NO POUCHES available - cannot pack")
            end
        end
    end
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[METH TASK] Employee", employeeId, "no task found - will rest")
    end
    return nil
end
...
local function GetNextTask(labId, employeeId)
    ...
    if not employeeConfig or not employeeConfig.allowedTasks then        
        if Config.laboratoire.debug then
            voltre.DebugPrint("[TASK] Employee", employeeId, "has no specialization config")
        end
        return nil
    end
    ...
    --voltre.DebugPrint("[TASK] Employee", employeeId, "checking", #plants, "plants for tasks (specialization:", employeeConfig.specialization, ")")
            ...
            if stats then
                if stats.stage == 5 and stats.growth >= 100 and isTaskAllowed("harvest") then
                    ...
                    if not EmployeeFailedTasks[harvestKey] or (os.time() - EmployeeFailedTasks[harvestKey]) > 60 then
                        EmployeeTaskLocks[taskKey] = employeeId
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "assigned HARVEST plant", plant.id)
                        end
                ...
                elseif stats.water < 30 and isTaskAllowed("water") then
                    ...
                    if not EmployeeFailedTasks[waterKey] or (os.time() - EmployeeFailedTasks[waterKey]) > 60 then
                        EmployeeTaskLocks[taskKey] = employeeId
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "assigned WATER plant", plant.id)
                        end
                ...
                elseif stats.fertilizer < 30 and isTaskAllowed("fertilize") then
                    ...
                    if not EmployeeFailedTasks[fertilizeKey] or (os.time() - EmployeeFailedTasks[fertilizeKey]) > 60 then
                        EmployeeTaskLocks[taskKey] = employeeId
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "assigned FERTILIZE plant", plant.id)
                        end
                ...
                if plantData and (plantData.hasBase == 0 or plantData.hasBase == 2) and isTaskAllowed("seed") then
                    ...
                    if not EmployeeFailedTasks[seedKey] or (os.time() - EmployeeFailedTasks[seedKey]) > 60 then
                        EmployeeTaskLocks[taskKey] = employeeId
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "assigned SEED plant", plant.id, "(hasBase:", plantData.hasBase, ")")
                        end
    ...
    if employee and employee.inventory then
        ...
        for k, v in pairs(employee.inventory) do
            if v.name == "weed_plant" and v.count > 20 and isTaskAllowed("deposit_fresh") then
                if Config.laboratoire.debug then
                    voltre.DebugPrint("[TASK] Employee", employeeId, "has", v.count, "weed_plant, needs to deposit")
                end
        ...
        if hasDryWeed then
            GetItemCountFromStorage(storageId, Config.laboratoire.items["pooch"], function(emptyPouchCount)
                if emptyPouchCount >= 15 then
                    ...
                    voltre.DebugPrint("[TASK] Storage has only", emptyPouchCount, "empty pouches (< 15)")
                end
            end)
            Wait(50)
            ...
            if hasPouches then
                ...
                for cutId, cutData in pairs(cuts) do
                    ...
                    if not EmployeeTaskLocks[taskKey] then
                        EmployeeTaskLocks[taskKey] = employeeId
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "assigned TREAT_WEED at table", cutId)
                        end
    ...
    if Config.laboratoire.debug then
        voltre.DebugPrint("[TASK] Employee", employeeId, "no task found - will rest")
    end
    return nil
end
            ...
            if task then
                ...
                if task.type == "water" then
                    ...
                    if success then
                        ...
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "FAILED water task for plant", task.plantId, "- blocked for 60s")
                        end
                    end
                elseif task.type == "fertilize" then
                    ...
                    if success then
                        ...
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "FAILED fertilize task for plant", task.plantId, "- blocked for 60s")
                        end
                    end
                elseif task.type == "seed" then
                    ...
                    if success then
                        ...
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "FAILED seed task for plant", task.plantId, "- blocked for 60s")
                        end
                    end
                elseif task.type == "harvest" then
                    ...
                    if success then
                        ...
                        if Config.laboratoire.debug then
                            voltre.DebugPrint("[TASK] Employee", employeeId, "FAILED harvest task for plant", task.plantId, "- blocked for 60s")
                        end
                    end
                ...
                if lab.employees[employeeId].fatigue >= 100 then
                    ...
                    --voltre.DebugPrint("[REST] Employee", employeeId, "exhausted (100%), returning to base for rest")
                    ...
                    if ped and DoesEntityExist(ped) then
                        ...
                        if returnSuccess then
                            --voltre.DebugPrint("[REST] Employee", employeeId, "at base, starting rest period")
                        end
                    end
                    ...
                    while lab.employees[employeeId].fatigue > 0 do
                        ...
                        Cache.SaveOne("illegals")
                        --voltre.DebugPrint("[REST] Employee", employeeId, "fatigue:", lab.employees[employeeId].fatigue, "%")
                    end
                    
                    EmployeeCurrentTask[labId][employeeId] = nil
                    --voltre.DebugPrint("[REST] Employee", employeeId, "fully rested, resuming work")
                else
        ...
        while AllWhileForEmployee[labId] and AllWhileForEmployee[labId][employeeId] do
            ...
            if task then
                ...
                if lab.employees[employeeId].fatigue >= 100 then
                    ...
                    voltre.DebugPrint("[REST] Meth Employee", employeeId, "exhausted (100%), returning to base for rest")
                    ...
                    EmployeeCurrentTask[labId][employeeId] = nil
                    voltre.DebugPrint("[REST] Meth Employee", employeeId, "fully rested, resuming work")
                else
                    local ped = createdEmployees[labId] and createdEmployees[labId][employeeId]
                    if ped and DoesEntityExist(ped) then
                        ReturnToBase(ped, labId, employeeId)
                ...
                elseif spot and spot.type == "entrance" then
                    ...
                    if spots and spots.entranceAnims and #spots.entranceAnims > 0 then
                        ...
                        if chosen.scenario then
                            TriggerClientEvent("voltre:labo:playEmployeeIdleAnim", -1, pedNetId, "scenario", chosen.scenario, -1)
                        else
                            TriggerClientEvent("voltre:labo:playEmployeeIdleAnim", -1, pedNetId, chosen.dict, chosen.anim, -1)
                        end
                    end
                elseif employeeConfig.sitChair and employeeConfig.anim then
                    TaskPlayAnim(createdEmployees[id][v.id], employeeConfig.anim[1], employeeConfig.anim[2], 8.0, 8.0, -1, 1, 0, false, false, false)
                        ...
                        while true do
                            ...
                            if GetEntityHealth(createdEmployees[id][v.id]) <= 0 and GetEntityMaxHealth(createdEmployees[id][v.id]) ~= 0 then
                                ...
                                Citizen.CreateThread(function()
                                    ...
                                    createdEmployees[id][v.id] = nil
                                    TriggerClientEvent("voltre:labo:removeEmployee", -1, id, v.id)
                                end)
                            end
                            Wait(5000)
                        end
...
ESX.RegisterServerCallback("voltre:labo:getEmployeeInfoFromPed", function(source, cb, pedNetId)
    local labId = nil
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    for labId, employees in pairs(createdEmployees) do
        for employeeId, ped in pairs(employees) do
            if DoesEntityExist(ped) then
                DeleteEntity(ped)
                --voltre.DebugPrint("[LABORATORY] Deleted employee ped: "..employeeId.." (Lab: "..labId..")")
            end
        end
    end
