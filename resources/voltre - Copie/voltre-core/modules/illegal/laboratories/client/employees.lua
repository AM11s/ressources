...
RegisterNetEvent("voltre:labo:syncEmployee", function(labId, employeeId, netId)
    voltre.DebugPrint("[EMPLOYEE SYNC] Received sync request - Lab: "..labId..", Employee: "..employeeId..", NetId: "..netId)
    ...
    Citizen.CreateThread(function()
        ...
        while attempts < maxAttempts do
            ...
            if ped and ped ~= 0 and DoesEntityExist(ped) then
                ...
                voltre.DebugPrint("[EMPLOYEE SYNC] SUCCESS - Employee "..employeeId.." synced (ped: "..ped..") after "..attempts.." attempts")
                
                -- Vérifier immédiatement que le ped existe toujours
                Wait(100)
                if DoesEntityExist(createdEmployees[labId][employeeId]) then
                    voltre.DebugPrint("[EMPLOYEE SYNC] Ped still exists after 100ms")
                else
                    voltre.DebugPrint("[EMPLOYEE SYNC] WARNING: Ped disappeared after sync!")
        ...
        voltre.DebugPrint("[EMPLOYEE SYNC] ERROR: Could not sync employee "..employeeId.." (netId: "..netId..") after "..maxAttempts.." attempts")
    end)
end)

RegisterNetEvent("voltre:labo:removeEmployee", function(labId, employeeId)
