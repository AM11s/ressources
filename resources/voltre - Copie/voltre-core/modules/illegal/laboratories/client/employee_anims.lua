...
RegisterNetEvent("voltre:labo:playEmployeeIdleAnim", function(pedNetId, dict, anim, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[IDLE ANIM CLIENT] Ped not found")
        return
    end
    ...
    if dict == "scenario" then
        ...
        if not HasAnimDictLoaded(dict) then
            voltre.DebugPrint("[IDLE ANIM CLIENT] Failed to load anim dict:", dict)
            return
        end
...
RegisterNetEvent("voltre:labo:playEmployeeWaterAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[WATER ANIM CLIENT] Ped not found")
        return
    end
    ...
    Citizen.CreateThread(function()
        ...
        if not HasModelLoaded(model) or not HasNamedPtfxAssetLoaded('core') or not HasAnimDictLoaded('weapon@w_sp_jerrycan') then
            voltre.DebugPrint("[WATER ANIM CLIENT] Failed to load resources")
            return
        end
...
RegisterNetEvent("voltre:labo:playEmployeeFertilizerAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[FERTILIZER ANIM CLIENT] Ped not found")
        return
    end
    ...
    Citizen.CreateThread(function()
        ...
        if not HasModelLoaded(model) or not HasNamedPtfxAssetLoaded('core') or not HasAnimDictLoaded('weapon@w_sp_jerrycan') then
            voltre.DebugPrint("[FERTILIZER ANIM CLIENT] Failed to load resources")
            return
        end
...
RegisterNetEvent("voltre:labo:playEmployeeSeedAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[SEED ANIM CLIENT] Ped not found")
        return
    end
    ...
    if not HasAnimDictLoaded('amb@medic@standing@kneel@base') then
        voltre.DebugPrint("[SEED ANIM CLIENT] Failed to load anim dict")
        return
    end
...
RegisterNetEvent("voltre:labo:playEmployeeHarvestAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[HARVEST ANIM CLIENT] Ped not found")
        return
    end
    ...
    if not HasAnimDictLoaded('amb@medic@standing@kneel@base') then
        voltre.DebugPrint("[HARVEST ANIM CLIENT] Failed to load anim dict")
        return
    end
...
RegisterNetEvent("voltre:labo:playEmployeeDryAreaAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[DRY AREA ANIM CLIENT] Ped not found")
        return
    end
    ...
    if not HasAnimDictLoaded('amb@medic@standing@kneel@base') then
        voltre.DebugPrint("[DRY AREA ANIM CLIENT] Failed to load anim dict")
        return
    end
...
RegisterNetEvent("voltre:labo:playEmployeeTreatAnim", function(pedNetId, iterations)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[TREAT ANIM CLIENT] Ped not found")
        return
    end
    ...
    Citizen.CreateThread(function()
        ...
        if not HasAnimDictLoaded('anim@amb@business@weed@weed_sorting_seated@') then
            voltre.DebugPrint("[TREAT ANIM CLIENT] Failed to load anim dict")
            return
        end
...
RegisterNetEvent("voltre:labo:playEmployeeStorageAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
    ...
    if not DoesEntityExist(ped) then
        voltre.DebugPrint("[STORAGE ANIM CLIENT] Ped not found")
        return
    end
    ...
    if not HasAnimDictLoaded('amb@medic@standing@kneel@base') then
        voltre.DebugPrint("[STORAGE ANIM CLIENT] Failed to load anim dict")
        return
    end
...
RegisterNetEvent("voltre:labo:playEmployeeMethCuveAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
...
RegisterNetEvent("voltre:labo:playEmployeeMethFourAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
...
RegisterNetEvent("voltre:labo:playEmployeeMethBreakAnim", function(pedNetId, duration)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
...
RegisterNetEvent("voltre:labo:client:unfreezeEmployee", function(pedNetId)
    local ped = NetworkGetEntityFromNetworkId(pedNetId)
...
RegisterNetEvent("voltre:labo:employeeAnim", function(labId, employeeId, animType, plantId)
    voltre.DebugPrint("[EMPLOYEE ANIM CLIENT] Animation request received - Lab: "..labId..", Employee: "..employeeId..", Type: "..animType)
    ...
    Citizen.CreateThread(function()
        ...
        while attempts < maxAttempts do
            employeePed = exports["voltre-core"]:GetEmployeePed(labId, employeeId)
            if employeePed and DoesEntityExist(employeePed) then
                voltre.DebugPrint("[EMPLOYEE ANIM CLIENT] Found employee ped after "..attempts.." attempts")
                break
            end
            attempts = attempts + 1
            Wait(100)
        ...
        if not employeePed or not DoesEntityExist(employeePed) then
            voltre.DebugPrint("[EMPLOYEE ANIM CLIENT] ERROR: Employee ped not found or doesn't exist (timeout 5000ms)")
            return
        end
        ...
        if EmployeeAnimThreads[labId][employeeId] then
            voltre.DebugPrint("[EMPLOYEE ANIM CLIENT] Employee already animating, skipping")
            return
        end
        ...
        if animType == "water" or animType == "fertilize" then
            -- voltre.DebugPrint("[EMPLOYEE ANIM] Starting "..animType.." animation")
            local plantData = exports["voltre-core"]:GetPlantStats(plantId)
            if plantId and plantData then
                local plantCoords = plantData.coords
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Plant coords: "..tostring(plantCoords))
                
                TaskGoToCoordAnyMeans(employeePed, plantCoords.x, plantCoords.y, plantCoords.z, 1.0, 0, 0, 786603, 0xbf800000)
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Employee walking to plant")
                ...
                Wait(500)
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Employee arrived at plant, playing animation")
                ...
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Playing kneel animation for "..animType)
                Wait(3000)
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Animation complete, returning to position")
        ...
        elseif animType == "harvest" then
            -- voltre.DebugPrint("[EMPLOYEE ANIM] Starting harvest animation")
            local plantData = exports["voltre-core"]:GetPlantStats(plantId)
            if plantId and plantData then
                local plantCoords = plantData.coords
                voltre.DebugPrint("[EMPLOYEE ANIM] Plant coords: "..tostring(plantCoords))
                ...
                TaskGoToCoordAnyMeans(employeePed, plantCoords.x, plantCoords.y, plantCoords.z, 1.0, 0, 0, 786603, 0xbf800000)
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Employee walking to harvest plant")
                ...
                TaskPlayAnim(employeePed, 'amb@medic@standing@kneel@base', 'base', 8.0, 8.0, -1, 1, 0, false, false, false)
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Playing harvest animation (12.5s)")
                Wait(12500)
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Harvest animation complete")
        ...
        elseif animType == "dry_weed" or animType == "collect_dry" or animType == "deposit_dry" then
            ...
            -- voltre.DebugPrint("[EMPLOYEE ANIM] Dry area coords: "..tostring(dryCoords))
            
            TaskGoToCoordAnyMeans(employeePed, dryCoords.x, dryCoords.y, dryCoords.z, 1.0, 0, 0, 786603, 0xbf800000)
            -- voltre.DebugPrint("[EMPLOYEE ANIM] Employee walking to dry area")
            ...
            TaskPlayAnim(employeePed, 'amb@medic@standing@kneel@base', 'base', 8.0, 8.0, -1, 1, 0, false, false, false)
            -- voltre.DebugPrint("[EMPLOYEE ANIM] Playing "..animType.." animation")
            ...
            -- voltre.DebugPrint("[EMPLOYEE ANIM] "..animType.." animation complete")
        ...
        elseif animType == "treat_weed" then
            -- voltre.DebugPrint("[EMPLOYEE ANIM] Starting treat_weed animation")
            ...
            if treatCoords then
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Treatment coords: "..tostring(treatCoords))
                TaskGoToCoordAnyMeans(employeePed, treatCoords.x, treatCoords.y, treatCoords.z, 1.0, 0, 0, 786603, 0xbf800000)
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Employee walking to treatment table")
                ...
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Employee seated at treatment table, creating props")
                ...
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Starting treatment sequence (5 iterations)")
                TreatOneWeed()
                TreatOneWeed()
                TreatOneWeed()
                TreatOneWeed()
                TreatOneWeed()
                -- voltre.DebugPrint("[EMPLOYEE ANIM] Treatment sequence complete")
        ...
        -- voltre.DebugPrint("[EMPLOYEE ANIM] Animation thread complete for "..animType)
