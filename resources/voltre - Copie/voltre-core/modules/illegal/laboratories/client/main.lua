Citizen.CreateThread(function()
    ...
    Add3DInteraction({
        ...
        Action = function()
            TriggerServerEvent("voltre:labo:leave")
        end,
        maxDistance = 0.6,
        maxDistance2 = 1.5,
        key = "E"
...
function HasLaboPermissions(id, perm) 
    ...
    local data = voltre.data.illegals.laboratories.list[id]
...
function GetAllLabo() 
    voltre.data.illegals.laboratories.loaded = false
    TriggerServerEvent("voltre:labo:get")
end 

function GetLaboInfo(name)
    return voltre.data.illegals.laboratories.list[name]
end
...
local function SetupLabo(id)
    Citizen.CreateThread(function()
        local data = voltre.data.illegals.laboratories.list[id]
        if not data then
            while voltre.data.illegals.laboratories.list[id] == nil do 
                Wait(100) 
            end
            data = voltre.data.illegals.laboratories.list[id]
        end 

        if devmode then
            RegisterCommand("testComputer_"..id, function()
                ESX.TriggerServerCallback("voltre:labo:getMarket", function(market) 
                    local labodata = exports["voltre-core"]:GetLaboInfo(id)
        ...
        if data.ownerType == "ind" then
            if ESX.PlayerData.idunique == data.owner then
                ESX.addBlips({
                    ...
                    category = nil,
                    position = voltre.fct.JsonCoordsToVect3(data.coords),
                    sprite = 499,
                    display = 4,
                    scale = 0.60,
                    color = 13
        ...
        Add3DInteraction({
            ...
            canSee = function(cb)
                if HasLaboPermissions(data.id, "access_lab") then
                    cb(true)
                    return
                elseif voltre.data.jobs.polices.list[ESX.PlayerData.job] ~= nil then
                    cb(true)
                    return
                else
                    cb(false)
            ...
            text = {
                ...
                lines = {
                    ...
                    --     right = function(cb)
                    --         local labodata = exports["voltre-core"]:GetLaboInfo(data.id)
                    ...
                    },
                    {
                        ...
                        action = function() 
                            ...
                            if (labodata.hasBuy and labodata.owner ~= nil) or labodata.dooropen then
                                TriggerServerEvent("voltre:labo:enter", labodata.id)
                                voltre.DebugPrint("Entrance triggered")
                            end
                        end,
                        canSee = function(cb) 
                            local labodata = exports["voltre-core"]:GetLaboInfo(data.id)
                    ...
                    },
                    {
                        ...
                        action = function() 
                            ...
                            TriggerServerEvent("voltre:labo:forceenter", data.id, timewait)
        ...
        if data.dropbox then 
            ...
            Add3DInteraction({
                ...
                Action = function()
                    ESX.TriggerServerCallback('voltre:getCoffre', function(coffredata, coffreid)
                        if coffredata then
                            local inventory = {
...
function openSellerMenu()
    ...
    if #voltre.data.illegals.laboratories.list == 0 then
    ...
    for k, v in pairs(voltre.data.illegals.laboratories.list) do
        if v.hasBuy == false and v.owner == "aucun" and v.ownerType == nil then
            ...
            table.insert(catalogItems, {
                ...
                price = v.price,
                image = "nui://voltre-cache/images/laboratories/" .. v.type .. ".png",
                options = {
    ...
    exports["voltre-core"]:OpenCatalog({
        ...
        items = catalogItems,
        imagePath = "nui://voltre-cache/images/laboratories/",
        currency = "$",
        callbackEvent = "voltre:labo:buy",
    })
end
...
function SetupLaboInside(id)
    ...
    Add3DInteraction({
        ...
        Action = function()
            ESX.TriggerServerCallback("voltre:labo:getMarket", function(market) 
                local result = exports["voltre-core"]:GetLaboInfo(id)
                exports["cuchi_computer"]:openLaptop("laboratory", {
                    laboratory = result, 
                    market = market,
                })
            ...
            text = {
                ...
                lines = {
                    ...
                    },
                    {
                        ...
                        action = function()
                            local result = exports["voltre-core"]:GetLaboInfo(id)
                            TriggerServerEvent("voltre:labo:setincamera", true, id)
                    ...
                    lines = {
                        {
                            ...
                            key = "E",
                            action = function()
                                local inventory = exports["voltre-core"]:GetPlayerInventaire()
                                local count = voltre.fct.input("Tu veux vendre combien de pochon ?")
                                ...
                                local price = voltre.fct.input("Tu veux vendre combien de $ le pochon ?")
                                if price == nil then return end
                                if type(price) ~= "number" then return end
                                TriggerServerEvent("voltre:labo:selltoDealer", id, count, price, "weed")
                            end,
                            canSee = function(cb)
                                local inventory = exports["voltre-core"]:GetPlayerInventaire()
                                local result = exports["voltre-core"]:GetLaboInfo(id)
                        ...
                        },
                        {
                            ...
                            action = function()
                                ...
                                if price == nil then return end
                                if type(price) ~= "number" then return end
                                TriggerServerEvent("voltre:labo:selltoDealer", id, count, price, "meth")
                            end,
                            canSee = function(cb)
                                local inventory = exports["voltre-core"]:GetPlayerInventaire()
                                local result = exports["voltre-core"]:GetLaboInfo(id)
                        ...
                        },
                        {
                            ...
                            key = "E",
                            action = function()
                                TriggerServerEvent("voltre:labo:takeReward", id)
                            end,
                            canSee = function(cb)
                                local result = exports["voltre-core"]:GetLaboInfo(id)
                                local inventory = exports["voltre-core"]:GetPlayerInventaire()
...
RegisterNetEvent("voltre:labo:updateEmployeesData", function(id, data)
    local data = voltre.data.illegals.laboratories.list[id]
    if data == nil then return end
    if PlayerState.Illegals.laboratories.inLabo == false then return end

    voltre.data.illegals.laboratories.list[id].employees = data
end)

local saveCoords = nil
local laboIplLoaded = false

RegisterNetEvent("voltre:labo:enter", function(id, loader)
    local data = voltre.data.illegals.laboratories.list[id]
    if data == nil then return end
    if PlayerState.Illegals.laboratories.inLabo ~= false then return end
    if loader then 
...
RegisterNetEvent("voltre:labo:exit", function(id)
    local data = voltre.data.illegals.laboratories.list[id]
    if data == nil then return end
    DoScreenFadeOut(100)
    if DoesEntityExist(CoffreEntity) then DeleteEntity(CoffreEntity) end
    ...
    for k,v in pairs(ChairsEntity) do if DoesEntityExist(v) then DeleteEntity(v) end end
    voltre.DisplayHud(false)
    SetEntityCoords(PlayerPedId(), voltre.fct.JsonCoordsToVect3(data.coords))
...
RegisterNetEvent("voltre:labo:exitTp", function(id)
    local data = voltre.data.illegals.laboratories.list[id]
    if data == nil then return end
    if DoesEntityExist(CoffreEntity) then DeleteEntity(CoffreEntity) end
    if DoesEntityExist(LadderEntity) then DeleteEntity(LadderEntity) end
...
RegisterNetEvent("voltre:labo:recevie", function(data)
    voltre.data.illegals.laboratories.list = data
    for k,v in pairs(voltre.data.illegals.laboratories.list) do
        SetupLabo(v.id)
    end
    voltre.data.illegals.laboratories.loaded = true
end)

RegisterNetEvent("voltre:labo:add", function(key, data)
    voltre.data.illegals.laboratories.list[key] = data
    SetupLabo(key)
end)

RegisterNetEvent("voltre:labo:edit", function(key, data, buy, laboExt)
    if data == nil then
        local id = key
        ...
        voltre.data.illegals.laboratories.list[key] = nil
    else
        voltre.data.illegals.laboratories.list[key] = data
        if PlayerState.Illegals.laboratories.inLabo == key then
            SetupIpl(voltre.data.illegals.laboratories.list[key], voltre.data.illegals.laboratories.list[key].type)
            SetupLaboInside(key)
        end
...
RegisterNetEvent("voltre:labo:someoneenter", function(id)
...
RegisterNetEvent("voltre:labo:forcedoor", function(id, data)
    voltre.data.illegals.laboratories.list[id].dooropen = data
end)

RegisterNetEvent("voltre:labo:start-tutorial", function(id, type, pos)
...
RegisterNetEvent("voltre:labo:start-weed-tutorial", function(id)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            elseif currentStep == "collect_dropbox" and not dropboxWaypoint then
                local dropboxCoords = voltre.data.illegals.laboratories.list[id].dropbox.coords
...
RegisterNetEvent("voltre:labo:start-weed-traitement-tutorial", function(id, resume)
    local laptopFocusDodge = false
    --exports["cuchi_computer"]:closeLaptop()
    --SetNuiFocus(true, false) -- to be up than laptop
    --laptopFocusDodge = true
    ...
    local labodata = voltre.data.illegals.laboratories.list[id]
        ...
        while true do
            ...
            elseif currentStep == "wait_drying" then
                if lastLaboInfo == nil or (GetGameTimer() - lastLaboInfo > 5000) then
                    SetResourceKvp(ESX.Config("serverName").."voltre:labo:tutorial:treatment", tostring(id))
...
RegisterCommand("resume-traitement-tuto", function()
    local kvpp = GetResourceKvpString(ESX.Config("serverName").."voltre:labo:tutorial:treatment")
    if kvpp and tonumber(kvpp) then
        TriggerEvent("voltre:labo:start-weed-traitement-tutorial", kvpp, true)
    end
end)