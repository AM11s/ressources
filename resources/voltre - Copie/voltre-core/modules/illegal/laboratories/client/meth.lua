function SetupMethLabo(id)
			...
			text = {
				...
				lines = {
					...
					-- Add pseudo
					{
						...
						key = "E",
						action = function()
							TriggerServerEvent("voltre:labo:methCuveAddIngredient", data.id, cuveId, "pseudo")
						end,
						canSee = function(cb)
							local result = GetLaboInfo(id)
							local cuve = result.treatementmeth.cuves[cuveId]
					...
					-- Add acide
					{
						...
						key = "F",
						action = function()
							TriggerServerEvent("voltre:labo:methCuveAddIngredient", data.id, cuveId, "acide")
						end,
						canSee = function(cb)
							local result = GetLaboInfo(id)
							local cuve = result.treatementmeth.cuves[cuveId]
					...
					-- Add phosphorus
					{
						...
						key = "G",
						action = function()
							TriggerServerEvent("voltre:labo:methCuveAddIngredient", data.id, cuveId, "phosphorus")
						end,
						canSee = function(cb)
							local result = GetLaboInfo(id)
							local cuve = result.treatementmeth.cuves[cuveId]
					...
					-- Add solvant
					{
						...
						key = "Y",
						action = function()
							TriggerServerEvent("voltre:labo:methCuveAddSolvant", data.id, cuveId)
						end,
						canSee = function(cb)
							local result = GetLaboInfo(id)
							local cuve = result.treatementmeth.cuves[cuveId]
					...
					-- Retrieve tray
					{
						...
						key = "H",
						action = function()
							TriggerServerEvent("voltre:labo:methCuveRetrieveTray", data.id, cuveId)
						end,
						canSee = function(cb)
							local result = GetLaboInfo(id)
							local cuve = result.treatementmeth.cuves[cuveId]
	...
	for breakId, v in pairs(Config.laboratoire.type[data.type].breakpoints) do
		...
		Add3DInteraction({
			...
			text = "Casser la meth",
			Action = function()
				local result = exports["voltre-core"]:GetAreCuting()
				if result then return end
				if exports["voltre-core"]:IsSceneOpen() then return end

				ESX.TriggerServerCallback("voltre:labo:canBreakMeth", function(can)
					...
					voltre.fct.game.RequestAndWaitModel("bkr_prop_meth_tray_01b")
					voltre.fct.game.RequestAndWaitModel("prop_custom_pooch_empty")
					voltre.fct.game.RequestAndWaitModel("prop_custom_methpile")
					...
					exports["voltre-core"]:OpenPropScene({
						...
						onZoneAction = function(zoneId, actionId, actionType, toolId)
							if actionType == "hold_complete" and actionId == "smash" and zoneId == "smash_zone" then
								...
								if smashStage == 1 then
									...
									exports["voltre-core"]:DeleteSceneProp("meth_tray")
									exports["voltre-core"]:AddSceneProp({
										id = "meth_tray",
										model = "tr_prop_meth_smashedtray_02",
										offset = vector3(0.0, 0.0, 0.02),
										rotation = vector3(0.0, 0.0, 0.0),
									})
									exports["voltre-core"]:SendSceneNotification("Premier coup! Continuez...", "info")
								elseif smashStage == 2 then
									...
									exports["voltre-core"]:SendSceneNotification("Plateau cassé! Encore un coup...", "info")
								elseif smashStage >= 3 then
									-- Final smash - tray empty, crystals appear
									TriggerServerEvent("voltre:labo:methSmashTray", laboId)
									exports["voltre-core"]:DeleteSceneProp("meth_tray")
									exports["voltre-core"]:UpdateZoneState("smash_zone", "completed", false)
									exports["voltre-core"]:RemoveSceneZone("smash_zone")
									...
									for _, prop in ipairs(allProps) do
										exports["voltre-core"]:AddSceneProp(prop)
									end
									for _, zone in ipairs(allZones) do
										exports["voltre-core"]:AddSceneZone(zone)
									end

									exports["voltre-core"]:SendSceneNotification("Cristaux révélés! Glissez-les dans les pochons", "success")
								end
							end
						end,
						...
						onZoneDrop = function(zoneId, actionId, itemId, toolId, sourceType, sourceId)
							if zoneId:match("^bag_") and sourceId:match("^crystal_zone_") then
								...
								exports["voltre-core"]:DeleteSceneProp(crystalPropId)
								exports["voltre-core"]:RemoveSceneZone(sourceId)
								...
								exports["voltre-core"]:DeleteSceneProp(bagPropId)
								exports["voltre-core"]:AddSceneProp({
									id = bagPropId,
								...
								exports["voltre-core"]:UpdateZoneState(zoneId, "completed", false)

								-- Server: pack 1 crystal = 1 pooch
								TriggerServerEvent("voltre:labo:methPackCrystal", laboId)

								-- Check if all crystals are packaged
								local remainingCrystals = 0
								for i = 1, numPiles do
									if exports["voltre-core"]:GetSceneProp("crystal_" .. i) then
								...
								if remainingCrystals <= 0 then
									Citizen.SetTimeout(1500, function()
										exports["voltre-core"]:SendSceneNotification("Tous les cristaux sont empaquetés!", "success")
										Citizen.SetTimeout(2000, function()
											exports["voltre-core"]:ClosePropScene()
											voltre.DisplayHud("3dinteractions", true)
										end)
									end)
								end
							end
				...
				lines = {
					...
					},
					{
						...
						action = function()
							...
							TriggerServerEvent("voltre:labo:setmethinfour", id, fourId)
					...
					},
					{
						...
						action = function()
							...
							Wait(8000)
							ClearPedTasks(PlayerPedId())
							TriggerServerEvent("voltre:labo:recolteMethFour", id, fourId)
							FreezeEntityPosition(PlayerPedId(), false)
							inDeposeMethFour = false
						end,
						canSee = function(cb)
...
RegisterNetEvent("voltre:labo:updatecuve", function(id, data)
    -- Ensure the lab data structure exists
    if not voltre.data.illegals.laboratories.list[id] then
        voltre.data.illegals.laboratories.list[id] = {}
    end
    if not voltre.data.illegals.laboratories.list[id].treatementmeth then
        voltre.data.illegals.laboratories.list[id].treatementmeth = {}
    end
    voltre.data.illegals.laboratories.list[id].treatementmeth.cuves = data
    ...
    for cuveId, _ in pairs(Config.laboratoire.type["meth"].cuves) do
        ...
        exports["voltre-core"]:Refresh3DInteraction(interactionId)
    end
end)

RegisterNetEvent("voltre:labo:updatefour", function(id, data)
    voltre.data.illegals.laboratories.list[id].treatementmeth.fours = data
end)