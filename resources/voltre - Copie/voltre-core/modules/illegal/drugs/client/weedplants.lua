...
RegisterNetEvent('voltre:weed:client:UseWeedSeed', function(rdn)
    ...
    local LaboType = nil
    if inLabo then
        LaboType = voltre.data.illegals.laboratories.list[inLabo].type
    ...
    while not planted do
        ...
        if hit == 1 then
            if inLabo and LaboType == "weed" then
                ...
                local closestSpot = FindClosestPlantSpot(vector3(dest.x, dest.y, dest.z), Labo.PlantCoords, voltre.data.illegals.laboratories.list[inLabo].bucket)
                if closestSpot then
                ...
                if toClose then
                    ...
                    if inLabo then
                        if LaboType == "weed" then
                            local closestSpot = FindClosestPlantSpot(vector3(dest.x, dest.y, dest.z), Labo.PlantCoords, voltre.data.illegals.laboratories.list[inLabo].bucket)
                            if closestSpot then
                                TriggerServerEvent("voltre:weed:server:CreateNewPlant", closestSpot.coords, rdn, inLabo, closestSpot.rotation)
                        ...
                        TriggerServerEvent("voltre:weed:server:CreateNewPlant", dest, rdn)
                    end
                    planted = false
                    seedUsed = false
                    ClearPedTasks(ped)
...
RegisterNetEvent('voltre:weed:client:recevieDataPlant', function(id, data)
    
    -- Vérifier que la plante existe dans PlantList
    if not PlantList[id] then
        voltre.DebugPrint("[WEED CLIENT] Plant", id, "not found in PlantList, ignoring update")
        return
    end
...
RegisterNetEvent('voltre:weed:client:recevie', function(data)
    voltre.fct.safe.wait(function() 
        for k,v in pairs(PlantList) do
...
RegisterNetEvent('voltre:weed:client:recevieNew', function(id, data)
	PlantList[id] = data
...
RegisterNetEvent("voltre:player:bucketChanged", function(bucket)
    InitWeedProps()
end)
...
Citizen.CreateThread(function()
    local loopWait = 2000
    Wait(5000)
    TriggerServerEvent('voltre:weed:server:load')
end)
...
function CreateWeedPlantInteraction2(data)
    ...
    voltre.fct.utils.CreateZoneFunction("weed_plant_"..data.id, 
        "circle", -- type
        ...
        function() 
            ...
            if hasSuit ~= "hazmat" and hasBase ~= 0 and hasBase ~= 2 then
                TriggerServerEvent("voltre:smells:updateSmell", 3, "weed")
                TriggerServerEvent("voltre:smells:setInSmellZone", "weed")
            end
    ...
    Citizen.CreateThread(function()
        --while GetResourceState("voltre-ui") ~= "started" do Wait(200) end
        Add3DInteraction({
            ...
            everytime = {
                [15000] = function()
                    ...
                    TriggerServerEvent("voltre:weed:requestPlantData", result.id)
                end,
            },
            text = {
                ...
                lines = {
...

                    {
                        ...
                        key = "G", 
                        action = function() 
                            if not voltre.fct.confirm("Voulez-vous vraiment récuperer cette plante ?") then return end
                            ...
                            TriggerServerEvent("voltre:weed:removePlantPot", id)
                            ...
                            exports["voltre-core"]:OpenPropScene({
                                ...
                                onZoneAction = function(zoneId, actionId, actionType, toolId)
                                    if actionType == "hold_complete" and actionId == "cut" then
                                        ...
                                        exports["voltre-core"]:DeleteSceneProp(budPropId)

                                        --exports["voltre-core"]:AddResult({ id = "weed_head", label = "Branche de weed", count = 1 })
                                        --exports["voltre-core"]:SendSceneNotification("Branches coupée !", "success")

                                        cutsRemaining = cutsRemaining - 1
                                        if cutsRemaining <= 0 then
                                            Citizen.SetTimeout(1500, function()
                                                --exports["voltre-core"]:SendSceneNotification("Récolte terminée !", "success")
                                                TriggerServerEvent("voltre:weed:harvest", plantId)
                    ...
                    },
                    {
                        ...
                        action = function() 
                            ...
                            Wait(12500)
                            FreezeEntityPosition(ped, false)
                            TriggerServerEvent("voltre:weed:add", id, "water")
                            ClearPedTasks(ped)
                            DeleteEntity(created_object)
                            StopParticleFxLooped(effect, 0)
                            --SetFollowPedCamViewMode(oldCam)
                            voltre.DisplayHud("3dinteractions", true)
                            TriggerServerEvent("voltre:weed:requestPlantData", id)
                        end,
                        canSee = function(cb)
                            local result = GetPlantStats(id)
                            if result == nil then cb(false) return end
                    ...
                    },
                    {
                        ...
                        action = function() 
                            ...
                            Wait(12500)
                            FreezeEntityPosition(ped, false)
                            TriggerServerEvent("voltre:weed:add", id, "fertilizer")
                            ClearPedTasks(ped)
                            DeleteEntity(created_object)
                            StopParticleFxLooped(effect, 0)
                            --SetFollowPedCamViewMode(oldCam)
                    ...
                    },
                    {
                        ...
                        key = "H", 
                        action = function() 
                            if not voltre.fct.confirm() then return end
                            local oldCam = GetFollowPedCamViewMode()
                            voltre.DisplayHud("3dinteractions", false)
                            --SetFollowPedCamViewMode(4)
                            local ped = PlayerPedId()
                            ...
                            Wait(5500)
                            FreezeEntityPosition(ped, false)
                            TriggerServerEvent("voltre:weed:add", result.id, "weed-seed-male")
                            ClearPedTasks(ped)
                            RemoveAnimDict('amb@medic@standing@kneel@base')
                            --RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
                            --SetFollowPedCamViewMode(oldCam)
                    ...
                    },
                    {
                        ...
                        action = function() 
                            ...
                            Wait(5000)
                            FreezeEntityPosition(ped, false)
                            TriggerServerEvent("voltre:weed:add", result.id, "weed-seed-female")
                            ClearPedTasks(ped)
                            RemoveAnimDict('amb@medic@standing@kneel@base')
                            --RemoveAnimDict('anim@gangops@facility@servers@bodysearch@')
                    ...
                    },
                    {
                        ...
                        action = function()
                            ...
                            FreezeEntityPosition(ped, true)
                            Wait(12500)
                            TriggerServerEvent("voltre:weed:upgrade:PlaceDirt", result.id)
                    ...
                    },
                    {
                        ...
                        action = function() 
                            ...
                            FreezeEntityPosition(ped, true)
                            Wait(12500)
                            TriggerServerEvent("voltre:weed:upgrade:PlantSeed", result.id)
                            local hasObjective = GetCurrentObjectives()
                            if hasObjective and hasObjective.id == "labo_weed_production" then
