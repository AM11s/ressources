...
local function StartFarm(drugs, type)
    ...
    while true do
        ...
        if distance > 30 then
            TriggerServerEvent("voltre:drugs:stop", drugs, type)
            break
        end
    end
end
        ...
        for _,z in pairs(Config.Drugs.Drugs) do
            for k,v in pairs(Config.Drugs.Drugs[_]) do
                ...
                if distance < 1.5 then
                    voltre.fct.draw.Text3DBar(Config.Drugs.RandomZone[k.._].x, Config.Drugs.RandomZone[k.._].y, Config.Drugs.RandomZone[k.._].z, "Appuyez sur [ ~g~E~w~ ] pour intéragir")
