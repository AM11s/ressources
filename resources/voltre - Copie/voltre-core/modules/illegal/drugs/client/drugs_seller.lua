Citizen.CreateThread(function()
    while not voltre.data.markers.loaded do Wait(100) end
    for k,v in pairs(Config.DrugSellers.Positions) do 
        local markId = "drugsSeller_npc_"..k
        voltre.data.markers.register(markId, {
            ...
            Job2 = nil,
            Action = function()
                voltre.data.markers.setVisible(markId, false)
                PlayUrl("knock_door", "sound/knockdoor.mp3", 0.2, false)
                Wait(500)
                ESX.TriggerServerCallback("voltre:drugsseller:arehere?", function(here, quantity, price, type, name, pedModel) 
                    ...
                    if here then
                        ...
                        voltre.data.markers.setVisible(markId, true)
                        voltre.data.markers.editLabel(markId, "Il n'y a l'air d'avoir personne.")
                        Wait(3000)
                        voltre.data.markers.editLabel(markId, "Appuyez sur [~y~E~s~] pour toquer à la porte")
                    end
                end, k)
            end
        })
            ...
            text = {
                ...
                lines = {
                    ...
                    },
                    {
                        ...
                        key = "E", 
                        action = function() 
                            local Quantity = voltre.fct.input("T'en veux combien ?")
                            ...
                            TriggerServerEvent("voltre:drugsseller:buy", id, Quantity)
