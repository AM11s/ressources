...
ESX.RegisterUsableItem('plantpot', function(source)
	...
	TriggerClientEvent('voltre:weed:client:UseWeedSeed', source, rdn)
end)

RegisterNetEvent('voltre:weed:server:load', function()
	local src = source
	while PlantLoaded == false do Wait(100) end
	TriggerClientEvent("voltre:weed:client:recevie", src, SaveData.Illegal.WeedPlants)
end)
        ...
        if inLabo and inLabo ~= 0 then
            for idunique, playerLabId in pairs(PeopleInLabo) do
                if playerLabId == inLabo then
                    ...
                    if xPlayer then
                        TriggerClientEvent("voltre:weed:client:recevieDataPlant", xPlayer.source, plantId, plantData)
						--voltre.DebugPrint("Update Plant for player :", xPlayer.source, "Plant ID :",plantId)
                    end
                end
            end
        else
            TriggerClientEvent("voltre:weed:client:recevieDataPlant", -1, plantId, plantData)
        end
    end
end
...
Citizen.CreateThread(function()
	...
	MySQL.Async.fetchAll('SELECT * FROM weedplants', {}, function(result)
		voltre.DebugPrint("[^1Drugs^7] Chargement des plantes de Weed en cours")
		...
		PlantLoaded = true
		voltre.DebugPrint("[^1Drugs^7] Plantes de Weed charger")
	end)
end)

RegisterNetEvent('voltre:weed:upgrade:PlaceDirt', function(id)
...
RegisterNetEvent('voltre:weed:upgrade:PlantSeed', function(id)
	if not SaveData.Illegal.WeedPlants[id] then return end
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:weed:destroyPlant', function(id)
	if not SaveData.Illegal.WeedPlants[id] then return end
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:weed:removePlantPot', function(id)
	if not SaveData.Illegal.WeedPlants[id] then return end
	local xPlayer = ESX.GetPlayerFromId(source)
	--if SaveData.Illegal.WeedPlants[id].owner.idunique ~= xPlayer.idunique then return end
	deletePlant(id)
...
RegisterNetEvent('voltre:weed:add', function(id, type)
	if not SaveData.Illegal.WeedPlants[id] then return end
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:weed:harvest', function(id)
	if not SaveData.Illegal.WeedPlants[id] then return end
...
RegisterNetEvent('voltre:weed:requestPlantData', function(id)
	if not SaveData.Illegal.WeedPlants[id] then return end
	SaveData.Illegal.WeedPlants[id].displayfertilizer = calcFertilizer(id)
	SaveData.Illegal.WeedPlants[id].displaywater = calcWater(id)
	SaveData.Illegal.WeedPlants[id].health = calcHealth(id)
...
RegisterNetEvent('voltre:weed:server:CreateNewPlant', function(coords, rdn, inLabo, rotation)
    ...
    if quantity ~= nil and quantity.count > 0 then
        ...
        MySQL.insert('INSERT into weedplants (owner, coords, time, fertilizer, water, gender, inLabo, rotation) VALUES (:owner,:coords, :time, :fertilizer, :water, :gender, :inLabo, :rotation)', {
			...
			TriggerClientEvent("voltre:weed:client:recevieNew", -1, data, SaveData.Illegal.WeedPlants[data])
        end)
    end
end)