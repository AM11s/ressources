            ...
            if v.here then
                if (os.time() - v.lastSync) > 60 then
                    ...
                    if needToGo < 50 then
                        v.here = false
                        v.lastSync = os.time()
                        --voltre.DebugPrint("[^1DrugsDealer^7] "..v.name.." vient de sortir de chez lui.")
                    end
                end
            else
                if (os.time() - v.lastSync) > 60 then
                    ...
                    if needToEnter < 50 then
                        v.here = true
                        v.lastSync = os.time()
                        --voltre.DebugPrint("[^1DrugsDealer^7] "..v.name.." vient de rentrer chez lui.")
                    end
                end
            end
        end
...
ESX.RegisterServerCallback("voltre:drugsseller:arehere?", function(source, cb, id)
...
RegisterNetEvent("voltre:drugsseller:buy", function(id, count)
