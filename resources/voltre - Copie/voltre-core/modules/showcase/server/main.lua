...
ESX.RegisterServerCallback('voltre:showcase:getData', function(src, cb)
...
RegisterNetEvent('voltre:showcase:setGroup', function(group)
    if not Showcase.IsEnabled() then return end
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterNetEvent('voltre:showcase:wipe', function()
    if not Showcase.IsEnabled() then return end
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
RegisterNetEvent('voltre:showcase:register', function()
    if not Showcase.IsEnabled() then return end
    local src = source
    TriggerClientEvent('voltre:newCreator:open', src, true)
end)
...
AddEventHandler('esx:playerLoaded', function(playerId)
    ...
    if xPlayer.getGroup() == 'user' then
        ...
        TriggerClientEvent("voltre:staff:recevieRequestGroup", xPlayer.source, { true, xPlayer.getGroup() })
