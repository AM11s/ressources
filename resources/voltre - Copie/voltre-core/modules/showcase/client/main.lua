...
CreateThread(function()
    ...
    SendNUIMessage({
        ...
        data = { version = GetResourceMetadata("voltre-core", 'version', 0), lastUpdate = cfg.lastUpdate or '?' },
    })
end)

-- Écran de bienvenue à chaque connexion (au spawn).
local function openWelcome()
    ESX.TriggerServerCallback('voltre:showcase:getData', function(data)
        setFocus(true)
...
RegisterNUICallback('showcase:setGroup', function(data, cb)
    if data and data.group then
        TriggerServerEvent('voltre:showcase:setGroup', data.group)
    end
    cb('ok')
end)
...
RegisterNUICallback('showcase:wipe', function(_, cb)
    setFocus(false)
    TriggerServerEvent('voltre:showcase:wipe')
    cb('ok')
end)

RegisterNUICallback('showcase:register', function(_, cb)
    setFocus(false)
    TriggerServerEvent('voltre:showcase:register')
    cb('ok')
end)
