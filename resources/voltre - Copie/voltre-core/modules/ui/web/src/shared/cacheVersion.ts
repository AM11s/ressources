...
/**
 * Build a versioned image URL from voltre-cache.
 * Usage: cacheImg('items/bread.webp') → 'nui://voltre-cache/images/items/bread.webp?v=1711234567890'
 *
...
export function cacheImg(path: string): string {
  ...
  return `nui://voltre-cache/images/${path}${_version ? `?v=${_version}` : ''}`;
}
