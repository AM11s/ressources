...
function App() {
    ...
    switch (action) {
      ...
      case 'setConfig':
        ...
        if (data?.serverColor || data?.data?.serverColor) {
          ...
          setServerConfig({
            serverName: data?.serverName || data?.data?.serverName || 'Voltre',
  ...
  return (
    <div className="voltre-ui-container" data-theme={globalConfig.theme} data-style={globalConfig.style}>
      {/* Interactions Module */}
      <InputDialog
      ...
      <WaveBgPlayground
        ...
        onClose={() => {
          setWavePlaygroundVisible(false);
          fetch(`https://voltre-core/wavePlayground:close`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }).catch(() => { });
        }}
        primaryColor={globalConfig.primaryColor}
      />
