...
const GetParentResourceName = () => 'voltre-core';
...
const Shop: React.FC<ShopProps> = ({ visible, onClose, primaryColor }) => {
  ...
  const loadImagesForCategory = useCallback(async (category: string, sex: string) => {
    ...
    for (let i = 0; consecutiveFails < 1; i++) {
      const url = `nui://voltre-ui/web/images/clothes/${sex}/${category}/${i}.webp`;
      try {
                ...
                <div className="shop-items-grid">
                  {filteredImages.map(id => {
                    ...
                    return (
                      ...
                      >
                        <img
                          src={`nui://voltre-ui/web/images/clothes/${shopData.playerSex}/${activeCategory}/${id}.webp`}
                          alt={`#${id}`}
                          loading="lazy"
                          draggable={false}
              ...
              return (
                <div key={category} className="shop-cart-item">
                  <div className="shop-cart-item-img">
                    <img
                      src={`nui://voltre-ui/web/images/clothes/${shopData.playerSex}/${category}/${item.item}.webp`}
                      alt={cat?.label}
                      draggable={false}
                      onError={(e) => { (e.target as HTMLImageElement).style.opacity = '0'; }}
                    />
