...
const nuiCallback = async (event: string, data: Record<string, any> = {}) => {
  try {
    await fetch(`https://voltre-core/${event}`, {
      method: 'POST',
...
const EXEC_RESOURCES = [
  { id: 'voltre-core', label: 'voltre-core', color: '#10b981' },
  { id: 'voltre-loader', label: 'voltre-loader', color: '#f59e0b', serverOnly: true },
  { id: 'voltre-stream', label: 'voltre-stream', color: '#3b82f6' },
  { id: 'voltre-ui', label: 'voltre-ui', color: '#8b5cf6' },
  { id: 'voltre-dealwp', label: 'voltre-dealwp', color: '#ec4899' },
];
...
const CodeExecutor: React.FC<CodeExecutorProps> = ({ results, setResults }) => {
  ...
  const [execResource, setExecResource] = useState('voltre-core');
