...
const ItemGrid: React.FC<ItemGridProps> = ({
  ...
  const renderItem = (item: InventoryItem, idx: number) => {
    ...
    return (
      <div
        key={`${id}-${item.name}-${item.slot ?? item.type2 ?? idx}`}
        className={`ni-item-slot ${isDragging && dragItem === item ? 'ni-dragging' : ''} ${bag ? 'ni-bag-item' : ''} ${isEquippedBag ? 'ni-bag-equipped' : ''} ${license ? 'ni-license-item' : ''} ${kevlar ? 'ni-kevlar-item' : ''}`}
  ...
  return (
    ...
    >
      <div className="ni-grid" style={gridStyle}>
        {items.length === 0 ? (
          <div className="ni-empty">{emptyText}</div>
        ) : (
