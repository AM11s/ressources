// PUBLIC RSA key used to verify JWT tokens signed by api.voltre.fr
// Replace this with the contents of your `keys/ui_jwt_public.pem` after running:
//   openssl genrsa -out keys/ui_jwt_private.pem 2048
//   openssl rsa -in keys/ui_jwt_private.pem -pubout -out keys/ui_jwt_public.pem
//
// The PRIVATE key stays on api.voltre.fr only. This PUBLIC key being in the
// bundle is fine: it can only verify tokens, never forge them.

export const UI_JWT_PUBLIC_KEY = `-----BEGIN PUBLIC KEY-----

4wIDAQAB
-----END PUBLIC KEY-----
`;

export const UI_JWT_ISSUER = 'api.voltre.fr';
export const UI_JWT_AUDIENCE = 'voltre-ui';
export const UI_VERIFY_ENDPOINT = 'https://api.voltre.fr/api/licenses/verify-ui';
export const UI_REPORT_ENDPOINT = 'https://api.voltre.fr/api/licenses/ui-report-rejection';
