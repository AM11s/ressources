...
RegisterNUICallback('chatResult', function(data, cb)
    voltre.DebugPrint("chatResult : " .. json.encode(data))
    ...
    if not data.canceled and data.message then
        ...
        if data.message:sub(1, 1) == '/' then
            ...
            voltre.DebugPrint("Commande executée : " .. data.message:sub(2))
        else
            TriggerServerEvent('Voltre:_chat:messageEntered', playerName, {r, g, b}, data.message)
        end
    end
...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    refreshCommands()
    voltre.InitPrint("Chat Module Loaded successfully")
end)
