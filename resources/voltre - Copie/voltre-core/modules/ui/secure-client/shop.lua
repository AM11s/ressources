-- Unified Shop - React UI Bridge (voltre-core)
...
function OpenShop(mode, brandId)
    ...
    if exports["voltre-core"]:playerIsDead() then return end

    isShopOpen = true
    shopMode = mode or "clothes"
    voltre.DisplayHud("always-display", false)
...
local function CloseShop(changeSkin)
    ...
    voltre.DisplayHud("additional-display", true)
    ...
    if isUtilsMode(shopMode) then
        ...
        if not changeSkin then
            ...
            TriggerServerEvent("voltre:tattoo:player:request:tattoo")
        end
        shopMode = "clothes"
        return
    end
    ...
    if not changeSkin then
        ...
        if next(shopInitialSkins) then
            ...
            TriggerEvent('Voltre:skinchanger:loadClothes', shopInitialSkins, shopInitialSkins)
        end
        shopChangedSkins = {}
        shopPreviewSkins = {}
        shopInitialSkins = {}
...
RegisterNUICallback("shop:clothes:pay", function(data, cb)
    local name = voltre.fct.input('Nom de la tenue', GetCurrentResourceName()) 
...
RegisterNUICallback("shop:acc:buyAccessories", function(data, cb)
    local name = voltre.fct.input("Nom de l'accessoire", GetCurrentResourceName())
    ...
    ESX.TriggerServerCallback('Voltre:accessories:buy', function(good)
        if good then
            if data.items then
...
RegisterNUICallback("shop:utils:apply", function(data, cb)
    ...
    if key then
        ...
        TriggerEvent("Voltre:skinchanger:change", key, variationId)
        if utilsType == "hair" then
            -- réinitialise le sub-texture pour hair
            TriggerEvent("Voltre:skinchanger:change", "hair_2", 0)
        end
    end
    cb('ok')
end)
...
RegisterNUICallback("shop:utils:pay", function(data, cb)
    ...
    if mode == "barber" or mode == "makeup" then
        ...
        ESX.TriggerServerCallback("Voltre:barber:purchase", function(ok)
            if ok then
    ...
    elseif mode == "tattoo" then
        ...
        -- Voltre:tattoo:purchase merge avec les tattoos existants en DB.
        TriggerServerEvent("Voltre:tattoo:purchase", total, serialized)
        CloseShop(true)
        cb({ success = true })
    else
        cb({ success = false })
