local HUDEditor = {
    isOpen = false,
    layoutKey = "voltre_hud_layout",
    tutorialKey = "voltre_hud_tutorial_done"
}
...
function HUDEditor.SaveLayout(layout)
    if not layout then
        voltre.DebugPrint("HUD Editor: Error: No layout provided")
        return false
    end
...
function HUDEditor.LoadLayout()
    ...
    if not layoutJson or layoutJson == "" then
        voltre.DebugPrint("HUD Editor: No saved layout found")
        return nil
    end
    ...
    if not success then
        voltre.DebugPrint("HUD Editor: Error decoding saved layout")
        return nil
    end
    ...
    for _, module in ipairs(layout) do
        if module.id == "minimap" then 
            ...
            TriggerEvent('voltre-hud:updateMapConfig', {
                mapType = mapType,
...
function HUDEditor.ResetLayout()
    ...
    voltre.DebugPrint("HUD Editor: Layout reset to defaults")
...
RegisterNUICallback("completeTutorial", function(data, cb)
    ...
    voltre.DebugPrint("HUD Tutorial: Marked as completed")
    cb("ok")
end)
...
RegisterCommand("hudreset_tutorial", function()
    ...
    voltre.DebugPrint("HUD Tutorial: Reset")
    if ESX and ESX.ShowNotification then
...
voltre.modules.ui.interactions.HUDEditor = HUDEditor

voltre.InitPrint("HUD Editor Loaded successfully")
