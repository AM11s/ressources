...
local function StartSitAnimation()
    ...
    if not HasAnimDictLoaded(Config.Creator.Animations[1]) then
        voltre.DebugPrint("[IDLE ANIM CLIENT] Failed to load anim dict:", Config.Creator.Animations[1])
        return
    end
...
local function ChangeSkinData(component, value)
    ...
    TriggerEvent('Voltre:skinchanger:change', component, value)
...
function OpenNewCreator(first)
    ...
    FreezeEntityPosition(PlayerPedId(), true)
    if voltre and voltre.DisplayHud then voltre.DisplayHud(false, 978) end
    ...
    TriggerEvent('Voltre:skinchanger:getData', function(comp, max)
...
local function CloseNewCreator()
    ...
    FreezeEntityPosition(PlayerPedId(), false)
    if voltre and voltre.DisplayHud then voltre.DisplayHud(true, 978) end
    DestroyCreatorCam()
            ...
            if genderOutfit then
                ...
                for _, component in ipairs(componentList) do
                    if genderOutfit[component] then
                        ...
                        TriggerEvent('Voltre:skinchanger:change', component, genderOutfit[component])
                    end
                end
...
RegisterNUICallback('newCreator:finishCreator', function(data, cb)
    ...
    TriggerEvent('Voltre:skinchanger:getSkin', function(skin)
        ...
        TriggerServerEvent('voltre:clothe:addClothinInvWithID', "top", "Tenue d'arriver", top, randId())
        TriggerServerEvent('voltre:clothe:addClothinInvWithID', "pants", "Tenue d'arriver", pant, randId())
        TriggerServerEvent('voltre:clothe:addClothinInvWithID', "shoes", "Tenue d'arriver", shoes, randId())

        TriggerServerEvent("Voltre:charCreator:finish", creatorCharData)
        TriggerServerEvent("voltre:esx:enter", false)
...
RegisterNetEvent("voltre:newCreator:open", OpenNewCreator)
