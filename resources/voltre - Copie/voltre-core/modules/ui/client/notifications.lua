...
function SendNotification(message, couleurProgress, timeout, progress, theme, exitAnim, pin_id)
    ...
    if message == nil then
        voltre.DebugPrint("^1NOTIFICATION ERROR: ^7Notification message is nil")
        return
    end
...
function SendAdvancedNotification(message, title, subject, couleurProgress, icon, timeout, progress, theme, exitAnim, pin_id)
    ...
    if title == nil then
        voltre.DebugPrint("^1NOTIFICATION ERROR: ^7Notification title is nil")
        return
    end
...
function SendCustomNotification(options, advanced)
    if type(options) ~= 'table' then
        voltre.DebugPrint("^1NOTIFICATION ERROR: ^7options passed to SendCustomNotification must be a table")
        return
    end
...
RegisterNUICallback("notificationResponse", function(data, cb)
    ...
    TriggerEvent('voltre:notificationResponse', data.id, data.response)
    cb('ok')
end)

-- Events
RegisterNetEvent("voltre:notification")
AddEventHandler("voltre:notification", SendNotification)

RegisterNetEvent("voltre:notificationAdvanced")
AddEventHandler("voltre:notificationAdvanced", SendAdvancedNotification)

RegisterNetEvent("voltre:notificationAccept")
AddEventHandler("voltre:notificationAccept", SendAcceptNotification)
...
RegisterCommand("testnotif_pinned", function()
    ...
    voltre.DebugPrint("Notification épinglée avec ID: " .. pin_id)
    
    -- Exemple: Unpin après 10 secondes
    SetTimeout(10000, function()
        UnpinNotification(pin_id)
        voltre.DebugPrint("Notification dépinglée")
    end)
end)
...
voltre.modules.notifications.initialized = true
voltre.InitPrint("Système de notifications chargé")