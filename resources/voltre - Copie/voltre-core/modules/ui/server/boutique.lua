...
-- Base URL for boutique images (from voltre-ui resource)
local IMG_BASE = "nui://voltre-cache/images/boutique/"
    ...
    for i = 1, 6 do
        ...
        if #candidates > 0 then
            ...
            if chosen.item.model then
                finalImage = "nui://voltre-cache/images/vehicles/" .. (chosen.item.model or "") .. ".webp"
            else
                finalImage = "nui://voltre-cache/images/weapons/" .. chosen.item.name .. ".webp"
            end
...
function BoutiqueServer.StartNightMarket(staffName)
    ...
    for i = 1, NIGHTMARKET_CARD_COUNT do
        ...
        if entry then
            ...
            if entry.type == "vehicle" and entry.item.model then
                cardImage = "nui://voltre-cache/images/vehicles/" .. entry.item.model .. ".webp"
            elseif entry.type == "weapon" and entry.item.name then
                cardImage = "nui://voltre-cache/images/weapons/" .. entry.item.name .. ".webp"
            end
    ...
    TriggerClientEvent('voltre:nightmarket:started', -1)
...
function BoutiqueServer.StopNightMarket()
    ...
    SaveNightMarketCache()
    TriggerClientEvent('voltre:nightmarket:stopped', -1)
end
...
RegisterServerEvent('voltre:nightmarket:start')
AddEventHandler('voltre:nightmarket:start', function()
    local source = source
...
RegisterServerEvent('voltre:nightmarket:stop')
AddEventHandler('voltre:nightmarket:stop', function()
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
    if not xPlayer.getPermission("GESTION_BOUTIQUE") then return end
...
ESX.RegisterServerCallback('voltre:nightmarket:isActive', function(source, cb)
    cb(BoutiqueServer.IsNightMarketActive())
end)

-- Server callback: get NightMarket data for NUI
ESX.RegisterServerCallback('voltre:nightmarket:getData', function(source, cb)
...
ESX.RegisterServerCallback('voltre:nightmarket:purchase', function(source, cb, cardId)
    local xPlayer = ESX.GetPlayerFromId(source)
...
function BoutiqueServer.FormatItems(category)
    ...
    if category == "vehicles" and Config.Boutique.Vehicles then
        for i, v in ipairs(Config.Boutique.Vehicles) do
            table.insert(items, {
                ...
                cells = v.cells,
                category = "vehicle",
                image = "nui://voltre-cache/images/vehicles/" .. (v.model or "") .. ".webp",
            })
        end
    elseif category == "weapons" and Config.Boutique.Weapons then
        for i, w in ipairs(Config.Boutique.Weapons) do
            local weaponImg = nil
            if w.name then
                weaponImg = "nui://voltre-cache/images/weapons/" .. w.name .. ".webp"
            end
            table.insert(items, {
                id = "weapon_" .. i,
                label = w.label,
...
ESX.RegisterServerCallback('voltre:boutique:getData', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    GetPlayerCoins(fivemId, function(coins)
        GetPlayerHistory(fivemId, function(history)
            GetPlayerFidelity(fivemId, function(havebuy, totalbuy)
                local dailyItems = BoutiqueServer.GenerateDailyShop(fivemId)
                ...
                local boutiqueLink = voltre.getConvarKey("boutiqueLink") or ""
...
ESX.RegisterServerCallback('voltre:boutique:getItems', function(source, cb, category)
    local items = BoutiqueServer.FormatItems(category)
    cb(items)
end)

ESX.RegisterServerCallback('voltre:boutique:purchase', function(source, cb, itemId, category, quantity, price)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then cb({ success = false, message = "Joueur introuvable" }) return end
        ...
        HasAlreadyPurchased(fivemId, itemLabel, function(alreadyOwned)
            ...
            DeductCoins(fivemId, xPlayer, totalPrice, string.format("Achat de : %s", itemLabel), function(success, newCoins)
                ...
                if vehicleModel then
                    ...
                    local oldCacheData = exports["voltre-core"]:GetCacheData("owned_vehicles")
                    local newplate = nil
                    ...
                    exports["voltre-core"]:EditCacheData("owned_vehicles", oldCacheData)
                end
    ...
    elseif category == "pack" then
        ...
        TriggerEvent('voltre:server:BuyPack', packKey)
...
ESX.RegisterServerCallback('voltre:boutique:openCrate', function(source, cb, crateId, quantity)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    DeductCoins(fivemId, xPlayer, totalPrice, string.format("Ouverture caisse : %s (x%d)", crate.label, quantity or 1), function(success, newCoins)
        ...
        voltre.DebugPrint(string.format("Boutique Crate: %s opened %s and got %s (rarity %d)",
...
-- History callback
ESX.RegisterServerCallback('voltre:boutique:getHistory', function(source, cb)
    local fivemId = GetFivemId(source)
    if not fivemId then cb({}) return end
...
ESX.RegisterServerCallback('voltre:boutique:getCoins', function(source, cb)
    local fivemId = GetFivemId(source)
...
-- Get weapon custom data from voltre-ui resource
local function GetWeaponCustomData()
    local ok, data = pcall(function()
        return exports["voltre-ui"]:GetWeaponCustomPrice()
    end)
...
ESX.RegisterServerCallback('voltre:boutique:getPlayerWeapons', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then cb({}) return end
...
ESX.RegisterServerCallback('voltre:boutique:buyWeaponComponent', function(source, cb, weaponName, componentHash, price)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then cb({ success = false, message = "Joueur introuvable" }) return end
...
ESX.RegisterServerCallback('voltre:boutique:getVipPurchaseInfo', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('voltre:boutique:buyVip', function(tierName)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
    ...
    if isVip and vipType then
        ...
        if targetLevel <= currentLevel then
            ...
            TriggerClientEvent('voltre:boutique:nuiCallback', source, { action = 'boutique:purchaseResult', success = false, message = msg })
            return
        end
    end
    ...
    DeductCoins(fivemId, xPlayer, price, string.format("Achat VIP %s (%d jours)", tierName, days), function(success, newCoins)
        if not success then
            ...
            TriggerClientEvent('voltre:boutique:nuiCallback', source, { action = 'boutique:purchaseResult', success = false, message = "Coins insuffisants" })
            return
        end
        ...
        MySQL.Async.fetchAll("SELECT * FROM vips WHERE identifier = @identifier", {
            ...
            TriggerClientEvent('voltre:boutique:nuiCallback', source, {
