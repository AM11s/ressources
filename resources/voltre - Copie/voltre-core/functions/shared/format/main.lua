...
voltre.fct.format.GetNbrOfNumber = function(string)
...
voltre.fct.format.isNumber = function(string)
	if voltre.fct.format.GetNbrOfNumber(string) == #string then 
		return true
	else
		return false
	end
...
voltre.fct.format.clearTableFunction = function(table)
	local newTable = {}
    for k, v in pairs(table) do
        local typeVal = type(v)
        if typeVal == "table" then
            newTable[k] = voltre.fct.format.clearTableFunction(v)
