...
voltre.fct.utils.SkyCam = LPH_NO_VIRTUALIZE(function(time)
...
voltre.fct.utils.SkyTransportCoords = LPH_NO_VIRTUALIZE(function(time, coords)
...
voltre.fct.utils.SkyTransportWipe = LPH_NO_VIRTUALIZE(function(time)
    if time == nil then time = 5000 end
	InitialSetup()
	...
	while true do
		...
		if GetGameTimer() - timer > time then
			TriggerServerEvent("voltre:wipeme")
			break
		end
	end
end)