voltre.fct.draw.HideHUDThisFrame = LPH_NO_VIRTUALIZE(function()
...
voltre.fct.draw.DrawInstructionalButtons = LPH_NO_VIRTUALIZE(function(buttons)
...
voltre.fct.draw.DrawRetroSprite = LPH_NO_VIRTUALIZE(function(textureDict, textureName, nums, node)
...
voltre.fct.draw.Text3D = LPH_NO_VIRTUALIZE(function(coords, text, size, a, font, dropShadow)
...
voltre.fct.draw.Text3DBar = LPH_NO_VIRTUALIZE(function(x, y, z, text)
...
Citizen.CreateThread(LPH_NO_VIRTUALIZE(function()
    while true do
        ...
        for k,v in pairs(draw_data) do
            ...
            if dst <= v.dst then
                near_drawer = true
                voltre.fct.draw.Text3D(v.pos, v.text, v.size)
            end
        end
...
Citizen.CreateThread(function()
    ...
    addDraw({
        ...
        text = "Direction notre discord > "..ESX.Config("serverColor")..(ESX.Config("serverDiscord") or "osnium FA"),
        size = 3.5,
