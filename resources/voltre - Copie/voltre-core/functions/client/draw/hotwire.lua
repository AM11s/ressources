...
local ShowRemaining = LPH_NO_VIRTUALIZE(function()
    voltre.fct.draw.Text2(voltre.fct.format.SecondsToClock(RemainingTime), 0.4, 0.485, 0.170, {49, 127, 99})
end)
...
voltre.fct.draw.StartHotwire = LPH_NO_VIRTUALIZE(function(cb)
    minigameLoop = true
    time = 3000
    ...
    while minigameLoop do
        for k, v in pairs(drawTable) do
            voltre.fct.draw.DrawRetroSprite(v[1], v[2], v[3], v[4])
        end
        if not win then
