	...
	while true do
		...
		if #activeBars > 0 then
			...
			for i,v in pairs(activeBars) do
				...
				voltre.fct.draw.Text(0, v.title, 0.425, (ScreenCoords.baseX - safeZoneX) + ScreenCoords.titleOffsetX, drawY + ScreenCoords.titleOffsetY, v.textColor, false, 2)
				...
				elseif v.text then
					voltre.fct.draw.Text(0, v.text, 0.425, (ScreenCoords.baseX - safeZoneX) + ScreenCoords.valueOffsetX, drawY + ScreenCoords.valueOffsetY, v.textColor, false, 2)
				elseif v.endTime then
					local remainingTime = math.floor(v.endTime - GetGameTimer())
					voltre.fct.draw.Text(0, SecondsToClock(remainingTime / 1000), 0.425, (ScreenCoords.baseX - safeZoneX) + ScreenCoords.valueOffsetX, drawY + ScreenCoords.valueOffsetY, remainingTime <= 0 and textColor or v.textColor, false, 2)
				end
			end
        else
            WaitActiveBars = 1500
...
voltre.fct.draw.AddTimerBar = function(title, itemData)
...
voltre.fct.draw.UpdateTimerBar = function(barIndex, itemData)
...
voltre.fct.draw.RemoveTimerBar = function()
	activeBars = {}
