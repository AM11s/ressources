if SaveData == nil then
    ...
    if SaveData == nil then
        voltre.DebugPrint("SaveData not init after timeout delay.")
        SaveData = {}
    end
end

local RESOURCE_NAME = 'voltre-core'
...
function Cache.Init(name, defaultValue)
    ...
    voltre.DebugPrint('^3Initializing cache:^7 ' .. name)
    ...
    if data and data.fragmented == true then
        ...
        voltre.DebugPrint(('^3Cache %s is fragmented (%d elements in %d files)^7'):format(name, data.totalElements, data.fragmentCount))
        return
    end
...
function Cache.Get(name, searchKey)
    ...
    voltre.DebugPrint(('^3[WARN] Loading all fragments for %s - this is expensive!^7'):format(name))
...
function Cache.SaveAll()
    ...
    voltre.DebugPrint('^2All caches saved^7')
end
...
function Cache.Reload(name)
    ...
    if rawData then
        ...
        voltre.DebugPrint('^3Cache reloaded:^7 ' .. name)
    end
end
...
function Cache.ReloadAll()
    ...
    voltre.DebugPrint('^2All caches reloaded^7')
end
...
SaveData.cacheLoad = true
voltre.DebugPrint('^2Cache system ready^7')
...
function Cache.Backup()
    ...
    print(('^2[voltre-core]^7 Backup créé: ^3%s^7'):format(date))
end
...
function Cache.Fragment(name)
    ...
    print(('^3[voltre-core]^7 Fragmentation de ^3%s^7 (%d éléments)...'):format(name, count))
    ...
    print(('^2[voltre-core]^7 ^3%s^7 fragmenté en %d parties'):format(name, fragmentIndex - 1))
    return true
end
...
function Cache.Cleanup(name, daysToKeep)
    ...
    if removed > 0 then
        ...
        Cache.SaveOne(name)
        print(('^2[voltre-core]^7 Nettoyage ^3%s^7: %d entrées supprimées (> %d jours)'):format(name, removed, daysToKeep))
    end
...
RegisterCommand('cache', function(source, args)
    ...
    if not args[1] then
        print([[^2[voltre-core]^7 Cache commands:
    ...
    if cmd == 'autosave' then
        ...
        if action == 'enable' then
            ...
            print('^2[voltre-core]^7 Auto-save ^2enabled^7')
        elseif action == 'disable' then
            Cache.SetAutoSave(false)
            print('^2[voltre-core]^7 Auto-save ^1disabled^7')
        else
            local status = Cache.IsAutoSaveEnabled() and '^2enabled^7' or '^1disabled^7'
            print('^2[voltre-core]^7 Auto-save is ' .. status)
        end
        
    elseif cmd == 'save' then
        if args[2] then
            Cache.SaveOne(args[2])
            print('^2[voltre-core]^7 Cache ^3' .. args[2] .. '^7 saved')
        else
            Cache.SaveAll()
        end
    ...
    elseif cmd == 'list' then
        print('^2[voltre-core]^7 Loaded caches:')
    ...
    elseif cmd == 'get' then
        if args[2] and SaveData.json[args[2]] then
            print('^2[voltre-core]^7 Cache ^3' .. args[2] .. '^7:')
            print(json.encode(SaveData.json[args[2]], { indent = true }))
        else
            print('^2[voltre-core]^7 Cache not found')
        end
    ...
    elseif cmd == 'cleanup' then
        if args[2] then
            ...
            if removed == 0 then
                print('^2[voltre-core]^7 Aucune entrée à supprimer')
            end
        else
            print('^2[voltre-core]^7 Usage: cache cleanup <name> [days]')
        end
        ...
        if args[2] then
            if FRAGMENTABLE_CACHES[args[2]] then
                ...
                if success then
                    ...
                    print('^2[voltre-core]^7 Cache trop petit pour être fragmenté (< ' .. FRAGMENT_THRESHOLD .. ' éléments)')
                end
            else
                print('^2[voltre-core]^7 Ce cache ne peut pas être fragmenté')
            end
        else
            print('^2[voltre-core]^7 Usage: cache fragment <name>')
        end
        
    elseif cmd == 'stats' then
        print('^2[voltre-core]^7 Cache statistics:')
        ...
        print('^2[voltre-core]^7 Unknown command. Use ^3cache^7 for help.')
    end
end, true)
...
voltre.InitPrint('^2Cache module loaded^7')