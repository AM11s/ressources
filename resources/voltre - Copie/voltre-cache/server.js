...
const IMAGES_PATH = 'resources/[voltre]/voltre-cache/images';
const TEMP_PATH = 'resources/[voltre]/voltre-cache/temp';
...
function bumpCacheVersion() {
    ...
    emitNet('voltre:cache:versionChanged', -1, cacheVersion);
}

onNet('voltre:cache:requestVersion', () => {
    emitNet('voltre:cache:versionChanged', source, cacheVersion);
});
...
function ensureDir(dirPath) {
    try {
        ...
        console.error(`[voltre-cache] Failed to create dir: ${dirPath}`, e.message);
    }
}
...
async function processAndSave(tempPngPath, finalPath, greenRemoval) {
    try {
        ...
        console.error(`[voltre-cache] Process error: ${e.message}`);
...
onNet('voltre:imagemaker:capture', async (type, filename, greenRemoval) => {
    ...
    try {
        exports['screenshot-basic'].requestClientScreenshot(
            ...
            async (err, savedPath) => {
                if (err) {
                    console.error(`[voltre-cache] Screenshot error: ${err}`);
        ...
        console.error(`[voltre-cache] Capture error: ${e.message}`);
        emitNet('voltre:imagemaker:captureResult', src, false, filename, type);
    }
});
...
console.log('[voltre-cache] Image server loaded');
