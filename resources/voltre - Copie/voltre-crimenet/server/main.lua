                ...
                CrimeNet.GetGroups(identifier, function(groups)
                    ...
                    CrimeNet.ResolvePlayerNames(contactIds, function(nameMap)
                        ...
                        local ok, _ = pcall(function()
                            exports["voltre-core"]:GoFast_GetReputation(identifier, function(rep)
                                buildAndSend(rep)
                            end)
                        end)
                        if not ok then
...
-- ============================================================================
RegisterServerEvent("Voltre:crimenet:incrementContactTrust")
AddEventHandler("Voltre:crimenet:incrementContactTrust", function(contactId, amount)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
...
-- BLACKLIST APPEAL
-- ============================================================================
RegisterServerEvent("Voltre:crimenet:removeBlacklist")
AddEventHandler("Voltre:crimenet:removeBlacklist", function(targetIdentifier)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
AddEventHandler("Voltre:gofast:complete", function(engineHealth)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end

    exports["voltre-core"]:GoFast_GetReputation(xPlayer.identifier, function(rep)
        if not rep then return end
        for _, contact in ipairs(CrimeNet.Config.NPCContacts) do
            if rep.xp >= contact.unlockXP then
                local trust = rep.contact_trust or {}
                if (trust[contact.id] or 0) < contact.maxTrust then
                    TriggerEvent("Voltre:crimenet:incrementContactTrust", contact.id, 1)
                    break
                end
            end
        end
    ...
    MySQL.Async.execute([[
        ...
        exports["voltre-core"]:GoFast_GetReputation(identifier, function(rep)
            if rep then
                ...
                TriggerEvent("Voltre:goFast:setXP", identifier, newXP)
...
RegisterServerEvent("Voltre:crimenet:missionStarted")
AddEventHandler("Voltre:crimenet:missionStarted", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
RegisterServerEvent("Voltre:crimenet:missionCompleted")
AddEventHandler("Voltre:crimenet:missionCompleted", function()
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
...
RegisterServerEvent("Voltre:crimenet:missionFailed")
AddEventHandler("Voltre:crimenet:missionFailed", function(reason)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer then return end
