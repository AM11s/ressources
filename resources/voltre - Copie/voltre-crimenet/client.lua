...
local function checkIllegalStatus()
    if ESX and ESX.PlayerData and ESX.PlayerData.job2 then
        ...
        local illegals = exports["voltre-core"]:getData("illegals") 
...
RegisterNUICallback("crimenet:appOpened", function(data, cb)
    ...
    if isIllegalMember then
        ...
        ESX.TriggerServerCallback("Voltre:gofast:getProfileData", function(result)
...
RegisterNUICallback("crimenet:requestMission", function(data, cb)
    TriggerServerEvent("Voltre:gofast:requestMission", "solo")
    cb("ok")
end)
...
RegisterNUICallback("crimenet:requestCrewMission", function(data, cb)
    if data and data.members and type(data.members) == "table" then
        ...
        TriggerServerEvent("Voltre:gofast:requestCrewMission", data.members)
    else
        -- Legacy fallback
        TriggerServerEvent("Voltre:gofast:requestMission", "crew")
    end
    cb("ok")
end)

RegisterNUICallback("crimenet:getCrewMembers", function(data, cb)
    ESX.TriggerServerCallback("Voltre:gofast:getCrewMembers", function(members)
        cb(members or {})
    end)
end)
    ...
    ESX.TriggerServerCallback("CrimeNet:acceptContract", function(success, result)
        if success then
            ...
            if result.type == 'crew' then
                TriggerServerEvent("Voltre:gofast:requestCrewMission", data.crewMembers)
            else
                TriggerServerEvent("Voltre:gofast:requestMission", 'solo')
            end
            TriggerServerEvent("Voltre:crimenet:missionStarted")
        end
...
AddEventHandler("Voltre:gofast:result", function(data)
    if activeContract then
        if data.success then
            TriggerServerEvent("Voltre:crimenet:missionCompleted")
        else
            TriggerServerEvent("Voltre:crimenet:missionFailed", data.reason or "FAILED")
        end
        activeContract = nil
    end
...
RegisterNetEvent("Voltre:gofast:profileRefresh")
AddEventHandler("Voltre:gofast:profileRefresh", function(data)
    if data then
