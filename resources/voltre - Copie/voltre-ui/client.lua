...
RegisterCommand('resetui', function()
    ...
    DisplayRadar(true)
    print("^2[Voltre-UI] Interface réinitialisée^0")
end, false)

RegisterCommand('setfocus_voltreui', function()
    SetNuiFocus(true, true)
end, false)

RegisterCommand('setunfocus_voltreui', function()
    SetNuiFocus(false, false)
end, false)
...
RegisterNetEvent("voltre:esx:pausemenu", function(bool)
    InPauseMenu = bool
end)
...
function Close(data)
    ...
    if menuType == "clothes" then
        ...
        if not shouldSave then
            ESX.TriggerServerCallback('Voltre:esx_skin:getPlayerSkin', function(skin)
                TriggerEvent('Voltre:skinchanger:loadClothes', skin, skin)
            end)
        else
            TriggerEvent('Voltre:skinchanger:getSkin', function(skina)
                TriggerEvent('Voltre:skinchanger:loadClothes', skina, Skins)
    ...
    elseif menuType == "accesories" then
        ...
        if not shouldSave then
            ...
            if next(initialAccessorySkins) then
                ...
                TriggerEvent('Voltre:skinchanger:loadClothes', initialAccessorySkins, initialAccessorySkins)
            end
            changedSkins = {}
            previewSkins = {}
            initialAccessorySkins = {}
...
RegisterNUICallback("updateVariation", function(data)
    ...
    TriggerEvent("Voltre:skinchanger:change", varType, data.number)
    SetPlayerControl(PlayerId(), false, 12)
end)
...
RegisterNUICallback('payItem', function(data, cb)
    name = voltre.fct.input("Nom du vêtement")
    ESX.TriggerServerCallback('Voltre:clothes:payone', function(good)
        if good then
...
RegisterNUICallback('previewAccessory', function(data, cb)
    ...
    if data.variation then
        TriggerEvent("Voltre:skinchanger:change", variation[data.type], data.variation)
    end
...
RegisterNUICallback('restoreAccessories', function(data, cb)
    if data.skins then
        for category, value in pairs(data.skins) do
            TriggerEvent("Voltre:skinchanger:change", category, value)
        end
    end
...
RegisterNUICallback('buyAccessory', function(data, cb)
    name = voltre.fct.input('Nom de l\'accessoire')
    ESX.TriggerServerCallback('Voltre:clothes:payone', function(good)
        if good then
...
RegisterNUICallback('pay', function(data, cb)
    name = voltre.fct.input('Nom de la tenue')
    ESX.TriggerServerCallback('Voltre:clothes:pay', function(good)
        if good then
            Close({type = "clothes", changeSkin = true})
        else
...
RegisterNetEvent("voltre-ui:setSafeZone", function(bool)
    voltre.DebugPrint("SafeZone event received: " .. tostring(bool))
    -- SendNUIMessage({
    --     type = "safezone",
    --     status = bool
    -- })
    TriggerEvent("voltre:safezone:update", { inSafezone = bool })
end)

RegisterNetEvent("voltre-ui:updateWarn", function(number, total)
    SendNUIMessage({
        type = "warn:update",
        number = number,
        total = total
...
RegisterNetEvent("voltre-ui:AnnounceImage", function(data)
    SendNUIMessage({
...
RegisterCommand("testWeazelNews", function()
    TriggerEvent("voltre-ui:AnnounceImage", {
...
--voltre.InitPrint("^2UI modules loaded")