...
RegisterNUICallback('outfit', function(data, cb)
    ...
    for name, outfit in pairs(outfits) do
        if name == value then
            ...
            TriggerEvent('Voltre:skinchanger:loadClothes', tempSkinTable, outfit)
...
RegisterNUICallback('updateInfo', function(data, cb)
    ...
    elseif field == "gender" then
        ...
        if value == "f" then
            ...
            TriggerEvent("Voltre:skinchanger:change", "sex", 1)
            TriggerEvent("Voltre:skinchanger:change", "glasses_1", -1)
            TriggerEvent("Voltre:skinchanger:change", "glasses_2", 0)
        elseif value == "m" then
            tempSkinTable["sex"] = 0
            TriggerEvent("Voltre:skinchanger:change", "sex", 0)
        end
...
function SaveCharacter(cb)
    ...
    TriggerEvent('Voltre:skinchanger:getSkin', function(skin)
        ...
        TriggerServerEvent('voltre:clothe:addClothinInvWithID', "top", "Tenue d'arriver", top, topid)
        TriggerServerEvent('voltre:clothe:addClothinInvWithID', "pants", "Tenue d'arriver", pant, pantsid)
        TriggerServerEvent('voltre:clothe:addClothinInvWithID', "shoes", "Tenue d'arriver", shoes, shoesid)

        if cb then cb() end

        TriggerServerEvent("Voltre:charCreator:finish", characterData)
...
RegisterNetEvent("VoltreUI:openCreator", OpenCharacterCreator)
