const illegalBossManager = {
    ...
    init() {
        ...
        $('.close-illegal-boss').on('click', () => {
            this.hide();
            fetch(`https://voltre-ui/illegal:close`, {
                method: 'POST',
    ...
    promoteMember(identifier) {
        $.post('https://voltre-ui/illegal:promote', JSON.stringify({ identifier: identifier }));
    },

    demoteMember(identifier) {
        $.post('https://voltre-ui/illegal:demote', JSON.stringify({ identifier: identifier }));
    },

    fireMember(identifier) {
        $.post('https://voltre-ui/illegal:fire', JSON.stringify({ identifier: identifier }));
    },

    updateSalary(gradeId, salary) {
        $.post('https://voltre-ui/illegal:updateSalary', JSON.stringify({ 
            gradeId: gradeId, 
            salary: salary 
        }));
    }
