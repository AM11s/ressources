...
const inventory = {
    ...
    initDragAndDrop() {
            ...
            if (e.button === 0) {
                ...
                if (itemSlot) {
                    ...
                    contextMenu.find('.use-btn').off('click').on('click', () => {
                        fetch(`https://voltre-ui/inventory:changeSlot`, {
                            method: 'POST',
                    ...
                    contextMenu.find('.rename-btn').off('click').on('click', () => {
                        fetch(`https://voltre-ui/inventory:renameItem`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ item: item })
                        });
                    ...
                    contextMenu.find('.give-btn').off('click').on('click', () => {
                        fetch(`https://voltre-ui/inventory:giveItem`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ item: item })
                        });
                    ...
                    contextMenu.find('.delete-btn').off('click').on('click', () => {
                        fetch(`https://voltre-ui/inventory:dropItem`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ item: item })
                        });
        ...
        function handleDrop(draggedItem, dropZone) {
            ...
            if (dropZone.classList.contains('shortcut-slot') && itemData.type === 'weapon') {
                ...
                fetch(`https://voltre-ui/shortcut:set`, {
                    method: 'POST',
                    headers: {
                ...
                if (itemData.type2 === slotType) {
                    fetch(`https://voltre-ui/shortcut:equipAccessory`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                ...
                if (startZoneIsEquipSlot && itemData.type === 'accessory') {
                    fetch(`https://voltre-ui/shortcut:removeAccessory`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
    ...
    initSearch() {
        ...
        searchInput.addEventListener('focus', () => {
            $.post('https://voltre-ui/inventory:searchFocus', JSON.stringify({}));
        });

        searchInput.addEventListener('blur', () => {
            $.post('https://voltre-ui/inventory:searchBlur', JSON.stringify({}));
        });
    ...
    setupEventListeners() {
        ...
        //         switch(action) {
        //             case 'use':
        //                 $.post('https://voltre-ui/inventory:useItem', JSON.stringify({
        //                     name: itemData.name,
        //                     slot: itemData.slot
        //                 }));
        //                 break;
        //             case 'give':
        //                 $.post('https://voltre-ui/inventory:startGiveItem', JSON.stringify({
        //                     name: itemData.name,
        //                     slot: itemData.slot
        //                 }));
        //                 break;
        //             case 'rename':
        //                 $.post('https://voltre-ui/inventory:startRename', JSON.stringify({
        //                     name: itemData.name,
        //                     slot: itemData.slot
        //                 }));
        //                 break;
        //             case 'delete':
        //                 $.post('https://voltre-ui/inventory:dropItem', JSON.stringify({
        //                     name: itemData.name,
        //                     slot: itemData.slot
        //                 }));
        //                 break;
        ...
        slots.forEach(grid => {
            ...
            grid.addEventListener('drop', (e) => {
                ...
                if (fromGrid === this.leftGrid && toGrid === this.rightGrid) {
                    $.post('https://voltre-ui/inventory:transferToRight', JSON.stringify({
                        name: itemData.name,
                        fromSlot: itemData.slot,
                        toSlot: toSlot
                    }));
                } else if (fromGrid === this.rightGrid && toGrid === this.leftGrid) {
                    $.post('https://voltre-ui/inventory:transferToLeft', JSON.stringify({
                        name: itemData.name,
                        fromSlot: itemData.slot,
                        toSlot: toSlot
                    }));
        ...
        case 'shortcut:setShortcut':
            ...
            if (shortcut.name !== 'none') {
                ...
                shortcutSlot.ondblclick = function (e) {
                    ...
                    fetch(`https://voltre-ui/shortcut:remove`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
...
document.addEventListener('keyup', (e) => {
    if (e.key === 'Escape' && inventory.isOpen) {
        $.post('https://voltre-ui/inventory:close', JSON.stringify({}));
    }
});
