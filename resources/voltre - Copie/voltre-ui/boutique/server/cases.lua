...
function GenerateLootbox(source, box, list)
    ...
    if chance <= minimalChance then
        ...
        gift.category = 4
        voltreamount = list[4][rand].amount
    ...
    elseif (chance > minimalChance and chance <= Config.Boutique.Crates.Chances.Legendary) then
        ...
        gift.category = 3
        voltreamount = list[3][rand].amount
    ...
    return finalList, gift.item, voltreamount
end
...
function OpenCaisse(source, type)
    ...
    local lists, result, voltre = GenerateLootbox(source, boxId, box)
    local giveReward = {
        ["vehicle"] = function(_s, license, player)
            ...
            if (identifier['fivem']) then
                ...
                while PlateExist do
                    newplate = voltre.fct.format.randomPlateText()
...
for k,v in pairs(Config.Boutique.Crates.List) do
    ESX.RegisterUsableItem(k, function(source)
        local xPlayer = ESX.GetPlayerFromId(source)
        TriggerClientEvent("voltre:inventory:closeinv", source)
        OpenCaisse(source, k)
...
AddEventHandler('sBoutique:openCaseWithAnimation', function(caseId)
    ...
    OnProcessCheckout(source, fprice, "Ouverture de caisse ("..caseData.label..")", function()
        ...
        -- Générer le résultat
        local lists, wonItemName, voltre = GenerateLootbox(source, boxId, box)
        ...
        if wonItemData then
            local giveReward = {
                ...
                ["item"] = function()
                    xPlayer.addInventoryItem(wonItemName, voltre or 1)
                end,
                ["weapon"] = function()
                    xPlayer.addWeapon(wonItemName, 250)
                end,
                ["money"] = function()
                    xPlayer.addAccountMoney('cash', voltre or 0)
                end
            }
        ...
        TriggerClientEvent("voltre:boutique:newCoinsAmount", source)
