...
local function addFidPoints(identifier, nbr, xPlayer)
    MySQL.Async.fetchAll("SELECT * FROM tebex_voltre_fidelite WHERE license = @license", {
        ...
        if result[1] ~= nil then
            ...
            MySQL.Sync.execute("UPDATE tebex_voltre_fidelite SET havebuy=@havebuy, totalbuy=@totalbuy WHERE license=@license", {
                ["@license"] = identifier,
                ["@havebuy"] = newhavebuy,
                ["@totalbuy"] = newtotal 
            })
            ...
            LiteMySQL:Insert('tebex_voltre_fidelite', {
                license = identifier,
                havebuy = havebuy,
                totalbuy = total
            }); 
...
--[[ESX.RegisterServerCallback('Voltre:getFidPoints', function(source, callback)
    local identifier = GetIdentifiers(source);
    if (identifier['fivem']) then
        local before, after = identifier['fivem']:match("([^:]+):([^:]+)")
        ...
        MySQL.Async.fetchAll("SELECT SUM(points) FROM tebex_players_wallet WHERE identifiers = @identifiers", {
            ...
            if (current ~= nil) then
                if (current >= price) then
                    ...
                    TriggerClientEvent("voltre:boutique:newCoinsAmount", source, getPoints(source))
                    Wait(1000)
                    MySQL.Async.fetchAll("SELECT * FROM tebex_voltre_fidelite WHERE license = @license", {
                        ['@license'] = after
                    }, function(result)
...
RegisterServerEvent('VoltreBoutique:buycoins')
AddEventHandler('VoltreBoutique:buycoins', function(coins, cost)
    local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('BoutiqueBucket:SetEntitySourceBucket', function(valeur)
    if valeur then
        exports["voltre-core"]:SetInInstance(source, source+1, "Boutique")
    else
        exports["voltre-core"]:SetInInstance(source, 0)
    end
end)
...
RegisterCommand('addPoints', function(source, args)
    if source == 0 then
        ...
        if (identifier['fivem']) then
            ...
            TriggerClientEvent("voltre:boutique:newCoinsAmount", args[1], getPoints(args[1]))
        else
        ...
        for i = 1, #AllPlayers, 1 do
            if AllPlayers[i] then
                ...
                if xPlayersAll then
                    ...
                    TriggerClientEvent("voltre:boutique:newCoinsAmount", AllPlayers[i], getPoints(AllPlayers[i]))
...
RegisterServerEvent('voltre:server:BuyPack')
AddEventHandler('voltre:server:BuyPack', function(type)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = GetIdentifiers(source)
    if xPlayer ~= nil then 
                            ...
                            if type == "Basic" or type == "Premium" then
                                MySQL.Async.fetchAll("SELECT * FROM vips WHERE identifier=@identifier",{
                                    ...
                                    if data[1] then
                                        ...
                                        TriggerClientEvent("voltre:boutique:newCoinsAmount", xPlayer.source, getPoints(xPlayer.source))
                                        ...
                                        exports["voltre-core"]:GetVIP(xPlayer.identifier)
                                        TriggerClientEvent('voltreupdateVIP', xPlayer.source, true)
                                    end
                                end)
                            else
