function AddToInv(identifier, clotheType, label, data, cb)
    MySQL.Async.execute('INSERT INTO vclothes (identifier, type, name, data) VALUES (@identifier, @type, @name, @data)', {
        ...
        pcall(function()
            exports["voltre-core"]:ClearClothesCache(identifier)
        end)
...
function EquipClothesSlot(source, clotheType, clotheId, clotheData)
    ...
    TriggerClientEvent("voltre:inventory:equipClothesSlot", source, clotheType, xPlayer.clothes_equiped[clotheType])
end

exports('EquipClothesSlot', EquipClothesSlot)

ESX.RegisterServerCallback('Voltre:clothes:pay', function(source, cb, items, paymentMethod, affichename)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    local function checkComplete()
        ...
        if completed >= total then
            voltre.DebugPrint("Clothes added to inventory and payed")
            cb(true, totalPrice)
        end
    end
...
ESX.RegisterServerCallback('Voltre:clothes:payone', function(source, cb, items, paymentMethod, group, affichename)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return cb(false) end
...
RegisterNetEvent('voltre-ui:rpannounce:sendMsg-staff', function(data)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.getGroup() == "user" then return end
    
    TriggerClientEvent('voltre-ui:AnnounceImage', -1, data)
end)


RegisterNetEvent('voltre-ui:rpannounce:sendMsg', function(data)
    local xPlayer = ESX.GetPlayerFromId(source)
