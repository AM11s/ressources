...
// Base paths
const VOLTRE_UI_IMAGES = 'resources/[voltre]/voltre-core/images';
const TEMP_PATH = 'resources/[voltre]/voltre-image-maker/temp';
...
function getTargetPath(type, filename) {
    switch(type) {
        case 'vehicle':
            return `${VOLTRE_UI_IMAGES}/vehicles/${filename}.webp`;
        case 'weapon':
            return `${VOLTRE_UI_IMAGES}/weapons/${filename}.webp`;
        case 'ped':
            return `${VOLTRE_UI_IMAGES}/peds/${filename}.webp`;
        case 'prop':
            return `${VOLTRE_UI_IMAGES}/props/${filename}.webp`;
        case 'clothing':
            return `${VOLTRE_UI_IMAGES}/clothes/${filename}.webp`;
        case 'utils':
            return `${VOLTRE_UI_IMAGES}/clothes/utils/${filename}.png`;
        default:
...
onNet('voltre:imagemaker:capture', async (type, filename, greenRemoval) => {
    const src = source;
        ...
        exports['screenshot-basic'].requestClientScreenshot(
            ...
            async (err, savedPath) => {
                if (err) {
                    ...
                    emitNet('voltre:imagemaker:captureResult', src, false, filename, type);
                    return;
                }

                const processed = await processAndSave(savedPath, targetPath, greenRemoval !== false);
                emitNet('voltre:imagemaker:captureResult', src, processed, filename, type);
            }
        );
    } catch(e) {
...
function scanClothingDir(gender) {
    const basePath = `${VOLTRE_UI_IMAGES}/clothes/${gender}`;
    const result = {};
    try {
...
function scanUtilsDir(gender) {
    const basePath = `${VOLTRE_UI_IMAGES}/clothes/utils/${gender}`;
    const result = {};
    try {
        if (!fs.existsSync(basePath)) return result;
...
onNet('voltre:imagemaker:scanImages', () => {
    const src = source;

    const data = {
        vehicles: scanDirectory(`${VOLTRE_UI_IMAGES}/vehicles`, '.webp'),
        weapons: scanDirectory(`${VOLTRE_UI_IMAGES}/weapons`, '.webp'),
        peds: scanDirectory(`${VOLTRE_UI_IMAGES}/peds`, '.webp'),
        props: scanDirectory(`${VOLTRE_UI_IMAGES}/props`, '.webp'),
        clothing: {
    ...
    emitNet('voltre:imagemaker:scanResult', src, data);
});
...
onNet('voltre:imagemaker:deleteImage', (type, filename) => {
    const src = source;
    const targetPath = getTargetPath(type, filename);
    ...
    try {
        ...
        emitNet('voltre:imagemaker:deleteResult', src, true, filename, type);
    } catch(e) {
        emitNet('voltre:imagemaker:deleteResult', src, false, filename, type);
    }
});
