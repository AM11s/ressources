-- ================================================================
-- Voltre Taxi — lb-phone custom app (client)
-- ----------------------------------------------------------------
-- - Enregistre l'app dans lb-phone
-- - Bridge UI ↔ voltre-core (demande de taxi, status, position)
-- ================================================================

local IDENTIFIER = "voltre_taxi"
...
local function addApp()
    ...
    if not ok then
        print("^1[VOLTRE-TAXI] AddCustomApp a échoué : " .. tostring(err) .. "^0")
    else
        print("^2[VOLTRE-TAXI] App enregistrée dans lb-phone^0")
    end
end
...
RegisterNUICallback("listDrivers", function(_, cb)
    ...
    ESX.TriggerServerCallback("voltre:taxi:listDrivers", function(res)
...
RegisterNUICallback("getMyRequest", function(_, cb)
    ...
    ESX.TriggerServerCallback("voltre:taxi:getMyRequest", function(state)
        if not state then
...
RegisterNUICallback("requestRide", function(data, cb)
    ...
    ESX.TriggerServerCallback("voltre:taxi:requestRide", function(ok, idOrErr)
        if ok then
            hasActiveRequest = true
...
RegisterNUICallback("cancelRequest", function(_, cb)
    if not ESX or not ESX.TriggerServerCallback then return cb({ ok = false }) end
    ESX.TriggerServerCallback("voltre:taxi:cancelMyRequest", function(ok)
        if ok then
            hasActiveRequest = false
...
-- ----------------------------------------------------------------
-- Ride HUD : reçu de ridemeter.lua (voltre-core) via event local
-- On le relaie dans la NUI du téléphone uniquement (pushUI).
-- ----------------------------------------------------------------
AddEventHandler('voltre:taxi:rideHUDMsg', function(action, data)
...
-- ----------------------------------------------------------------
RegisterNetEvent("voltre:taxi:requestAccepted", function(payload)
    ...
    pcall(function()
        if exports["lb-phone"] and exports["lb-phone"].OpenApp then
            exports["lb-phone"]:OpenApp("Voltre Taxi")
        end
    end)
end)

RegisterNetEvent("voltre:taxi:driverPos", function(coords)
    if type(coords) ~= "table" then return end
    lastDriverCoords = { x = coords.x, y = coords.y }
    pushUI("taxi:driverPos", lastDriverCoords)
end)

RegisterNetEvent("voltre:taxi:requestEnded", function()
    hasActiveRequest = false
    lastDriverCoords = nil
    pushUI("taxi:ended", {})
    if ESX and ESX.ShowNotification then
