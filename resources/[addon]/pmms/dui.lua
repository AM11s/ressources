...
function DuiBrowser:waitForConnection()
	...
	local timeout = GetGameTimer() + Config.PMMS.dui.timeout
	...
	if self.initDone then
		...
		print(("Failed to initialize DUI browser: Could not connect to %s within %d ms"):format(self.duiUrl, Config.PMMS.dui.timeout))
		return false
	end
end
...
function DuiBrowser:enableScaleform()
	...
	if self:loadScaleform() then
		...
		ScaleformMovieMethodAddParamInt(0)
		ScaleformMovieMethodAddParamInt(Config.PMMS.dui.screenWidth)
		ScaleformMovieMethodAddParamInt(Config.PMMS.dui.screenHeight)
...
function DuiBrowser:new(mediaPlayerHandle, model, renderTarget, scaleform, url)
	...
	if useHttps then
		self.duiUrl = Config.PMMS.dui.urls.https
	else
		if Config.PMMS.dui.urls.http then
			self.duiUrl = Config.PMMS.dui.urls.http
		else
			self.duiUrl = ("http://%s/%s/dui/"):format(GetCurrentServerEndpoint(), thisResource)
		end
	end

	self.duiObject = CreateDui(self.duiUrl .. "?resourceName=" .. thisResource, Config.PMMS.dui.screenWidth, Config.PMMS.dui.screenHeight)
	...
	if self.renderTarget or self.scaleform then
		...
		if self.scaleform then
			self.sfName = self.scaleform.name or Config.PMMS.defaultScaleformName
		end
	end
