...
local function isUrlAllowed(url)
	for _, pattern in ipairs(Config.PMMS.allowedUrls) do
...
local function addMediaPlayer(handle, options)
	...
	options.offset = 0
	options.videoSize = Clamp(options.videoSize, 10, 100, Config.PMMS.defaultVideoSize)
	options.paused = false

	if options.filter == nil then
		options.filter = Config.PMMS.enableFilterByDefault
	end

	if options.attenuation then
		options.attenuation.sameRoom = Clamp(options.attenuation.sameRoom, 0.0, 10.0, Config.PMMS.defaultSameRoomAttenuation)
		options.attenuation.diffRoom = Clamp(options.attenuation.diffRoom, 0.0, 10.0, Config.PMMS.defaultDiffRoomAttenuation)
	else
		options.attenuation = {
			sameRoom = Config.PMMS.defaultSameRoomAttenuation,
			diffRoom = Config.PMMS.defaultDiffRoomAttenuation
		}
	end

	options.diffRoomVolume = Clamp(options.diffRoomVolume, 0.0, 1.0, Config.PMMS.defaultDiffRoomVolume)

	options.range = Clamp(options.range, 0, Config.PMMS.maxRange, Config.PMMS.defaultRange)
...
local function getRandomPreset()
	...
	for preset, info in pairs(Config.PMMS.presets) do
...
local function resolvePreset(url, title, filter, video)
	...
	if Config.PMMS.presets[url] then
		return Config.PMMS.presets[url]
	else
		return {
			url = url,
			title = title,
...
local function startDefaultMediaPlayers()
	for _, mediaPlayer in ipairs(Config.PMMS.defaultMediaPlayers) do
...
local function syncSettings()
	TriggerClientEvent("pmms:loadSettings", -1, Config.PMMS.models, Config.PMMS.defaultMediaPlayers)
end
...
local function loadSettings()
	...
	if models then
		for key, info in pairs(models) do
			...
			if Config.PMMS.models[model] then
				Config.PMMS.models[model].renderTarget = info.renderTarget
				Config.PMMS.models[model].label = info.label
				Config.PMMS.models[model].filter = info.filter
				Config.PMMS.models[model].volume = info.volume
				Config.PMMS.models[model].attenuation = info.attenuation
				Config.PMMS.models[model].diffRoomVolume = info.diffRoomVolume
				Config.PMMS.models[model].range = info.range
				Config.PMMS.models[model].isVehicle = info.isVehicle
				Config.PMMS.models[model].scaleform = info.scaleform
			else
				Config.PMMS.models[model] = info
			end
		end
	end
	...
	if defaultMediaPlayers then
		for _, defaultMediaPlayer in ipairs(defaultMediaPlayers) do
			...
			local dmp = GetDefaultMediaPlayer(Config.PMMS.defaultMediaPlayers, defaultMediaPlayer.position)
			...
			if dmp then
				...
				table.insert(Config.PMMS.defaultMediaPlayers, defaultMediaPlayer)
			end
		end
	end
...
local function addModel(model, data)
	if Config.PMMS.models[model] then
		if not data.renderTarget then
			data.renderTarget = Config.PMMS.models[model].renderTarget
		else
			Config.PMMS.models[model].renderTarget = data.renderTarget
		end
		if not data.label then
			data.label = Config.PMMS.models[model].label
		else
			Config.PMMS.models[model].label = data.label
		end
		Config.PMMS.models[model].filter = data.filter
		Config.PMMS.models[model].volume = data.volume
		Config.PMMS.models[model].attenuation = data.attenuation
		Config.PMMS.models[model].diffRoomVolume = data.diffRoomVolume
		Config.PMMS.models[model].range = data.range
		Config.PMMS.models[model].isVehicle = data.isVehicle
		Config.PMMS.models[model].scaleform = data.scaleform
	else
		Config.PMMS.models[model] = data
	end
...
local function addEntity(coords, data)
	local defaultMediaPlayer = GetDefaultMediaPlayer(Config.PMMS.defaultMediaPlayers, coords)
	...
	if defaultMediaPlayer then
		...
		table.insert(Config.PMMS.defaultMediaPlayers, data)
	end
...
local function removeModel(model)
	if not Config.PMMS.models[model] then
		return
	end

	local data = Config.PMMS.models[model]

	Config.PMMS.models[model] = nil
...
local function removeEntity(coords)
	...
	for i = 1, #Config.PMMS.defaultMediaPlayers do
		if IsSameEntity(coords, Config.PMMS.defaultMediaPlayers[i].position) then
			data = table.remove(Config.PMMS.defaultMediaPlayers, i)
			break
		end
	end
...
AddEventHandler("pmms:start", function(handle, options)
	...
	if mediaPlayers[handle] then
		...
		if Config.PMMS.presets[options.url] then
			options.title  = Config.PMMS.presets[options.url].title
			options.filter = Config.PMMS.presets[options.url].filter or false
			options.video  = Config.PMMS.presets[options.url].video or options.visualization ~= nil
			options.url    = Config.PMMS.presets[options.url].url
		...
		elseif IsPlayerAceAllowed(source, "pmms.customUrl") then
			...
			errorMessage(source, "You must select from one of the pre-defined songs (" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "presets)")
		end
	end
end)
...
AddEventHandler("pmms:setAttenuation", function(handle, sameRoom, diffRoom)
	...
	mediaPlayers[handle].attenuation = {
		sameRoom = Clamp(sameRoom, 0.0, 10.0, Config.PMMS.defaultSameRoomAttenuation),
		diffRoom = Clamp(diffRoom, 0.0, 10.0, Config.PMMS.defaultDiffRoomAttenuation)
	}
end)
...
AddEventHandler("pmms:setDiffRoomVolume", function(handle, diffRoomVolume)
	...
	mediaPlayers[handle].diffRoomVolume = Clamp(diffRoomVolume, 0.0, 1.0, Config.PMMS.defaultDiffRoomVolume)
end)
...
AddEventHandler("pmms:setRange", function(handle, range)
	...
	mediaPlayers[handle].range = Clamp(range, 0.0, Config.PMMS.maxRange, Config.PMMS.defaultRange)
end)
...
AddEventHandler("pmms:setVideoSize", function(handle, size)
	...
	mediaPlayers[handle].videoSize = Clamp(size, 10, 100, Config.PMMS.defaultVideoSize)
end)
...
AddEventHandler("pmms:loadSettings", function()
	TriggerClientEvent("pmms:loadSettings", source, Config.PMMS.models, Config.PMMS.defaultMediaPlayers)
end)
...
RegisterCommand(Config.PMMS.commandPrefix, function(source, args, raw)
	TriggerClientEvent("pmms:showControls", source)
end, true)
...
RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "play", function(source, args, raw)
	if #args > 0 then
		local options = {}
...
RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "pause", function(source, args, raw)
	TriggerClientEvent("pmms:pauseClosestMediaPlayer", source)
end, true)

RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "stop", function(source, args, raw)
	TriggerClientEvent("pmms:stopClosestMediaPlayer", source)
end, true)

RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "status", function(source, args, raw)
	TriggerClientEvent("pmms:toggleStatus", source)
end, true)

RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "presets", function(source, args, raw)
	TriggerClientEvent("pmms:listPresets", source)
end, true)

RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "vol", function(source, args, raw)
	if #args < 1 then
...
RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl", function(source, args, raw)
	if #args < 1 then
		print("Usage:")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl list")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl lock <handle>")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl unlock <handle>")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl mute <handle>")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl unmute <handle>")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl loop <handle> <on|off>")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl next <handle>")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl pause <handle>")
		print("  " .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl stop <handle>")
...
RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "add", function(source, args, raw)
	local model = args[1]
	local label = args[2]
...
RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "fix", function(source, args, raw)
	TriggerClientEvent("pmms:reset", source)
end, true)

RegisterCommand(Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "refresh_perms", function(source, args, raw)
