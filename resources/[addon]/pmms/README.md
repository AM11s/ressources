...
The default permissions allow members of `group.admin` full access to pmms, while other players will only be able to perform basic interactions with media players and only be able to play preset songs (those defined in `Config.PMMS.presets` in [config.lua](config.lua)).
...
Even with this ace, URLs will be restricted to those allowed by `Config.PMMS.allowedUrls`, which includes generally safe sites such as YouTube. To allow players to use URLs from other sites, you can either add the appropriate pattern to `Config.PMMS.allowedUrls`, or uncomment or add the following line to remove the restriction entirely:
...
| Option     | Description                                                              |
|------------|--------------------------------------------------------------------------|
| `model`    | The entity model to use. If omitted, `Config.PMMS.defaultModel` will be used. |
