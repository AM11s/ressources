...
local function notify(args)
	...
	if not args.duration then
		args.duration = Config.PMMS.notificationDuration;
	end
...
local function isMediaPlayer(entity)
	...
	return Config.PMMS.models[model] ~= nil
end
...
local function forEachMediaPlayer(func)
	...
	if Config.PMMS.allowPlayingFromVehicles then
...
local function getClosestMediaPlayer()
	return getClosestMediaPlayerEntity(GetEntityCoords(PlayerPedId()), Config.PMMS.maxDiscoveryDistance)
end

local function getEntityLabel(handle, entity)
	local defaultMediaPlayer = GetDefaultMediaPlayer(Config.PMMS.defaultMediaPlayers, GetEntityCoords(entity))
	...
	if defaultMediaPlayer and defaultMediaPlayer.label then
		...
		if model and Config.PMMS.models[model] and Config.PMMS.models[model].label then
			return Config.PMMS.models[model].label
...
local function startMediaPlayer(handle, options)
	...
	if NetworkDoesNetworkIdExist(handle) then
		...
		if Config.PMMS.models[options.model] then
			options.renderTarget = Config.PMMS.models[options.model].renderTarget
		end
		...
		if options.isVehicle == nil then
			if Config.PMMS.models[options.model] then
				options.isVehicle = Config.PMMS.models[options.model].isVehicle
			else
...
local function startClosestMediaPlayer(options)
	...
	if not mediaPlayer then
		if Config.PMMS.showNotifications then
	...
	if #presets == 0 then
		...
		for _, preset in ipairs(presets) do
			TriggerEvent("chat:addMessage", {
				args = {preset, Config.PMMS.presets[preset].title}
			})
		end
	end
end
...
local function updateUi()
	...
	if uiIsOpen then
		...
		forEachMediaPlayer(function(entity)
			...
			if clHandle then
				...
				local distance = #(pos - mediaPos)
				local isNearby = distance <= Config.PMMS.maxDiscoveryDistance
	...
	SendNUIMessage({
		type = "updateUi",
		color = "rgba("..ESX.Config("r")..", "..ESX.Config("g")..", "..ESX.Config("b")..", 0.4)",
		uiIsOpen = uiIsOpen,
		activeMediaPlayers = json.encode(activeMediaPlayers),
		usableMediaPlayers = json.encode(usableMediaPlayers),
		presets = json.encode(Config.PMMS.presets),
		maxDiscoveryDistance = Config.PMMS.maxDiscoveryDistance,
...
local function createMediaPlayerEntity(options, networked)
	local model = options.model or Config.PMMS.defaultModel
...
local function getEntityModelAndRenderTarget(handle)
	...
	if type(handle) == "vector3" then
		entity = getLocalMediaPlayer(handle, GetEntityCoords(PlayerPedId()), Config.PMMS.maxRange)
	elseif NetworkDoesNetworkIdExist(handle) then
		entity = NetworkGetEntityFromNetworkId(handle)
	else
		return
	...
	if Config.PMMS.models[model] then
		return entity, model, Config.PMMS.models[model].renderTarget
	else
		return entity, model
	end
end
	...
	if Config.PMMS.isRDR then
		...
		if not duiBrowser then
			...
			if mediaPos and (model or scaleform) then
				...
				if #(viewerPos - mediaPos) < (data.range or Config.PMMS.maxRange) then
...
RegisterNUICallback("startup", function(data, cb)
	...
	cb {
		isRDR = Config.PMMS.isRDR,
		enableFilterByDefault = Config.PMMS.enableFilterByDefault,
		defaultSameRoomAttenuation = Config.PMMS.defaultSameRoomAttenuation,
		defaultDiffRoomAttenuation = Config.PMMS.defaultDiffRoomAttenuation,
		defaultDiffRoomVolume = Config.PMMS.defaultDiffRoomVolume,
		defaultRange = Config.PMMS.defaultRange,
		maxRange = Config.PMMS.maxRange,
		defaultScaleformName = Config.PMMS.defaultScaleformName,
		defaultVideoSize = Config.PMMS.defaultVideoSize,
		audioVisualizations = Config.PMMS.audioVisualizations,
...
RegisterNUICallback("setMediaPlayerDefaults", function(data, cb)
	...
	if entity and coords then
		defaults = GetDefaultMediaPlayer(Config.PMMS.defaultMediaPlayers, coords) or Config.PMMS.models[GetEntityModel(entity)]
	else
		for _, dmp in ipairs(Config.PMMS.defaultMediaPlayers) do
...
AddEventHandler("pmms:loadSettings", function(models, defaultMediaPlayers)
	Config.PMMS.models = models

	-- Keep local entity handles of default media players
	for _, dmp1 in ipairs(Config.PMMS.defaultMediaPlayers) do
		if dmp1.handle then
	...
	Config.PMMS.defaultMediaPlayers = defaultMediaPlayers
end)
...
Citizen.CreateThread(function()
	if Config.PMMS.autoDisableStaticEmitters then
		restoreStaticEmitters()
	end

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix, "Open the media player control panel.")

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "play", "Play something on the closest media player.", {
	...
	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "pause", "Pause the closest media player.")

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "stop", "Stop the closest media player.")

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "status", "Show/hide the status of the closest media player.")

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "presets", "List available presets.")

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "vol", "Adjust the base volume of all media players.", {
		{name = "volume", help = "0-100"}
	})

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "add", "Add or modify a media player model preset.", {
	...
	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "fix", "Reset all media players to fix issues.")

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "ctl", "Advanced media player control.")

	TriggerEvent("chat:addSuggestion", "/" .. Config.PMMS.commandPrefix .. Config.PMMS.commandSeparator .. "refresh_perms", "Refresh permissions for all clients.")
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if canWait then
			if Config.PMMS.autoDisableStaticEmitters and staticEmittersDisabled then
				restoreStaticEmitters()
			...
			if Config.PMMS.autoDisableStaticEmitters and not staticEmittersDisabled then
				disableStaticEmitters()
				staticEmittersDisabled = true
			end

			if Config.PMMS.autoDisableIdleCam and not disableIdleCam then
				invalidateIdleCams()
				disableIdleCam = true
			end
...
Citizen.CreateThread(function()
	while true do
		...
		for handle, info in pairs(mediaPlayers) do
			...
			if not Config.PMMS.isRDR and Config.PMMS.autoDisableVehicleRadio then
				local entity
		...
		for _, mediaPlayer in ipairs(Config.PMMS.defaultMediaPlayers) do
			if mediaPlayer.spawn then
				local nearby = #(myPos - mediaPlayer.position) <= Config.PMMS.defaultMediaPlayerSpawnDistance
